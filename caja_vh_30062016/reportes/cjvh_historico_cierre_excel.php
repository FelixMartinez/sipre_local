...
while ($row = mysql_fetch_assoc($rs)) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C".$contFila, date("d-m-Y",strtotime($row['fechaAperturaCaja'])));
	$objPHPExcel->getActiveSheet()->setCellValue("D".$contFila, utf8_encode($row['empleado_apertura']));
	$objPHPExcel->getActiveSheet()->setCellValue("E".$contFila, date("d-m-Y "."H:i:s", strtotime($row['ejecucion_apertura'])));
	$objPHPExcel->getActiveSheet()->setCellValue("F".$contFila, utf8_encode($row['empleado_cierre']));
	$objPHPExcel->getActiveSheet()->setCellValue("G".$contFila, date("d-m-Y "."H:i:s", strtotime($row['ejecucion_cierre'])));
