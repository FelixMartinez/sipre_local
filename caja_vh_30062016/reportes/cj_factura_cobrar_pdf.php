...
if ($totalRows > 0) {
	...
	while ($contFila<1) {
		...
		if ($contFila % 45 == 1) {
			...
			$pdf->Ln();
			$pdf->Cell(562,10,"FACTURAS POR COBRAR",0,0,'C');
			...
			$arrayCol = array("EMPRESA\n\n","FECHA REGISTRO","NRO. FACTURA\n\n","NRO. CONTROL\n\n","CLIENTE\n\n","ESTADO FACTURA","SALDO FACTURA","TOTAL FACTURA");
		...
		while ($rowDetalle = mysql_fetch_assoc($rsDetalle)){
			...
			$pdf->Cell($arrayTamCol[5],12,utf8_encode($rowDetalle['descripcion_estado_factura']),'LR',0,'C',true);
			$pdf->Cell($arrayTamCol[6],12,number_format($rowDetalle['saldoFactura'],2,".",","),'LR',0,'R',true);
			$pdf->Cell($arrayTamCol[7],12,number_format($rowDetalle['total'],2,".",","),'LR',0,'R',true);
			$pdf->Ln();
