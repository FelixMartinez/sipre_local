...
GROUP BY cierre.fechaCierre ORDER BY fechaCierre DESC",
...
while ($rowMaestro = mysql_fetch_assoc($rsMaestro)) {
	...
	FROM cj_verificacion_cierre
		...
		INNER JOIN ".$cierreCajaPpal." cierre ON (cj_verificacion_cierre.id_cierre = cierre.idCierre)
...
if (isset($arrayPagina)) {
	foreach ($arrayPagina as $indice => $valor) {
		...
		$excel->add_row(array("","","","","","","","","","","","","","","","","","","",""));
		$excel->add_row(array("HISTORICO DE CIERRE - ".$nombreCajaPpal."|19"), 'trTitulo');
		$excel->add_row(array("","","","","","","","","","","","","","","","","","","",""));
...
$excel->download('SIPRE_Historico_Cierre_'.str_replace(" ","_",$nombreCajaPpal).'.xls');
?>