	...
	while ($contFila<1) {
		...
		// DETALLE DEL LSITADO
			...
			FROM an_pedido ped_vent
				...
				LEFT OUTER JOIN an_unidad_fisica uni_fis ON (ped_vent.id_unidad_fisica = uni_fis.id_unidad_fisica)
				LEFT OUTER JOIN an_presupuesto pres_vent ON (pres_vent.id_presupuesto = ped_vent.id_presupuesto)
				LEFT OUTER JOIN an_presupuesto_accesorio pres_vent_acc ON (pres_vent.id_presupuesto = pres_vent_acc.id_presupuesto)
				LEFT OUTER JOIN an_uni_bas uni_bas ON (uni_fis.id_uni_bas = uni_bas.id_uni_bas)
				...
				LEFT OUTER JOIN an_version vers ON (uni_bas.ver_uni_bas = vers.id_version) 
