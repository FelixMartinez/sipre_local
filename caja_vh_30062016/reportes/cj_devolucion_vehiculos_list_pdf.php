...
if ($totalRows > 0) {
	...
	while ($contFila<1) {
		...
		if ($contFila % 45 == 1) {
			...
			$pdf->Cell(562,5,"NOTAS DE CRÉDITO DE VEHÍCULOS",0,0,'C');
		...
		$sqlBusq .= $cond.sprintf(" WHERE idDepartamentoOrigenFactura IN (%s) AND anulada LIKE 'NO' ",
			...
			while ($rowDetalle = mysql_fetch_assoc($rsDetalle)){
				...
				if ($estadoAnticipo == 0){
		 		...
		 		if($condicionDePago == 0){
					...
					$pdf->Cell($arrayTamCol[1],12,date("d-m-Y", strtotime($fechaRegistroFactura)),'LR',0,'C',true);
