...
if ($totalRows > 0) {
	...
	while ($contFila<1) {
		...
		if ($contFila % 45 == 1) {
			...
			$pdf->Ln();
			$pdf->Cell(562,10,"LISTADO DE COBRANZAS",0,0,'C');
			...
			$arrayTamCol = array("100","40","40","40","94","40","60","50","50","50");
		...
		while ($rowDetalle = mysql_fetch_assoc($rsDetalle)){
			...
			$pdf->Cell($arrayTamCol[1],12,date("d-m-Y", strtotime($rowDetalle['fechaRegistroFactura'])),'LR',0,'C',true);
			$pdf->Cell($arrayTamCol[2],12,utf8_encode($rowDetalle['numeroFactura']),'LR',0,'R',true);
			$pdf->Cell($arrayTamCol[3],12,utf8_encode($rowDetalle['numeroControl']),'LR',0,'R',true);
			$pdf->Cell($arrayTamCol[4],12,utf8_encode($rowDetalle['nombre_cliente']),'LR',0,'L',true);
			$pdf->Cell($arrayTamCol[5],12,date("d-m-Y", strtotime($rowDetalle['fechapago'])),'LR',0,'C',true);
			$pdf->Cell($arrayTamCol[6],12,utf8_encode($rowDetalle['nombreFormaPago']),'LR',0,'C',true);
			$pdf->Cell($arrayTamCol[7],12,number_format($rowDetalle['montopagado'],2,".",","),'LR',0,'R',true);
			$pdf->Cell($arrayTamCol[8],12,number_format($rowDetalle['saldoFactura'],2,".",","),'LR',0,'R',true);
			$pdf->Cell($arrayTamCol[9],12,number_format($rowDetalle['montoTotalFactura'],2,".",","),'LR',0,'R',true);
			$pdf->Ln();
			
			$fill = !$fill;
					
			$totalPagadoFactura += $rowDetalle['montopagado'];
			$saldoFinalFactura += $rowDetalle['saldoFactura'];
			$totalFinalFactura += $rowDetalle['montoTotalFactura'];
		}
		...
		$pdf->Cell(220,14,"",'T',0,'L',true);
		$pdf->SetFillColor(204,204,204,204);
		$pdf->Cell(194,14,"TOTALES: ",1,0,'R',true);
		$pdf->Cell(50,14,number_format($totalPagadoFactura,2,".",","),1,0,'R',true);
