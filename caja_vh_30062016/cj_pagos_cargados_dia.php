...
if ($_GET['acc'] == 2) { // 2 = Corte de Caja
	...
	FROM (SELECT
		...
		SELECT
			cxc_trf.id_empresa AS id_empresa,
			6 AS tipoDoc,
			4 AS formaPago
		FROM cj_cc_transferencia cxc_trf
		WHERE cxc_trf.idCaja IN (%s) AND cxc_trf.tomadoEnCierre in (0,2)
			AND cxc_trf.fecha_transferencia = %s
		GROUP BY cxc_trf.id_empresa, 3) AS q
		...
		if ($totalRowsPago > 0) {
			...
			while ($rowPago = mysql_fetch_array($rsPago)) {
				...
				switch ($rowPago['tabla']) {
					case 'an_pagos' : // FACTURA
						$aVerDcto = sprintf("<a href=\"javascript:verVentana('reportes/cjvh_recibo_pago_pdf.php?idRecibo=%s', 960, 550);\" class=\"noprint\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>",
					...
					case 'cj_cc_detalleanticipo' : // ANTICIPO
						$aVerDcto = sprintf("<a href=\"javascript:verVentana('reportes/cjvh_recibo_impresion_pdf.php?idRecibo=%s', 960, 550);\" class=\"noprint\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>",
							$rowPago['id_recibo_pago']);
						break;
...
$rowVentaCredito = mysql_fetch_assoc($rsVentaCredito); ?>
    ...
    <div id="divButtons" name="divButtons" style="display:none">
        <table border="0" width="100%">
        ...
        <tr>
            ...
            <form id="frmCerrar" name="frmCerrar" method="post">
                ...
                <button type="button" id="btnCerrarCaja" name="btnCerrarCaja" onclick="if (confirm('Desea Cerrar La Caja?')){ xajax_cerrarCaja(); }" style="display:none" value="Cerrar Caja"><table align="center" cellpadding="0" cellspacing="0"><tr><td>&nbsp;</td><td><img src="../img/iconos/lock.png"/></td><td>&nbsp;</td><td>Cerrar Caja</td></tr></table></button>
