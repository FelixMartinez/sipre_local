...
function aperturaCaja($frmApertura){
	...
	$horaApertura = date("h:i:s"); // HORA DE APERTURA SIEMPRE SERÁ LA FECHA ACTUAL.
	...
	if (!$rsConfig410) { errorAperturaCaja($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	if ($rowConfig410['valor'] == 1) { // 0 = Caja Propia, 1 = Caja Empresa Principal
		...
		if (!$rsEmpresa) { errorAperturaCaja($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	if (mysql_num_rows($consulta) > 0) {
		...
		if (mysql_num_rows($consultaConsultaFecha)) {
			...
			if (!$rsDeposito){ errorAperturaCaja($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			...
			if ($totalRowsDeposito > 0) {
				...
				if (!$Result1){ errorAperturaCaja($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
...
function cargarDatosCaja(){
	...
	if (in_array($rowApertura['statusAperturaCaja'],array(1))) {
		...
		if ($rowApertura['fechaAperturaCaja'] != $fecha) {
			$objResponse->alert("La caja esta abierta con fecha: ".date("d-m-Y",strtotime($rowApertura['fechaAperturaCaja'])).". Cierrela y aperture nuevamente.");
		}
	} else {
		$objResponse->script("
