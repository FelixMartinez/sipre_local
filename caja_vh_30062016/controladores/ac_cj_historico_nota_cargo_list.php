		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			if ($row['saldoNotaCargo'] < $row['total']) {
				if (in_array($row['id_modulo'],array(2,4))) {
					$aVerDcto = sprintf("javascript:verVentana('../caja_vh/reportes/cjvh_recibo_pago_pdf.php?idTpDcto=2&id=%s', 960, 550);",
						$row['idNotaCargo']);
				} else if (in_array($row['id_modulo'],array(0,1,3))) {
					$aVerDcto = sprintf("javascript:verVentana('../caja_rs/reportes/cjrs_recibo_pago_pdf.php?idTpDcto=2&id=%s', 960, 550);",
						$row['idNotaCargo']);
				}
			} else {
				$aVerDcto = "";
			}
				$htmlTb .= (strlen($aVerDcto) > 0) ? "<a href=\"".$aVerDcto."\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>" : "";
			$htmlTb .= "</td>";
			// MODIFICADO ERNESTO
