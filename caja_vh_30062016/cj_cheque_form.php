...
if(!(validaAcceso("cj_cheque_list","insertar"))) {
...
include("controladores/ac_cj_cheque_form.php");
...
<head>
	...
	<title>.: SIPRE <?php echo cVERSION; ?> :. <?php echo $nombreCajaPpal; ?> - Cheque</title>
	...
	function validarFrmAgregarDcto(tipoDcto) {
		...
		&& validarCampo('lstTipoCheque', 't', 'lista') == true
		&& validarCampo('txtMontoPago', 't', 'monto') == true)) {
			validarCampo('txtIdCliente', 't', '');
			validarCampo('lstTipoCheque', 't', 'lista');
	...
	function validarFrmDcto(){
		tipoCheque = byId('lstTipoCheque').value;		
		error = false;
		
		/*if(inArray(tipoCheque, [1,3])){//tipo cliente y pnd requiere cliente
			if (!(validarCampo('txtIdCliente','t','') == true)){					
				validarCampo('txtIdCliente','t','');
				error = true;
			}
		...
		&& validarCampo('lstTipoCheque','t','') == true
		...
		if (error == true) {
			...
			if(tipoCheque == "1") {
				mensaje = "¿Seguro deseas generar el cheque?";
			} else if(tipoCheque == "2") {
				mensaje = "¿Seguro deseas generar pago de anticipo suplidor?";
			} else if(tipoCheque == "3") {
				mensaje = "¿Seguro deseas generar pago de anticipo PND?";
			}
			
			if (confirm(mensaje)) { 
				xajax_guardarCheque(xajax.getFormValues('frmDcto'), xajax.getFormValues('frmDetallePago'), xajax.getFormValues('frmListaDctoPagado'));
			}
		}
	}
	...
	function asignarTipoCheque(lstTipoCheque){
		if (limpiarCheque() != true) {
			selectedOption('lstTipoCheque', byId('hddTipoCheque').value);
		} else {
			byId('hddTipoCheque').value = lstTipoCheque;
		}
		
		if (inArray(lstTipoCheque, [1,3])) { // 1 = Cliente, 3 = PND
			if (lstTipoCheque == 1) { // 1 = Cliente
				...
				byId('tituloAnticipos').innerHTML = "Agregar Anticipo a Cobrar (Bono Suplidor):";	
			} else if (lstTipoCheque == 3) { // 3 = PND
				byId('btnAgregarFactura').style.display = "";
				byId('btnAgregarNotaDebito').style.display = "";
				byId('btnAgregarAnticipo').style.display = "";
		...
		} else if (lstTipoCheque == 2) { // 2 = Bono Suplidor
			byId('btnAgregarFactura').style.display = "";
			byId('btnAgregarNotaDebito').style.display = "";
			byId('btnAgregarAnticipo').style.display = "";
			byId('btnAgregarAnticipoOtro').style.display = "";
	...
	function calcularPagos(){
		...
		xajax_calcularPagos(byId('txtMontoPago').value, xajax.getFormValues('frmListaDctoPagado'), xajax.getFormValues('frmDcto'));
	}
	
	function copiarMontoCheque(){
		var saldo = byId('txtMontoPago').value;
		byId('txtMontoRestante').value = saldo;
		calcularPagos();
	}
	
	function limpiarCheque(){
		if ((byId('cbx4') != undefined && confirm('Se elimaran todos los documentos a Cobrar. ¿Seguro desea cambiar el tipo de cheque?'))
		|| byId('cbx4') == undefined) {
			...
			// Quito anticipos y reestablesco montos del cheque
...
<div id="divGeneralPorcentaje">
	...
	<div id="divInfo" class="print">
		...
		<tr>
			<td class="tituloPaginaCaja">Cheque</td>
		</tr>
		<tr>
			<td>&nbsp;</td>
		</tr>
