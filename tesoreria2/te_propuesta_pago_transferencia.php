...
include("controladores/ac_te_propuesta_pago_tr.php");
...
<head>
   ...
   <link rel="stylesheet" type="text/css" href="../js/domDragTesoreria.css"/>
    ...
    <script>
	function radioChecado(){
		if ($('rdoTodas').checked == true){
			//$('tdDiasVencimiento').style.display = 'none';
			$('hddTipoFactura').value = 0;
		}
		else if ($('rdoSinVencer').checked == true){
			//$('tdDiasVencimiento').style.display = 'none';
			$('hddTipoFactura').value = 1;
		}
		else{
			//$('tdDiasVencimiento').style.display = '';
			$('hddTipoFactura').value = 2;
		}
		//xajax_buscarFactura(xajax.getFormValues('frmBuscar'));
	}
	...
	function validar(){
		...
		if (validarCampo('txtMontoAPagar','t','') == true && $('txtMontoAPagar').value >= 0){
			...
			$('divFlotante').style.display = 'none';
                        ...
                        if($('hddTipoDocumento').value == 0){
                            xajax_buscarFactura(xajax.getFormValues('frmBuscar'));
                        }else{
                            xajax_buscarNotaCargo(xajax.getFormValues('frmBuscar'));
                        }
	...
	function confirmarCambio(prove_o_banco){
		...
		if ($('arrayIdFactura').value != ""){
			 if(confirm("Si selecciona otro " + prove_o_banco + " se borraran las facturas seleccionadas de la propuesta de pago desea continuar?")){
				...
				$('arrayCodigoRetenido').value = "";
				$('arrayIdRetenion').value = "";
    ...
    <div id="divInfo" class="print">
        ...
        <tr class="solo_print">
				...
				<td align="right">
					...
					<table width="100%" border="0">
                        ...
                        <tr>
                            ...
                            <td align="left" colspan="3" id="tdSelCuentas"><select name="selCuenta" id="selCuenta"><option value="-1">Seleccione</option></select></td>
                    ...
                    <td align="right" class="tituloCampo" width="110"><span class="textoRojoNegrita">*</span>Cuenta:</td>
                            <td align="left" width="301">
                                <input type="text" id="txtNumCuenta" name="txtNumCuenta" size="30" style="text-align:center" />
                            </td>
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                        ...
                        <tr>
                            <td align="left" colspan="6" id="tdEditar">
                            	<button type="button" id="btnAgregarFactura"  name="btnAgregarFactura" onclick="document.getElementById('buscarFact').value = 1; xajax_buscarFactura(xajax.getFormValues('frmBuscar'));" title="Seleccionar Empresa" disabled="disabled">Agregar Factura</button>
                                <button type="button" id="btnAgregarNotaCargo"  name="btnAgregarNotaCargo" onclick="document.getElementById('buscarFact').value = 2; xajax_buscarNotaCargo(xajax.getFormValues('frmBuscar'));" title="Agregar Nota Cargo" disabled="disabled">Agregar Nota Cargo</button>
            ...
            <tr class="noprint" id="trBotones">
                <td align="right">
                    <button type="button" id="btnGuardar" name="btnGuardar" onclick="this.disabled = true; validarGuardado();" style="cursor:default"><table align="center" cellpadding="0" cellspacing="0"><tr><td>&nbsp;</td><td><img src="../img/iconos/save.png"/></td><td>&nbsp;</td><td>Guardar</td></tr></table></button>
...
<div id="divFlotante" class="root" style="cursor:auto; display:none; left:0px; position:absolute; top:0px; z-index:2;">
    ...
    <table border="0" id="tblFactura" width="500">
        <tr>
            <td>
            	<fieldset><legend><span style="color:#990000">Datos Factura</span></legend>
            	<table border="0">
                	<tr>
                	...
                	<tr>
                        <td align="right" class="tituloCampo" >Numero Factura.</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="hddBaseImponible" onkeyup="xajax_asignarDetallesRetencion(document.getElementById('selRetencionISLR').value,'SI');" name="hddBaseImponible" /></td>
                    </tr>
                    <tr>
                    	<td align="right" class="tituloCampo" >Monto a Pagar</td>
                        <td><input type="text" id="txtMontoAPagar" name="txtMontoAPagar" size="20" style="text-align:right" onkeypress="return validarSoloNumerosRealesTesoreria(event);" onblur="validarMonto(); calcularRetencionMano();" onfocus="$('btnAceptar').disabled = '';"/></td>
                    </tr>
                    <tr>
...
<div id="divFlotanteDoc" class="root" style="position:absolute; cursor:auto; display:none; left:0px; top:0px; z-index:1; ">
	<div id="divFlotanteTituloDoc" class="handle"><table><tr><td id="tdFlotanteTituloDoc" width="100%"></td></tr></table></div>
  	<table border="0" id="tblFacturasNcargos" width="600px">
    <tr id="trBuscarDocumento">
    	<td>
        	<form id="frmBuscarDocumento" name="frmBuscarDocumento" onsubmit="$('btnBuscarDocumento').click(); return false;" style="margin:0">
            	<table>
                <tr>
                	<td align="right" class="tituloCampo" width="115">Num Documento:</td>
                	<td>
                        <input type="hidden" id="buscarFact" name="buscarFact" value = "1"/>
                    	<input type="text" id="txtCriterioBusq" name="txtCriterioBusq" onkeyup="$('btnBuscarDocumento').click();"/>
					</td>
                    <td><input type="button" id="btnBuscarDocumento" name="btnBuscarDocumento" onclick="xajax_buscarDocumento($('hddIdEmpresa').value,$('hddIdProveedorCabecera').value, '', '',xajax.getFormValues('frmBuscarDocumento'),xajax.getFormValues('frmBuscar'));" value="Buscar..."/></td>
                </tr>
                </table>
            </form>
        </td>
    	...
    	<td>
        	<form id="frmBuscarCliente" name="frmBuscarCliente" onsubmit="$('btnBuscarCliente').click(); return false;" style="margin:0">
                ...
                <tr>
                	...
                	<td>
                    	<input type="text" id="txtCriterioBusqProveedor" name="txtCriterioBusqProveedor" onkeyup="$('btnBuscarCliente').click();"/>
					</td>
...
<script>
//xajax_listarFacturas('','','','',xajax.getFormValues('frmBuscar'));
xajax_comboRetencionISLR();
$('hddIdPropuesta').value = <?php echo $_GET['id_propuesta']; ?>;
	
	if (<?php echo $_GET['id_propuesta']; ?> != 0){
...
<script language="javascript">
	...
	Drag.init(theHandle, theRoot);
	var theHandle = document.getElementById("divFlotanteTituloProv");
	var theRoot   = document.getElementById("divFlotanteProv");
	Drag.init(theHandle, theRoot);
	var theHandle = document.getElementById("divFlotanteTituloDoc");
	var theRoot   = document.getElementById("divFlotanteDoc");
	Drag.init(theHandle, theRoot);
</script>