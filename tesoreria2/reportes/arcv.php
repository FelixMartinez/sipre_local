...
$query = sprintf("SELECT *  FROM vw_te_retencion_cheque WHERE id_proveedor = %s AND anulado IS NULL",$proveedor);
		...
		for ($mes = 1; $mes <= 12; $mes++) {
			...
			if(fmod($mes,2)== 0){
				...
				$queryRetencion = sprintf("SELECT 
								...
								WHERE (fecha_registro >= '".$ano."-".$mes."-01' AND fecha_registro <= '".$ano."-".$mes."-31' )
									AND id_proveedor = %s AND anulado IS NULL
								GROUP BY porcentaje_retencion", $proveedor);
