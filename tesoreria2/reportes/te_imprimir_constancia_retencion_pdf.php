...
$idCheque = $_GET['id'];
$tipoDocumento = $_GET['documento']; //1 cheque, 2 transferencia, 3 retencion, envia el id dependiendo del documento cheque/transf para buscar todas las retenciones, id retencion una sola retencino (reimpresion)
$contador = 0;
...
while ($row = mysql_fetch_assoc($rs)){
    ...
    if($row['tipo_documento'] == 0){//CHEQUE
        $queryVerificarMonto = sprintf("SELECT monto_pagar 
                                        ...
                                        WHERE id_factura = %s AND id_cheque = %s AND tipo_documento = %s",
                                valTpDato($row['id_factura'],'int'),
                                valTpDato($row['id_cheque'],"int"), 
                                valTpDato($row['tipo'],"int"));
    }elseif($row['tipo_documento'] == 1){//TRANSFERENCIA         
        $queryVerificarMonto = sprintf("SELECT monto_pagar
