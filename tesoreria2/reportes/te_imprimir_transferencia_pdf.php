...
$query = sprintf("SELECT 
  ...
  (case when(te_transferencia.beneficiario_proveedor = 0) then(select te_beneficiarios.nombre_beneficiario from te_beneficiarios where(te_transferencia.id_beneficiario_proveedor = te_beneficiarios.id_beneficiario)) when(te_transferencia.beneficiario_proveedor = 1) then(select cp_proveedor.nombre from cp_proveedor where(te_transferencia.id_beneficiario_proveedor = cp_proveedor.id_proveedor)) end) AS nombre,
...
imagestring($img,1,430,90,": ".date("d-m-Y", strtotime($row['fecha_registro'])),$textColor);
...
imagestring($img,1,0,390,"-------------------------------------------------------------------------------------------------------------------",$textColor);
	...
	$r = imagepng($img,"../img/tmp/"."comprobante_transferencia".$contador.'.png');
	$contador++;
}
...
for ($i = 0;$i < $contador;$i++){
	...
	$pdf->Image("../img/tmp/comprobante_transferencia".$i.".png", 15, 55, 580, 680);
