...
$rowConAnt = mysql_fetch_array($rsConAnt);
	...
	if ($idEmpresa != "") {
		...
		$pdf->Cell(562,16,"RESUMEN CONCILIACION  ".date("d-m-Y",strtotime($rowConciliacion['fecha']))."",0,0,'C');
		$pdf->Ln();
		...
		$pdf->Cell(45,12,date("d-m-Y",$fecha),0,0,'C');
		$pdf->Ln();
		$pdf->Cell('',3);
		$pdf->Ln();
...
/*		$pdf->SetFillColor(234,244,255);
		...
		$pdf->Cell(50,14,date("d-m-Y",strtotime($row['fecha_registro'])),'LR',0,'C',true);
...
if ($totalRows > 0) {
	...
	while ($row = mysql_fetch_assoc($rs)) {
		...
		$pdf->Cell($arrayTamCol[2],14,date("d-m-Y",strtotime($row['fecha_registro'])),'LR',0,'C',true);
		...
		$pdf->Cell($arrayTamCol[5],14,$row['id_estado_cuenta'],'LR',0,'C',true);
