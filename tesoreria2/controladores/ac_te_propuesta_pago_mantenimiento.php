<?php
function aprobarPropuesta($formPropuesta,$aux){//ojo no es idpropuesta sino un FORMULARIO
...
/*	if ($rowMonto['monto'] > $rowPropuesta['saldo_tem'] && $aux == 1){
		$objResponse->script("document.getElementById('txtClaveAprobacion').className = 'inputInicial';");
		
		$objResponse->script("if (confirm('El monto del cheque es mayor que el saldo en banco ".utf8_encode("�")."Desea Sobregirar la Cuenta?') == true){
				document.getElementById('divFlotante').style.display = '';
				centrarDiv(document.getElementById('divFlotante'));
				document.getElementById('tdFlotanteTitulo').innerHTML = 'Aprobaci".utf8_encode("�")."n';
				document.getElementById('tblClaveAprobacion').style.display = '';
			}");
		
		$objResponse->assign("txtIdPropuesta","value",$formPropuesta['hddIdPropuestaA']);
		$objResponse->assign("hddIdPropuesta","value",$formPropuesta['hddIdPropuestaA']);
		$objResponse->assign("txtClaveAprobacion","value","");
		return $objResponse;
	}*/
	...
	$rowChequera = mysql_fetch_array($rsChequera);
        ...
        //if(trim($formPropuesta["numeroChequeManual"]) != ""){  
            //$numeroChequeGenerado = $formPropuesta["numeroChequeManual"];
        //}else{
	...
	$queryUpdate = sprintf("UPDATE te_propuesta_pago SET estatus_propuesta = 1, id_cheque = %s WHERE id_propuesta_pago = %s",$idCheque,$formPropuesta['hddIdPropuestaA']);
	...
	$saldoTem = $rowCuenta['saldo_tem'] - $rowPropuesta['monto_pagar'];
	$Diferido=  $rowCuenta['Diferido'] - $rowPropuesta['monto_pagar'];
	$updateCuenta = sprintf("UPDATE cuentas SET saldo_tem = '%s', Diferido = '%s' WHERE idCuentas = %s ;",$saldoTem,$Diferido, $rowChequera['id_cuenta']);
	
	if (!$updateCuenta) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
	...
	while ($rowPropuestaDetalle = mysql_fetch_array($rsPropuestaDetalle)){
            ...
            if (!$consultaCpPago){ errorGuardarDcto($objResponse); return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
                ...
                if($rowPropuestaDetalle['tipo_documento']==0){//FACTURA
                        ...
                        $queryActualizaSaldo = sprintf("UPDATE cp_notadecargo SET saldo_notacargo = '%s', estatus_notacargo = '%s'  WHERE id_notacargo = '%s'",0,1,$rowPropuestaDetalle['id_factura']);
                        $rsActualizaSaldo = mysql_query($queryActualizaSaldo);
                        if (!$rsActualizaSaldo) { errorGuardarDcto($objResponse); return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
...
function listarPropuestas($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	while ($rowPropuesta = mysql_fetch_assoc($rsLimitPropuesta)){
		...
		$htmlTb.= "<tr align=\"left\" class=\"".$clase."\" onmouseover=\"this.className='trSobre';\" onmouseout=\"this.className='".$clase."';\" height=\"24\">";
			$htmlTb .= "<td align=\"center\">".$rowPropuesta['id_propuesta_pago']."</td>";
			$htmlTb .= "<td align=\"center\">".date("d-m-Y",strtotime($rowPropuesta['fecha_propuesta_pago']))."</td>";
			$htmlTb .= "<td align=\"center\">".proveedor($rowPropuesta['id_proveedor'])."</td>";
			$htmlTb .= "<td align=\"center\">".utf8_encode($rowPropuesta['nombreBanco'])."</td>";
			$htmlTb .= "<td align=\"center\">".$rowPropuesta['numeroCuentaCompania']."</td>";
...
function listarProveedores($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL){
	...
	//$queryProveedor = "SELECT * FROM cp_proveedor";
	$rsProveedor = mysql_query($queryProveedor);
        if (!$rsProveedor) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
	
	//$queryLimitProveedor = $queryProveedor." LIMIT ".$maxRows." OFFSET ".$startRow.";";
...
function verificarClave($valForm){
	...
	$queryClave = sprintf("SELECT contrasena FROM vw_pg_claves_modulos WHERE id_usuario = %s AND id_clave_modulo = 33",valTpDato($_SESSION['idUsuarioSysGts'],'int'));
...
function CuentaBanco($clave,$id){
  ...
  INNER JOIN bancos ON (cuentas.idBanco = bancos.idBanco) WHERE  te_chequeras.id_chq = '%s'",$id);
	...
	if ($clave == 1){$respuesta = utf8_encode($row['nombreBanco']);}
