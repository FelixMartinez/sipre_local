<?php 
function listarChequeAnulado($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	$startRow = $pageNum * $maxRows;
		...
		$objResponse->assign("tdReferenciaPagina","innerHTML","Cheques Anulados");		
		$colspan = 12;
	...
	if ($valCadBusq[3] != "" && $valCadBusq[4] != ""){
		if ($cadenaFiltro == ""){
			$cadenaFiltro = sprintf(" WHERE fecha_cheque BETWEEN '%s' AND '%s'",
                                date("Y-m-d",strtotime($valCadBusq[3])),
                                date("Y-m-d",strtotime($valCadBusq[4])));
                }else{
			$cadenaFiltro .= sprintf(" AND fecha_cheque BETWEEN '%s' AND '%s'",
                                date("Y-m-d",strtotime($valCadBusq[3])),
                                date("Y-m-d",strtotime($valCadBusq[4])));
                }
	}
	...
	$query = sprintf("SELECT * FROM te_cheques_anulados
	...
	$htmlTblIni .= "<table border=\"0\" width=\"100%\">";
        ...
        $htmlTh .= "<tr class=\"tituloColumna\">";
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "10%", $pageNum, "id_empresa", $campOrd, $tpOrd, $valBusq, $maxRows, "Empresa");
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "10%", $pageNum, "nombre", $campOrd, $tpOrd, $valBusq, $maxRows, "Proveedor");
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "6%", $pageNum, "numero_cheque", $campOrd, $tpOrd, $valBusq, $maxRows, "N&uacute;mero Cheque");			
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "8%", $pageNum, "monto_cheque", $campOrd, $tpOrd, $valBusq, $maxRows, "Monto Cheque");
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "7%", $pageNum, "fecha_cheque", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Cheque");			
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "7%", $pageNum, "fecha_registro", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Anulado");
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "40%", $pageNum, "concepto", $campOrd, $tpOrd, $valBusq, $maxRows, "Concepto");			
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "15%", $pageNum, "", $campOrd, $tpOrd, $valBusq, $maxRows, "Banco Compa&ntilde;ia");			
            $htmlTh .= ordenarCampo("xajax_listarChequeAnulado", "15%", $pageNum, "", $campOrd, $tpOrd, $valBusq, $maxRows, "Cuenta Compa&ntilde;ia");			
            $htmlTh .= "<td></td>";
	$htmlTh .= "</tr>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\">";
			...
			$htmlTb .= "<td align=\"center\">".date("d-m-Y",strtotime($row['fecha_cheque']))."</td>";
			$htmlTb .= "<td align=\"center\">".date("d-m-Y",strtotime($row['fecha_registro']))."</td>";
			$htmlTb .= "<td align=\"center\">".utf8_encode($row['concepto'])."</td>";
			$htmlTb .= "<td align=\"center\">".banco($row['id_chequera'])."</td>";
			$htmlTb .= "<td align=\"center\">".cuenta($row['id_chequera'])."</td>";
			...
			$htmlTf .= "<tr class=\"tituloCampo\">";
				...
				$htmlTf .= "<td align=\"center\" class=\"tituloColumna\" width=\"210\">";
					...
					$htmlTf .= "<tr align=\"center\">";
						$htmlTf .= "<td width=\"25\">";
						if ($pageNum > 0) {
							$htmlTf .= sprintf("<a class=\"puntero\" onclick=\"xajax_listarChequeAnulado(%s,'%s','%s','%s',%s);\">%s</a>",
						...
						$htmlTf .= "<td width=\"100\">";
							...
							$htmlTf .= sprintf("<select id=\"pageNum\" name=\"pageNum\" onchange=\"xajax_listarChequeAnulado(%s,'%s','%s','%s',%s)\">",
...
function buscarCheque($valForm) {
	...
	$objResponse->script(sprintf("xajax_listarChequeAnulado(0,'fecha_registro','DESC','%s|%s|%s|%s|%s|%s|%s');",
		$valForm['selEmpresa'],
		$valForm['selEstado'],
		$valForm['txtBusq'],
		$valForm['txtFecha'],
...
function banco($id){
	...
	$query = sprintf("SELECT te_chequeras.id_chq, te_chequeras.id_cuenta, cuentas.idCuentas, cuentas.idBanco, bancos.idBanco, bancos.nombreBanco FROM cuentas INNER JOIN te_chequeras ON ( cuentas.idCuentas = te_chequeras.id_cuenta ) INNER JOIN bancos ON ( bancos.idBanco = cuentas.idBanco ) WHERE te_chequeras.id_chq = '%s'",$id);
	$rs = mysql_query($query) or die(mysql_error());
	$row = mysql_fetch_array($rs);
...
function cuenta($id){
	...
	$query = sprintf("SELECT te_chequeras.id_chq, te_chequeras.id_cuenta, cuentas.idCuentas, cuentas.numeroCuentaCompania FROM cuentas INNER JOIN te_chequeras ON (cuentas.idCuentas = te_chequeras.id_cuenta) WHERE te_chequeras.id_chq = '%s'",$id);
	$rs = mysql_query($query) or die(mysql_error());
	$row = mysql_fetch_array($rs);
...
/**
 * Carga informacion referente a la propuesta de un cheque
 * @param int $idPropuesta Id de la propuesta del cheque
 * @return \xajaxResponse Llena informacion en la vista
 */
...
$xajax->register(XAJAX_FUNCTION,"listarChequeAnulado");
