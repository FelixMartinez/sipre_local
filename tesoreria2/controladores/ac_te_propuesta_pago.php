...
function asignarFactura($valForm,$idFactura,$tipoDocumento){
	...
	if ($montoPagar < 1){
		$queryMontoPagarFacturaPropuesta = sprintf("SELECT monto_pagar, monto_retenido FROM te_propuesta_pago_detalle WHERE id_factura = %s AND id_propuesta_pago = %s",valTpDato($idFactura,"int"),valTpDato($_GET['id_propuesta'],"int"));
	...
	if($tipoDocumento==0){//factura
            ...
            $objResponse->script(sprintf("
                    ...
                    $idFactura, utf8_encode($rowFactura['observacion_factura']),
...
/////////////
	...
	//$objResponse->assign("txtDiferido","value",number_format($monto_propuesta,"2",".",","));	
...
SELECT te_propuesta_pago_detalle_transferencia.id_factura FROM te_propuesta_pago_detalle_transferencia WHERE te_propuesta_pago_detalle_transferencia.tipo_documento <> 1)%s %s %s",
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr class=\"".$clase."\" onmouseover=\"$(this).className = 'trSobre';\" onmouseout=\"$(this).className = '".$clase."';\">";
			$htmlTb .= "<td align='center'>"."<button type=\"button\" onclick=\"validarChequeraSeleccionada(".$row['id_factura'].",0);\" title=\"Seleccionar Factura\"><img src=\"../img/iconos/ico_aceptar.gif\"/></button>"."</td>";
...
SELECT te_propuesta_pago_detalle_transferencia.id_factura FROM te_propuesta_pago_detalle_transferencia WHERE te_propuesta_pago_detalle_transferencia.tipo_documento <> 0) %s %s",
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$contFila++;
				...
				$htmlTb .= "<tr class=\"".$clase."\" onmouseover=\"$(this).className = 'trSobre';\" onmouseout=\"$(this).className = '".$clase."';\">";
			$htmlTb .= "<td align='center'>"."<button type=\"button\" onclick=\"validarChequeraSeleccionada(".$row['id_notacargo'].",1);\" title=\"Seleccionar Nota Cargo\"><img src=\"../img/iconos/ico_aceptar.gif\"/></button>"."</td>";
