...
function anularTransferencia($idTrans, $borrarISLR = "NO"){
	...
	if ($rowTransferencia['id_documento']==0){//si es cero es porque puede tener propuestas de pago
		...
		$queryFacturasPropuesta = sprintf("SELECT 
                    ...
                    FROM
                    te_propuesta_pago_transferencia
		...
		if($rowTransferencia['tipo_documento']==0){//si es factura
                        $queryFacturaSaldo = sprintf("SELECT saldo_factura FROM cp_factura WHERE id_factura = %s;",$rowTransferencia['id_documento']);
                        $rsFacturaSaldo  = mysql_query($queryFacturaSaldo);
                        if (!$rsFacturaSaldo) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
                        $rowFacturaSaldo  = mysql_fetch_array($rsFacturaSaldo );

                        $queryRetenido = sprintf("SELECT * FROM te_retencion_cheque WHERE id_factura = %s AND id_cheque = %s;",$rowTransferencia['id_documento'], $rowTransferencia['id_transferencia']);
			...
			if (!$rsUpdateFactura) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
                    ...
                    $queryNotaCargo = sprintf("SELECT saldo_notacargo FROM cp_notadecargo WHERE id_notacargo = %s;",$rowTransferencia['id_documento']);
                    $rsNotaCargo  = mysql_query($queryNotaCargo);
                    if (!$rsNotaCargo) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
                    $rowNotaCargo  = mysql_fetch_array($rsNotaCargo);
	...
	$rsTransferenciaAnulada = mysql_query($queryTransferenciaAnulada);
        ...
        $idTransferenciaAnulada = mysql_insert_id();//Debe usarlo Ernesto en contabilidad para devovler!!!
...
function asignarFactura($idFactura){
	...
	$objResponse = new xajaxResponse();
        ...
        $objResponse->script("document.getElementById('divFlotante1').style.display = 'none';
                            document.getElementById('selRetencionISLR').disabled = '';
                            calcularRetencion();");
...
function comboEstado(){
	...
	$query = sprintf("SELECT * FROM te_estados_principales ORDER BY id_estados_principales");
		$rs = mysql_query($query);
                if(!$rs) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
		$html = "<select id=\"selEstado\" name=\"selEstado\" onChange=\"xajax_buscarTransferencia(xajax.getFormValues('frmBuscar'))\">";
...
function guardarTransferencia($valForm){
	...
	if ($valForm['hddTipoDocumento']==0){
        ...
        $queryCpPago = sprintf("INSERT INTO cp_pagos_documentos(id_documento_pago,tipo_documento_pago, tipo_pago, id_documento, fecha_pago, numero_documento, banco_proveedor, banco_compania, cuenta_proveedor, cuenta_compania, monto_cancelado, id_empleado_creador)
                        VALUE('%s', '%s', 'Transferencia', '%s', NOW(), '%s', '%s', '%s', '%s', '%s', '%s', %s)",
                        $valForm['txtIdFactura'],
                        $tipoDocumento,
                        $idTransferencia,
                        $valForm['numTransferencia'],
	...
	if ($valForm['txtIdFactura'] != ''){
            ...
            if ($valForm['txtMontoRetencionISLR'] != 0){
                    $queryRetencion = sprintf("INSERT INTO te_retencion_cheque (id_factura, id_cheque, id_retencion, base_imponible_retencion, sustraendo_retencion, porcentaje_retencion, monto_retenido, codigo, tipo_documento, tipo)
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		if ($_GET['acc'] == 3){
			$htmlTb .= "<td align='center' class=\"puntero\" title=\"Anular Transferencia\">"."<img src=\"../img/iconos/ico_quitar.gif\" onclick=\"if (confirm('Desea anular la Transferencia ".$row['numero_transferencia']."?') == true){
				document.getElementById('divFlotanteAnular').style.display = '';
				centrarDiv(document.getElementById('divFlotanteAnular'));
				document.getElementById('tdFlotanteTitulo').innerHTML = 'Eliminar Transferencia';
				document.getElementById('txtNumTransferencia').value = '".$row['numero_transferencia']."';
				document.getElementById('hddIdTransferencia').value = '".$row['id_transferencia']."';}\"/>"."</td>";
...
function verificarClaveAnular($valForm){
	...
	$queryClave = sprintf("SELECT contrasena FROM vw_pg_claves_modulos WHERE id_usuario = %s AND id_clave_modulo = 47",valTpDato($_SESSION['idUsuarioSysGts'],'int'));
	$rsClave = mysql_query($queryClave);
	...
	if (mysql_num_rows($rsClave)){
		...
		if ($rowClave['contrasena'] == $valForm['txtClaveAnular']){
			...
			$objResponse->script("document.getElementById('divFlotanteAnular').style.display = 'none';");
		}
		else
			$objResponse->alert("Clave Errada.");
	}
...
function verPropuesta($idPropuesta){    
    ...
    $queryPropuesta = sprintf("SELECT te_propuesta_pago_transferencia.fecha_propuesta_pago, 
...
FROM
  cuentas
  INNER JOIN bancos ON (cuentas.idBanco = bancos.idBanco) WHERE   cuentas.idCuentas = '%s'",$id);
	$rs = mysql_query($query) or die(mysql_error());
	$row = mysql_fetch_array($rs);
