...
WHERE te_estado_cuenta.id_cuenta = '%s' AND te_estado_cuenta.tipo_documento = 'CH' AND (te_estado_cuenta.estados_principales = '1' or te_estado_cuenta.estados_principales = '2')",$valCadBusq[0]);
	...
	while ($rowChequera = mysql_fetch_assoc($rsLimitChequera)) {
		...
		$rowProveedor = mysql_fetch_array($rsProveedor);
			...
			$htmlTb .= "<tr class=\"".$clase."\">";
			$htmlTb .= "<td>".$rowChequera['numero_documento']."</td>";
			$htmlTb .= "<td>".utf8_encode($rowBeneficiario['nombre_beneficiario'])."</td>";
			$htmlTb .= "<td>".$rowChequera['monto']."</td>";
			$htmlTb .= "<td>".$entregado."</td>";
			$htmlTb .= "</tr>";	
		}			
...
WHERE te_estado_cuenta.id_cuenta = '%s' AND te_estado_cuenta.tipo_documento = 'TR' AND (te_estado_cuenta.estados_principales = '1' or te_estado_cuenta.estados_principales = '2')",$valCadBusq[0]);
...
WHERE te_estado_cuenta.id_cuenta = '%s' AND te_estado_cuenta.tipo_documento = 'ND' AND (te_estado_cuenta.estados_principales = '1' or te_estado_cuenta.estados_principales = '2')",$valCadBusq[0]);
	...
	while ($rowChequera = mysql_fetch_assoc($rsLimitChequera)) {
		...
		if($rowChequera['control_beneficiario_proveedor'] == 1 ){
		...
		else{ if($rowChequera['control_beneficiario_proveedor'] == 0 ){
...
WHERE te_estado_cuenta.id_cuenta = '%s' AND te_estado_cuenta.tipo_documento = 'NC' AND (te_estado_cuenta.estados_principales = '1' or te_estado_cuenta.estados_principales = '2')",$valCadBusq[0]);
...
function listarDeposito($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL){
	...
	$queryChequeras = sprintf("SELECT * FROM te_estado_cuenta WHERE id_cuenta = '%s' AND tipo_documento = 'DP' AND (estados_principales = '1' or estados_principales = '2')",$valCadBusq[0]);
	$rsChequeras = mysql_query($queryChequeras) or die(mysql_error());
...
function cargarDatosCuenta($id){
	...
	//$montoTotal = ($countTotalDeposito+$countTotalCredito+$rowCuenta['saldo'])-($countTotalCheques + $countTotalDebito);
	$montoTotal = ($countTotalDeposito+$countTotalCredito)-($countTotalCheques + $countTotalDebito + $countTotalTransferencia);
	//$objResponse -> alert($montoTotal);
