...
function asignarBanco($idBanco, $valores = "") {
	...
	if ($totalRowsBanco > 0) {
		$select = "<select id=\"lstMesesFinanciar\" name=\"lstMesesFinanciar\" class=\"inputHabilitado\" onchange=\"percent();\">";
		...
		$select = ($totalRowsFactor > 0) ? $select : 
	...
	/*$objResponse->assign("monto_seguro","value",numformat(floatval($rowBanco['porcentaje_flat']),2,'.',','));
	$objResponse->assign("inicial_poliza","value",numformat(floatval($rowBanco[1]),2,'.',','));*/
	...
	$objResponse->assign("capameses_financiar","innerHTML",$select);
	if ($totalRowsBanco > 0) {
...
function asignarPoliza($idPoliza) {
	...
	$objResponse->assign("monto_seguro","value",number_format($rowPoliza['contado_poliza'], 2, ".", ","));
	$objResponse->assign("inicial_poliza","value",number_format($rowPoliza['inicial_poliza'], 2, ".", ","));
	$objResponse->assign("cuotas_poliza","value",number_format($rowPoliza['cuotas_poliza'], 2, ".", ","));
	$objResponse->assign("meses_poliza","value",$rowPoliza['meses_poliza']);
...
function formPedido($idEmpresa, $numeroPresupuesto, $idPresupuesto, $idPedido, $idFactura, $frmPedido) {
	...
	if (($idPresupuesto > 0 || ($idEmpresa > 0 && $numeroPresupuesto > 0)) && !($idPedido > 0)) {
		...
		$objResponse->loadCommands(cargaLstEmpleado("", "lstGerenteVenta", "2,300"));
		...
		if ($idFactura > 0) {
			...
			$html .= "<tr align=\"center\">";
				$html .= "<td height=\"25\" width=\"25\"><img src=\"../img/iconos/exclamation.png\"/></td>";
				$html .= "<td>"."Edición de la Factura Nro. ".$rowFactura['numeroFactura']."</td>";
			$html .= "</tr>";
			$html .= "</table>";
		...
		$objResponse->assign("observaciones","innerHTML",$rowPedido['observacion']);
		
		$objResponse->loadCommands(cargaLstEmpleado($rowPedido['gerente_ventas'], "lstGerenteVenta", "2,300"));
		$objResponse->assign("txtFechaVenta","value",date("d-m-Y", strtotime($rowPedido['fecha_gerente_ventas'])));
		$objResponse->loadCommands(cargaLstEmpleado($rowPedido['administracion'], "lstGerenteAdministracion", "3"));
		$objResponse->assign("txtFechaAdministracion","value",date("d-m-Y", strtotime($rowPedido['fecha_administracion'])));
		
		$objResponse->assign("txtFechaReserva","value",date("d-m-Y", strtotime($rowPedido['fecha_reserva_venta'])));
		$objResponse->assign("txtFechaEntrega","value",date("d-m-Y", strtotime($rowPedido['fecha_entrega'])));
		
		$objResponse->assign("txtPrecioRetoma","value",number_format($rowPedido['precio_retoma'], 2, ".", ","));
		$objResponse->assign("txtFechaRetoma","value",date("d-m-Y", strtotime($rowPedido['fecha_retoma'])));
