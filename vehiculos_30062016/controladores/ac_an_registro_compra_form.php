	...
	switch ($idModoCompra) {
		...
		case 2 : // 2 = Importacion
			...
			if ($rowMonedaLocal['incluir_impuestos'] == 1 && $frmDcto['lstNacionalizar'] == 1) { // 0 = No, 1 = Si
				...
				if (!$rsIva) { errorGuardarDcto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
...
function listaAdicional($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$query = sprintf("SELECT acc.*,
		(CASE acc.iva_accesorio
			WHEN 1 THEN
				(SELECT iva FROM pg_iva WHERE tipo = 1 AND estado = 1 AND activo = 1 ORDER BY iva)
			ELSE
				'-'
		END) AS iva
...
function guardarNotaCreditoPago($idFactura, $idNotaCargo, $idMotivo, $idRetencionCabezera = "", $porcIva = "", $porcRetencion = "", $ivaRetenido = "") {
	...
	if ($idFactura > 0 && !($idNotaCargo > 0) && $idRetencionCabezera > 0) {
		...
		if ($idRetencionCabezera > 0) {
			...
			$observacionNC = "NOTA DE CREDITO POR PLAN MAYOR, CUENTA POR PAGAR REFLEJADA EN LA NOTA DE CARGO NRO. ".$row['numero_notacargo'];
			$subTotalNC = $rowFactura['subtotal_factura'];
...
function insertarItemAdicional($contFila, $hddIdAccesorioItm, $idFacturaDetalle = "") {
	...
	if ($idFacturaDetalle > 0) {
		...
		WHERE tipo = 1 AND estado = 1 AND activo = 1
...
function insertarItemOtroCargo($contFila, $hddIdGastoCargo = "", $idFacturaCompraGasto = "") {
	...
	$txtFechaFacturaGasto = ($txtFechaFacturaGasto == "" && $totalRows > 0) ? implode("-", array_reverse(explode("-", $row['fecha_origen']))) : $txtFechaFacturaGasto;
...
function insertarItemUnidad($contFila, $idPedidoDetalle, $idFacturaDetalle = "") {
	...
	$descripUnidad .= "<tr>";
		...
		$descripUnidad .= sprintf("<td><input type=\"text\" id=\"txtSerialCarroceria%s\" name=\"txtSerialCarroceria%s\" class=\"inputHabilitado\" style=\"text-align:left\" value=\"%s\"></td>", 
			$contFila, $contFila, utf8_encode($rowPedidoDet['serial_carroceria']));
	...
	$descripUnidad .= "<tr>";
		...
		$descripUnidad .= "<td align=\"right\" class=\"tituloCampo\"><span class=\"textoRojoNegrita\">*</span>Fecha Fabricación:</td>";
			...
			$contFila, $contFila, implode("-",array_reverse(explode("-",$rowPedidoDet['fecha_fabricacion']))));
	...
	$descripUnidad .= "</tr>";
	$descripUnidad .= "<tr>";
		$descripUnidad .= "<td align=\"right\" class=\"tituloCampo\"><span class=\"textoRojoNegrita\">*</span>Registro Legalización:</td>";
	...
	if ($rowPedidoDet['id_combustible'] == 2 || $rowPedidoDet['id_combustible'] == 5) {
		...
		$descripUnidad .= "<tr>";
			...
			$descripUnidad .= sprintf("<td><input type=\"text\" id=\"txtFechaCilindro%s\" name=\"txtFechaCilindro%s\" class=\"inputHabilitado\" value=\"%s\"></td>", 
				$contFila, $contFila, implode("-",array_reverse(explode("-",$rowPedidoDet['fecha_elaboracion_cilindro']))));
		$descripUnidad .= "</tr>";
	}
	$descripUnidad .= "</table>";
	...
	$htmlItmPie = sprintf("$('#trItmPie').before('".
		...
		jQuery(function($){
			$(\"#txtFechaFabricacion%s\").maskInput(\"99-99-9999\",{placeholder:\" \"});
		});
