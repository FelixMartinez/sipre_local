...
function cargaLstVendedor($selId = "") {
	...
	$html = "<select id=\"lstEmpleado\" name=\"lstEmpleado\" class=\"inputHabilitado\" onchange=\"byId('btnBuscar').click();\" style=\"width:99%\">";
...
function listaFacturaVenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	if ($valCadBusq[1] != "" && $valCadBusq[2] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		if ($valCadBusq[3] == "2") {
			$sqlBusq .= $cond.sprintf("ped_vent.fecha_entrega BETWEEN %s AND %s",
	...
	FROM cj_cc_factura_detalle_accesorios cxc_fact_det_acc
		...
		INNER JOIN an_pedido ped_vent ON (cxc_fact.numeroPedido = ped_vent.id_pedido) %s", $sqlBusq);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				...
				$htmlTb .= "</tr>";
				$htmlTb .= "<tr>";
					$htmlTb .= "<td>"."<b>Fecha Entrega:</b> ".date("d-m-Y", strtotime($row['fecha_entrega']))."</td>";
				$htmlTb .= "</tr>";
				$htmlTb .= "</table>";
			$htmlTb .= "</td>";
