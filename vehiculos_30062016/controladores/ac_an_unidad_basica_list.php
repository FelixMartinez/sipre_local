...
function buscarUnidadBasica($frmBuscar) {
	...
	$valBusq = sprintf("%s|%s|%s|%s|%s|%s|%s",
		...
		implode(",",$frmBuscar['lstAnoBuscar']),
...
function cargaLstAnoBuscar($selId = "") {
	...
	$html = "<select ".(($totalRows > 2) ? "multiple": "")." id=\"lstAnoBuscar\" name=\"lstAnoBuscar\" class=\"inputHabilitado\" onchange=\"byId('btnBuscar').click();\" style=\"width:99%\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
