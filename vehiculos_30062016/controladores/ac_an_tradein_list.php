...
function formEditarTradeIn($idTradeIn) {
	...
	$objResponse->loadCommands(asignarCliente($row['id'], $idEmpresa));
...
function generarNuevoAnticipo($frmAjusteInventario, $esNuevo = 0){
	...
	if ($esNuevo) { // Viene de vehiculo ya registrado
        ...
        $idUnidadFisica = $frmAjusteInventario['txtIdUnidadFisicaAjuste'];
        $allowance = $frmAjusteInventario['txtAllowance'];
        $payoff = $frmAjusteInventario['txtPayoff'];
        $acv = $frmAjusteInventario['txtAcv'];
        $totalCredito = $frmAjusteInventario['txtCreditoNeto'];
        ...
        $allowance = $frmAjusteInventario['txtAllowance_2'];
        $payoff = $frmAjusteInventario['txtPayoff_2'];
        $acv = $frmAjusteInventario['txtAcv_2'];
        $totalCredito = $frmAjusteInventario['txtCreditoNeto_2'];
	...
	VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
		...
		valTpDato($totalCredito, "real_inglesa"),
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		while ($rowUnidadFisica = mysql_fetch_assoc($rsUnidadFisica)) {
			...
			$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
				...
				$htmlTb .= "</td>";
				$htmlTb .= "<td>";
				if (!in_array($rowUnidadFisica['estado_venta'],array("VENDIDO", "ENTREGADO"))) {
