...
function anularUnidad($frmAnularUnidad, $frmPedido, $frmListaPedidoCompra) {
	...
	if ($row['id_proveedor_plan_mayor'] > 0) {
		...
		$queryNotaCargo = sprintf("SELECT * FROM cp_notadecargo nota_cargo
		...
		if ($rowNotaCargo['saldo_notacargo'] > 0) {
			...
			VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
				...
				valTpDato("CANCELACION DE LA NOTA DE CARGO Nro.".$numeroNotaCargo." DE VEHICULO POR PLAN MAYOR, POR DEVOLUCION DE LA UNIDAD", "text"),
			...
			VALUE (%s, %s, %s, %s);",
				...
				valTpDato(date("Y-m-d"), "date"),
				valTpDato("4", "int")); // 1 = Factura, 2 = Nota Débito o Cargo, 3 = Anticipo, 4 = Nota Crédito
			mysql_query("SET NAMES 'utf8';");
			$Result1 = mysql_query($insertSQL);
			if (!$Result1) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			mysql_query("SET NAMES 'latin1';");
				...
				foreach ($arrayObj1 as $indice1 => $valor1) {
					...
					if ($valor1[0] == $valor) {
						...
						if ($idFormaPago > 4) {
							...
							$idModulo = 2; // 0 = Repuestos, 1 = Sevicios, 2 = Vehiculos, 3 = Administracion
							$diasCredito = $rowProv['diascredito'];
							...
							VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
								...
								valTpDato(date("Y-m-d"), "date"),
								valTpDato(suma_fechas("Y-m-d",date("Y-m-d"),$diasCredito), "date"),
								valTpDato($idModulo, "int"),
								valTpDato(0, "int"), // 0 = No Cancelado, 1 = Cancelado, 2 = Parcialmente Cancelado
								valTpDato("NOTA DE CARGO DE VEHICULO POR PLAN MAYOR", "text"),
...
function listaCarta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td align=\"center\">".date("d-m-Y", strtotime($row['fechaCartaSolicitud']))."</td>";
...
function listaPedidoCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".date("d-m-Y", strtotime($row['fecha_pedido']))."</td>";
...
function listaPedidoDetalle($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna texto_10px\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaPedidoDetalle", "8%", $pageNum, "id_unidad_fisica", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Unidad Física");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			if ($row['estado'] != 6 && $row['estado_compra'] != "REGISTRADO") {
				if (xvalidaAcceso(NULL,"an_registrar_vehiculo_form","insertar")) {
