...
function listaExpediente($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$query = sprintf("SELECT 
		cxp_fact_det_unidad.id_factura,
		cxp_fact.id_empresa,
	...
	FROM cp_factura_detalle_unidad_importacion cxp_fact_det_unidad_imp
		INNER JOIN cp_factura_detalle_unidad cxp_fact_det_unidad ON (cxp_fact_det_unidad_imp.id_factura_detalle_unidad = cxp_fact_det_unidad.id_factura_detalle_unidad)
		...
		INNER JOIN pg_monedas moneda_local ON (cxp_fact.id_moneda = moneda_local.idmoneda) %s
	
	UNION
	
	SELECT 
		cxp_fact_det_acc.id_factura,
		cxp_fact.id_empresa,
		cxp_fact_imp.numero_expediente,
		acc.id_accesorio,
		acc.nom_accesorio,
	...
	FROM cp_factura_detalle_accesorio_importacion cxp_fact_det_acc_imp
		INNER JOIN cp_factura_detalle_accesorio cxp_fact_det_acc ON (cxp_fact_det_acc_imp.id_factura_detalle_accesorio = cxp_fact_det_acc.id_factura_detalle_accesorio)
		...
		INNER JOIN pg_monedas moneda_origen ON (cxp_fact_imp.id_moneda_tasa_cambio = moneda_origen.idmoneda)
		INNER JOIN cp_factura cxp_fact ON (cxp_fact_imp.id_factura = cxp_fact.id_factura)
		INNER JOIN pg_monedas moneda_local ON (cxp_fact.id_moneda = moneda_local.idmoneda) %s", $sqlBusq, $sqlBusq2);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"right\">".number_format($row['costo_unitario'], 3, ".", ",")." ".$abrevMonedaOrigen."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($row['gasto_unitario'], 3, ".", ",")." ".$abrevMonedaOrigen."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($row['costo_cif'], 3, ".", ",")." ".$abrevMonedaOrigen."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($row['tasa_cambio'], 3, ".", ",")." ".$abrevMonedaLocal."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($row['costo_cif_nacional'], 3, ".", ",")." ".$abrevMonedaLocal."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($row['tarifa_adv'], 3, ".", ",")." ".$abrevMonedaLocal."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($row['gastos_import_nac_unitario'], 3, ".", ",")." ".$abrevMonedaLocal."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($row['gastos_import_unitario'], 3, ".", ",")." ".$abrevMonedaLocal."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($row['costo_unitario_final'], 3, ".", ",")." ".$abrevMonedaLocal."</td>";
