...
function cargaLstModulo($selId = ""){
	...
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$totalRows = mysql_num_rows($rs);
	$html = "<select ".(($totalRows > 1) ? "multiple" : "")." id=\"lstModulo\" name=\"lstModulo\" class=\"inputHabilitado\" onchange=\"byId('btnBuscarDocumento').click();\" style=\"width:99%\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function formAjusteInventario($frmUnidadFisica, $existeUnidadFisica) {
	...
	if ($existeUnidadFisica == 1) {
		...
		if ($row['estado_venta'] == "ACTIVO FIJO") {
			...
			$objResponse->assign("txtCostoCompra","value",number_format($row['precio_compra'], 2, ".", ","));
			$objResponse->assign("txtMontoDepreciado","value",number_format($rowDepreciacionActivo['depreciacionacumulada'], 2, ".", ","));
			$txtSubTotal = $row['precio_compra'] - $rowDepreciacionActivo['depreciacionacumulada'];
		} else {
			$txtSubTotal = $row['precio_compra'] - $row['costo_depreciado'];
		}
...
function formListaDocumento() {
	...
	$objResponse->assign("txtFechaDesdeBuscarDocumento","value",date("01-m-Y"));
	$objResponse->assign("txtFechaHastaBuscarDocumento","value",date("d-m-Y"));
...
function listaNotaCreditoCxC($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL, $frmUniFisAgregado){
	...
	while ($row = mysql_fetch_assoc($rsLimit)){
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".((strtotime($row['fechaRegistroFactura'])) ? date("d-m-Y",strtotime($row['fechaRegistroFactura'])) : "")."</td>";
			...
			switch ($row['idDepartamentoNotaCredito']) {
				...
				$htmlTb .= (strlen($aVerDctoAux) > 0) ? "<a href=\"".$aVerDctoAux."\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"".utf8_encode("Nota de Crédito PDF")."\"/></a>" : "";
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
...
function listaUnidadFisica($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL) {
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		$htmlTh .= ordenarCampo("xajax_listaUnidadFisica", "2%", $pageNum, "uni_fis.id_unidad_fisica", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Nro. Unidad Física"));
		...
		$htmlTh .= ordenarCampo("xajax_listaUnidadFisica", "8%", $pageNum, "estado_venta", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Estado Venta"));
		$htmlTh .= ordenarCampo("xajax_listaUnidadFisica", "14%", $pageNum, "alm.id_empresa", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Empresa"));
		...
		while ($rowUnidadFisica = mysql_fetch_assoc($rsUnidadFisica)) {
			...
			$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
				...
				$htmlTb .= "<td align=\"center\">";
					...
					$htmlTb .= ($rowUnidadFisica['dias_inventario'] > 0) ? "<br><span class=\"textoNegrita_9px\">".utf8_encode($rowUnidadFisica['dias_inventario']." días")."</span>" : "";
				$htmlTb .= "</td>";
...
function listaValeSalida($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL, $frmUniFisAgregado) {
	...
	// 12 = VEHICULOS USADOS
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("(filtro_orden.id_filtro_orden IN (12)
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaValeSalida", "8%", $pageNum, "placa", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Placa"));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".date("d-m-Y", strtotime($row['fecha_vale']))."</td>";
			$htmlTb .= "<td align=\"right\">".$row['numero_vale']."</td>";
			$htmlTb .= "<td align=\"center\">".date("d-m-Y", strtotime($row['tiempo_orden']))."</td>";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"javascript:verVentana('../servicios/sa_imprimir_historico_vale.php?valBusq=%s|2|3', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Vale de Salida PDF\"/></a>",
					$row['id_vale_salida']);
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
	}
...
function insertarItemDctoAgregado($contFila, $hddIdUnidadFisicaAgregado = "", $idTipoMovimientoAgregado = "", $tipoDctoAgregado = "", $idDocumento = "") {
	...
	switch ($tipoDctoAgregado) {
		...
		case "NC" :
			if ($idTipoMovimientoAgregado == 2) { // 2 = ENTRADA
				...
				$idModulo = $row['idDepartamentoNotaCredito'];
				$nombreCliente = $row['nombre_cliente'];
				$idMotivo = $row['id_motivo'];
				$descripcionMotivo = $row['descripcion_motivo'];
	...
	if ($totalRowsUniFisAgregado > 0) {
		...
		$empleadoAnuladoUniFisAgregado = (strlen($rowUniFisAgregado['nombre_empleado_anulado']) > 0) ? "<div align=\"center\"><span class=\"texto_9px\">Anulado por:</span> <span class=\"textoNegrita_9px\">".$rowUniFisAgregado['nombre_empleado_anulado']."<br>(".date("d-m-Y",strtotime($rowUniFisAgregado['fecha_anulado'])).")</span></div>" : "";
	}
		...
		case 2 : // 2 = ENTRADA
			switch ($tipoDctoAgregado) {
				...
				case "NC" :
					switch ($idModulo) {
						case 0 : // REPUESTOS
							$aVerDctoAux = sprintf("../repuestos/reportes/iv_devolucion_venta_pdf.php?valBusq=%s", $idDocumento);
							break;
						case 1 : // SERVICIOS
							//if ($rowUniFisAgregado['aplicaLibros'] == 1) {
								$aVerDctoAux = sprintf("../servicios/reportes/sa_devolucion_venta_pdf.php?valBusq=%s", $idDocumento);
							//}
							break;
						case 2 : // VEHICULOS
							$aVerDctoAux = sprintf("../vehiculos/reportes/an_devolucion_venta_pdf.php?valBusq=%s", $idDocumento);
							break;
						case 3 : // ADMINISTRACION
							$aVerDctoAux = sprintf("../repuestos/reportes/ga_devolucion_venta_pdf.php?valBusq=%s", $idDocumento);
							break;
						default : $aVerDctoAux = "";
					}
	...
	$htmlItmPie = sprintf("$('#trItmPie').before('".
		...
		$contFila, $clase,
			...
			date("d-m-Y", strtotime($fechaDcto)),
			$classUniFisAgregado,
				"100%",
					$aVerDcto,
					$imgDctoModulo,
