...
if ($verifica != 52) { //el año esta incompleto, lo crea por semanas:
	for ($mes = 1; $mes <= 12; $mes++) {
		$primerdiames = mktime(0,0,0,$mes,1,$ano);
		//$fecha_actual=$primerdiames;
		for ($i = 1; $i < date('t',$primerdiames); $i += 7) {
		
			$fecha_actual = mktime(0, 0, 0, $mes, $i, $ano);
			
			$existe = "SELECT COUNT(*) FROM an_listado_semanal WHERE ".setmysqlfecha(date("d-m-Y",$fecha_actual))." BETWEEN semana_inicio AND semana_fin;";
			...
			if (intval(getmysql($existe)) == 0) {
				...
				//echo date('d-m-Y',$domingo), ' a ', date('d-m-Y',$sabado).'<br />';//PERFECTO!!!
				$sqlinsert = "INSERT INTO an_listado_semanal(semana_inicio, semana_fin, numero_semana)
				VALUES (".setmysqlfecha(date("d-m-Y",$domingo)).", ".setmysqlfecha(date("d-m-Y",$sabado)).", (date_format(".setmysqlfecha(date("d-m-Y",$domingo)).",'%U')-date_format(".setmysqlfecha(date("d-m-Y",$primerdiames)).",'%U')));";
...
$emeses = array(1 => "ENERO","FEBRERO","MARZO","ABRIL","MAYO","JUNIO","JULIO","AGOSTO","SEPTIEMBRE","OCTUBRE","NOVIEMBRE","DICIEMBRE");
...
<div id="divGeneralPorcentaje">
    ...
    <tr>
        <td class="tituloPaginaVehiculos">Informe de Clientes Nuevos<br/><span class="texto_10px">(Clientes Atendidos Mensualmente <?php echo $emeses[$mes].' '.$ano; ?>)</span></td>
    </tr>
    <tr>
        <td>&nbsp;</td>
    </tr>
    ...
    <tr>
        <td>
            ...
            for ($i = 1; $i <= 12; $i++) {
                ...
                echo "<option ".$selected." value=\"".$i."\">".$emeses[$i]."</option>";
            } ?>
            </select>
        </td>
        <td>
        ...
        <td align="center" width="100%">Reporte Generado el <?php echo date('d-m-Y').' - '.getmysql("select date_format(now(),'%h:%i %p');") ?></td>
    </tr>
    </table>
</form>
	<?php
