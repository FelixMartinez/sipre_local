...
if ($idFactura > 0) {
	validaModulo("cj_facturacion_vehiculos_list",insertar);
	...
	SELECT CONCAT(numeracion_pedido,'(',((SELECT COUNT(an_ped_vent.id_factura_cxc) FROM an_pedido an_ped_vent
										WHERE an_ped_vent.id_empresa = an_pedido.id_empresa
											AND an_ped_vent.id_factura_cxc = ".valTpDato($idFactura, "int").") + 1),')'), ".
