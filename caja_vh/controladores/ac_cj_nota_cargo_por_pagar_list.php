...
function cargaLstOrientacionPDF($selId = "", $bloquearObj = false){
	...
	$onChange = ($bloquearObj == true) ? "onchange=\"selectedOption(this.id,'".$selId."');\"" : "onchange=\"xajax_imprimirNotaCargo(xajax.getFormValues('frmBuscar'));\"";
...
function listaNotaDebito($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "6%", $pageNum, "numeroNotaCargo", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Nro. Nota de Débito"));
		...
		$htmlTh .= ordenarCampo("xajax_listaNotaDebito", "8%", $pageNum, "montoTotalNotaCargo", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Total Nota de Débito"));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				...
				$htmlTb .= "</tr>";
				$htmlTb .= ($row['id_motivo'] > 0) ? "<tr><td class=\"textoNegrita_9px\">".$row['id_motivo'].".- ".utf8_encode($row['descripcion_motivo'])."</td></tr>" : "";
				$htmlTb .= (strlen($row['observacionNotaCargo']) > 0) ? "<tr><td class=\"textoNegritaCursiva_9px\">".utf8_encode($row['observacionNotaCargo'])."</td></tr>" : "";
				$htmlTb .= "</table>";
			$htmlTb .= "</td>";
