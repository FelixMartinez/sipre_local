...
function anularTransferencia($formAnulacion){
	...
	if(!xvalidaAcceso($objResponse,"cj_devolucion_transferencia_list","insertar")){ return $objResponse; }
	...
	// ACTUALIZO EL ESTADO DE LA TRANSFERENCIA A ANULADO (0 = Anulado, 1 = Activo)
	$query = sprintf("UPDATE cj_cc_transferencia SET
		estatus = 0,
		fecha_anulado = %s,
...
function imprimirTransferencia($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjvh_devolucion_transferencia_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
