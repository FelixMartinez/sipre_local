...
function cargarDcto($idFactura){
	...
	$nombreCondicionPago = ($idCondicionPago == 0) ? "CRÃ‰DITO" : "CONTADO";
	...
	if ($totalRowsVehiculo > 0) {
		...
		$divDescripcionItm .= "<tr>";
			...
			$divDescripcionItm .= "<td align=\"right\" class=\"tituloCampo\">".utf8_decode("VersiÃ³n:")."</td>"."<td>".$rowVehiculo['nom_version']."</td>";
		$divDescripcionItm .= "</tr>";
		$divDescripcionItm .= "<tr>";
			$divDescripcionItm .= "<td align=\"right\" class=\"tituloCampo\">".utf8_decode("AÃ±o:")."</td>";
		...
		$divDescripcionItm .= "</tr>";
		$divDescripcionItm .= "<tr>";
			$divDescripcionItm .= "<td align=\"right\" class=\"tituloCampo\">".utf8_decode("Nro. VehÃ­culo:")."</td>";
		...
		$divDescripcionItm .= "</tr>";
		$divDescripcionItm .= "<tr>";
			$divDescripcionItm .= "<td align=\"right\" class=\"tituloCampo\">".utf8_decode("Registro LegalizaciÃ³n:")."</td>";
		...
		if (in_array($rowVehiculo['com_uni_bas'],array(2,5))) {
			...
			$divDescripcionItm .= "<tr>";
				...
				$divDescripcionItm .= "<td align=\"right\" class=\"tituloCampo\">CÃ³digo Ãšnico:</td>";
...
function guardarDcto($frmDcto, $frmListaArticulo, $frmTotalDcto){
	...
	// ACTUALIZA LA NUMERACIÃ“N DEL DOCUMENTO
	...
	if ($totalRowsFacturaVehiculo > 0) { // FUE AGREGADA POR VENTAS DE VEHÃ�CULOS
		if (isset($arrayObj)) {
			foreach($arrayObj as $indice => $valor) {
				if (isset($frmListaArticulo['cbx']) && in_array($valor, $frmListaArticulo['cbx'])) { // VERIFICA SI EL ITEM ESTA EN EL DETALLE
					$frmListaArticuloAux = $frmListaArticulo;
		...
		while ($rowFADetVehic = mysql_fetch_array($rsFADetVehic)) {
			...
			// ACTUALIZA EL ESTADO DE VENTA DEL VEHÃ�CULO
			...
			while($rowTradeInCxC = mysql_fetch_assoc($rsTradeInCxC)) {
				...
				if ($idDocumento > 0) {
					...
					if ($tipoDocumento == 'ND_CXC') {
						...
						$totalRowsSaldoDcto = mysql_num_rows($rsSaldoDcto);
						$rowSaldoDcto = mysql_fetch_assoc($rsSaldoDcto);
						if ($totalRowsSaldoDcto > 0) { return $objResponse->alert("La Nota de CrÃ©dito Nro. ".$rowSaldoDcto['numeracion_nota_credito']." del Cliente ".$rowSaldoDcto['nombre_cliente']." presenta un saldo negativo"); }
	...
	if (in_array($rowFact['estadoFactura'],array(0,2))) { // 0 = No Cancelado, 2 = Parcialmente Cancelado
		...
		WHERE emp_num.id_numeracion = %s
			AND (emp_num.id_empresa = %s OR (aplica_sucursales = 1 AND emp_num.id_empresa = (SELECT suc.id_empresa_padre FROM pg_empresa suc
																							WHERE suc.id_empresa = %s)))
		ORDER BY aplica_sucursales DESC LIMIT 1;",
			valTpDato(45, "int"), // 45 = Recibo de Pago VehÃ­culos
			valTpDato($idEmpresa, "int"),
			valTpDato($idEmpresa, "int"));
		$rsNumeracion = mysql_query($queryNumeracion);
		if (!$rsNumeracion) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	$objResponse->alert("Nota de CrÃ©dito Guardada con Ã‰xito");
