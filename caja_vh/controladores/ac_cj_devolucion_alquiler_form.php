...
function guardarDcto($frmDcto, $frmListaPrecio, $frmListaAccesorio, $frmListaAdicional, $frmTotalDcto){
	...
	VALUE (%s, %s, %s, %s);",
		...
		valTpDato("4", "int")); // 1 = FA, 2 = ND, 3 = AN, 4 = NC, 5 = CH
	$Result1 = mysql_query($insertSQL);
	if (!$Result1) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	if (in_array($rowFact['estadoFactura'],array(0,2))) { // 0 = No Cancelado, 2 = Parcialmente Cancelado
		...
		if ($rowFact['estadoFactura'] == 0) { // 0 = No Cancelado
			...
			$insertSQL = sprintf("INSERT INTO an_pagos (id_factura, fechaPago, formaPago, numeroDocumento, bancoOrigen, bancoDestino, montoPagado, numeroFactura, tomadoEnComprobante, tomadoEnCierre, idCaja, id_apertura, idCierre, estatus, id_encabezado_v)
