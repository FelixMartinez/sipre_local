...
function exportarTransferencia($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cjvh_transferencia_historico_excel.php?valBusq=".$valBusq."','_self');");
...
function imprimirTransferencia($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjvh_transferencia_historico_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				...
				$htmlTb .= "</tr>";
				$htmlTb .= (strlen($row['observacion_transferencia']) > 0) ? "<tr><td class=\"textoNegritaCursiva_9px\">".utf8_encode($row['observacion_transferencia'])."</td></tr>" : "";
				$htmlTb .= "</table>";
			$htmlTb .= "</td>";
			...
			$htmlTb .= "<td>";
			if ($row['saldo_transferencia'] > 0 && $row['estatus'] == 1) {
				$htmlTb .= sprintf("<a href=\"cj_transferencia_form.php?id=%s\" target=\"_self\"><img src=\"../img/iconos/application_view_columns_add.png\" title=\"Pagar Dctos.\"/></a>",
					$row['id_transferencia']);
			}
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"../cxc/cc_transferencia_form.php?id=%s&acc=0\" target=\"_blank\"><img src=\"../img/iconos/ico_view.png\"/ title=\"Ver\"></a>",
					$row['id_transferencia']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			if (in_array($row['id_departamento'], array(2,4))){ // 2 = Vehiculos, 4 = Alquiler
				$aVerDctoAux = sprintf("../caja_vh/reportes/cjvh_recibo_impresion_pdf.php?idTpDcto=6&id=%s", $row['id_transferencia']);
			} else if (in_array($row['id_departamento'], array(0,1,3))){ // 0 = Repuestos, 1 = Servicios, 3 = Administración
				$aVerDctoAux = sprintf("../caja_rs/reportes/cjrs_recibo_impresion_pdf.php?idTpDcto=6&id=%s", $row['id_transferencia']);
			}
