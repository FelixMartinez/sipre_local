...
function cargaLstTipoCheque($selId = "", $bloquearObj = false){//si es puerto rico, permitir cambio y uso de tipo de cheque suplidor
	...
	$class = ($bloquearObj == true) ? "" : "class=\"inputHabilitado\"";
	$onChange = ($bloquearObj == true) ? "onchange=\"selectedOption(this.id,'".$selId."');\"" : "onchange=\"asignarTipoCheque(this.value);\"";
    ...
    if (in_array($rowConfig403['valor'],array(3))) { // 3 = Puerto Rico
		...
		foreach ($array as $indice => $valor) {
			...
			if ($totalRows == 1) { $objResponse->call(asignarTipoCheque,$indice); }
...
function cargarDcto($idCheque){
	...
	if ($idCheque > 0) {
		...
		if (!($rowCheque['estatus'] == 1)) {
			$objResponse->alert(utf8_encode("El cheque no puede ser aplicado porque se encuentra anulado."));
			
			return $objResponse->script(sprintf("window.location.href='cj_cheque_list.php';"));
		}
		...
		$objResponse->call(asignarTipoCheque,$rowCheque['tipo_cheque']);
		...
		FROM (SELECT
				...
				WHERE cxc_pago.id_forma_pago IN (2)
					AND cxc_pago.estatus IN (1)
					AND cxc_pago.id_cheque IS NOT NULL) AS q
		...
		byId('txtMontoPago').className = 'inputHabilitado';
		byId('txtObservacionCheque').className = 'inputHabilitado';
		...
		$objResponse->loadCommands(cargaLstTipoCheque((($idCajaPpal == 1) ? "" : 1)));
		...
		$objResponse->call(asignarTipoCheque,(($idCajaPpal == 1) ? "" : 1));
		$objResponse->loadCommands(cargaLstTipoDcto());
	}
...
function guardarCheque($frmDcto, $frmDetallePago, $frmListaDctoPagado){
	$objResponse = new xajaxResponse();
	...
	if (!xvalidaAcceso($objResponse,"cj_cheque_list","insertar")){ return $objResponse; }
	
	$idCheque = $frmDcto['hddIdCheque'];
	$idEmpresa = $frmDcto['txtIdEmpresa'];
	$idModulo = $frmDcto['lstModulo'];
	$idCliente = $frmDcto['txtIdCliente'];
	$lstTipoCheque = $frmDcto['lstTipoCheque'];
	
	$idBancoCliente = $frmDetallePago['selBancoCliente'];
	$idBancoCompania = 1;
	$numeroCuentaCliente = trim($frmDetallePago['txtNumeroCuenta']);
	$numeroCuentaCompania = "-";
	$numeroCheque = trim($frmDetallePago['txtNumeroDctoPago']);
	$txtMontoPago = str_replace(",", "", $frmDetallePago['txtMontoPago']);
	$saldoCheque = str_replace(",", "", $frmListaDctoPagado['txtMontoRestante']);
	
	$Result1 = validarAperturaCaja($idEmpresa, date("Y-m-d"));
	if ($Result1[0] != true && strlen($Result1[1]) > 0) { return $objResponse->alert($Result1[1]); }
	
	
	if ($txtMontoPago <= 0) { return $objResponse->alert("El monto del cheque no debe ser cero ni negativo");}	
	if ($saldoCheque < 0) { return $objResponse->alert("El saldo del cheque no debe ser negativo"); }	
	if ($idCliente == "") { return $objResponse->alert("Debe seleccionar un cliente"); }
	
	if (in_array($lstTipoCheque,array(2,3))) { // 2 = tipo bono, 3 = tipo pnd
		if ($frmListaDctoPagado['cbxDctoAgregado'] == "") { return $objResponse->alert("Debe seleccionar almenos un anticipo"); }				
		//if ($saldoCheque != 0) { return $objResponse->alert("Debe usar el saldo completo del cheque para pagar anticipos"); }
	}
	...
	if ($idCheque > 0) {
		...
		$insertSQL = sprintf("INSERT INTO cj_cc_cheque (id_cliente, tipo_cheque, id_banco_cliente, cuenta_cliente, monto_neto_cheque, saldo_cheque, total_pagado_cheque, fecha_cheque, id_empleado_registro, observacion_cheque, estado_cheque, numero_cheque, id_departamento, id_empresa, tomadoEnComprobante, tomadoEnCierre, idCaja, id_apertura)
		VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",									
			valTpDato($idCliente, "int"),
			valTpDato($lstTipoCheque, "int"),
		...
		VALUE (%s, %s, %s, %s);",
			...
			valTpDato($idCheque, "int"),
			valTpDato(date("Y-m-d", strtotime($frmDcto['txtFecha'])), "date"),
			valTpDato("5", "int")); // 1 = FA, 2 = ND, 3 = AN, 4 = NC, 5 = CH, 6 = TB
		...
		$Result1 = mysql_query($insertSQL);
		if (!$Result1) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		$idReporteImpresionCheque = mysql_insert_id();
		
		// TODO CHEQUE INGRESA INMEDIATAMENTE A CAJA:	
						...
						if ($rowPago['idCierre'] > 0) {
							if ($rowPago['tipo_documento'] == 1) { // 1 = FA, 2 = ND, 3 = NC, 4 = AN, 5 = CH, 6 = TB (Relacion Tabla tipodedocumentos)
								...
								if (in_array($idCaja,array(1))) { // 1 = CAJA DE VEHICULOS, 2 = CAJA DE REPUESTOS Y SERVICIOS
									...
									$insertSQL = sprintf("INSERT INTO an_pagos (id_factura, fechaPago, formaPago, numeroDocumento, bancoOrigen, bancoDestino, cuentaEmpresa, montoPagado, numeroFactura, tipoCheque, id_cheque, tipo_transferencia, id_transferencia, tomadoEnComprobante, tomadoEnCierre, idCaja, id_apertura, estatus, id_encabezado_v)
									...
									// INSERTA EL NUEVO PAGO CON LA APERTURA DE CAJA ACTUAL
									$insertSQL = sprintf("INSERT INTO sa_iv_pagos (id_factura, fechaPago, formaPago, numeroDocumento, bancoOrigen, bancoDestino, cuentaEmpresa, montoPagado, numeroFactura, tipoCheque, id_cheque, tipo_transferencia, id_transferencia, tomadoEnComprobante, tomadoEnCierre, idCaja, id_apertura, estatus, id_encabezado_rs)
									SELECT id_factura, %s, formaPago, numeroDocumento, bancoOrigen, bancoDestino, cuentaEmpresa, montoPagado, numeroFactura, tipoCheque, id_cheque, tipo_transferencia, id_transferencia, %s, %s, idCaja, %s, %s, %s
	...
	$objResponse->alert("Cheque guardado correctamente");
	...
	if (!($frmDcto['hddIdCheque'] > 0)) {
		if (in_array($idCajaPpal, array(1))){
			$objResponse->script(sprintf("verVentana('reportes/cjvh_recibo_impresion_pdf.php?idTpDcto=5&id=%s',960,550)", $idCheque));
		} else if (in_array($idCajaPpal, array(2))){
			$objResponse->script(sprintf("verVentana('reportes/cjrs_recibo_impresion_pdf.php?idTpDcto=5&id=%s',960,550)", $idCheque));
		}
	}
	
	$objResponse->script(sprintf("
	cerrarVentana = true;
	window.location.href='cj_cheque_list.php';"));
...
function listaAnticipo($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 5, $totalRows = NULL){
	...
	if ($valCadBusq[3] == "ANTICIPO") {
		...
		if ($valCadBusq[2] == 2) { // 2 = Bono Suplidor
			...
			return $objResponse->alert("Debe seleccionar tipo de cheque Bono Suplidor o PND para agregar anticipos");
		}
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
...
function listaFacturaVenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 5, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			if (in_array($row['id_modulo'], array(2,4))){ // 2 = Vehiculos, 4 = Alquiler
				$aVerDctoAux = sprintf("../caja_vh/reportes/cjvh_recibo_impresion_pdf.php?idTpDcto=1&id=%s", $row['idFactura']);
			} else if (in_array($row['id_modulo'], array(0,1,3))){ // 0 = Repuestos, 1 = Servicios, 3 = Administración
				$aVerDctoAux = sprintf("../caja_rs/reportes/cjrs_recibo_impresion_pdf.php?idTpDcto=1&id=%s", $row['idFactura']);
			}
				$htmlTb .= (strlen($aVerDctoAux) > 0) ? "<a href=\"javascript:verVentana('".$aVerDctoAux."', 960, 550);\"><img src=\"../img/iconos/print.png\" title=\"".("Recibo(s) de Pago(s)")."\"/></a>" : "";
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
...
function listaNotaDebito($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 5, $totalRows = NULL){
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
			if (in_array($row['id_modulo'], array(2,4))){ // 2 = Vehiculos, 4 = Alquiler
				$aVerDctoAux = sprintf("../caja_vh/reportes/cjvh_recibo_impresion_pdf.php?idTpDcto=2&id=%s", $row['idNotaCargo']);
			} else if (in_array($row['id_modulo'], array(0,1,3))){ // 0 = Repuestos, 1 = Servicios, 3 = Administración
				$aVerDctoAux = sprintf("../caja_rs/reportes/cjrs_recibo_impresion_pdf.php?idTpDcto=2&id=%s", $row['idNotaCargo']);
			}
				$htmlTb .= (strlen($aVerDctoAux) > 0) ? "<a href=\"javascript:verVentana('".$aVerDctoAux."', 960, 550);\"><img src=\"../img/iconos/print.png\" title=\"".("Recibo(s) de Pago(s)")."\"/></a>" : "";
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
