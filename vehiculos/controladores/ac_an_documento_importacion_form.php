...
function calcularFacturaGasto($frmFacturaGasto) {
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			...
			$estatusIva = 1;
			$queryIva = sprintf("SELECT * FROM pg_iva iva WHERE iva.idIva = %s;", valTpDato($frmFacturaGasto['lstIvaItmFacturaGasto'.$valor], "int"));
	...
	if (isset($arrayIva)) {
		foreach ($arrayIva as $indice => $valor) {
			$queryIva = sprintf("SELECT * FROM pg_iva iva WHERE iva.idIva = %s;", valTpDato($arrayIva[$indice][0], "int"));
			$rsIva = mysql_query($queryIva);
			if (!$rsIva) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			$rowIva = mysql_fetch_assoc($rsIva);
...
function cargaLstRetencionImpuesto($selId = "") {
	...
	$query = sprintf("SELECT * FROM pg_iva iva WHERE tipo = 5 AND estado = 1 ORDER BY iva");
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	...
	if ($sinAlmacen == false) {
		...
		if (isset($arrayObj)) {
			foreach ($arrayObj as $indice => $valor) {
				...
				$queryIva = sprintf("SELECT * FROM pg_iva iva WHERE iva.idIva = %s;", $idIva);
				$rsIva = mysql_query($queryIva);
				if (!$rsIva) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				$rowIva = mysql_fetch_assoc($rsIva);
