...
function buscarModelo($frmBuscarModelo, $frmAsignacion) {
	$objResponse = new xajaxResponse();
...
function formAsignacion($idAsignacion, $frmAsignacion) {
	...
	if ($idAsignacion > 0) {
		...
		$objResponse->assign("txtFecha","value",utf8_encode(date("d-m-Y",strtotime($row['fecha_asignacion']))));
		...
		$objResponse->assign("txtFechaCierreCompra","value",utf8_encode(date("d-m-Y",strtotime($row['fecha_cierre_compra']))));
		$objResponse->assign("txtFechaCierreVenta","value",utf8_encode(date("d-m-Y",strtotime($row['fecha_cierre_venta']))));
...
function formUnidadAsignacion() {
	...
	$objResponse->script("byId('btnBuscarModelo').click();");
...
function guardarAsignacion($frmAsignacion, $frmListaAsignacion) {
	...
	if ($idAsignacion > 0) {
		if (!xvalidaAcceso($objResponse,"an_asignacion_list","editar")) { errorGuardarAsignacion($objResponse); return $objResponse; }
		...
		mysql_query("SET NAMES 'utf8'");
		$Result1 = mysql_query($updateSQL);
		if (!$Result1) { errorGuardarAsignacion($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		mysql_query("SET NAMES 'latin1';");
		...
		$query = sprintf("SELECT * FROM an_asignacion WHERE idAsignacion = %s;",
			valTpDato($idAsignacion, "int"));
		$rs = mysql_query($query);
		if (!$rs) { errorGuardarAsignacion($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		$row = mysql_fetch_assoc($rs);
		...
		if (count($arrayCantidadInvalida) > 0) {
			...
			errorGuardarAsignacion($objResponse);
			...
			if (isset($arrayObj)) {
				foreach ($arrayObj as $indice => $valor) {
					...
					if ($row['estatus_asignacion'] == 1) { // 0 = Abierta, 1 = Cerrada, 2 = Aceptada, 3 = Confirmada
						if (!xvalidaAcceso($objResponse,"an_respuesta","editar")) { errorGuardarAsignacion($objResponse); return $objResponse; }
						...
						if (!xvalidaAcceso($objResponse,"an_confirmacion","editar")) { errorGuardarAsignacion($objResponse); return $objResponse; }
		...
		if (!xvalidaAcceso($objResponse,"an_asignacion_list","insertar")) { errorGuardarAsignacion($objResponse); return $objResponse; }
	...
	$queryPedidoDet = sprintf("SELECT * FROM an_det_asignacion WHERE idAsignacion = %s;",
		valTpDato($idAsignacion, "int"));
	$rsPedidoDet = mysql_query($queryPedidoDet);
	if (!$rsPedidoDet) { errorGuardarAsignacion($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	while ($rowPedidoDet = mysql_fetch_assoc($rsPedidoDet)) {
		$existRegDet = false;
		if (isset($arrayObj)) {
			foreach ($arrayObj as $indice => $valor) {
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			if (strlen($frmAsignacion['hddIdUnidadBasica'.$valor]) > 0 && !($frmAsignacion['hddIdDetalleAsignacion'.$valor] > 0)) {
				if (!xvalidaAcceso($objResponse,"an_asignar_vehiculo","editar")) { errorGuardarAsignacion($objResponse); return $objResponse; }
...
function insertarUnidadAsignacion($frmUnidadAsignacion, $frmAsignacion) {
	...
	if (count($arrayObj) < $rowConfig203['valor']) {
		...
		errorInsertarUnidadAsignacion($objResponse);
...
function listaAsignacion($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".utf8_encode(date("d-m-Y",strtotime($row['fecha_asignacion'])))."</td>";
...
function errorGuardarAsignacion($objResponse) {
	$objResponse->script("
	byId('btnGuardarAsignacion').disabled = false;
	byId('btnCancelarAsignacion').disabled = false;");
}
...
function errorInsertarUnidadAsignacion($objResponse) {
	$objResponse->script("
