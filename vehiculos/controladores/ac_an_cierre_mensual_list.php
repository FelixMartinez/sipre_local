...
// 	if ($idEmpresa > 0 && $arrayFecha != '' && $valCierre) {
		...
		$query = sprintf("SELECT idFactura FROM cj_cc_encabezadofactura enfac
					WHERE (YEAR(enfac.fechaRegistroFactura) < %s OR (YEAR(enfac.fechaRegistroFactura) = %s AND MONTH(enfac.fechaRegistroFactura) <= %s ))
					AND enfac.estatus_factura = %s AND enfac.estadoFactura = %s;",
...
// 		print_r('alto');exit;
		$query = sprintf("SELECT * FROM cj_cc_notacredito
			...
			AND (YEAR(fechaNotaCredito) < %s OR (YEAR(fechaNotaCredito) = %s AND MONTH(fechaNotaCredito) <= %s ))
			AND estatus_nota_credito = %s ORDER BY IdNotaCredito ASC;",
				valTpDato(2, "int"),
				valTpDato($anoCierre,"int"),
				valTpDato($anoCierre,"int"),
				valTpDato($mesCierre,"int"),
