...
function cargaLstMarcaModeloVersion($idLstOrigen, $nombreObjeto, $padreId = "", $selId = "") {
	...
	if (($posList + 1) != count($arraySelec) - 1)
		$onChange = "onchange=\"xajax_cargaLstMarcaModeloVersion('".$arraySelec[$posList+1]."', '".$nombreObjeto."', this.value);\"";
...
function guardarVersion($frmVersion, $frmListaVersion) {
	...
	if ($frmVersion['hddIdVersion'] > 0) {
		if (!xvalidaAcceso($objResponse,"an_version_list","editar")) { errorGuardarVersion($objResponse); return $objResponse; }
		...
		if (!$Result1) {
			errorGuardarVersion($objResponse);
			if (mysql_errno() == 1062) {
		...
		if (!xvalidaAcceso($objResponse,"an_version_list","insertar")) { errorGuardarVersion($objResponse); return $objResponse; }
...
function listaVersion($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaVersion", "50%", $pageNum, "des_version", $campOrd, $tpOrd, $valBusq, $maxRows, "Descripción");
...
function errorGuardarVersion($objResponse) {
	$objResponse->script("
	byId('btnGuardarVersion').disabled = false;
	byId('btnCancelarVersion').disabled = false;");
}
?>