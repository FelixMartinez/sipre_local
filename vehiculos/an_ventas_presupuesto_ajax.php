...
if (isset($_GET['ajax_cedula'])) {
	...
	if ($cadena != "") {
		...
		"<tr class=\"tituloCampo\">".
			...
			"<td><a href=\"javascript:cancelarCliente();\"><img border=\"0\" src=\"../img/iconos/cross.png\" alt=\"Cerrar\"/></a></td>".
		"</tr>".
		"</table>";
		...
		if ($totalRows > 0) {
			while ($row = mysql_fetch_array($r)) {
				...
				echo "<tr class=\"".$clase."\" onclick=\"javascript:cargarXML('cliente', '".$row['id']."', byId('txtIdEmpresa').value, '".$row['id']."');\" style=\"cursor:pointer\" height=\"24\">";
					echo "<td align=\"right\" width=\"22%\">".utf8_encode($row['ci_cliente'])."</td>";
					echo "<td width=\"62%\">".utf8_encode($row['nombre_cliente'])."</td>";
		...
		if ($totalRows > 0) {
			while ($row = mysql_fetch_assoc($r)) {
				...
				echo "<tr class=\"".$clase."\" onclick=\"javascript:cargarXML('vehiculo', '".$row['id_uni_bas']."', '".$row['id_empresa']."', '".$_GET['idCliente']."');\" style=\"cursor:pointer\" height=\"24\">";
					echo "<td>"."<img border=\"0\" src=\"../img/iconos/tick.png\" title=\"Seleccionar\"/>"."</td>";
					echo "<td width=\"22%\">".utf8_encode($row['nom_marca'])."</td>";
					echo "<td width=\"78%\">".utf8_encode($row['vehiculo'])."</td>";
				echo "</tr>";
			}
		} else {
			echo "<tr>";
	...
	if ($cadena != "") {
		conectar();
		$iva = doubleval(getmysql("SELECT iva FROM pg_iva WHERE tipo = 6 AND estado = 1 AND activo = 1 ORDER BY iva;"));
		...
		"<tr class=\"trResaltar3\">".
			"<td align=\"left\" style=\"padding:2px;\" width=\"100%\"><strong><em>Paquetes:</em></strong></td>".
			"<td><a href=\"javascript:cancelarAdicional();\"><img border=\"0\" src=\"../img/iconos/cross.png\" alt=\"Cerrar\"/></a></td>".
		"</tr>".
		"</table>";
		...
		echo "<div class=\"overflowlist\">";
			...
			if ($totalRows > 0) {
				while ($row = mysql_fetch_row($r)) {
					...
					echo "<tr>";
						echo "<td>";
						echo "<form id=\"paq".$row[0]."\" style=\"margin:0px;\">";
							echo "<fieldset><legend class=\"legend\">".utf8_encode($row[1])." (".utf8_encode($row[2]).")</legend>";
								echo "<table border=\"0\" width=\"100%\">";
								if ($totalRows2 > 0) {
									$contFila = 0;
									while ($row2 = mysql_fetch_array($r2)) {
										...
										$ivaAcc = ($row2['iva_accesorio'] == '1') ? getmysql("SELECT iva FROM pg_iva WHERE tipo = 6 AND estado = 1 AND activo = 1 ORDER BY iva;"): '0';
										...
										echo (fmod($contFila, 2) == 1) ? "<tr align=\"left\" class=\"".$clase."\" height=\"24\">" : "";
											...
											echo "<td width=\"50%\">";
												echo utf8_encode($row2['nom_accesorio']).
									...
									echo "<tr>";
										echo "<td align=\"center\" colspan=\"4\"><button type=\"button\" onclick=\"insertarPaquete(".$row[0].");\" style=\"cursor:default\" value=\"Agregar Paquete\"><table align=\"center\" cellpadding=\"0\" cellspacing=\"0\"><tr><td>&nbsp;</td><td><img src=\"../img/iconos/plus.png\"/></td><td>&nbsp;</td><td>Agregar Paquete</td></tr></table></button></td>";
									echo "</tr>";
								} else {
									echo "<td>";
										echo "<table cellpadding=\"0\" cellspacing=\"0\" class=\"divMsjError\" width=\"100%\">";
										echo "<tr>";
											echo "<td width=\"25\"><img src=\"../img/iconos/ico_fallido.gif\" width=\"25\"/></td>";
											echo "<td align=\"center\">No existen accesorios en este paquete</td>";
										echo "</tr>";
										echo "</table>";
									echo "</td>";
								}
			...
			echo "<table border=\"0\" class=\"tabla\" cellpadding=\"2\" width=\"100%\">";
			echo "<tr class=\"trResaltar7\">";
				echo "<td align=\"left\" style=\"padding:2px;\"><strong><em>"."Otros Adicionales:"."</em></strong></td>";
			echo "</tr>";
			if ($totalRows3 > 0) {
				while ($row3 = mysql_fetch_array($r3)) {
					...
					$ivaAcc = ($row3['iva_accesorio'] == '1') ? getmysql("SELECT SUM(iva) AS iva FROM pg_iva WHERE tipo = 6 AND estado = 1 AND activo = 1 ORDER BY iva;") : '0';
					
					echo "<tr class=\"".$clase."\" height=\"24\">";
						echo sprintf("<td align=\"left\" style=\"cursor:pointer\" onclick=\"insertarAdicional('acc%s','%s','%s','%s','%s','%s','%s');\">",
							$row3['id_accesorio'],
							$row3['precio_con_iva'],
							...
							echo "<tr>";
								echo "<td><img src=\"../img/iconos/plus.png\"/></td>";
								echo "<td>&nbsp;</td>";
								echo "<td width=\"80%\">".utf8_encode($row3['nom_accesorio'])."</td>";
								echo "<td width=\"20%\">".utf8_encode($row3['descripcion_tipo_accesorio'])."</td>";
							echo "</tr>";
							echo "</table>";
						echo "</td>";
					echo "</tr>";
