	...
	if (isset($_FILES['userfile'])) {
		...
		if (!( strpos(strtolower ($nombre_archivo), $type) && ($tamano_archivo < 100000))) { //(strpos($tipo_archivo, "gif") || strpos($tipo_archivo, "jpeg") || strpos($tipo_archivo, "png")  ||
			...
			if (move_uploaded_file($_FILES['userfile']['tmp_name'], $name_result)) {
				...
				/*if ($backup){
					echo '<form onsubmit="return false;"><button type="button" onclick="window.location=\'cargarvehiculo.php?rollback='.$name_result.'\';"><img border="0" src="../iconos/delete.png" style="padding:2px; vertical-align:middle;" /> Cancelar</button>
				</form>';
				}*/
				echo '</body></html>';
			} else {
