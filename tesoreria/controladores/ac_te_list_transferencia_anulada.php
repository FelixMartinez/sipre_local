<?php 
function listarTransferenciaAnulada($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	$startRow = $pageNum * $maxRows;
		...
		$objResponse->assign("tdReferenciaPagina","innerHTML","Transferencias Anuladas");		
		$colspan = 12;
	...
	if ($valCadBusq[0] != 0){
		if ($valCadBusq[0] == -1)
			if ($cadenaFiltro == "")
				$cadenaFiltro = sprintf(" WHERE te_transferencias_anuladas.id_empresa = %s",$_SESSION['idEmpresaUsuarioSysGts']);
			else
				$cadenaFiltro .= sprintf(" AND te_transferencias_anuladas.id_empresa = %s",$_SESSION['idEmpresaUsuarioSysGts']);
		else
			if ($cadenaFiltro == "")
				$cadenaFiltro = sprintf(" WHERE te_transferencias_anuladas.id_empresa = %s",$valCadBusq[0]);
			else
				$cadenaFiltro .= sprintf(" AND te_transferencias_anuladas.id_empresa = %s",$valCadBusq[0]);
	}
	...
	if ($valCadBusq[3] != "" && $valCadBusq[4] != ""){
		if ($cadenaFiltro == ""){
			$cadenaFiltro = sprintf(" WHERE fecha_transferencia BETWEEN '%s' AND '%s'",
                                date("Y-m-d",strtotime($valCadBusq[3])),
                                date("Y-m-d",strtotime($valCadBusq[4])));
                }else{
			$cadenaFiltro .= sprintf(" AND fecha_transferencia BETWEEN '%s' AND '%s'",
                                date("Y-m-d",strtotime($valCadBusq[3])),
                                date("Y-m-d",strtotime($valCadBusq[4])));
                }
	}
        ...
        if ($valCadBusq[6] != ""){
		if ($cadenaFiltro == ""){
			$cadenaFiltro = sprintf(" WHERE observacion LIKE %s",
                                                        valTpDato("%%".$valCadBusq[6]."%%","text"));
                }else{
			$cadenaFiltro .= sprintf(" AND observacion LIKE %s",
                                                        valTpDato("%%".$valCadBusq[6]."%%","text"));
                }
	}
	
	$query = "SELECT te_transferencias_anuladas.*, cuentas.numeroCuentaCompania, bancos.nombreBanco, pg_empresa.nombre_empresa, cp_proveedor.nombre
		  ...
		  LEFT JOIN cp_proveedor ON te_transferencias_anuladas.id_beneficiario_proveedor = cp_proveedor.id_proveedor
                  ...
                  INNER JOIN bancos ON cuentas.idBanco = bancos.idBanco ".$cadenaFiltro;
        
        //$sqlOrd = ($campOrd != "") ? sprintf(" ORDER BY %s %s", $campOrd." ".$tpOrd.",numero_transferencia+0", $tpOrd) : "";
        
        if($campOrd == "fecha_transferencia" OR $campOrd == "fecha_registro"){//agrupar por fecha y luego ordenar por numero
	...
	//$objResponse->alert($queryLimit); //alerta
	...
	$htmlTblIni .= "<table border=\"0\" width=\"100%\">";
        ...
        $htmlTh .= "<tr class=\"tituloColumna\">";
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "10%", $pageNum, "nombre_empresa", $campOrd, $tpOrd, $valBusq, $maxRows, "Empresa");
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "10%", $pageNum, "nombre", $campOrd, $tpOrd, $valBusq, $maxRows, "Proveedor");
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "10%", $pageNum, "numero_transferencia+0", $campOrd, $tpOrd, $valBusq, $maxRows, "N&uacute;mero Transferencia");			
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "8%", $pageNum, "monto_transferencia", $campOrd, $tpOrd, $valBusq, $maxRows, "Monto Transferencia");
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "7%", $pageNum, "fecha_transferencia", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Transferencia");			
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "7%", $pageNum, "fecha_registro", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Anulado");
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "40%", $pageNum, "observacion", $campOrd, $tpOrd, $valBusq, $maxRows, "Concepto");			
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "15%", $pageNum, "nombreBanco", $campOrd, $tpOrd, $valBusq, $maxRows, "Banco Compa&ntilde;ia");			
            $htmlTh .= ordenarCampo("xajax_listarTransferenciaAnulada", "15%", $pageNum, "numeroCuentaCompania", $campOrd, $tpOrd, $valBusq, $maxRows, "Cuenta Compa&ntilde;ia");			
            $htmlTh .= "<td></td>";
        $htmlTh .= "</tr>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\">";
			...
			$htmlTb .= "<td align=\"center\">".$row['nombreBanco']."</td>";
			...
			$htmlTf .= "<tr class=\"tituloCampo\">";
				...
				$htmlTf .= "<td align=\"center\" class=\"tituloColumna\" width=\"210\">";
					...
					$htmlTf .= "<tr align=\"center\">";
						$htmlTf .= "<td width=\"25\">";
						if ($pageNum > 0) {
							$htmlTf .= sprintf("<a class=\"puntero\" onclick=\"xajax_listarTransferenciaAnulada(%s,'%s','%s','%s',%s);\">%s</a>",
						...
						$htmlTf .= "<td width=\"100\">";
							...
							$htmlTf .= sprintf("<select id=\"pageNum\" name=\"pageNum\" onchange=\"xajax_listarTransferenciaAnulada(%s,'%s','%s','%s',%s)\">",
	...
	if (!($totalRows > 0)) {
		$htmlTb .= "<td colspan=\"".$colspan."\" class=\"divMsjError\">";
        ...
        $objResponse->assign("tdListadoTransferencias","innerHTML",$htmlTblIni.$htmlTf.$htmlTh.$htmlTb.$htmlTf.$htmlTblFin);
...
function buscarTransferencia($valForm) {
	...
	$objResponse->script(sprintf("xajax_listarTransferenciaAnulada(0,'fecha_registro','DESC','%s|%s|%s|%s|%s|%s|%s');",
		$valForm['selEmpresa'],
		$valForm['selEstado'],
		$valForm['txtBusq'],
		$valForm['txtFecha'],
...
function comboEmpresa($idTd,$idSelect,$selId){
	...
	if ($selId)
		...
		$rs = mysql_query($query);
                if(!$rs) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
		$html = "<select id=\"".$idSelect."\" name=\"".$idSelect."\" onChange=\"xajax_buscarTransferencia(xajax.getFormValues('frmBuscar'))\">";
...
$xajax->register(XAJAX_FUNCTION,"listarTransferenciaAnulada");
