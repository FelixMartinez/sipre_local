...
function listadoEstadoCuentaAplicados($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 2000, $totalRows = NULL) {
	$objResponse = new xajaxResponse();
	...
	$htmlx.="<table align=\"right\"  border=\"0\" width=\"60%\">";
		...
		$htmlx.="<tr>";
			...
			$htmlx.="<td align=\"right\"><input name=\"txtSaldoBanco\" id=\"txtSaldoBanco\" type=\"text\" readonly=\"readonly\" value = \"".$valCadBusq[2]."\"  />
										 <input name=\"hddSaldoBanco\" id=\"hddSaldoBanco\" type=\"hidden\" value = \"".$valCadBusq[2]."\" />
...
//                        ."{ xajax_guardarConciliacion(xajax.getFormValues('frmListadoEstadoCuenta1'),xajax.getFormValues('frmBuscar'),'NO'); } ");
//            }
//        }
	...
	for ($cont = 0; $cont <= strlen($valForm['hddObj']); $cont++){
		...
		if ($caracter != "|" && $caracter != ""){
           ...
           // $valFormBuscar['txtFecha'],
			date("Y-m-d",strtotime($valFormBuscar['txtFecha'])),
			$valForm['hddSaldo'],
...
function listAuditoria($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL){
	...
	while ($rowAuditoria = mysql_fetch_assoc($rsLimitAuditoria)) {
		...
		$htmlTb .= "<tr class=\"".$clase."\">";
			$htmlTb .= "<td align=\"center\">".date("d/m/Y",strtotime($row['fecha_conciliacion']))."</td>";
