...
$idCheque = $_GET['id'];
$codigo = getBarcode($idCheque,'../img/tmp/img_codigo',2,1,25,"c",0);
...
if ($rowConfig403['valor'] == 1) { //VENEZUELA
    ...
    $pdf->Image("../img/tmp/img_codigo.png", '6', '13', '1.5', '0.5');
	...
	$posY += 52;
	imagestring($img,$tamanoLetra,90,$posY,$nombreBeneficiarioProveedor,$textColor);
	...
	imagestring($img,$tamanoLetra,80,125, utf8_decode(abreviacionCentavos($numerosLetras,$row['monto_cheque'])),$textColor);
	...
	imagestring($img,$tamanoLetra,50,450,strtoupper($concepto[0]),$textColor);
	imagestring($img,$tamanoLetra,50,460,strtoupper($concepto[1]),$textColor);
	imagestring($img,$tamanoLetra,50,470,strtoupper($concepto[2]),$textColor);
...
//        imagestring($img,$tamanoLetra,485,90,str_pad(number_format("13437.57", 2, ".", ","), 12," ", STR_PAD_LEFT),$textColor);
        ...
        while($rowFactuasNotas = mysql_fetch_assoc($rsFacturasNotas)){
			...
			$cuentaPagarTotal += $rowFactuasNotas["total_cuenta_pagar"];
            $descuentoTotal += $rowFactuasNotas["descuento"];
            $montoPagarTotal += $rowFactuasNotas["monto_pagar"];
			$montoRetencionTotal += $rowFactuasNotas["monto_retenido"];
            ...
            imagestring($img,$tamanoLetra,345,$y,str_pad(number_format($rowFactuasNotas["total_cuenta_pagar"], 2, ".", ","), 10," ", STR_PAD_LEFT),$textColor);
            ...
            //}
            if($rowFactuasNotas["monto_retenido"] !=0){
                imagestring($img,$tamanoLetra,418,$y,str_pad(number_format($rowFactuasNotas["monto_retenido"], 2, ".", ","), 9," ", STR_PAD_LEFT),$textColor);
            }
