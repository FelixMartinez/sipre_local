...
// Cabecera de página
	...
	$query = sprintf("SELECT * FROM vw_ga_articulos WHERE id_articulo = %s", $_GET['idArt']);
	...
	$pdf->SetFont('Arial','B',12);
		$pdf->Cell(55,5,utf8_decode("Tipo De Articuló:"),0,0,'L'); // Título
	$pdf->SetFont('Arial','',12);
...
//CONSULTA LOS ARTICULOS ALTERNO
		$pdf->Cell(195,7,utf8_decode("Artísculos Sustitutos"),1,0,'C'); // Título
		$pdf->Ln();
	$querySust = sprintf("SELECT id_articulo_codigo_sustituto, ga_articulos_codigos_sustitutos.id_articulo,id_articulo_sustituto,
							   codigo_articulo,vw_ga_articulos.descripcion, existencia
						...
						WHERE ga_articulos_codigos_sustitutos.id_articulo = %s;",$_GET['idArt']); 
	$rsSust = mysql_query($querySust) or die(mysql_error());
	$numSust = mysql_num_rows($rsSust);
	
	$header = array(utf8_decode("Codigo Articuló"), utf8_decode("Descripción"), utf8_decode("Existencia"));
	$pdf->SetFont('Arial','B',12);
	...
	if(!$numSust){
		...
		$pdf->Cell(195,7,utf8_decode("No Exite Artísculos Sustitutos"),1,0,'C'); // Título
	} else{
...
//CONSULTA LOS ARTICULOS ALTERNO	
		$pdf->SetFont('Arial','B',12);// Arial bold 15
		$pdf->Cell(195,7,utf8_decode("Artísculos Alterno"),1,0,'C'); // Título
		$pdf->Ln(7);
	$queryAlte = sprintf("SELECT id_articulo_codigo_alterno, ga_articulos_codigos_alternos.id_articulo,id_articulo_alterno,
							   codigo_articulo,vw_ga_articulos.descripcion, existencia
						...
						WHERE ga_articulos_codigos_alternos.id_articulo = %s;",$_GET['idArt']); 
	$rsAlte = mysql_query($queryAlte) or die(mysql_error());
	$numAlte = mysql_num_rows($rsAlte);
	
	$header = array(utf8_decode("Codigo Articuló"), utf8_decode("Descripción"), "Existencia");
	$pdf->SetFont('Arial','B',12);
	$pdf->headerTable($header, $tabDatos);
	...
	if(!$numAlte){
		$pdf->SetFont('Arial','B',10);
		$pdf->Ln();// Salto de línea
		$pdf->Cell(195,7,utf8_decode("No Exite Artísculos Alterno"),1,0,'C'); // Título
	} else{
