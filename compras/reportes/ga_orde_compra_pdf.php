...
// Cabecera de página
	...
	function Header(){
		...
		$queryEmpresa = "SELECT nombre_empresa,rif,logo_familia,web,nit FROM vw_iv_empresas_sucursales WHERE id_empresa_reg = '".$_GET['session']."'"; //id
	...
	$sqlProve = sprintf("SELECT id_orden_compra, ga_orden_compra.id_proveedor,  cp_proveedor.nombre AS nombre_proveedor,cp_proveedor.rif AS rif_proveedor,
					   		  nit,cp_proveedor.correo AS correo_proveedor,contacto AS persona_contacto,  cp_proveedor.telefono AS telf_proveedor,
							 CONCAT_WS(' ',cp_proveedor.telfcontacto, otrotelf) AS telf_contacto_proveedor,  cp_proveedor.direccion AS direccion_proveedor,
				...
				WHERE id_orden_compra = %s", $_GET['idOrdenComp']);
	...
	$sqlOrdeComp = sprintf("SELECT 
		id_orden_compra,fecha_entrega,fecha_cotizacion,ga_orden_compra.observaciones,segun_cotizacion,tipo_transporte, ga_orden_compra.id_solicitud_compra, 
		CONCAT_WS('-',codigo_empresa,numero_solicitud) AS num_solicitud, fecha_solicitud,
		ga_orden_compra.id_empresa, nombre_empresa, pg_empresa.rif AS rif_empresa, pg_empresa.nit AS nit_empresa, pg_empresa.direccion AS direccion_empresa, telefono1,telefono2,telefono3,telefono3,
		...
		monto_letras,subtotal_descuento
	FROM ga_orden_compra
	...
	if($rowOrdeComp['fecha_cotizacion'] != NULL){
		$fechCotizacion = date('d-m-Y',strtotime($rowOrdeComp['fecha_cotizacion']));
	}
	...
	$sqlDetallOrdeComp =sprintf("
			SELECT ga_orden_compra_detalle.id_orden_compra_detalle,ga_orden_compra_detalle.id_orden_compra,
				ga_orden_compra_detalle.id_articulo,descripcion,codigo_articulo,ga_orden_compra_detalle.cantidad,
				ga_orden_compra_detalle.pendiente,ga_orden_compra_detalle.precio_unitario,
				ga_orden_compra_detalle.id_iva,ga_orden_compra_detalle.iva,ga_orden_compra_detalle.tipo,
				ga_orden_compra_detalle.id_cliente,(precio_unitario * cantidad) AS subtotal,
				ga_articulos.codigo_articulo,ga_articulos.descripcion,ga_tipos_unidad.id_tipo_unidad,
	...
	//CONSULTA GASTO
	$sqlGastos= sprintf("SELECT ga_orden_compra_gasto.porcentaje_monto AS porcentaje_monto, ga_orden_compra_gasto.monto AS monto,
								pg_gastos.nombre as nombre_gasto, if(pg_gastos.estatus_iva = 1,'*','') AS iva
	...
	$sqlIva = sprintf("SELECT ga_orden_compra_iva.iva as iva, sum(ga_orden_compra_iva.`base_imponible`) AS base, SUM(ga_orden_compra_iva.`subtotal_iva`) as subtotal, observacion
							...
							ORDER BY ga_orden_compra_iva.iva;",$_GET['idOrdenComp']);
	...
	$pdf->SetFont('Arial','',11);
	$pdf->Cell(25,5,date("d-m-Y", strtotime($rowOrdeComp['fecha_entrega'])),0,0,'L'); // Título  
	$pdf->Ln();
	$pdf->Cell(140);
	$pdf->SetFont('Arial','B',11);
  ...
  $pdf->SetFont('Arial','',11); 
  $pdf->MultiCell(170,5,utf8_decode(cambiaLetras($rowProve['direccion_proveedor'])),0,'L', false); // Multilinea
  $pdf->Ln(); 
  $pdf->SetFont('Arial','B',11);
  ...
  $pdf->SetFont('Arial','',11); 
  $pdf->Cell(85,5,cambiaLetras($rowOrdeComp['nombre_empresa']),0,0,'L'); // Título  
  $pdf->SetFont('Arial','B',11);
  ...
  $pdf->Cell(20,5,utf8_decode("Dirección:"),0,0,'L'); // Título
  $pdf->SetFont('Arial','',11); 
  $pdf->MultiCell(170,5,utf8_decode(cambiaLetras($rowOrdeComp['direccion_empresa'])),0,'L', false); // Multilinea
  $pdf->Ln(); 
  $pdf->SetFont('Arial','B',12);
  $pdf->Cell(39,5,"Fecha de Entrega:",0,0,'L'); // Título
  $pdf->SetFont('Arial','',12); 
  $pdf->Cell(151,5,date("d-m-Y", strtotime($rowOrdeComp['fecha_entrega'])),0,0,'L'); // Título  
  $pdf->Ln(); 	
  $header = array(utf8_decode("N°"), "Catn.", utf8_decode("Código"), utf8_decode("Descripción"), "Precio Por Unidad","SubTotal");
  $pdf->SetFont('Arial','B',11);
  $pdf->headerTable($header);
	...
	while($rowDetallOrdeComp = mysql_fetch_array($rsDetallOrdeComp)){
		...
		$total[] = $SubTotal;
		$tabDatos = array($num++,$rowDetallOrdeComp["cantidad"],cambiaLetras(substr($rowDetallOrdeComp["codigo_articulo"],0,14)),
					cambiaLetras(substr($rowDetallOrdeComp["descripcion"],0,43)),
					number_format($rowDetallOrdeComp["precio_unitario"], 2, ".", ","),number_format($SubTotal, 2, ".", ","));
		$pdf->SetFont('Arial','',11);
...
//var_dump($iva);
	...
	$pdf->SetFont('Arial','B',11);
	$pdf->Cell(40,5,"total:",1,0,'R'); // Título 
	$pdf->Cell(24,5,number_format($totalOrdenCompra, 2, ".", ","),1,0,'R'); // Título
	$pdf->Ln(10);
	$pdf->SetFont('Arial','',11);
	$pdf->MultiCell(190,5,utf8_decode("Observaciones: ".cambiaLetras($rowOrdeComp['observaciones'])),1,'L', false); // Multilinea
