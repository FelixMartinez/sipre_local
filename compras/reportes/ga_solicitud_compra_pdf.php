...
// Cabecera de página
	...
	$query = sprintf("SELECT id_solicitud_compra, fecha_solicitud, id_unidad_centro_costo, tipo_compra, id_proveedor, nombre_proveedor, justificacion_proveedor,
						...
						fecha_empleado_condicionamiento,tipo_seccion
	...
	$pdf->SetFont('Arial','',12); 
	$pdf->Cell(25,5,date("d-m-Y", strtotime($rowS['fecha_solicitud'])),0,0,'L'); // Título  
	$pdf->Ln();
	...
	while($rowsD = mysql_fetch_array($rsD)){
		...
		$tabDatos = array(substr($rowsD["codigo_articulo_prov"],0,14),
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(153,5,"total:",1,0,'R'); // Título
	...
	$pdf->Cell(95,5,cambiaLetras($rowS["nombre_proveedor"]),1,0,'L'); // Título
	$pdf->MultiCell(95,5,cambiaLetras($rowS["justificacion_proveedor"]),1,'L',false); // Título
	$pdf->Ln();	
	$pdf->MultiCell(190,5,utf8_decode("Observaciones: ").cambiaLetras($rowS["observaciones_proveedor"]),1,'L',false); // Título
	$pdf->Ln(10);
	$pdf->SetFont('Arial','B',12);
