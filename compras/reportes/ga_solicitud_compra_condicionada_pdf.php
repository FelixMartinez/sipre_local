...
// Cabecera de página
	...
	$query = sprintf("SELECT id_solicitud_compra, fecha_solicitud, id_unidad_centro_costo, tipo_compra, id_proveedor, nombre_proveedor, justificacion_proveedor,
						observaciones_proveedor,sustitucion, presupuestado, justificacion_compra, id_estado_solicitud_compras, id_empleado_solicitud,
						fecha_empleado_solicitud, id_empleado_aprobacion, fecha_empleado_aprobacion, id_empleado_conformacion, fecha_empleado_conformacion,
						id_empleado_proceso, fecha_empleado_proceso, fecha_creacion, numero_actualizacion, codigo_unidad_centro_costo,
						nombre_unidad_centro_costo, nombre_departamento, codigo_departamento, nombre_empresa, codigo_empresa, id_departamento,
						numero_solicitud, id_empresa, id_empleado_solicitud, id_empleado_condicionamiento, motivo_condicionamiento,
						fecha_empleado_condicionamiento,tipo_seccion,id_estado_solicitud_compras
					FROM vw_ga_solicitudes
						LEFT JOIN ga_tipo_seccion ON ga_tipo_seccion.id_tipo_seccion = vw_ga_solicitudes.tipo_compra
					WHERE id_solicitud_compra = %s", $_GET['idSolCom']);
	$rs = mysql_query($query) or die(mysql_error());
	$rowS = mysql_fetch_array($rs);
	
	//CONSULTA DETALLES	
	$queryD = sprintf("SELECT id_detalle_solicitud_compra, id_solicitud_compra, id_articulo, cantidad, precio_sugerido,
							fecha_requerida, estado_proceso, codigo_articulo, id_marca, id_tipo_articulo, codigo_articulo_prov,
							descripcion, id_subseccion, stock_maximo, stock_minimo, unidad, foto
						FROM vw_ga_detalle_articulos_solicitud_compra
							WHERE id_solicitud_compra =  %s", $_GET['idSolCom']);
	$rsD = mysql_query($queryD);
	...
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(20,5,"Empresa:",0,0,'R'); // Título
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(120,5,cambiaLetras($rowS['nombre_empresa']),0,0,'L'); // Título
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(25,5,"Fecha:",0,0,'R'); // Título
	$pdf->SetFont('Arial','',12); 
	$pdf->Cell(25,5,$rowS['fecha_solicitud'],0,0,'L'); // Título  
	$pdf->Ln();
	
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(30,5,"Departamento:",0,0,'R'); // Título
	$pdf->SetFont('Arial','',12); 
	...
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(190,5,"Descripcion del Material o Servicio",0,0,'C'); // Título
	$pdf->Ln();
	$header = array("Cantidada", "Unidad", utf8_decode("Código"), utf8_decode("Descripción"), "Precio","Fecha R");
	$pdf->SetFont('Arial','B',12);
	...
	while($rowsD = mysql_fetch_array($rsD)){
		...
		$tabDatos = array($rowsD["cantidad"],$rowsD["unidad"],$rowsD["codigo_articulo_prov"],cambiaLetras($rowsD["descripcion"]),
						  $rowsD["precio_sugerido"],$fechaRequerida);
		$pdf->SetFont('Arial','',12);
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell(140,5,"total:",1,0,'R'); // Título
	$pdf->Cell(50,5,array_sum($total),1,0,'L'); // Título
	...
	if(!$rowS["fecha_empleado_solicitud"] == "" ){
		$fechaSolicitud = date("d-m-Y", strtotime($rowS["fecha_empleado_solicitud"]));
	}	
	if(!$rowS["fecha_empleado_aprobacion"] == "" ){
		$fechaAprobacion = date("d-m-Y", strtotime($rowS["fecha_empleado_aprobacion"]));
	}	
	if(!$rowS["fecha_empleado_conformacion"] == "" ){
		$fechaConformacion = date("d-m-Y", strtotime($rowS["fecha_empleado_conformacion"]));
	}
	if(!$rowS["fecha_empleado_proceso"] == "" ){
		$fechaProceso = date("d-m-Y", strtotime($rowS["fecha_empleado_proceso"]));
	}
	if(!$rowS["fecha_empleado_condicionamiento"] == "" ){
		$fechaCondicion = date("d-m-Y", strtotime($rowS["fecha_empleado_condicionamiento"]));
	}	
