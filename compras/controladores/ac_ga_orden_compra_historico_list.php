...
function buscar($frmBuscar) {
	...
	$numSolicitud = (strpos($frmBuscar['textNroSolicitud'],"-")) ? substr($frmBuscar['textNroSolicitud'], 4) : $frmBuscar['textNroSolicitud'];

		$valBusq = sprintf("%s|%s|%s|%s|%s|%s|%s|%s",
			$frmBuscar['lstEmpresa'],
			$numSolicitud,
			$frmBuscar['textNroOrden'],
			$frmBuscar['txtFechaDes'],
			$frmBuscar['txtFechaHas'],
			$frmBuscar['tipoPago'],
			$frmBuscar['estatusOrden'],
			$frmBuscar['criterioBuscar']);
...
function orderCompraHead($idOrdenCompra){
	$objResponse = new xajaxResponse();
		...
		$sqlOrdCompa = sprintf("SELECT id_orden_compra, vw_ga_historico_ordenes.id_solicitud_compra, fecha,
									vw_ga_historico_ordenes.id_empresa, CONCAT_WS('-',codigo_empresa,numero_solicitud) AS numero_solicitud
									, logo_familia
...
function datosProveedor($idOrdenCompra){
	...
	$sql = sprintf("SELECT id_orden_compra, ga_orden_compra.id_proveedor,  cp_proveedor.nombre AS nombre_proveedor,cp_proveedor.rif AS rif_proveedor,
...
function datosCompras($idOrdeCompra){
	...
	$sql = sprintf("SELECT id_orden_compra,fecha_entrega,fecha_cotizacion,tipo_transporte, ga_orden_compra.id_solicitud_compra, numero_solicitud, fecha_solicitud,
					...
					WHERE id_orden_compra = %s",$idOrdeCompra);	
	...
	$rowsOrdCompa = mysql_fetch_array($queryOrdCompa);
			...
			$tablaDtaCmp = "<table width=\"90%\" border=\"0\" align=\"center\">";
				...
				$tablaDtaCmp .= "<tr>";
					...
					$tablaDtaCmp .= "<td align=\"left\">".date('d - m - Y',strtotime($rowsOrdCompa['fecha_entrega']))."</td>";
...
function detallesOrdenCompras($idOrdenCompra){
	...
	$sqlOrdCompDetalles = 
			sprintf("SELECT
						ga_orden_compra_detalle.id_orden_compra_detalle,ga_orden_compra_detalle.id_orden_compra,
	...
	$sql = sprintf("SELECT id_orden_compra,fecha_entrega,fecha_cotizacion,tipo_transporte,tipo_pago,subtotal,subtotal_descuento, porcentaje_descuento,
						   monto_letras,condiciones_pago,ga_orden_compra.observaciones,ga_orden_compra.id_solicitud_compra,numero_solicitud,
						   fecha_solicitud
					FROM ga_orden_compra
						  LEFT JOIN ga_solicitud_compra ON ga_solicitud_compra.id_solicitud_compra = ga_orden_compra.id_solicitud_compra
					WHERE id_orden_compra = %s",$idOrdenCompra);	
	...
	$sqlIva = sprintf("SELECT ga_orden_compra_iva.iva as iva, sum(ga_orden_compra_iva.`base_imponible`) AS base, SUM(ga_orden_compra_iva.`subtotal_iva`) as subtotal, observacion
							...
							ORDER BY ga_orden_compra_iva.iva;",$idOrdenCompra);
			...
			$tablaContCmp .= "<tr>";
				...
				if ($iva != NULL){
					foreach($iva as $key=>$value){
							$tablaContCmp .= "<tr>";
							$tablaContCmp .= "<td align=\"right\" width=\"67%\">".$value['observacion'].' '.$value['iva'].'%'."</td>";
...
function listHistCompOrden($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL) {
	...
	if($valCadBusq[2] != ""){
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf(" ga_orden_compra.id_orden_compra = %s",
	...
	$query = sprintf("SELECT ga_orden_compra.id_orden_compra,
...
//$objResponse->alert($queryLimit);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= sprintf("<td><img title=\"Ver Orden de Compra\" src=\"../img/iconos/ico_view.png\" class=\"modalImg\" onclick=\"xajax_verHistCompOrden(%s)\"/></td>",
				$row['id_orden_compra']);//window.open('ga_orden_compra_imprimir_pdf.php?view=1&id=".$row['id_orden_compra']."');
			$htmlTb .= sprintf("<td><img title=\"Orden de Compra en PDF\" src=\"../img/iconos/page_white_acrobat.png\" onclick=\"window.open('reportes/ga_orde_compra_pdf.php?idOrdenComp=%s&session=%s');\"/></td>",
