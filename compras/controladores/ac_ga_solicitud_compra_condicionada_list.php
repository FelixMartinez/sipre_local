...
function aporbarSolicitudCompras($idSolicitudCompras,$idObj){
	$objResponse = new xajaxResponse();
		$sql = sprintf("SELECT id_solicitud_compra,ga_solicitud_compra.id_estado_solicitud_compras, estado_solicitud_compras
						...
						WHERE id_solicitud_compra = %s",$idSolicitudCompras);
...
function cargarSolicitudCompras($idSolicitudCompras){
	$objResponse = new xajaxResponse();
		...
		$sql = sprintf("
		SELECT id_solicitud_compra,CONCAT_WS('-',codigo_empresa,numero_solicitud) AS numSolicitud,
			   ga_solicitud_compra.id_empresa,codigo_empresa,nombre_empresa,fecha_solicitud,
			   ga_solicitud_compra.id_unidad_centro_costo,codigo_unidad_centro_costo,nombre_unidad_centro_costo,
			   pg_unidad_centro_costo.id_departamento,nombre_departamento,codigo_departamento,tipo_compra,
			   id_proveedor,justificacion_proveedor,observaciones,sustitucion,presupuestado,justificacion_compra,
			   id_estado_solicitud_compras,
         id_empleado_solicitud, CONCAT_WS(' ', a.nombre_empleado, a.apellido) AS nombre_empleado_solictud,
         a.cedula AS num_empleado_solicitud, fecha_empleado_solicitud,
         id_empleado_aprobacion, CONCAT_WS(' ', b.nombre_empleado, b.apellido) AS nombre_empleado_aprobacion,
			   b.cedula AS num_empleado_aprobacion,fecha_empleado_aprobacion,
         id_empleado_conformacion, CONCAT_WS(' ', c.nombre_empleado, c.apellido) AS nombre_empleado_conformacion,
         c.cedula AS num_empleado_conformacion,fecha_empleado_conformacion,
         id_empleado_proceso, CONCAT_WS(' ', d.nombre_empleado, d.apellido) AS nombre_empleado_proceso,
			   d.cedula AS num_empleado_proceso,fecha_empleado_proceso,
         fecha_creacion,
         id_empleado_condicionamiento, CONCAT_WS(' ', e.nombre_empleado, e.apellido) AS nombre_empleado_condicionamiento,
         e.cedula AS num_empleado_condicionamiento,fecha_modificacion,numero_actualizacion,
		...
		$objResponse->assign("fechSolicitud","value",$rows['fecha_solicitud']);
		$objResponse->assign("codEmpresa","value",$rows['codigo_empresa']);
		$objResponse->assign("nombEmpresa","value",utf8_encode($rows['nombre_empresa']));
		$objResponse->assign("idEmpresa","value",$rows['id_empresa']);
		$objResponse->assign("codDepartamento","value",$rows['codigo_departamento']);
		$objResponse->assign("nombDepartamento","value",utf8_encode($rows['nombre_departamento']));
		$objResponse->assign("idDepartamento","value",$rows['id_departamento']);
		$objResponse->assign("codCentroCosto","value",$rows['codigo_unidad_centro_costo']);
		$objResponse->assign("nombCentroCosto","value",utf8_encode($rows['nombre_unidad_centro_costo']));
		$objResponse->assign("idCentroCosto","value",$rows['id_unidad_centro_costo']);
		...
		$objResponse->assign("justificacionProveedor","value",utf8_encode($rows['justificacion_proveedor']));
		$objResponse->assign("ObservacionProveedor","value",utf8_encode($rows['observaciones']));
		...
		$objResponse->assign("justificacionCompra","value",utf8_encode($rows['justificacion_compra']));
		
		if($rows['id_estado_solicitud_compras']!= 2){}
			
		$objResponse->script("$('#tabAprobacion').show();");
		//empleado solicitud
		$objResponse->assign("tdNombFirmaS","innerHTML",utf8_encode($rows['nombre_empleado_solictud']));
		$objResponse->assign("tdnumEmplS","innerHTML",utf8_encode($rows['num_empleado_solicitud']));
		if($rows["fecha_empleado_solicitud"] != "" ){
			$fechaS = date("d-m-Y", strtotime($rows["fecha_empleado_solicitud"]));
		}
		$objResponse->assign("tdfechaS","innerHTML",$fechaS);
		//empleado aprobacion
		$objResponse->assign("tdNombFirmaA","innerHTML",utf8_encode($rows['nombre_empleado_aprobacion']));
		$objResponse->assign("tdnumEmplA","innerHTML",utf8_encode($rows['num_empleado_aprobacion']));
		if($rows["fecha_empleado_aprobacion"] != "" ){
			$fechaA = date("d-m-Y", strtotime($rows["fecha_empleado_aprobacion"]));
		}		
		$objResponse->assign("tdfechaA","innerHTML",$fechaA);
		//empleado conformacion
		$objResponse->assign("tdNombFirmaC","innerHTML",utf8_encode($rows['nombre_empleado_conformacion']));
		$objResponse->assign("tdnumEmplC","innerHTML",utf8_encode($rows['num_empleado_conformacion']));
		if($rows["fecha_empleado_conformacion"] != "" ){
			$fechaC = date("d-m-Y", strtotime($rows["fecha_empleado_conformacion"]));
		}		
		$objResponse->assign("tdfechaC","innerHTML",$fechaC);
		//empleado proceso
		$objResponse->assign("tdNombFirmaP","innerHTML",utf8_encode($rows['nombre_empleado_proceso']));
		$objResponse->assign("tdnumEmplP","innerHTML",utf8_encode($rows['num_empleado_proceso']));
		if($rows["fecha_empleado_proceso"] != "" ){
			$fechaP = date("d-m-Y", strtotime($rows["fecha_empleado_proceso"]));
		}		
		...
		if($rows["fecha_empleado_condicionamiento"] != "" ){
			$fechaConRe = date("d-m-Y", strtotime($rows["fecha_empleado_condicionamiento"]));
		}
...
function cargarDetallesSolComp($idSolCompras, $idArticulo){
	...
	$query = sprintf("SELECT id_detalle_solicitud_compra,id_solicitud_compra,
			  			ga_detalle_solicitud_compra.id_articulo,
			  			unidad,codigo_articulo,descripcion,
			  			cantidad, precio_sugerido,fecha_requerida
					  ...
					  WHERE ga_detalle_solicitud_compra.id_articulo = %s AND id_solicitud_compra = %s;",$idArticulo,$idSolCompras);
	...
	$inputfecha = sprintf("<input type='text' class='inputHabilitado' readonly='readonly' id='txtFecha%s' name='txtFecha[]' size='10px' value='%s' style='text-align:center; border:0px'>",
		$rows["id_articulo"],$rows["fecha_requerida"]);
 		...
 		$trTd .= "<tr id='tr".$rows["id_articulo"]."' class='textoGris_11px trEliminar'>";
			...
			$trTd .= "<td align='center'>".$rows["codigo_articulo"]."</td>";
...
function combLstEstCompra() {//ESTADO DE COMPRA
	$objResponse = new xajaxResponse();
		...
		$html .= "<select id=\"lisEstCompras\" name=\"lisEstCompras\" class=\"inputHabilitado\">";
...
function combLstTipCompra() { //TIPO COMPRAS
	$objResponse = new xajaxResponse();
		...
		$html .= "<select id=\"lisTipCompras\" name=\"lisTipCompras\" class=\"inputHabilitado\">";
			$html .= "<option value=\"\">[ Seleccione ]</option>";
...
function guardarSolicitud($formval){
	...
	$rowsEmpl = mysql_fetch_array($rsEmpl);
		...
		if($formval['idSolicitudCompras'] > 0){
			if (xvalidaAcceso($objResponse,"ga_solicitud_compra_list","editar")) {
			...
			if (xvalidaAcceso($objResponse,"ga_solicitud_compra_list","insertar")){
				...
				$rowsNumSol = mysql_fetch_array($queryNumSol);
				 	//return $objResponse->alert($rowsNumSol);
					
				$sqlSolComp = sprintf("INSERT INTO ga_solicitud_compra (id_empresa, id_estado_solicitud_compras, 
							fecha_empleado_solicitud, fecha_creacion, fecha_solicitud, numero_solicitud, 
							id_unidad_centro_costo, tipo_compra, id_proveedor, presupuestado, sustitucion, 
							observaciones, justificacion_compra, justificacion_proveedor,id_empleado_solicitud)
							VALUE
							(%s,%s,CURRENT_DATE(),NOW(), CURRENT_DATE(),%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
							valTpDato($formval['idEmpresa'], "int"),
							1,
			...
			if($formval['artAgregadoSolComp'] != 0){
				...
				$fechRequerida = $formval['txtFecha'][$indice];
					...
					$sqlDetArt = sprintf("INSERT INTO ga_detalle_solicitud_compra 
								(id_solicitud_compra,id_articulo,cantidad,precio_sugerido,fecha_requerida) 
								VALUE (%s,%s,%s,%s,'%s');",
							  $idSolCompras,$valorArt[0],$cant,$precio,
							  date("Y-m-d", strtotime($fechRequerida)));
							  //$objResponse->alert($sqlDetArt);
...
function fromSolicitud($tipo, $idSolCompras = ""){
	...
	switch($tipo){
		case "nuevo"://nueva solicitus
			...
			$objResponse->assign("fechSolicitud","value",date("d-m-Y"));
		...
		case "editar": //edita nueva solicitud
			...
			$objResponse->loadCommands(cargarSolicitudCompras($idSolCompras));
				...
				while($rowsDetSolComp = mysql_fetch_array($queryDetSolComp)){
					//$objResponse->alert($rowsDetSolComp['id_articulo']);
...
function listadoArticulo($pageNum = 0, $campOrd = "codigo_articulo", $tpOrd = "ASC", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	if ($valCadBusq[0] != "-1" && $valCadBusq[0] != ""){
		$sqlBusq .= sprintf(" WHERE (id_articulo LIKE %s
	...
	$query = sprintf("SELECT id_articulo, codigo_articulo, ga_articulos.descripcion AS articulos_descripcion,
        				ga_articulos.id_tipo_articulo, ga_tipos_articulos.descripcion AS descripcion_tipos_articulos
					  	FROM ga_articulos
					  LEFT JOIN ga_tipos_articulos ON ga_articulos.id_tipo_articulo = ga_tipos_articulos.id_tipo_articulo 
					  %s",$sqlBusq);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\"  height=\"24\">";
			...
			$htmlTb .= "<td align=\"left\">".htmlentities($row['articulos_descripcion'])."</td>";
			$htmlTb .= "<td>".htmlentities($row['descripcion_tipos_articulos'])."</td>";
		$htmlTb .= "</tr>";
	}
	...
	$objResponse->assign("tdFlotanteTitulo3","innerHTML","Articulos");
...
function listadoEmpresas($pageNum = 0, $campOrd = "nombre_empresa", $tpOrd = "ASC", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$btn = "<button id=\"btnBuscar\" title=\"Buscar\" onclick=\"xajax_BuscarempDepaUnidadCentroCosto(xajax.getFormValues('frmBuscar2'),'Empresa');\" style=\"cursor:default\" name=\"btnBuscar\" type=\"button\">Buscar</button>";
	
	$objResponse->script("openImg(byId('divFlotante2'));");
	$objResponse->assign("tdFlotanteTitulo2","innerHTML","Empresa");
	$objResponse->assign("tdBtnBuscar","innerHTML",$btn);
...
function listadoDepartamento($pageNum = 0, $campOrd = "nombre_departamento", $tpOrd = "ASC", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$sqlBusq .= sprintf(" WHERE id_empresa = %s",
		valTpDato($valCadBusq[0],"int"));
	}
	
	if ($valCadBusq[1] != "-1" && $valCadBusq[01] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("  (id_departamento LIKE %s
	...
	$query = sprintf("SELECT * FROM pg_departamento  %s", $sqlBusq);
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listadoDepartamento", "15%", $pageNum, "id_departamento", $campOrd, $tpOrd, $valBusq, $maxRows, "Id");
		$htmlTh .= ordenarCampo("xajax_listadoDepartamento", "15%", $pageNum, "codigo_departamento", $campOrd, $tpOrd, $valBusq, $maxRows, "Código");
		$htmlTh .= ordenarCampo("xajax_listadoDepartamento", "70%", $pageNum, "nombre_departamento", $campOrd, $tpOrd, $valBusq, $maxRows, "Nombre Departamento");
	$htmlTh .= "</tr>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\"  height=\"24\">";
			$htmlTb .= sprintf("<td>"."<button type=\"button\" onclick=\"xajax_asignarEmpDepartamentoCento(%s,%s);byId('btnCancelar2').click();vaciarCampo();\" title=\"Seleccionar Empresa\"><img src=\"../img/iconos/ico_aceptar.gif\"/></button>"."</td>",
			2,$row['id_departamento']);
			$htmlTb .= "<td align=\"center\">".$row['id_departamento']."</td>";
			$htmlTb .= "<td align=\"center\">".$row['codigo_departamento']."</td>";
			$htmlTb .= "<td align=\"left\">".htmlentities($row['nombre_departamento'])."</td>";
		$htmlTb .= "</tr>";
	}
	...
	$btn = "<button id=\"btnBuscar\" title=\"Buscar\" onclick=\"xajax_BuscarempDepaUnidadCentroCosto(xajax.getFormValues('frmBuscar2'),'Departamento');\" style=\"cursor:default\" name=\"btnBuscar\" type=\"button\">Buscar</button>";
...
function listadoCentroCosto($pageNum = 0, $campOrd = "nombre_unidad_centro_costo", $tpOrd = "ASC", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listadoCentroCosto", "15%", $pageNum, "id_unidad_centro_costo", $campOrd, $tpOrd, $valBusq, $maxRows, "Id");
		$htmlTh .= ordenarCampo("xajax_listadoCentroCosto", "15%", $pageNum, "codigo_unidad_centro_costo", $campOrd, $tpOrd, $valBusq, $maxRows, "Código");
		$htmlTh .= ordenarCampo("xajax_listadoCentroCosto", "70%", $pageNum, "nombre_unidad_centro_costo", $campOrd, $tpOrd, $valBusq, $maxRows, "Nombre Unidad Centro Costo");
	$htmlTh .= "</tr>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\"  height=\"24\">";
			$htmlTb .= sprintf("<td>"."<button type=\"button\" onclick=\"xajax_asignarEmpDepartamentoCento(%s,%s);byId('btnCancelar2').click();\" title=\"Seleccionar Empresa\"><img src=\"../img/iconos/ico_aceptar.gif\"/></button>"."</td>",
			3,$row['id_unidad_centro_costo']);
			$htmlTb .= "<td>".$row['id_unidad_centro_costo']."</td>";
			$htmlTb .= "<td>".$row['codigo_unidad_centro_costo']."</td>";
			$htmlTb .= "<td align=\"left\">".htmlentities($row['nombre_unidad_centro_costo'])."</td>";
		$htmlTb .= "</tr>";
	}
	...
	$btn = "<button id=\"btnBuscar\" title=\"Buscar\" onclick=\"xajax_BuscarempDepaUnidadCentroCosto(xajax.getFormValues('frmBuscar2'),'centroCosto');\" style=\"cursor:default\" name=\"btnBuscar\" type=\"button\">Buscar</button>";
	$btnLim = "<button id=\"btnLimpiar\" title=\"Limpiar\" style=\"cursor:default\" name=\"btnLimpiar\" type=\"button\" 
                    onblur=\"document.forms['frmBuscar2'].reset(); byId('btnBuscar').click();\">";
	/*
...
function listadoSolicitudCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {//LISTA TODAS LAS SOLICITUDES
	$objResponse = new xajaxResponse();
	...
	if($valCadBusq[0] != "" && $valCadBusq[0] != ""){
		$sqlBusq .= sprintf(" AND id_empresa = %s",
			valTpDato($valCadBusq[0],"int"));
	}
	
	if($valCadBusq[1] != "" && $valCadBusq[1] != ""){
		if(strpos($valCadBusq[1], "-")){
				$cadena = substr($valCadBusq[1], 4);
		} else {
			$cadena = $valCadBusq[1];	
		}
		$sqlBusq .= sprintf(" AND numero_solicitud = %s",$cadena);
	}
	
	if($valCadBusq[2] != "" && $valCadBusq[2] != ""){
		$sqlBusq .= sprintf(" AND tipo_compra = %s",
			valTpDato($valCadBusq[2],"int"));
	}
	
	if($valCadBusq[3] != "" && $valCadBusq[3] != ""){
		$sqlBusq .= sprintf(" AND vw_ga_solicitudes.id_estado_solicitud_compras = %s",
			valTpDato($valCadBusq[3],"int"));
	}

	if($valCadBusq[4] != ""){
		$sqlBusq .= sprintf(" AND (numero_solicitud LIKE %s
			OR nombre_empresa LIKE %s
			OR nombre_departamento LIKE %s
			OR nombre_unidad_centro_costo LIKE %s
	...
	$query = sprintf("SELECT vw_ga_solicitudes.*, cp_proveedor.nombre AS nombre_proveedor,
					...
					WHERE vw_ga_solicitudes.id_estado_solicitud_compras in (6,7)
						 AND vw_ga_solicitudes.id_empleado_condicionamiento is not null %s",$sqlBusq);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\"  height=\"24\">";
			...
			$htmlTb .= "<td>".$row['numero_solicitud']."</td>";
			$htmlTb .= "<td align=\"center\">".utf8_encode($row['nombre_empresa'])."</td>";
			$htmlTb .= "<td align=\"\">".utf8_encode($row['nombre_departamento'])."</td>";
			$htmlTb .= "<td align=\"\">".utf8_encode($row['nombre_unidad_centro_costo'])."</td>";
			$htmlTb .= "<td align=\"\" >".utf8_encode($row['nombre_proveedor'])."</td>";
...
function generarFecha(){ //suma 8 dias a la fecha actual
	...
	for($i=1; $i <= 8;  $i++){
		$fechaHoy = date("d-m-Y", strtotime($fechaHoy."+ 1 day"));	
		$diaSemana = date("w", strtotime($fechaHoy));
		if($diaSemana == 0){//domingo
			$fechaHoy = date("d-m-Y",strtotime($fechaHoy."+ 1 day"));
		}elseif($diaSemana == 6){//sabado
			$fechaHoy = date("d-m-Y",strtotime($fechaHoy."+ 2 day"));
		}
	} 	
