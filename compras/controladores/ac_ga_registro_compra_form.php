...
function cargaLstRetencionImpuesto($selId = "") {
	...
	$html = "<select id=\"lstRetencionImpuesto\" name=\"lstRetencionImpuesto\" class=\"inputHabilitado\" style=\"width:150px\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
		$html .= "<option ".(($selId == 0 && strlen($selId) > 0) ? "selected=\"selected\"" : "")." value=\"0\">".utf8_encode("Sin Retención")."</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function formAlmacen($hddNumeroArt, $frmListaArticulo) {
	...
	$objResponse->assign("tdFlotanteTitulo","innerHTML",utf8_encode("Distribuir Artículo en Almacen"));
	$objResponse->script("
...
function guardarDcto($frmDcto, $frmListaArticulo, $frmTotalDcto) {
	...
	if (!($totalRowsConfig402 > 0)) {
		errorGuardarDcto($objResponse,__LINE__);
		return $objResponse->alert(utf8_encode("Falta establecer el parámetro de configuración \"Código 402\" para realizar el registro de compra"));
	}
	
	// VALIDA QUE LA FECHA DEL REGISTRO ESTE ENTRE LA FECHA EN CURSP
	$txtFechaRegistroCompra = date("d-m-Y");
	$txtFechaProveedor = $frmDcto['txtFechaProveedor'];
	if ($valor[0] == 1) {
		if ((date("Y",strtotime($txtFechaProveedor)) == date("Y",strtotime("-".$valor[2]." month",strtotime(date("d-m-Y"))))
	...
	FROM ga_almacenes alm
		INNER JOIN ga_calles calle ON (alm.id_almacen = calle.id_almacen)
	...
	//INSERTA LOS DATOS DE LA FACTURA
		$insertSQL = sprintf("INSERT INTO cp_factura (id_empresa, numero_factura_proveedor, numero_control_factura, fecha_factura_proveedor,
		 id_proveedor, fecha_origen, fecha_vencimiento, id_modulo, id_orden_compra, estatus_factura, observacion_factura, tipo_pago,
		 monto_exento, monto_exonerado, subtotal_factura, porcentaje_descuento, subtotal_descuento, total_cuenta_pagar,saldo_factura,
		 aplica_libros, fecha_registro, id_empleado_creador) VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
			valTpDato($idEmpresa, "int"),
	...
	// INSERTA EL MOVIMIENTO
		$insertSQL = sprintf("INSERT INTO ga_movimiento (id_tipo_clave_movimiento, id_clave_movimiento, id_documento, fecha_movimiento, id_cliente_proveedor, referencia1, referencia2, remision, tipo_costo, fecha_captura, ultima_partida_usada, id_usuario, credito, id_moneda_extranjera_doc, id_moneda_extranjera_doc_cambio, id_moneda_extranjera_ref, id_moneda_extranjera_ref_cambio) VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
		...
		if ($frmTotalDcto['lstRetencionImpuesto'] > 0 && $existeIva == true) {
			...
			foreach($frmTotalDcto['hddIdIva'] as $indice => $valorIva) {
				if ($frmTotalDcto['txtSubTotalIva'.$valorIva] > 0) {
					...
					$insertPagoDoc = sprintf("INSERT INTO cp_pagos_documentos ( id_documento_pago, tipo_documento_pago, tipo_pago, id_documento,
						fecha_pago, numero_documento, banco_proveedor, banco_compania, cuenta_proveedor, cuenta_compania, monto_cancelado)
...
*/		}
	...
	// RECORRE EL DETALLE DE LA ORDEN PARA VERIFICAR Y ASI ACTULIZAR LOS PENDIENTES
		...
		errorGuardarDcto($objResponse,__LINE__);
		$objResponse->alert(utf8_encode("Existen artículos los cuales no tienen una ubicacion asignada"));
	}
...
//$objResponse->alert($queryLimit);
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listadoArticulos", "15%", $pageNum, "codigo_articulo", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Código"));
		$htmlTh .= ordenarCampo("xajax_listadoArticulos", "50%", $pageNum, "descripcion", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Descripción"));
...
function listadoArticulosAlmacen($valForm) {
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= "<td width=\"38%\">".utf8_encode("Código")."</td>";
		$htmlTh .= "<td width=\"50%\">".utf8_encode("Descripción")."</td>";
...
function  listadoGastos($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listadoGastos", "10%", $pageNum, "porcentaje", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Retenciónes"));
	$htmlTh .= "</tr>";
		...
		"<tr id=\"trItmGasto0\">".
			"<td colspan=\"7\">".
				...
				"<table width=\"%s\" class=\"divMsjInfo\">".
					"<tr>".
						"<td><img width=\"25\" src=\"../img/iconos/ico_info2.gif\"></td>".
