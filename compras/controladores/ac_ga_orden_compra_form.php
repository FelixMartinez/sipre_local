...
function asignarArticulo($idPedidoDet, $numeroArt, $valForm) {
	...
	$objResponse->assign("txtCodigoArt","value",$rowArticulo['codigo_articulo']);
...
function cargarDcto($idPedido, $frmListaArticulo, $frmTotalDcto) {
	...
	$objResponse->script(sprintf("$('#trItmPie').before('".
		...
		$contFila, $clase,//tr 301
			...
			$contFila,//307
				...
				$contFila,preg_replace("/[\r?|\n?]/","<br>",htmlentities($rowArticulo['descripcion'])),//317
	...
	$objResponse->assign("txtFechaPreparado","value",date("d-m-Y", strtotime($rowPedido['fecha_solicitud'])));
...
function guardarOrden($frmDcto, $frmListaArticulo, $frmTotalDcto) {
	...
	$objResponse->script(sprintf("window.open('reportes/ga_orde_compra_pdf.php?idOrdenComp=%s&session=%s', 960, 550)",
		$idOrdenCompra,
...
function listaProveedor($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaProveedor", "20%", $pageNum, "rif_proveedor", $campOrd, $tpOrd, $valBusq, $maxRows, htmlentities("C.I. / R.I.F."));
