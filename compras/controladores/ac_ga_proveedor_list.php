...
function cargaLstBancos($selId = "") {
	...
	$html = "<select id=\"lstBanco\" name=\"lstBanco\" class=\"inputHabilitado\" style=\"width:150px\">";
	while ($rowBanco = mysql_fetch_assoc($rsBanco)) {
		$seleccion = ($selId == $rowBanco['idBanco']) ? "selected=\"selected\"" : "";
		
		$html .= "<option value=\"".$rowBanco['idBanco']."\" ".$seleccion.">".htmlentities($rowBanco['nombreBanco'])."</option>";
	}
	$html .= "</select>";
...
function cargaLstFormaPago($selId = "") {
	...
	foreach ($arrayFormaPago as $indice => $valor) {
		...
		$html .= "<option ".$selected." value=\"".htmlentities($arrayFormaPago[$indice])."\">".htmlentities($arrayFormaPago[$indice])."</option>";
	}
	$html .= "</select>";
...
function cargalstMonedas($selId = "") {
	...
	while ($rowMoneda = mysql_fetch_assoc($rsMoneda)) {
		$seleccion = ($selId == $rowMoneda['idmoneda']) ? "selected=\"selected\"" : "";
		
		$html .= "<option value=\"".$rowMoneda['idmoneda']."\" ".$seleccion.">".htmlentities($rowMoneda['descripcion'])."</option>";
	}
	$html .= "</select>";
...
function cargaLstPais($selId = "") {
	...
	$html = "<select id=\"lstPais\" name=\"lstPais\" class=\"inputHabilitado\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
		$seleccion = ($selId == $row['id_origen']) ? "selected=\"selected\"" : "";
		
		$html .= "<option value=\"".$row['id_origen']."\" ".$seleccion.">".htmlentities($row['nom_origen'])."</option>";
	}
	$html .= "</select>";
...
function cargaLstRetencion($selId = "") {
	...
	$html = "<select id=\"lstRetencion\" name=\"lstRetencion\" class=\"inputHabilitado\" style=\"width:150px\">";
	while ($rowRetencion = mysql_fetch_assoc($rsRetencion)) {
		$seleccion = ($selId == $rowRetencion['id']) ? "selected=\"selected\"" : "";
...
function formProveedor($idProveedor, $frmProveedor) {
	...
	if ($idProveedor > 0) {
		...
		$objResponse->assign("txtFechaCracion","value",date("d-m-Y",strtotime($rowProveedor['fechacreacion'])));
		...
		$objResponse->assign("txtFechaCracion","value",date("d-m-Y"));
...
function guardarProveedor($frmProveedor, $frmListaProveedor) {
	...
	if (isset($arrayValidar)) {
		...
		if ($valido == false) {
			...
			errorGuardarProveedor($objResponse);
	...
	if (isset($arrayValidar)) {
		...
		if ($valido == false && strlen($frmProveedor['txtCedulaContacto']) > 0) {
	...
	if ($frmProveedor['hddIdProveedor'] != "") {
		...
		&& !xvalidaAcceso($objResponse,"cp_proveedor_list_credito","editar")) { errorGuardarProveedor($objResponse); return $objResponse; }
		...
		if (!$Result1) { errorGuardarProveedor($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		...
		if ($frmProveedor['lstTipoProveedor'] == "Si") {
			...
			if (!$rsProvCred) { errorGuardarProveedor($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			$rowProvCred = mysql_fetch_assoc($rsProvCred);
			
			if ($rowProvCred['id_credito'] > 0) {
				if (!xvalidaAcceso($objResponse,"cp_proveedor_list_credito","editar")) { errorGuardarProveedor($objResponse); return $objResponse; }
				...
				if (!xvalidaAcceso($objResponse,"cp_proveedor_list_credito","insertar")) { errorGuardarProveedor($objResponse); return $objResponse; }
			...
			if ($frmProveedor['lstPlanMayor'] == 1) {
				...
				if (!$rsProvCredFormaPago) { errorGuardarProveedor($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		...
		&& !xvalidaAcceso($objResponse,"cp_proveedor_list_credito","insertar")) { errorGuardarProveedor($objResponse); return $objResponse; }
...
function listaProveedor($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listaProveedor", "11%", $pageNum, "rif", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode($spanProvCxP));
		$htmlTh .= ordenarCampo("xajax_listaProveedor", "39%", $pageNum, "nombre", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Nombre"));
		$htmlTh .= ordenarCampo("xajax_listaProveedor", "15%", $pageNum, "telefono", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Teléfono"));
		$htmlTh .= ordenarCampo("xajax_listaProveedor", "25%", $pageNum, "correo", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Correo"));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		switch ($row['status']) {
			case "Inactivo" : $imgEstatusPedido = "<img src=\"../img/iconos/ico_rojo.gif\" title=\"Inactivo\"/>"; break;
...
function errorGuardarProveedor($objResponse) {
	$objResponse->script("
	byId('btnGuardarProveedor').disabled = false;
	byId('btnCancelarProveedor').disabled = false;");
}
