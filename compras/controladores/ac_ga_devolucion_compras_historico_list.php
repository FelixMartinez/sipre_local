...
function formNotaCredito($idFacturaCompra) {
	$objResponse = new xajaxResponse();
//$objResponse->alert($idFacturaCompra);	
	//BUSCA LOS DATOS DE LA FACTURA DE COMPRA
	$queryFact = sprintf("SELECT id_movimiento,ga_movimiento.id_clave_movimiento,id_documento,
				id_factura,id_empresa,numero_factura_proveedor,numero_control_factura,fecha_factura_proveedor,fecha_origen,cp_factura.id_proveedor,cp_factura.id_modulo,aplica_libros,
				nombre AS nombre_proveedor,
				descripcion AS descripcion_clave_momiento, pg_clave_movimiento.tipo AS tipo_clave_momiento,
	...
	$objResponse->assign("txtFechaRegistroNotaCredito","value",date("d-m-Y"));
...
function guardarDct($frmDcto, $frmBuscar){
	$objResponse = new xajaxResponse();
	...
	if ($frmDcto['lstAplicaLibro'] == 0) {//SI NO VA A APLICAR A LIBRO
		...
		$queryNumeracion = sprintf("SELECT * FROM pg_empresa_numeracion
				WHERE id_numeracion = (
					SELECT clave_mov.id_numeracion_documento FROM pg_clave_movimiento clave_mov
		...
		$updateNumDocSQL = sprintf("UPDATE pg_empresa_numeracion SET numero_actual = (numero_actual + 1)
			WHERE id_empresa_numeracion = %s;",
			 valTpDato($idEmpresaNumeracion, "int"));
		$Result1 = mysql_query($updateNumDocSQL);
	...
	$insertDevolSQL = sprintf("INSERT INTO cp_notacredito (id_empresa, id_proveedor, saldo_notacredito, 
			 fecha_notacredito, observacion_notacredito, estado_notacredito, id_documento, tipo_documento, 
			 subtotal_notacredito, subtotal_descuento, id_departamento_notacredito, monto_exonerado_notacredito,
			 monto_exento_notacredito, aplica_libros_notacredito, numero_nota_credito, numero_control_notacredito,
			 fecha_registro_notacredito, id_motivo, id_empleado_creador)
			SELECT id_empresa, id_proveedor, %s, %s, %s, %s, id_factura, 'FA', subtotal_factura, subtotal_descuento, id_modulo,
				monto_exonerado, monto_exento, %s, %s, %s, %s, %s, %s
			FROM cp_factura
		WHERE id_factura = %s;",
	...
	/*MODIFICAR LA TABLA DE GA_MOVIMIETNO CON EL SCRIPT SIGUIENTE	
	ALTER TABLE `ga_movimiento` ADD `id_tipo_clave_movimiento` INT NULL AFTER `id_movimiento`
	ALTER TABLE `ga_movimiento` ADD `tipo_documento_movimiento` INT NOT NULL COMMENT '1 = Vale Entrada / Salida, 2 = Nota Credito' AFTER `id_clave_movimiento` 
	ALTER TABLE `ga_movimiento` CHANGE `tipo_documento_movimiento` `tipo_documento_movimiento` INT( 11 ) NULL COMMENT '1 = Vale Entrada / Salida, 2 = Nota Credito'
	
	ALTER TABLE `erp_automotriz`.`ga_movimiento` ADD CONSTRAINT `FK_ga_movimiento_1` FOREIGN KEY `FK_ga_movimiento_1` (`id_clave_movimiento`)
    REFERENCES `pg_clave_movimiento` (`id_clave_movimiento`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE;
		
	ALTER TABLE `erp_automotriz`.`ga_movimiento` ADD CONSTRAINT `FK_ga_movimiento_3` FOREIGN KEY `FK_ga_movimiento_3` (`id_usuario`)
    REFERENCES `pg_usuario` (`id_usuario`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE;*/
	
	/*CREAR campo id_notacredito en la tabla ga_movimiento*/
	$insertMovSQL = sprintf("INSERT INTO ga_movimiento (id_tipo_clave_movimiento, id_clave_movimiento,tipo_documento_movimiento, 
		id_documento,fecha_movimiento, id_cliente_proveedor, tipo_costo, fecha_captura, id_usuario, credito)
			SELECT %s, %s, %s, %s, %s, id_cliente_proveedor, tipo_costo, NOW(), %s, credito
				FROM ga_movimiento
			WHERE id_documento = %s AND id_tipo_clave_movimiento = 1;",
	...
	$insertSQL = sprintf("INSERT INTO ga_movimiento_detalle (id_movimiento, id_articulo, cantidad, precio, costo, 
		porcentaje_descuento, subtotal_descuento, tipo_costo, promocion, id_moneda_costo, id_moneda_costo_cambio)
			SELECT 
				%s,
				mov_det.id_articulo,
				mov_det.cantidad,
			...
			WHERE mov.id_documento = %s AND mov.id_tipo_clave_movimiento = 1;",
	...
	$insertSQL = sprintf("INSERT INTO ga_kardex (id_documento,id_articulo,tipo_movimiento,cantidad,
					id_clave_movimiento,estado,fecha_movimiento,observacion,hora_movimiento)
			SELECT 
	...
	$query = sprintf("SELECT * FROM ga_kardex
	WHERE id_documento = %s
	...
	if (!$rs) { errorGuardarDcto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
 	while ($row = mysql_fetch_assoc($rs)) {
		...
		$queryArtEmp = sprintf("SELECT * FROM vw_ga_articulos_empresa_ubicacion WHERE id_articulo = %s ",
...
$objResponse->alert("BUSCA EL ULTIMO COSTO DEL ARTICULO \n".$queryCostoArt);
	...
	$insertSQL = sprintf("INSERT INTO cp_notacredito_gastos (id_notacredito, id_gastos_notacredito, tipo_gasto_notacredito, porcentaje_monto, monto_gasto_notacredito, estatus_iva_notacredito, id_iva_notacredito, iva_notacredito, id_modo_gasto, afecta_documento, id_factura_importacion, id_condicion_gasto)
	SELECT 
		%s,
		id_gasto,
		tipo,
...
$objResponse->script("byId('btnBuscar').click();"); 
	...
	//$objResponse->loadCommands(listaRegistroCompra(0,'id_factura','DESC',$_SESSION['idEmpresaUsuarioSysGts']."|".$frmBuscar['']."|".$frmBuscar['']));
...
function listaRegistroCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf(" activa IS NULL ");
	...
	FROM vw_ga_facturas_compra  %s", $sqlBusq); //AND activa IS NOT NULL en cp_factura
$objResponse->alert($query);
...
function listaDetalleFactura($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 6, $totalRows = NULL) {
	...
	$query = sprintf("SELECT id_factura_detalle, id_factura, cp_factura_detalle.id_articulo,
			  codigo_articulo, descripcion,
			  cantidad, pendiente, devuelto,precio_unitario,iva, (cantidad * precio_unitario) AS total_articulo
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= sprintf("<td align=\"center\"><input type=\"text\" id=\"txtCodArt%s\" name=\"txtCodArt%s\" readonly=\"readonly\" value =\"%s\" class=\"inputSinFondo\" style=\"text-align:center\" size=\"10\"/></td>",
				$row['id_factura_detalle'],$row['id_factura_detalle'],$row['codigo_articulo']);
			$htmlTb .= sprintf("<td align=\"left\">%s</td>",$row['descripcion']);
...
$xajax->register(XAJAX_FUNCTION,"guardarDct");
