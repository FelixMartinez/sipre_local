...
function formSubSeccion() {
	$objResponse = new xajaxResponse();
//$objResponse->alert("eadj");
	if (xvalidaAcceso($objResponse,"ga_subseccion_list","insertar")) {
		$objResponse->script("
			$('hddIdSubSeccion').value = '';
			byId('txtSubSeccion').className = 'inputHabilitado';
			byId('lstEstatu').className = 'inputHabilitado';");
...
function cargarSubSeccion($idSubSeccion) {
	...
	if (xvalidaAcceso($objResponse,"ga_subseccion_list","editar")) {
		...
		$objResponse->assign("lstEstatu","value",$rowSubSeccion['estatu']);
...
function cargaLstSeccion($selId = "", $tipo) {
	...
	$querySeccion = sprintf("SELECT * FROM ga_secciones ORDER BY descripcion");
	...
	while ($rowSeccion = mysql_fetch_assoc($rsSeccion)) {
		$seleccion = "";
		if ($selId == $rowSeccion['id_seccion'])
			$seleccion = "selected='selected'";
		
		$html .= "<option value=\"".$rowSeccion['id_seccion']."\" ".$seleccion.">".htmlentities($rowSeccion['descripcion'])."</option>";
	}
	$html .= "</select>";
...
function guardarSubSeccion($valForm) {
	...
	if ($valForm['hddIdSubSeccion'] > 0) {
		...
		if (xvalidaAcceso($objResponse,"ga_subseccion_list","editar")) {
			...
			WHERE id_subseccion = %s",
				...
				valTpDato($valForm['lstEstatu'], "text"),
	...
	$objResponse->alert("Sub-Sección Guardada con Exito");
	
	$objResponse->script("byId('btnCancelar').click();");
	
	$objResponse->script("byId('btnBuscar').click();");
	//$objResponse->loadCommands(listadoSubSecciones(0,'descripcion_subseccion','ASC'));
...
function eliminarSubSeccion($idSubseccion) {
	...
	if (xvalidaAcceso($objResponse,"ga_subseccion_list","eliminar")) {
		if (isset($idSubseccion)) {
			mysql_query("START TRANSACTION;");
			
				$deleteSQL = sprintf("UPDATE ga_subsecciones SET estatu = 0 WHERE id_subseccion = %s",valTpDato($idSubseccion, "int"));
			...
			$objResponse->alert("Se elimino la Sub-Sección con Exito");
...
function eliminarSubSeccionBloque($valForm) {
	$objResponse = new xajaxResponse();
	...
	if (xvalidaAcceso($objResponse,"ga_subseccion_list","eliminar")) {
		if (isset($valForm['cbxSubSec'])) {
			...
			$nun = count($valForm['cbxSubSec']);
			
			$objResponse->alert("Se eliminado ".$nun." Sub-Sección con Exito");
...
function listadoSubSecciones($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= sprintf("<td><input id=\"cbxItm\" name=\"cbxSubSec[]\" type=\"checkbox\" value=\"%s\"></td>",$row['id_subseccion']);
...
$xajax->register(XAJAX_FUNCTION,"eliminarSubSeccionBloque");
