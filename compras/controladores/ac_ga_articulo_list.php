...
$objResponse = new xajaxResponse();
	...
	switch($tipo){
		...
		case "buscar":
			...
			$objResponse->assign("txtBusCodigoArticulo","value",$rows['codigo_articulo']);
			$objResponse->assign("txtBusCodigoArtProv","value",$rows['codigo_articulo_prov']);
			$objResponse->assign("txtBusDescripcion","value",$rows['descripcion']);
		...
		case "editar":
			...
			$objResponse->assign("txtCodigoArticulo","value",$rows['codigo_articulo']);
			$objResponse->assign("txtCodigoArtPro","value",$rows['codigo_articulo_prov']);
			$objResponse->assign("txtDescripcion","value",$rows['descripcion']);
		...
		case "ver2":
			$objResponse->loadCommands(cargaLstMarca( $rows['id_marca'], $tipo));
			$objResponse->loadCommands(cargaLstTipoArticulo($rows['id_tipo_articulo'], $tipo));
			$objResponse->assign("txtCodigoArtVer","value",$rows['codigo_articulo']);
			$objResponse->assign("txtCodigoArtProVer","value",$rows['codigo_articulo_prov']);
...
function cargarArtAlterno($idArtculo){
	...
	$query = sprintf("SELECT id_articulo_codigo_alterno,ga_articulos_codigos_alternos.id_articulo,id_articulo_alterno,
					  	vw_ga_articulos.descripcion,codigo_articulo,existencia
					  ...
					  WHERE ga_articulos_codigos_alternos.id_articulo = %s;",$idArtculo);
	$rs =mysql_query($query);
...
function cargarArtSustituto($idArtculo){
	$objResponse = new xajaxResponse();
	//$objResponse->alert($idArtculo);
	$query = sprintf("SELECT id_articulo_codigo_sustituto, ga_articulos_codigos_sustitutos.id_articulo,
						id_articulo_sustituto, codigo_articulo, vw_ga_articulos.descripcion, existencia
					...
					WHERE ga_articulos_codigos_sustitutos.id_articulo = %s;",$idArtculo);
	$rs =mysql_query($query);
...
function cargaLstTipoArticulo($selId = "",$tipo) {
	...
	switch($tipo){
		...
		case "buscador"; //carga en el formualrio de art sustituto y alterno
			$select = "<select id=\"lstTipoArticuloArtBus2\" name=\"lstTipoArticuloArtBus2\" class=\"inputInicial\" OnFocus=\"this.blur()\">";
...
function guardarArticulo($valorForm){
	$objResponse = new xajaxResponse();
		...
		if($valorForm['hddIdArticulo'] > 0){
			if (xvalidaAcceso($objResponse,"ga_articulo_list","editar")) {
				...
				UPDATE ga_articulos SET
						...
						id_tipo_unidad = %s
				WHERE id_articulo = %s;",
						valTpDato($valorForm['txtCodigoArticulo'], "text"),
						valTpDato($valorForm['lstMarcaArt'], "int"),
						valTpDato($valorForm['lstTipoArticuloArt'], "int"),
						valTpDato($valorForm['txtCodigoArtPro'], "text"),
						valTpDato($valorForm['txtDescripcion'], "text"),
			...
			if (xvalidaAcceso($objResponse,"ga_articulo_list","insertar")){
				$query = sprintf("INSERT INTO ga_articulos
					(codigo_articulo,id_marca,id_tipo_articulo,codigo_articulo_prov,descripcion,foto,id_subseccion,id_tipo_unidad,id_empresa_creador) 
		...
		$objResponse->loadCommands(listadoArticulos("","","",$_SESSION['idEmpresaUsuarioSysGts']."|". 1));
...
function listadoDctosPedComp($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 5, $totalRows = NULL) {
	...
	$objResponse->assign("txtArticulo","innerHTML",htmlentities($rowArticulo['descripcion']));
