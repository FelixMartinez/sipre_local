...
function formSeccion() {
	...
	if (xvalidaAcceso($objResponse,"ga_seccion_list","insertar")) {
		$objResponse->script("
			...
			byId('lstTipoSeccion').className = 'inputHabilitado';
			byId('lstEstatus').className = 'inputHabilitado';");
		
		//$objResponse->script("xajax_cargaLstTipoSeccion('', 'nuevo');");
...
function cargarSeccion($idSeccion) {
	...
	if (xvalidaAcceso($objResponse,"ga_seccion_list","editar")) {
...
function guardarSeccion($valForm) {
	...
	$objResponse->alert("Sección Guardada con Exito");
...
function eliminarSeccion($idSeccion){
	...
	if (xvalidaAcceso($objResponse,"ga_seccion_list","eliminar")) {
		if (isset($idSeccion)) {
			mysql_query("START TRANSACTION;");
			...
			$objResponse->loadCommands(listadoSecciones(0,'descripcion','ASC'));
		}
	}
...
function eliminarSeccionBloque($valForm) {
	$objResponse = new xajaxResponse();
	...
	if (xvalidaAcceso($objResponse,"ga_seccion_list","eliminar")) {
		if (isset($valForm['cbxItm'])) {
			...
			$objResponse->alert("Se a eliminado: ".$num." con exito ");
			
				$objResponse->loadCommands(listadoSecciones(
				$valForm['pageNum'],
				$valForm['campOrd'],
				$valForm['tpOrd'],
				$valForm['valBusq']));
...
function listadoSecciones($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL) {
	...
	if ($valCadBusq[1] != "-1" && $valCadBusq[1] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("(descripcion LIKE %s) OR (corta LIKE %s)",
			valTpDato("%".$valCadBusq[1]."%", "text"),valTpDato("%".$valCadBusq[1]."%", "text"));
	}
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a class=\"modalImg\" id=\"aEditar%s\" rel=\"#divFlotante\" onclick=\"abrirDivFlotante1(this, 'tblSeccion', '%s');\"><img class=\"puntero\" src=\"../img/iconos/pencil.png\" title=\"Editar\"/></a>",
					$contFila,
					$row['id_seccion']);
			$htmlTb .= "</td>";
...
$xajax->register(XAJAX_FUNCTION,"eliminarSeccionBloque");
