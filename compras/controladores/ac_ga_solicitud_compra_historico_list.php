...
function BuscarSolicituComp($valorFrom){//HACE LA BUSQUEDA
	$objResponse = new xajaxResponse();
		...
		$valBus = sprintf("%s|%s|%s|%s|%s|%s",
			...
			$valorFrom["txtFechaSol"],
			$valorFrom["txtFechaSolHasta"]);
...
function listadoSolicitudCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL) {//LISTA TODAS LAS SOLICITUDES
	...
	if($valCadBusq[4] != "" && $valCadBusq[4] != 0 && $valCadBusq[4] != NULL && $valCadBusq[5] != "" && $valCadBusq[5] != 0 && $valCadBusq[5] != NULL){
		$sqlBusq .= sprintf(" AND fecha_solicitud BETWEEN %s AND %s",
			valTpDato(date("Y-m-d", strtotime($valCadBusq[4])),"text"),
			valTpDato(date("Y-m-d", strtotime($valCadBusq[5])),"date"));
	}

	$query = sprintf("SELECT vw_ga_solicitudes.*,SUM(cantidad * precio_sugerido) AS total_precio_cantidad, cp_proveedor.nombre AS nombre_proveedor, CONCAT_WS('-', codigo_empresa, numero_solicitud) AS numero_solicitud,
						...
						(SELECT COUNT(id_detalle_solicitud_compra) FROM ga_detalle_solicitud_compra
							WHERE ga_detalle_solicitud_compra.id_solicitud_compra = vw_ga_solicitudes.id_solicitud_compra) AS items
					...
					WHERE vw_ga_solicitudes.id_estado_solicitud_compras = 5 %s
...
//$objResponse->alert($queryLimit);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\"  height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".date("d-m-Y", strtotime($row['fecha_solicitud']))."</td>";
