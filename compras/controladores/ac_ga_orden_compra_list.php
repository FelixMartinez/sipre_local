...
function buscar($valForm) {
	...
	$numSolicitud = (strpos($valForm["txtNroSolict"],"-")) ? substr($valForm["txtNroSolict"], 4) : $valForm["txtNroSolict"] ;
...
function listaPedidoCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$query = sprintf("SELECT vw_ga_sol_comp.id_empresa,id_solicitud_compra,nombre_empresa,id_estado_solicitud_compras, fecha_solicitud,
					CONCAT_WS('-',(SELECT codigo_empresa FROM pg_empresa where pg_empresa.id_empresa = vw_ga_sol_comp.id_empresa),numero_solicitud)AS numero_solicitud,
					tipo_compra,nombre_porveedor,observaciones,items,total,id_estado_solicitud_compras
...
//$objResponse->alert($query);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>".utf8_encode($row['nombre_porveedor'])."</td>";
