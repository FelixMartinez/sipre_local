		...
		switch($tipoAsg){
			...
			case 3://unidad centro de costo
				...
				$objResponse->script("document.getElementById('btnCentroCosto').disabled = true;");
					break; 
			default:
				$sql = sprintf("SELECT id_proveedor, nombre FROM cp_proveedor WHERE id_proveedor = %s", $id);
					break;
		}
...
function BuscarSolicituComp($valorFrom){//HACE LA BUSQUEDA
	...
	$numSolicitud = (strpos($valorFrom["txtNroSolicitud"],"-")) ? substr($valorFrom["txtNroSolicitud"], 4) : $valorFrom["txtNroSolicitud"] ;
		...
		$valBus = sprintf("%s|%s|%s|%s|%s|%s|%s",
			...
			$valorFrom["txtFechaDes"],
			$valorFrom["txtFechaHas"],
...
function cargarSolicitudCompras($idSolicitudCompras){
	$objResponse = new xajaxResponse();
		...
		$sql = sprintf("SELECT id_solicitud_compra,CONCAT_WS('-',codigo_empresa,numero_solicitud) AS numSolicitud,
			   ...
			   ga_solicitud_compra.id_proveedor,e.nombre, justificacion_proveedor,ga_solicitud_compra.observaciones,sustitucion,presupuestado,justificacion_compra,
         ...
         fecha_creacion,fecha_modificacion,numero_actualizacion,
			   motivo_condicionamiento,id_empleado_condicionamiento,fecha_empleado_condicionamiento
		FROM ga_solicitud_compra
		...
		if($rows["fecha_solicitud"] != "" ){
			$fechSolicitud = date("d-m-Y", strtotime($rows["fecha_solicitud"]));
		}
		$objResponse->assign("fechSolicitud","value",$fechSolicitud);
...
function guardarSolicitud($formval){
	...
	if($formval['idSolicitudCompras'] > 0){
		...
		$sqlSolComp = sprintf("INSERT INTO ga_solicitud_compra (id_empresa, id_estado_solicitud_compras,  fecha_empleado_solicitud, fecha_creacion, 
			fecha_solicitud, numero_solicitud, id_unidad_centro_costo, tipo_compra, id_proveedor, presupuestado, sustitucion, observaciones, 
			justificacion_compra, justificacion_proveedor,id_empleado_solicitud)
		VALUE
...
function listadoArticulo($pageNum = 0, $campOrd = "codigo_articulo", $tpOrd = "ASC", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\"  height=\"24\">";
			...
			$htmlTb .= "<td>".$row['descripcion_tipos_articulos']."</td>";
		$htmlTb .= "</tr>";
	}
...
function listadoEmpresas($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	//$btn = "<button id=\"\" title=\"Buscar\" onclick=\";\" style=\"cursor:default\" name=\"btnBuscar\" type=\"button\">Buscar</button>";
...
function listadoSolicitudCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {//LISTA TODAS LAS SOLICITUDES
	...
	if($valCadBusq[0] != "" && $valCadBusq[0] != 0 && $valCadBusq[0] != NULL){
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("id_empresa = %s",
			valTpDato($valCadBusq[0],"int"));
	}
	
	if($valCadBusq[1] != "" && $valCadBusq[1] != 0 && $valCadBusq[1] != NULL){
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("numero_solicitud = %s",
			($valCadBusq[1]));
	}
	
	if($valCadBusq[2] != "1-" && $valCadBusq[2] != "" && $valCadBusq[2] != 0 && $valCadBusq[2] != NULL){
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("tipo_compra = %s",
			valTpDato($valCadBusq[2],"int"));
	}
	
	if($valCadBusq[3] != "" && $valCadBusq[3] != 0 && $valCadBusq[3] != NULL){
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("vw_ga_solicitudes.id_estado_solicitud_compras = %s",
			valTpDato($valCadBusq[5],"int"));
	}
//$objResponse->alert($valCadBusq[4]."  ".$valCadBusq[5]);
	if($valCadBusq[4] != "" && $valCadBusq[4] != 0 && $valCadBusq[4] != NULL && $valCadBusq[5] != "" && $valCadBusq[5] != 0 && $valCadBusq[5] != NULL){
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	...
	if($valCadBusq[6] != "" && $valCadBusq[6] != 0 && $valCadBusq[6] != NULL){
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	...
	$query = sprintf("SELECT vw_ga_solicitudes.*, SUM(cantidad * precio_sugerido) AS Precio_cantidad , 
						cp_proveedor.nombre AS nombre_proveedor, CONCAT_WS('-', codigo_empresa, numero_solicitud) AS numero_solicitud,
						ga_estado_solicitud_compra.estado_solicitud_compras, ga_tipo_seccion.tipo_seccion,
						(SELECT COUNT(id_detalle_solicitud_compra) FROM ga_detalle_solicitud_compra 
							WHERE id_solicitud_compra = vw_ga_solicitudes.id_solicitud_compra GROUP BY id_solicitud_compra) AS ITEMS
						...
						LEFT JOIN ga_detalle_solicitud_compra ON ga_detalle_solicitud_compra.id_solicitud_compra = vw_ga_solicitudes.id_solicitud_compra
							%s 
						GROUP BY ga_detalle_solicitud_compra.id_solicitud_compra ",$sqlBusq);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\"  height=\"24\">";
			...
			$htmlTb .= "<td>".date("d-m-Y", strtotime($row['fecha_solicitud']))."</td>";
			$htmlTb .= "<td align=\"\">".$row['tipo_seccion']."</td>";
			$htmlTb .= "<td align=\"\" >".$row['nombre_proveedor']."</td>";
...
function procesarSolicitud($fromVal){
	...
	$objResponse->script("byId('buttBuscarList').click();");	
...
function insertarItemArticuloSolicitud($contFila, $idArticulo = "", $idSolCompras = "") {
	...
	$fecha = ($idSolCompras != "") ? strtotime(date("d-m-Y",$rows["fecha_requerida"])) : generarFecha();
	...
	$htmlItmPie = sprintf("$('#trItemsSolicitudArt').before('".
		...
		$contFila, $clase, $contFila,//tr
			...
			"5%",$contFila,//id	
			$rows["unidad"],
			"15%",$rows["codigo_articulo"],
			"35%",decodificar($rows["descripcion"]),
			$contFila,$contFila,$fecha = ($idSolCompras != "") ? date("d-m-Y", strtotime($rows["fecha_requerida"])) : generarFecha() ,//imput fecha
