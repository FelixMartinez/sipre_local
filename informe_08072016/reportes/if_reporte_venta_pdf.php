...
$sqlBusq2 .= $cond.sprintf("fact_vent.idDepartamentoOrigenFactura IN (2)
...
$queryVentaMensual = sprintf("SELECT
	...
	COUNT((CASE MONTH(fact_vent.fechaRegistroFactura)
		WHEN %s THEN
			fact_vent.idFactura
	END)) AS nro_unidades_vendidas,
	
	COUNT(fact_vent.idFactura) AS nro_unidades_vendidas_acumulado,
	
	SUM((CASE MONTH(fact_vent.fechaRegistroFactura)
		WHEN %s THEN
			fact_det_vehic.precio_unitario
	END)) AS monto_facturado_vehiculo,
	
	SUM(fact_det_vehic.precio_unitario) AS monto_facturado_vehiculo_acumulado
FROM cj_cc_encabezadofactura fact_vent
  INNER JOIN cj_cc_factura_detalle_vehiculo fact_det_vehic ON (fact_vent.idFactura = fact_det_vehic.id_factura)
  INNER JOIN an_unidad_fisica uni_fis ON (fact_det_vehic.id_unidad_fisica = uni_fis.id_unidad_fisica)
...
SELECT 
	...
	((-1) * COUNT((CASE MONTH(nota_cred.fechaNotaCredito)
		WHEN %s THEN
			nota_cred.idNotaCredito
	END))) AS nro_unidades_vendidas,
	
	((-1) * COUNT(nota_cred.idNotaCredito)) AS nro_unidades_vendidas_acumulado,
	
	((-1) * SUM((CASE MONTH(nota_cred.fechaNotaCredito)
		WHEN %s THEN
			nota_cred_det_vehic.precio_unitario
	END))) AS monto_facturado_vehiculo,
	
	((-1) * SUM(nota_cred_det_vehic.precio_unitario)) AS monto_facturado_vehiculo_acumulado
FROM cj_cc_nota_credito_detalle_vehiculo nota_cred_det_vehic
	INNER JOIN cj_cc_notacredito nota_cred ON (nota_cred_det_vehic.id_nota_credito = nota_cred.idNotaCredito)
	...
	LEFT JOIN cj_cc_encabezadofactura fact_vent ON (nota_cred.idDocumento = fact_vent.idFactura) %s
GROUP BY uni_fis.id_condicion_unidad;",
	valTpDato($valFecha[0], "date"),
	valTpDato($valFecha[0], "date"),
	$sqlBusq,
...
FROM cj_cc_factura_detalle_accesorios fact_det_acc
	INNER JOIN cj_cc_encabezadofactura fact_vent ON (fact_det_acc.id_factura = fact_vent.idFactura)
	INNER JOIN an_pedido ped_vent ON (fact_vent.numeroPedido = ped_vent.id_pedido)
	INNER JOIN vw_pg_empleados vw_pg_empleado ON (fact_vent.idVendedor = vw_pg_empleado.id_empleado) %s
...
FROM cj_cc_nota_credito_detalle_accesorios nota_cred_det_acc
	RIGHT JOIN cj_cc_notacredito nota_cred ON (nota_cred_det_acc.id_nota_credito = nota_cred.idNotaCredito)
...
$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
$sqlBusq .= $cond.sprintf("fact_vent.idDepartamentoOrigenFactura IN (2)
AND fact_det_acc.id_tipo_accesorio IN (1,3)");

$sqlBusq2 = "";
$cond = (strlen($sqlBusq2) > 0) ? " AND " : " WHERE ";
$sqlBusq2 .= $cond.sprintf("fact_vent.idDepartamentoOrigenFactura IN (2)
AND nota_cred_det_acc.id_tipo_accesorio IN (1,3)
AND nota_cred.idDepartamentoNotaCredito IN (2)
AND nota_cred.tipoDocumento LIKE 'FA'");
...
FROM (
	SELECT
		...
		SUM((CASE MONTH(fact_vent.fechaRegistroFactura)
			WHEN %s THEN
				fact_det_acc.cantidad
		END)) AS cant_facturado_accesorio,
		
		SUM((CASE MONTH(fact_vent.fechaRegistroFactura)
			WHEN %s THEN
				fact_det_acc.precio_unitario
		END)) AS monto_facturado_accesorio,
		
		SUM((CASE MONTH(fact_vent.fechaRegistroFactura)
			WHEN %s THEN
				fact_det_acc.costo_compra
		END)) AS monto_costo_facturado_accesorio,
		
		SUM(fact_det_acc.precio_unitario) AS monto_facturado_accesorio_acumulado,
		SUM(fact_det_acc.costo_compra) AS monto_facturado_costo_accesorio_acumulado
	FROM cj_cc_factura_detalle_accesorios fact_det_acc
		INNER JOIN an_accesorio acc ON (fact_det_acc.id_accesorio = acc.id_accesorio)
		INNER JOIN cj_cc_encabezadofactura fact_vent ON (fact_det_acc.id_factura = fact_vent.idFactura)
		INNER JOIN an_pedido ped_vent ON (fact_vent.numeroPedido = ped_vent.id_pedido)
		INNER JOIN vw_pg_empleados vw_pg_empleado ON (fact_vent.idVendedor = vw_pg_empleado.id_empleado) %s
	GROUP BY vw_pg_empleado.id_empleado, fact_det_acc.id_accesorio
	...
	SELECT 
		...
		((-1) * SUM(nota_cred_det_acc.precio_unitario)) AS monto_facturado_accesorio_acumulado,
		((-1) * SUM(nota_cred_det_acc.costo_compra)) AS monto_facturado_costo_accesorio_acumulado
	FROM cj_cc_nota_credito_detalle_accesorios nota_cred_det_acc
		...
		INNER JOIN an_pedido ped_vent ON (fact_vent.numeroPedido = ped_vent.id_pedido)
		INNER JOIN vw_pg_empleados vw_pg_empleado ON (fact_vent.idVendedor = vw_pg_empleado.id_empleado) %s
	GROUP BY vw_pg_empleado.id_empleado, nota_cred_det_acc.id_accesorio
...
$queryVentaAsesor = sprintf("SELECT
	...
	SUM(fact_det_vehic.precio_unitario) AS monto_facturado_vehiculo_acumulado
FROM cj_cc_encabezadofactura fact_vent
	LEFT JOIN cj_cc_factura_detalle_vehiculo fact_det_vehic ON (fact_vent.idFactura = fact_det_vehic.id_factura)
	LEFT JOIN an_unidad_fisica uni_fis ON (fact_det_vehic.id_unidad_fisica = uni_fis.id_unidad_fisica)
	INNER JOIN an_pedido ped_vent ON (fact_vent.numeroPedido = ped_vent.id_pedido)
	LEFT JOIN an_solicitud_factura ped_comp_det ON (uni_fis.id_pedido_compra_detalle = ped_comp_det.idSolicitud)
	INNER JOIN vw_pg_empleados vw_pg_empleado ON (fact_vent.idVendedor = vw_pg_empleado.id_empleado) %s
...
SELECT 
	...
	((-1) * SUM(nota_cred_det_vehic.precio_unitario)) AS monto_facturado_vehiculo_acumulado
FROM cj_cc_nota_credito_detalle_vehiculo nota_cred_det_vehic
	RIGHT JOIN cj_cc_notacredito nota_cred ON (nota_cred_det_vehic.id_nota_credito = nota_cred.idNotaCredito)
...
$queryVentaModelo = sprintf("SELECT
	...
	SUM(fact_det_vehic.precio_unitario) AS monto_unidades_vendidas_acumulado
FROM cj_cc_encabezadofactura fact_vent
	LEFT JOIN cj_cc_factura_detalle_vehiculo fact_det_vehic ON (fact_vent.idFactura = fact_det_vehic.id_factura)
	LEFT JOIN an_unidad_fisica uni_fis ON (fact_det_vehic.id_unidad_fisica = uni_fis.id_unidad_fisica)
	INNER JOIN an_pedido ped_vent ON (fact_vent.numeroPedido = ped_vent.id_pedido)
...
SELECT
	...
	((-1) * SUM(nota_cred_det_vehic.precio_unitario)) AS monto_unidades_vendidas_acumulado
FROM cj_cc_nota_credito_detalle_vehiculo nota_cred_det_vehic
	RIGHT JOIN cj_cc_notacredito nota_cred ON (nota_cred_det_vehic.id_nota_credito = nota_cred.idNotaCredito)
	LEFT JOIN an_unidad_fisica uni_fis ON (nota_cred_det_vehic.id_unidad_fisica = uni_fis.id_unidad_fisica)
	LEFT JOIN an_solicitud_factura ped_comp_det ON (uni_fis.id_pedido_compra_detalle = ped_comp_det.idSolicitud)
...
FROM cj_cc_encabezadofactura fact_vent
	...
	INNER JOIN an_pedido ped_vent ON (fact_vent.numeroPedido = ped_vent.id_pedido)
	LEFT JOIN an_solicitud_factura ped_comp_det ON (uni_fis.id_pedido_compra_detalle = ped_comp_det.idSolicitud)
	INNER JOIN vw_iv_empresas_sucursales vw_iv_emp_suc ON (fact_vent.id_empresa = vw_iv_emp_suc.id_empresa_reg) %s
GROUP BY fact_vent.id_empresa, uni_fis.id_condicion_unidad
...
GROUP BY nota_cred.id_empresa, uni_fis.id_condicion_unidad
...
imagestring($img,1,0,$posY,str_pad(utf8_decode("REPORTE DE VENTA ".$valFecha[0]."-".$valFecha[1]), 152, " ", STR_PAD_BOTH),$textColor);
		
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// RESUMEN MENSUAL
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
$posY += 20;
imagestring($img,1,0,$posY,str_pad(utf8_decode("RESUMEN MENSUAL"), 152, " ", STR_PAD_BOTH),$textColor);
...
if (isset($arrayVentaMensual)) {
	foreach ($arrayVentaMensual as $indice => $valor) {
		...
		imagestring($img,1,170,$posY,str_pad(number_format($arrayFila[1],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,305,$posY,str_pad(number_format($arrayFila[2],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,440,$posY,str_pad(number_format($arrayFila[3],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,575,$posY,str_pad(number_format($arrayFila[4],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,710,$posY,str_pad(number_format($arrayFila[5],2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
...
imagestring($img,1,170,$posY,str_pad(number_format(round($arrayTotalVentaMensual['nro_unidades_vendidas'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
imagestring($img,1,305,$posY,str_pad(number_format(round($arrayTotalVentaMensual['nro_unidades_vendidas_acumulado'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
imagestring($img,1,440,$posY,str_pad(number_format(round($arrayTotalVentaMensual['monto_facturado_vehiculo'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
imagestring($img,1,575,$posY,str_pad(number_format(round($arrayTotalVentaMensual['monto_facturado_vehiculo_acumulado'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
imagestring($img,1,710,$posY,str_pad(number_format(round($arrayTotalVentaMensual['porcentaje_participacion'],2),2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
...
if (isset($arrayVentaAsesor)) {
	$posY += 20;
	imagestring($img,1,0,$posY,str_pad(utf8_decode("VENTAS POR ASESOR"), 152, " ", STR_PAD_BOTH),$textColor);
	...
	imagestring($img,1,170,$posY,str_pad(number_format(round($arrayTotalVentaAsesor['nro_unidades_vendidas'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,305,$posY,str_pad(number_format(round($arrayTotalVentaAsesor['nro_unidades_vendidas_acumulado'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,440,$posY,str_pad(number_format(round($arrayTotalVentaAsesor['monto_facturado_vehiculo'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,575,$posY,str_pad(number_format(round($arrayTotalVentaAsesor['monto_facturado_vehiculo_acumulado'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,710,$posY,str_pad(number_format(round($arrayTotalVentaAsesor['porcentaje_participacion'],2),2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
}
...
if (isset($arrayVentaAccesorio)) {
	$posY += 20;
	imagestring($img,1,0,$posY,str_pad(utf8_decode("ACCESORIOS INSTALADOS POR ASESOR"), 98, " ", STR_PAD_BOTH),$textColor);
	...
	foreach ($arrayVentaAccesorio as $indice => $valor) {
		...
		imagestring($img,1,170,$posY,str_pad(number_format($arrayFila[1],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,305,$posY,str_pad(number_format($arrayFila[2],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,440,$posY,str_pad(number_format($porcParticipacion,2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
	...
	imagestring($img,1,170,$posY,str_pad(number_format(round($arrayTotalVentaAccesorio[1],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,305,$posY,str_pad(number_format(round($arrayTotalVentaAccesorio[2],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,440,$posY,str_pad(number_format(round($arrayTotalVentaAccesorio[3],2),2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
}
...
if (isset($arrayVentaAdicional)) {
	...
	$posY += 20;
	imagestring($img,1,0,$posY,str_pad(utf8_decode("ADICIONALES INSTALADOS POR ASESOR"), 152, " ", STR_PAD_BOTH),$textColor);
	...
	foreach ($arrayVentaAdicional as $indice => $valor) {
		...
		if (isset($arrayVentaAdicional[$indice]['array_adicional'])) {
			foreach ($arrayVentaAdicional[$indice]['array_adicional'] as $indice2 => $valor2) {
				...
				imagestring($img,1,305,$posY,str_pad(number_format($arrayFila[1],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
				imagestring($img,1,440,$posY,str_pad(number_format($arrayFila[2],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
				imagestring($img,1,575,$posY,str_pad(number_format($arrayFila[3],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
				imagestring($img,1,710,$posY,str_pad(number_format($arrayFila[4],2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
			}
		}
	...
	imagestring($img,1,305,$posY,str_pad(number_format(round($arrayTotalVentaAdicional['cant_facturado_accesorio'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,440,$posY,str_pad(number_format(round($arrayTotalVentaAdicional['monto_facturado_accesorio'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,575,$posY,str_pad(number_format(round($arrayTotalVentaAdicional['monto_facturado_accesorio_acumulado'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,710,$posY,str_pad(number_format(round($arrayTotalVentaAdicional['porcentaje_participacion'],2),2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
$posY += 20;
imagestring($img,1,0,$posY,str_pad(utf8_decode("VENTAS POR MODELO DE VEHÍCULO"), 152, " ", STR_PAD_BOTH),$textColor);
	...
	foreach ($arrayVentaModelo as $indice => $valor) {
		...
		if (isset($arrayVentaModelo[$indice]['array_modelo'])) {
			foreach ($arrayVentaModelo[$indice]['array_modelo'] as $indice2 => $valor2) {
				...
				imagestring($img,1,320,$posY,str_pad(number_format($arrayFila[1],2,".",","), 20, " ", STR_PAD_LEFT),$textColor);
				imagestring($img,1,420,$posY,str_pad(number_format($arrayFila[2],2,".",","), 20, " ", STR_PAD_LEFT),$textColor);
				imagestring($img,1,520,$posY,str_pad(number_format($arrayFila[3],2,".",","), 20, " ", STR_PAD_LEFT),$textColor);
				imagestring($img,1,620,$posY,str_pad(number_format($arrayFila[4],2,".",","), 20, " ", STR_PAD_LEFT),$textColor);
				imagestring($img,1,720,$posY,str_pad(number_format($arrayFila[5],2,".",","), 8, " ", STR_PAD_LEFT),$textColor);
			}
		}
...
imagestring($img,1,320,$posY,str_pad(number_format(round($arrayTotalVentaModelo['cant_unidades_vendidas'],2),2,".",","), 20, " ", STR_PAD_LEFT),$textColor);
imagestring($img,1,420,$posY,str_pad(number_format(round($arrayTotalVentaModelo['cant_unidades_vendidas_acumulado'],2),2,".",","), 20, " ", STR_PAD_LEFT),$textColor);
imagestring($img,1,520,$posY,str_pad(number_format(round($arrayTotalVentaModelo['monto_unidades_vendidas'],2),2,".",","), 20, " ", STR_PAD_LEFT),$textColor);
imagestring($img,1,620,$posY,str_pad(number_format(round($arrayTotalVentaModelo['monto_unidades_vendidas_acumulado'],2),2,".",","), 20, " ", STR_PAD_LEFT),$textColor);
imagestring($img,1,720,$posY,str_pad(number_format(round($arrayTotalVentaModelo['porcentaje_participacion'],2),2,".",","), 8, " ", STR_PAD_LEFT),$textColor);
...
if (isset($arrayVentaEmpresa)) {
	$posY += 20;
	imagestring($img,1,0,$posY,str_pad(utf8_decode("VENTAS POR EMPRESA"), 152, " ", STR_PAD_BOTH),$textColor);
	...
	imagestring($img,1,170,$posY,str_pad(number_format(round($arrayTotalVentaEmpresa['nro_unidades_vendidas'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,305,$posY,str_pad(number_format(round($arrayTotalVentaEmpresa['nro_unidades_vendidas_acumulado'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,440,$posY,str_pad(number_format(round($arrayTotalVentaEmpresa['monto_facturado_vehiculo'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,575,$posY,str_pad(number_format(round($arrayTotalVentaEmpresa['monto_facturado_vehiculo_acumulado'],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,710,$posY,str_pad(number_format(round($arrayTotalVentaEmpresa['porcentaje_participacion'],2),2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
}
...
if (isset($arrayVentaContado)) {
	$posY += 20;
	imagestring($img,1,0,$posY,str_pad(utf8_decode("OPERACIONES A CONTADO"), 152, " ", STR_PAD_BOTH),$textColor);
	...
	foreach ($arrayVentaContado as $indice => $valor) {
		...
		imagestring($img,1,440,$posY,str_pad(number_format($arrayFila[3],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,575,$posY,str_pad(number_format($arrayFila[4],2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,710,$posY,str_pad(number_format($porcParticipacion,2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
	...
	imagestring($img,1,170,$posY,str_pad(number_format(round($arrayTotalVentaContado[1],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,305,$posY,str_pad(number_format(round($arrayTotalVentaContado[2],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,440,$posY,str_pad(number_format(round($arrayTotalVentaContado[3],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,575,$posY,str_pad(number_format(round($arrayTotalVentaContado[4],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,710,$posY,str_pad(number_format(round($arrayTotalVentaContado[5],2),2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
}
...
if (isset($arrayVentaCredito)) {
	...
	$posY += 20;
	imagestring($img,1,0,$posY,str_pad(utf8_decode("OPERACIONES A CRÉDITO"), 152, " ", STR_PAD_BOTH),$textColor);
	...
	imagestring($img,1,170,$posY,str_pad(number_format(round($arrayTotalVentaCredito[1],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,305,$posY,str_pad(number_format(round($arrayTotalVentaCredito[2],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,440,$posY,str_pad(number_format(round($arrayTotalVentaCredito[3],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,575,$posY,str_pad(number_format(round($arrayTotalVentaCredito[4],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,710,$posY,str_pad(number_format(round($arrayTotalVentaCredito[5],2),2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
...
if (isset($arraySeguros)) {
	...
	$posY += 20;
	imagestring($img,1,0,$posY,str_pad(utf8_decode("COMPAÑIA DE SEGUROS"), 152, " ", STR_PAD_BOTH),$textColor);
	...
	imagestring($img,1,170,$posY,str_pad(number_format(round($arrayTotalSeguros[1],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,305,$posY,str_pad(number_format(round($arrayTotalSeguros[2],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,440,$posY,str_pad(number_format(round($arrayTotalSeguros[3],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,575,$posY,str_pad(number_format(round($arrayTotalSeguros[4],2),2,".",","), 26, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,710,$posY,str_pad(number_format(round($arrayTotalSeguros[5],2),2,".",","), 10, " ", STR_PAD_LEFT),$textColor);
