...
if (!function_exists("valTpDato")) {
	function valTpDato($theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "") {
		...
		switch ($theType) {
			...
			case "campo":			$theValue = ($theValue != "") ? trim($theValue) : NULL; break;
...
if (!function_exists("ordenarCampo")) {
	function ordenarCampo($funcion, $tamanoColumna, $pageNum, $campoBD, $campOrd, $tpOrd, $valBusq, $maxRows, $nombreColumna) {
		...
		$html .= sprintf("<td class=\"puntero\" width=\"%s\" onclick=\"%s(%s,'%s','%s','%s',%s);\"><table cellpadding=\"0\" cellspacing=\"0\" width=\"%s\"><tr align=\"center\"><td width=\"8px\"></td><td>%s</td><td width=\"8px\">%s</td></tr></table></td>",
			$tamanoColumna,
			$funcion,
			$pageNum,
			$campoBD,
