...
function generarComprasAdORG($idFactura=0,$Desde="",$Hasta=""){
$con = ConectarBD();
...
$exec = EjecutarExec($con,$SqlStr) or die($SqlStr."<br>Line: ".__LINE__);
	...
	$SqlStr = "SELECT
		...
		,(SELECT SUM(cantidad*precio_unitario)  FROM  ".$_SESSION['bdEmpresa'].".cp_factura_detalle WHERE  id_factura=a.id_factura AND d.id_subseccion = c.id_subseccion) as debecxp
	FROM
	...
	while ($row = ObtenerFetch($exec)) {
		...
		if(buscarDoc($id,$cc,$ct,$dt,$fecha)==0){
			...
			// Compras sin descuentos
				ingresarRenglon($cuentacontable,$descripcion,$debecxp,$Haber,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
				//NO VA - 28/07/2014
				...
				if(!in_array($id, $arrayIdFactura)){
					//echo("cuenta iva ".$cuentaiva."de ".$documento);
...
//ingresarRenglon($cuentaCXC,$descripcionConcepto.'CXC',$montoCXC,0,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);		
				ingresarRenglon($cuentaVenta,$descripcion,$montoCXC,0,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);		
			...
			ingresarRenglon($cuentaCXC,$descripcionConcepto,$Debe,$Haber,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'H',$cc,$id);
...
$con = ConectarBD();
	...
	while ($rowFact = ObtenerFetch($execFact)){
		...
		//echo ("Nota: ".$Nota." -ID: ".$id);//=>Nota: 51 -ID: 51
		...
		if(buscarDoc($id,$cc,$ct,$dt,$fecha)==0){
			...
			while ($row = ObtenerFetch($exec)) {
				...
				if($id_cond <> 1){
					 $cuentaCXC = buscarContable(88,$idcliente,$sucursal);
					//echo("<br>".$id_cond."(usado)-".$descripcion."-cuenta N°- ".$cuentaCXC);
				}else{
					$cuentaCXC = buscarContable(19,$idcliente,$sucursal);	
					//echo("<br>".$id_cond."(nuevo)-".$descripcion."-cuenta N°- ".$cuentaCXC);
				}
					
				if ($row[0] != 1){// vehículos usados
					//echo("es un usado".$idcliente.$sucursal);
					$cuentacontable =  buscarContable(3,$id_modelo,$sucursal);//inventario vehiculos usados

					//se busca por la acción CXC Vehiculos Usados
					// $cuentaCXC = buscarContable(88,$idcliente,$sucursal);	   
					///
				...
				}else{//vehículos nuevos	
					//echo("es un nuevo");			
					$cuentacontable =  buscarContable(2,$id_modelo,$sucursal);//inventario vehiculos nuevos

					//se busca por la acción CXC Vehiculos
					// $cuentaCXC = buscarContable(19,$idcliente,$sucursal);	   
					///
...
$SqlStr = "select
       ...
       ,b.iva as porcentaje_iva
	   ,a.id_empresa
	   ,(SELECT sum(precio_unitario) FROM ".$_SESSION['bdEmpresa'].".cj_cc_factura_detalle_accesorios as a1  WHERE id_tipo_accesorio = 1 AND a.idfactura = a1.id_factura) AS montot1
		from ".$_SESSION['bdEmpresa'].".cj_cc_encabezadofactura a
			...
			while ($row = ObtenerFetch($exec)) {
				  ...
				  if ($EntroVehiculo == ""){
					 ...
					 //$cuentaVenta = buscarContable(36,$id_accesorio,$sucursal);// 23 es compras adminstriativas en el encabezado y 5 es en el detalle de los fijos.
					  if($Nota == 0){  
						 $cuentaVenta = buscarContable(29,$id_accesorio,$sucursal);// para los vehiculos nuevos
					  }else{	 
					 ...
					 //se comenta acá y se coloca lineas antes para distinguir de si es usado o no eh vehículo y buscar la cuenta en base a ese criterio
					 //$cuentaCXC = buscarContable(19,$idcliente,$sucursal);	   
			 ...
			 // - PRUEBA DE ACCESORIOS TIPO 1 06/11/2015 --- LA-------
				$cuentaacc = buscarContable(36,$idcliente,$sucursal);
				ingresarRenglon($cuentaacc,"TIPO 1",$debetipo1,0,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
				ingresarRenglon($cuentaacc,"TIPO 1",0,$debetipo1,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'H',$cc,$id);
			// FIN ------------------------------------------------------
             ...
             if ($EntroVehiculo == ""){
			  ...
			  if ($EntroVehiculo == "S"){
				  ...
				  //$montoiva = bcadd($montoivaA,$montoivaV,2);16-04-2011
					...
					//la siguiente linea se comenta como prueba		
				  //$cuentaCXC = buscarContable(19,$idcliente,$sucursal);	   
				   // Compras sin descuentos
				if ($Nota ==0){	
						   ingresarRenglon($cuentaVenta,$descripcion,$Debe,$Haber,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'H',$cc,$id);
				 ...
				 //   Fin Compras sin descuentos
						...
						if ($Nota ==0){		 
						//es la cuentacxc la que hay q ver x donde integrarla si es usado o no
							ingresarRenglon($cuentaCXC,$descripcion,$montoCXC,0,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);		
								    /* Para insertar los terceros */
										ingresarEnlacesTerceros($cuentaCXC,$fecha,$montoCXC,0,$idcliente
										,"cj_cc_cliente","id|concat(nombre,' ',apellido)",$descripcion);
					...
					if ($Nota ==0){		 	
						...
						//ingresarRenglon($cuentaCosto,$NC.$descripcion."aaaa".$id,0,$Costo,$NroCompra,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
						ingresarRenglon($cuentaCosto,$NC.$descripcion,0,$Costo,$documentoNC,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
						...
						//este renglon no deberia aparecer aquí?
						
						//ingresarRenglon($cuentacontable,$NC.$descripcion."bbbb",$Costo,0,$NroCompra,$ct,$dt,$cc,$icomprobant,$fecha,'H',$cc,$id);
						
						ingresarRenglon($cuentacontable,$NC.$descripcion,$Costo,0,$documentoNC,$ct,$dt,$cc,$icomprobant,$fecha,'H',$cc,$id);
...
/*F 7*/
//**************************VENTAS SERVICIOS****************************************
function generarVentasSeORG($idFactura=0,$Desde="",$Hasta="",$Nota=0){
	$con = ConectarBD();
	$SqlStr = "select a.idfactura
		,a.idcliente
		,a.fecharegistrofactura
...
//		$cuentaCXC = buscarContable(5,$idcliente,$sucursal);	
		...
		if(buscarDoc($id,$cc,$ct,$dt,$fecha)==0){	
			...
			while ($row = ObtenerFetch($exec)) {
				...
				if(($idfiltro==9)||($idfiltro==10)||($idfiltro==9)){
					if($idfiltro==9){					
...
$SqlStr = "select a.idnotacredito
      ...
      ,(select subtotal_iva from ".$_SESSION['bdEmpresa'].".cj_cc_nota_credito_iva where idNotaCredito = a.idNotaCredito)
      ,(select cuenta from detalleintegracion x1 where x1.idencabezado = 26 and x1.idobjeto = 15 and x1.sucursal = 1) as cuentaiva
      ,a.idcliente as idcliente
      ,(select concat(nombre,' ',apellido) from ".$_SESSION['bdEmpresa'].".cj_cc_cliente x3 where x3.id = a.idcliente) as descliente
      ,a.subtotal_descuento
...
$con = ConectarBD();
	...
	$SqlStr.="GROUP BY c.fecha_pago,a.iddepartamentoorigenfactura, a.id_empresa, a.idcliente, c.id_encabezado_rs
		ORDER BY a.numerofactura";
	$icomprobant = 0;
	$exec = EjecutarExec($con,$SqlStr) or die($SqlStr." " .mysql_error());
...
function generarConceptosAnticiposVeORG($idTran=0,$Desde="",$Hasta=""){
$con = ConectarBD();
	$ct='19';
	$dt='07';
	$cc='05';
...
$con = ConectarBD();
	...
	while ($row = ObtenerFetch($exec)) {
		...
		//filtro para distinguir cahs back/bono dealer del resto para integrar por las nuevas acciones
		if($id_concepto == 1){
			$sqlDesCond = "SELECT a.idPago
						,a.id_factura
						,a.numeroFactura
						,c.id_unidad_fisica
						,b.id_anticipo 
						FROM ".$_SESSION['bdEmpresa'].".an_pagos a INNER JOIN ".$_SESSION['bdEmpresa'].".cj_cc_anticipo_concepto b ON a.numeroDocumento = b.id_anticipo 		
						INNER JOIN ".$_SESSION['bdEmpresa'].".cj_cc_factura_detalle_vehiculo c ON a.id_factura=c.id_factura 
						WHERE b.id_anticipo = ".$id." and fecha_registro BETWEEN '".date('Y-m-d',strtotime($Desde))."' AND '". date('Y-m-d',strtotime($Hasta))."'";
			
			$execDC = EjecutarExec($con,$sqlDesCond) or die($sqlDesCond." " .mysql_error());
			
			while($rowDC = ObtenerFetch($execDC)){
				$id_fact = 	$rowDC[1];
				$id_uni_fis = $rowDC[3];
			...
			$SqlCond = "SELECT a.id_unidad_fisica
					,a.id_uni_bas
					,a.id_condicion_unidad
					FROM ".$_SESSION['bdEmpresa'].".an_unidad_fisica a
					WHERE a.id_unidad_fisica = ".$id_uni_fis;
								
					
			$execC = EjecutarExec($con,$SqlCond) or die($SqlCond." " .mysql_error());
			
			//echo("se ejecuto ".$SqlCond);
			
			while($rowC = ObtenerFetch($execC)){
				$id_cond_un = $rowC[2];				
				
				if($id_cond_un == 1){//vehículo nuevo
					$cuentaConcepto = buscarContable(89,$id_uni_fis,$sucursal);//cash back/bono dealer nuevos
				}elseif($id_cond_un == 2){//vehículo usado
					$cuentaConcepto = buscarContable(90,$id_uni_fis,$sucursal);//cash back/bono dealer nuevos
				}else{//vehículo usado particular
					$cuentaConcepto = buscarContable(91,$id_uni_fis,$sucursal);//cash back/bono dealer nuevos
				}	
...
$exec =  EjecutarExec($con,$SqlStr) or die($SqlStr."<br>Line: ".__LINE__); 
	...
	$SqlStr = "SELECT
		...
		,b.total_credito
		,c.id_condicion_unidad
		,b.allowance
		,b.payoff
	FROM ".$_SESSION['bdEmpresa'].".an_vale_entrada a INNER JOIN ".$_SESSION['bdEmpresa'].".an_tradein b ON a.id_vale_entrada = b.id_vale_entrada INNER JOIN ".$_SESSION['bdEmpresa'].".an_unidad_fisica c ON a.id_unidad_fisica = c.id_unidad_fisica";
	// and x.tipo_documento_movimiento = 1  es para entradas y salidas
	// and x1.tipo = 2 es para entradas y 4 es para salidas
	...
	while ($row = ObtenerFetch($exec)) {
		...
		if (is_null($cuentacontable) or $cuentacontable == ""){
			if($condicion==1){
				$cuentacontable = buscarContable(2,0,$sucursal); //vehiculos nuevos
			}else{				
				$cuentacontable = buscarContable(3,0,$sucursal);// vehiculos usados
			}
		}
		$descripcion = $row[4];
		$Debe = $row[5];
		...
		if(buscarDoc($id,$cc,$ct,$dt,$fecha)==0){
			...
			//$cuentaCXP = buscarContable(65,0,$sucursal); // esta ya no es una cuenta por pagar si no una cuenta de vale de entrada... Este era el que estaba originalmente
			/*if($costo_neto <> 0){
				$cuentaCXP = buscarContable(13,0,$sucursal);
			}else{
				$cuentaCXP = buscarContable(19,0,$sucursal);
			}*///esta condicion sustituia a la original... pero no es correcta
			
			if(($costo_neto == 0)||($costo_neto > $payoff)){
				$cuentaCXP = buscarContable(13,0,$sucursal);
			}
			
			if($costo_neto < $payoff){
				$cuentaCXP = buscarContable(19,0,$sucursal);
			}
...
function generarNotasCreditoCpAd($idTran=0,$Desde="",$Hasta=""){
	...
	$SqlStr = "SELECT
		...
		,'' AS cuentacontable
		,a.id_documento	
		,b.subtotal_descuento
		,b.id_factura
	FROM
		".$_SESSION['bdEmpresa'].".cp_notacredito a inner join ".$_SESSION['bdEmpresa'].".cp_factura b ON a.id_documento = b.id_factura
	WHERE
...
//	echo $SqlStr;
	...
	while ($row = ObtenerFetch($exec)) {
		...
		if ($facturaAnt != $id){
			$SqlStr="SELECT sum(ivaRetenido) FROM ".$_SESSION['bdEmpresa'].".cp_retenciondetalle WHERE idfactura = $idDocumento";
			//echo $SqlStr;
			$exec3 = EjecutarExec($con,$SqlStr) or die($SqlStr."<br>Line: ".__LINE__); 
			$row = ObtenerFetch($exec3);
			if ($row[0] > 0){
		...
		if(!is_null($idDocumento)){ // SI POSEE GASTOS
			...
			$SqlStr = "SELECT
						...
						WHERE a.id_documento = ".$idDocumento." and id_departamento_notacredito = 0";
			if($idTran != 0){
			...
			//echo("<br>".$documento." gasto: ".$sumaMontoGasto." id doc".$idDocumento);		
		...
		if(buscarDoc($id,$cc,$ct,$dt,$fecha) == 0){
			...
			if ($Descuento == 0){			
				...
				$cuentaDescuento = buscarContable(6,2,$sucursal);
//				$montoCXP = bcadd($monto,$montoiva,2);
				$montoCXP = bcsub($montoTotal,$Descuento,2);
				//$montoCXP = bcsub($monto,$Descuento,2);
				
				ingresarRenglon($cuentaDescuento,$descripcion,$Descuento,0,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
				ingresarRenglon($cuentaCXP,$descripcion,$montoCXP,0,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
			}
			//**	
...
//echo "<pre>";
/*foreach(debug_backtrace() as $key => $array){
	$funciones .= $array["function"]." ";
	$array["function"]." ";
}

$descripcion = $funciones.$descripcion;
*/
