...
function validarCampo(idObj, requerido, formato) {
			...
			|| (formato == 'numeroControl' && formatoNumeroControl(idObj) == false)) {
				...
				if (document.getElementById(idObj).readOnly == false || document.getElementById(idObj).disabled == false) {
					switch (document.getElementById(idObj).className) {
						...
						case 'inputCompletoErrado' : 	clase = "inputCompleto"; break;
						case 'inputSinFondoErrado' : 	clase = "inputSinFondo"; break;
						default : 						clase = document.getElementById(idObj).className; break;
					}
				} else {
					clase = "inputInicial";
	...
	if (inArray(clase, ["inputInicial","inputHabilitado","inputCompleto","inputSinFondo","inputCompletoHabilitado","inputTotal",""])) {
		return true;
	} else if (inArray(clase, ["inputErrado","inputCompletoErrado","inputSinFondoErrado"])) {
		return false;
	}
}
...
function formatoFecha(idObj) {
	texto = document.getElementById(idObj).value;
	var b= /^(0[1-9]|1\d|2\d|3[0-1])-(0[1-9]|1[0-2])-\d{4}$/;
	var c= /^(0[1-9]|1\d|2\d|3[0-1])\/(0[1-9]|1[0-2])\/\d{4}$/;
