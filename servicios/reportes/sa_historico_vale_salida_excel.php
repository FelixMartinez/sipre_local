...
FROM sa_orden orden
	...
	GROUP BY sa_vale_salida.id_vale_salida ORDER BY sa_vale_salida.numero_vale DESC  ",
	$join,
	$sqlBusq
	);
...
while ($rowMaestro = mysql_fetch_assoc($rsMaestro)) {
	...
	$arrayCol[$contFila][5] = utf8_encode($rowMaestro['nombre_empleado'])." ";
	$arrayCol[$contFila][6] = utf8_encode($rowMaestro['nombre_tecnico']);
	$arrayCol[$contFila][7] = utf8_encode($rowMaestro['nombre_tipo_orden']);
	$arrayCol[$contFila][8] = utf8_encode($rowMaestro['nombre_cliente']);
	$arrayCol[$contFila][9] = utf8_encode($rowMaestro['nom_uni_bas']);
	$arrayCol[$contFila][10] = utf8_encode($rowMaestro['placa']);
	$arrayCol[$contFila][11] = utf8_encode($rowMaestro['chasis']);
	$arrayCol[$contFila][12] = utf8_encode($rowMaestro['nombre_estado']);
	$arrayCol[$contFila][13] = ($rowMaestro['id_orden_retrabajo']);
	$arrayCol[$contFila][14] = round($rowMaestro['total_orden'],2);
	
	// TOTALES
	$arrayTotalPagina[13] += $arrayCol[$contFila][14];
}
...
$excel->download('Historico vales de salida de servicios.xls');
