	...
	function load_page($args=''){//$page,$maxrows,$order,$ordertype,$capa,
		...
		if($rec){
			...
			foreach($rec as $v){
				...
				$html.='<tr><td>'.$v->codigo_tempario.'</td><td>'.$v->descripcion_tempario.'</td><td align="center">'.$v->descripcion_modo.'</td><td align="center">'.$v->operador.'</td><td>'.$v->precio.'</td><td>'.$v->costo.'</td><td>'.$v->descripcion_seccion.'</td><td>'.$v->descripcion_subseccion.'</td></tr>';
