...
////implementando xajax;
	...
	//require_once("../control/iforms.inc.php");//comentado gregor
...
function load_page($args=''){//$page,$maxrows,$order,$ordertype,$capa,
		...
		if($rec){
			$html.='<table class="order_table">';
				...
				$html.='<tr><td style="background-color: #bfbfbf; font-weight:bold;">PAQUETE</td><td style="background-color: #bfbfbf; font-weight:bold;">Descripci&oacute;n</td><!--<td style="background-color: #bfbfbf; font-weight:bold;">Unidades</td>-->';
				$html.='<td style="background-color: #bfbfbf; font-weight:bold;">Codigo</td><td style="background-color: #bfbfbf; font-weight:bold;">Descripci&oacute;n</td><td style="background-color: #bfbfbf; font-weight:bold;">Unidad</td><td style="background-color: #bfbfbf; font-weight:bold;">Cantidad</td>';				
				if($esCombo){ 
			...
			foreach($rec as $v){
				...
				if($recdet){
					foreach($recdet as $temp){
						$html.='<tr><td>'.$v->codigo_paquete.'</td><td>'.$v->descripcion_paquete.'</td><!--<td>'.$lista_unidades.'</td>-->';
						$html.='<td>'.$temp->codigo_tempario.'</td><td>'.$temp->descripcion_tempario.'</td><td>'.$temp->descripcion_modo.'</td><td>N/A</td>';
				...
				if($recdet){
					foreach($recdet as $rep){
						$html.='<tr><td>'.$v->codigo_paquete.'</td><td>'.$v->descripcion_paquete.'</td><!--<td>'.$lista_unidades.'</td>-->';
						$html.='<td>'.$rep->codigo_articulo.'</td><td>'.$rep->descripcion.'</td><td>'.$rep->unidad.'</td><td>'.$rep->cantidad.'</td>';
