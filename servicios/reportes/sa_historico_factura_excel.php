...
$queryMaestro = sprintf("SELECT *,
	...
	cliente.telf AS telf_cliente,
	cliente.otrotelf AS otrotelf_cliente,
...
FROM sa_orden orden
	...
	GROUP BY fact_vent.idFactura ORDER BY fact_vent.numeroFactura DESC ",
	$join,
	$sqlBusq);
...
while ($rowMaestro = mysql_fetch_assoc($rsMaestro)) {
	...
	$arrayCol[$contFila][6] = utf8_encode($rowMaestro['nombre_empleado'])." ";
	$arrayCol[$contFila][7] = utf8_encode($rowMaestro['nombre_tecnico']);
	$arrayCol[$contFila][8] = utf8_encode($rowMaestro['nombre_tipo_orden']);
	$arrayCol[$contFila][9] = utf8_encode($rowMaestro['nombre_cliente']);
	$arrayCol[$contFila][10] = utf8_encode($rowMaestro['cedula_cliente']);
	$arrayCol[$contFila][11] = utf8_encode($rowMaestro['direccion_cliente']);
	$arrayCol[$contFila][12] = utf8_encode($rowMaestro['telf_cliente']);
	$arrayCol[$contFila][13] = utf8_encode($rowMaestro['otrotelf_cliente']);
	$arrayCol[$contFila][14] = utf8_encode($rowMaestro['correo_cliente']);
	$arrayCol[$contFila][15] = utf8_encode($rowMaestro['nom_uni_bas']);
	$arrayCol[$contFila][16] = utf8_encode($rowMaestro['nom_modelo']);
	$arrayCol[$contFila][17] = utf8_encode($rowMaestro['nom_ano']);
	$arrayCol[$contFila][18] = utf8_encode($rowMaestro['kilometraje']);
	$arrayCol[$contFila][19] = utf8_encode($rowMaestro['motivo']);
	$arrayCol[$contFila][20] = utf8_encode($rowMaestro['placa']);
	$arrayCol[$contFila][21] = utf8_encode($rowMaestro['chasis']);
	$arrayCol[$contFila][22] = utf8_encode($rowMaestro['nombre_estado']);
	$arrayCol[$contFila][23] = ($rowMaestro['id_orden_retrabajo']);
	$arrayCol[$contFila][24] = $rowMaestro['total'];
	
	// TOTALES
	$arrayTotalPagina[24] += $arrayCol[$contFila][24];
}
...
$excel->download('Historico facturas de servicios.xls');
