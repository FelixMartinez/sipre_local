...
//necesito un unico id porque si abres 2 pdf al mismo tiempo uno puede borrar el otro o usar la misma imagen o codigo de barras ya previamente generada!! por eso se borran al final, pero si los borras y el otro no genera, dara error de que no encuentra la imagen
$UID = uniqid();
$contador = 1;//paginas

$esGarantia = intval($_GET['garantia']);//1 si

$valCadBusq = explode("|",$_GET['valBusq']);
//$valCadBusq[0]; ID DEL DOCUMENTO
//$valCadBusq[1]; 1 = presupuesto y cotizacion, 0 o else o !=1 = Orden de servicio
//$valCadBusq[2]; 3 = vale de salida
...
if ($valCadBusq[1] == 1){//presupuesto y cotizacion
	...
	$numeroBarras = $datosNumeroPresupuesto["numero_presupuesto"];
	...
	if($valCadBusq[2] == 3){//vale salida
		...
		$numeroBarras = $datosNumeroValeSalida["numero_vale"];
	}else{//orden
		...
		$numeroBarras = $datosNumeroOrden["numero_orden"];		
	}
...
$pais = $rowConfig403['valor'];

if($pais != 3){//sino es puerto rico
    //RIF EMPRESA
...
//if($valCadBusq[2] == 3)
//	imagestring($img,1,375,82,$rowCliente['numero_vale'],$textColor);
//else
...
//VALE
imagestring($img,1,376,92,$rowCliente['numeracion_recepcion'],$textColor);
imageline($img,10,100,602,100,$textColor);
...
$queryFalla = sprintf("SELECT id_recepcion_falla, descripcion_falla, diagnostico_falla, respuesta_falla FROM sa_recepcion_falla WHERE id_recepcion = %s",
...
if (mysql_num_rows($rsFalla) > 0){
	if($pais == "3"){//puerto rico
		while($rowFalla = mysql_fetch_assoc($rsFalla)){//guardo para usar despues
			$arrayFallas[$rowFalla['id_recepcion_falla']] = $rowFalla;
		}
	}else{//vzla panama
...
if ($valCadBusq[1] == 1){//presupuesto y cotizacion
	...
	if($valCadBusq[2] == 3){ //vale de salida
		...
		$queryRepuestosGenerales = sprintf("SELECT
			...
			sa_det_orden_articulo.id_falla
		FROM
			vw_iv_articulos
...
if (mysql_num_rows($rsOrdenDetRep) > 0){
	if($pais == "3"){//puerto rico
		...
		while ($rowOrdenDetRep = mysql_fetch_assoc($rsOrdenDetRep)) {
			...
			if($pais == 2 || $pais == 3){//si es puerto o panama
				$cantidadDisponibleActual = cantidadDisponibleActual($rowOrdenDetRep['id_articulo'],$idEmpresa);
			}else{//si es vzla
				$cantidadDisponibleActual = cantidadDisponibleActualLote($rowOrdenDetRep['id_articulo_costo']);
			}
			
			imagestring($img,1,325,$posY,str_pad($rowOrdenDetRep['id_articulo_costo']." ".$cantidadDisponibleActual, 17, " ", STR_PAD_LEFT),$textColor);
...
if ($valCadBusq[1] == 1){
	...
	if($valCadBusq[2] == 3){
		$queryFactDetTemp = sprintf("SELECT
			...
			sa_det_orden_tempario.id_falla
		FROM
...
//ITEMS POR FALLAS SOLO PUERTO RICO
/****************************************************************************/

if($pais == "3"){//puerto rico
	imagestring($img,1,100,$posY,"FALLA",$textColor);
	//imagestring($img,1,290,$posY,"CAUSA",$textColor);
	imagestring($img,1,440,$posY,"CORRECCION",$textColor);
	$posY += 10;
	$margenIzq = 50;
	
	foreach($arrayFallas as $idRecepcionFalla => $arrayFalla){
		//$posY += 10;
		imagestring($img,1,10,$posY,strtoupper(substr($arrayFalla['descripcion_falla'],0,55)),$textColor);
		imagestring($img,1,328,$posY,strtoupper(substr($arrayFalla['respuesta_falla'],0,55)),$textColor);
		
		//$posY += 10;
		//imagestring($img,1,10,$posY,strtoupper(substr($arrayFalla['descripcion_falla'],55,55)),$textColor);
		//imagestring($img,1,328,$posY,strtoupper(substr($arrayFalla['respuesta_falla'],55,55)),$textColor);
		
		$posY += 10;
		//REPUESTOS POR FALLA
		foreach($arrayRepuestos as $arrayRepuesto){
			if($idRecepcionFalla == $arrayRepuesto["id_falla"]){
				//imagestring($img,1,300,$posY,"REPUESTOS",$textColor);	
				//imagestring($img,1,420,$posY,"CANT.",$textColor);

				if($valCadBusq[1] == 1){//si es presupuesto no mostrar codigo
					$codigoRepuesto = "";
				}else{
					$codigoRepuesto = elimCaracter($arrayRepuesto['codigo_articulo'],";");
				}

				if($arrayRepuesto['id_paquete']){
					$provienePaquete = "(P)";
				}else{
					$provienePaquete = "";
				}
				imagestring($img,1,25,$posY,str_pad("REP.", 18, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,10+$margenIzq,$posY,str_pad(substr($codigoRepuesto,0,18), 18, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,100+$margenIzq,$posY,str_pad(strtoupper(substr($solicitudRep.$provienePaquete." ".$arrayRepuesto['descripcion'],0,50).$anex), 53, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,350,$posY,str_pad($arrayRepuesto['cantidad'], 17, " ", STR_PAD_LEFT),$textColor);
				if(!$esGarantia){
					imagestring($img,1,460,$posY,str_pad(number_format($arrayRepuesto['precio_unitario'],2,".",","), 15, " ", STR_PAD_LEFT),$textColor);				
				imagestring($img,1,543,$posY,str_pad(number_format(($arrayRepuesto['cantidad']*$arrayRepuesto['precio_unitario']),2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
				}
				
				$totalRepuestos += $arrayRepuesto['cantidad']*$arrayRepuesto['precio_unitario'];
				$posY += 10;
		...
		//MANOS DE OBRA POR FALLA
		foreach($arrayTemparios as $arrayTempario){
			if($idRecepcionFalla == $arrayTempario["id_falla"]){
				$anex = (strlen($arrayTempario['descripcion_tempario']) > 53) ? "..." : "";

				$caractCantTempario = ($arrayTempario['id_modo'] == 1) ? number_format($arrayTempario['precio_por_tipo_orden']/100,2,".",",") : number_format(1,2,".",",");

				$caracterPrecioTempario = ($arrayTempario['id_modo'] == 1) ? $arrayTempario['precio_tempario_tipo_orden'] : $arrayTempario['precio_por_tipo_orden'];

				$cantidad = $caractCantTempario."/MEC:".sprintf("%04s",$arrayTempario['id_mecanico']);
				$precioUnit = number_format($caracterPrecioTempario,2,".",",");
				$total = number_format($arrayTempario['total_por_tipo_orden'],2,".",",");

				if($arrayTempario['id_paquete']){
					$provienePaqueteTempario = "(P) ";
				}else{
					$provienePaqueteTempario = "";
				}
				
				imagestring($img,1,25,$posY,str_pad("MO.", 18, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,10+$margenIzq,$posY,str_pad(substr($arrayTempario['codigo_tempario'],0,16), 18, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,100+$margenIzq,$posY,str_pad(strtoupper(substr($provienePaqueteTempario.$arrayTempario['descripcion_tempario'],0,45).$anex), 53, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,370,$posY,str_pad($cantidad, 17, " ", STR_PAD_LEFT),$textColor);
				if(!$esGarantia){
					imagestring($img,1,460,$posY,str_pad($precioUnit, 15, " ", STR_PAD_LEFT),$textColor);
					imagestring($img,1,543,$posY,str_pad($total, 12, " ", STR_PAD_LEFT),$textColor);
				}
				$totalManoDeObra += $arrayTempario['total_por_tipo_orden'];
				$posY += 10;
		...
		//TOT POR FALLA
		foreach($arrayTots as $arrayTot){
			if($idRecepcionFalla == $arrayTot["id_falla"]){
				$cantidad = "1";
				$precioUnit = number_format($arrayTot['monto_subtotal'] + ($arrayTot['monto_subtotal'] * $arrayTot['porcentaje_tot'] / 100),2,".",",");
				$total = number_format($arrayTot['monto_subtotal'] + ($arrayTot['monto_subtotal'] * $arrayTot['porcentaje_tot'] / 100),2,".",",");

				imagestring($img,1,25,$posY,str_pad("TOT", 18, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,10+$margenIzq,$posY,str_pad($arrayTot['numero_tot'], 18, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,100+$margenIzq,$posY,str_pad(str_replace("\\n"," ",str_replace("\t","",substr($arrayTot['observacion_factura'],0,70))), 53, " ", STR_PAD_RIGHT),$textColor);
				...
				$totalTOT += $arrayTot['monto_subtotal'] + ($arrayTot['monto_subtotal'] * $arrayTot['porcentaje_tot'] / 100);
				$posY += 10;
		...
		foreach($arrayNotas as $arrayNota){
			if($idRecepcionFalla == $arrayNota["id_falla"]){
				$anex = (strlen($arrayNota['descripcion_nota']) > 53) ? "..." : "";
				$cantidad = "1";
				$precioUnit = number_format($arrayNota['precio'],2,".",",");
				$total = number_format($arrayNota['precio'],2,".",",");
				
				imagestring($img,1,25,$posY,str_pad("NOTA", 18, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,10+$margenIzq,$posY,$posY,$textColor);
				imagestring($img,1,100+$margenIzq,$posY,str_pad(strtoupper(substr($arrayNota['descripcion_nota'],0,50).$anex), 53, " ", STR_PAD_RIGHT),$textColor);
				imagestring($img,1,370,$posY,str_pad($cantidad, 17, " ", STR_PAD_LEFT),$textColor);
				if(!$esGarantia){
					imagestring($img,1,460,$posY,str_pad($precioUnit, 15, " ", STR_PAD_LEFT),$textColor);
					imagestring($img,1,543,$posY,str_pad($total, 12, " ", STR_PAD_LEFT),$textColor);
				}
				$totalNotas += $arrayNota['precio'];
				$posY += 10;	
	...
	$sinFallaAsociada = false;//compruebo si hay items sin falla asociada
	foreach($arrayRepuestos as $arrayRepuesto){
		if($arrayRepuesto["id_falla"] == ""){
			$sinFallaAsociada = true;
		}
	}
	
	foreach($arrayTemparios as $arrayTempario){
		if($arrayTempario["id_falla"] == ""){
			$sinFallaAsociada = true;
		}
	}
	
	foreach($arrayTots as $arrayTot){
		if($arrayTot["id_falla"] == ""){
			$sinFallaAsociada = true;
		}
	}
	
	foreach($arrayNotas as $arrayNota){
		if($arrayNota["id_falla"] == ""){
			$sinFallaAsociada = true;
		}
	}
	
	if($sinFallaAsociada){
		imagestring($img,1,10,$posY,"-SIN FALLA ASOCIADA ",$textColor);
		$posY += 10;
	}	
	
	//REPUESTOS SIN FALLA
	foreach($arrayRepuestos as $arrayRepuesto){
		if(!array_key_exists($arrayRepuesto["id_falla"], $arrayFallas) ){
			//imagestring($img,1,300,$posY,"REPUESTOS",$textColor);	
			//imagestring($img,1,420,$posY,"CANT.",$textColor);
	...
	//MANOS DE OBRA SIN FALLA
	foreach($arrayTemparios as $arrayTempario){
		if(!array_key_exists($arrayTempario["id_falla"], $arrayFallas) ){
			$anex = (strlen($arrayTempario['descripcion_tempario']) > 53) ? "..." : "";
	...
	//TOT SIN FALLA
	foreach($arrayTots as $arrayTot){
		if(!array_key_exists($arrayTot["id_falla"], $arrayFallas) ){
			$cantidad = "1";
			$precioUnit = number_format($arrayTot['monto_subtotal'] + ($arrayTot['monto_subtotal'] * $arrayTot['porcentaje_tot'] / 100),2,".",",");
			$total = number_format($arrayTot['monto_subtotal'] + ($arrayTot['monto_subtotal'] * $arrayTot['porcentaje_tot'] / 100),2,".",",");
	...
	//NOTAS SIN FALLA
	foreach($arrayNotas as $arrayNota){
		if(!array_key_exists($arrayNota["id_falla"], $arrayFallas) ){
			$anex = (strlen($arrayNota['descripcion_nota']) > 53) ? "..." : "";
			$cantidad = "1";
			$precioUnit = number_format($arrayNota['precio'],2,".",",");
			$total = number_format($arrayNota['precio'],2,".",",");
	...
	$totalManoDeObra = number_format($totalManoDeObra,2,".",",");
	$totalTOT = number_format($totalTOT,2,".",",");
	$totalNotas = number_format($totalNotas,2,".",",");
	 
	imagestring($img,1,10,$posY,str_pad("SUB-TOTAL:          REPUESTOS ".$totalRepuestos."          TEMPARIOS ".$totalManoDeObra."          TOT ".$totalTOT."          NOTAS ".$totalNotas, 12, " ", STR_PAD_LEFT),$textColor);*/
...
//pruebas de relleno
/*for ($i = 1; $i <= 26; $i++){
imagestring($img,1,10,$posY,str_pad($i."-", 18, " ", STR_PAD_BOTH),$textColor);
...
imagestring($img,1,543,$posY,str_pad($posY."-", 12, " ", STR_PAD_BOTH),$textColor);
$posY += 10;
}*/
...
$posY = 10;
$auxY1 = $posY;
imageline($img2,395,$posY,602,$posY,$textColor);
$posY += 2;
//TEXTO "SUB-TOTAL"
imagestring($img2,1,415,$posY,str_pad("SUB-TOTAL:", 25, " ", STR_PAD_LEFT),$textColor);
//SUB-TOTAL
if(!$esGarantia){
	imagestring($img2,1,540,$posY,str_pad(number_format($rowCliente['subtotal'],2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
}
$posY += 8;
imageline($img2,395,$posY,602,$posY,$textColor);
$posY += 2;
...
//TEXTO "DESCUENTO"
imagestring($img2,1,415,$posY,str_pad("DESCUENTO:", 25, " ", STR_PAD_LEFT),$textColor);
//DESCUENTO
if(!$esGarantia){
	imagestring($img2,1,540,$posY,str_pad(number_format($rowCliente['subtotal_descuento'] + $descuentoDetalles,2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
}
$posY += 8;
imageline($img2,395,$posY,602,$posY,$textColor);
$posY += 2;
...
//TEXTO "MONTO EXENTO"
imagestring($img2,1,415,$posY,str_pad("MONTO EXENTO:", 25, " ", STR_PAD_LEFT),$textColor);
if(!$esGarantia){
	imagestring($img2,1,540,$posY,str_pad(number_format($rowCliente["monto_exento"],2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
}
$posY += 8;
imageline($img2,395,$posY,602,$posY,$textColor);
$posY += 2;
...
while($rowIvas = mysql_fetch_assoc($rsIvas)){
    ...
    if($esGarantia){//si es garantia imprimo sin base
    	imagestring($img2,1,398,$posY,str_pad($rowIvas["observacion"]." ".$rowIvas["iva"], 27, " ", STR_PAD_LEFT),$textColor);
	}else{
		imagestring($img2,1,398,$posY,str_pad($rowIvas["observacion"]." ".$rowIvas["iva"]." ".$rowIvas["base_imponible"], 27, " ", STR_PAD_LEFT),$textColor);
    
    	imagestring($img2,1,540,$posY,str_pad(number_format($rowIvas["subtotal_iva"],2,".",","), 12, " ", STR_PAD_LEFT),$textColor);	
	}
...
if($cotizacion){//CONTINGENCIA COTIZACION BORRAR CUANDO PUEDA
	imagestring($img2,1,415,$posY,str_pad(nombreIva($rowCliente["id_iva"])." ".$rowCliente["iva"]." ".$rowCliente["base_imponible"].":", 25, " ", STR_PAD_LEFT),$textColor);
    
    imagestring($img2,1,540,$posY,str_pad(number_format($rowCliente["subtotal_iva"],2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
    $posY += 8;
    imageline($img2,395,$posY,602,$posY,$textColor);
    $posY += 2;
}
...
//TEXTO "TOTAL"
imagestring($img2,1,415,$posY,str_pad("TOTAL:", 25, " ", STR_PAD_LEFT),$textColor);
//TOTAL
//$totalPresupuesto = round($rowCliente['subtotal'] - $nuevoSubtotalDescuento - $descuentoDetalles + $iva,2);//antes
if(!$esGarantia){
	imagestring($img2,1,540,$posY,str_pad(number_format($rowCliente["total"],2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
}
$posY += 8;
$auxY2 = $posY;
imageline($img2,395,$posY,602,$posY,$textColor);
imageline($img2,395,$auxY1,395,$auxY2,$textColor);
imageline($img2,539,$auxY1,539,$auxY2,$textColor);
imageline($img2,602,$auxY1,602,$auxY2,$textColor);
$posY += 2;
...
$rutaCodigoBarras = "tmp/img_codigo-".$UID.".png";
$rutaImgPie = "tmp/ordenPie-".$UID.".png";
$rutaImgCabezera = "tmp/ordenCabecera-".$UID.".png";

//CODIGO DE BARRA
//crea codigo de barra, quito extension y punto porque la crea automaticamente luego
getBarcode($numeroBarras,substr($rutaCodigoBarras, 0, -4));

$p = imagepng($img2,$rutaImgPie);
$s = imagepng($imgCabecera,$rutaImgCabezera);

$pdf = new FPDF('P','cm','LETTER');

//cierra la ultima actual, si solo es 1 esa debe cerrar aqui tambien
$rutaImgOrden = "tmp/orden-".$UID."-".$contador.".png";
imagepng($img,$rutaImgOrden) or die("Error generando PNG");

for($i=1; $i<=$contador; $i++){
	$pdf->AddPage('P',$tamanoPaginaPixel);//$tamanoPaginaPixel
		
	$rutaImgOrden = "tmp/orden-".$UID."-".$i.".png";
	$pdf->Image($rutaImgOrden, 0, 0);
	$pdf->Image($rutaImgCabezera, 0, 0);

	//LOGO EMPRESA
	$pdf->Image("../../".$ruta_logo, '0.2', '0.2', '4', '1.5', '','');
	$pdf->Image($rutaCodigoBarras, 17, 0.3, '', '', '','');
	...
	if($i == $contador){
		$posPie = 19.5;
		//$pdf->AddPage('P',$tamanoPaginaPixel);//$tamanoPaginaPixel
		$pdf->Image($rutaImgPie, 0, $posPie);		
	}	
}



unlink($rutaCodigoBarras);//elimino la imagen para evitar duplicacion o que la tome otro pdf que no es
unlink($rutaImgPie);
unlink($rutaImgCabezera);
...
function cantidadDisponibleActualLote($idArticuloCosto){
    if($idArticuloCosto){
...
//EN Y 542 es la linea ultima de item antes del pie de pagina
//EN Y 732 termina una pag, final del pie de pagina
//El limite tambien depende de las fallas, alrededor de 35 tomando en cuenta titulos 
//subtitulos y subtotal de items, tomar encuenta tambien que en factura es otro limite
function generarPagina(){//detecta en que linea esta y genera una nueva pagina si paso la linea
	global $posY;
	global $UID;
	global $contador;
	global $img;
	
	if($posY > 542){//multi pagina
		$rutaImgOrden = "tmp/orden-".$UID."-".$contador.".png";
		imagepng($img,$rutaImgOrden) or die("Error generando imagen linea: ".__LINE__);//cierra la que escribe actual, png
		
		$img = @imagecreate(612, 792) or die("No se puede crear la imagen linea: ".__LINE__);//crea la nueva pa continuar
		$backgroundColor = imagecolorallocate($img, 255, 255, 255);
		$textColor = imagecolorallocate($img, 0, 0, 0);
		
		$posY = 52;//las nuevas paginas se reinician en linea 52 para el encabezado empresa.
		$contador++;
	}
}
