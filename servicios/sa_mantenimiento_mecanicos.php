...
//implementando xajax;
		...
		if($rec){
			if($rec->getNumRows()==0){
				...
				foreach($rec as $v){
					...
					if($rec->id_equipo_mecanico){
						$btnOtroEquipo = '<img src="'.getUrl('img/iconos/add.png').'" width="16" border="0" title="Duplicar a otro equipo" onClick="xajax_cargarOtroEquipo('.$rec->id_mecanico.',\'edit\');">';
					}else{
						$btnOtroEquipo = "";
					}
					...
					<td align="center"><img src="'.getUrl('img/iconos/view.png').'" width="16" border="0" title="Ver" onClick="xajax_cargar('.$rec->id_empleado.',\'view\');"></td>
					<td align="center"><img src="'.getUrl('img/iconos/edit.png').'" width="16" border="0" title="Editar" onClick="xajax_cargar('.$rec->id_empleado.',\'edit\');"></td>
					<td align="center">'.$btnOtroEquipo.'</td>
					</tr>';
	...
	function cargarOtroEquipo($idMecanico){
		$objResponse = getResponse();
		
		$sql = sprintf("SELECT id_mecanico, nombre_completo, id_equipo_mecanico
						FROM sa_v_mecanicos
						WHERE id_mecanico = %s",
		...
		$sql = "SELECT id_equipo_mecanico, nombre_equipo, nombre_empresa
				FROM sa_equipos_mecanicos
				INNER JOIN pg_empresa ON sa_equipos_mecanicos.id_empresa = pg_empresa.id_empresa";
		$rs = mysql_query($sql);
		if(!$rs){ return $objResponse->alert("Error: ".mysql_error()."\n\nLinea: ".__LINE__); }
		
		while($rowEquipos = mysql_fetch_assoc($rs)){
			$equiposArray[$rowEquipos['id_equipo_mecanico']] = $rowEquipos['nombre_empresa']." - ".$rowEquipos['nombre_equipo'];
		}
		$inputSelect = inputSelect('id_equipo_mecanico_otro',$equiposArray,$row['id_equipo_mecanico'],'',false);		
		
		$objResponse->assign("id_mecanico_otro","value",$row['id_mecanico']);
		$objResponse->assign("nombre_completo_mecanico","innerHTML",$row['nombre_completo']);
		$objResponse->assign("td_otro_equipo","innerHTML",$inputSelect);
		
		$objResponse->script("setDivWindow('edit_window2','title_window2',true);");
	...
	function guardarOtroEquipo($frmOtroEquipo){
		$objResponse = getResponse();
		
		if($frmOtroEquipo['id_mecanico_otro'] == "" || $frmOtroEquipo['id_equipo_mecanico_otro'] == ""){
			return $objResponse->alert("Falta informacion del mecanico o del equipo");
		}
		...
		$sql = sprintf("SELECT *
					...
					valTpDato($frmOtroEquipo['id_mecanico_otro'],"int"),
					valTpDato($frmOtroEquipo['id_equipo_mecanico_otro'],"int"));
		$rs = mysql_query($sql);
		if(!$rs){ return $objResponse->alert("Error: ".mysql_error()."\n\nLinea: ".__LINE__); }
		
		if(mysql_num_rows($rs) > 0){
			return $objResponse->alert("Ya se encuentra en ese equipo");
		}
		
		$sql = sprintf("INSERT INTO sa_mecanicos
							SELECT 	
									NULL,									
									id_empleado, 
								    nivel, 
		...
		$objResponse->script('cargar(); close_window("edit_window2");');
	...
	xajaxRegister('guardarOtroEquipo');
	xajaxRegister('cargar_empresas');
	xajaxRegister('cargar');
	xajaxRegister('cargarOtroEquipo');
...
<div class="window" id="edit_window" style="visibility:hidden;min-width:400px;max-width:400px;">
 ...
 (Tabla Empleados) al especificar un cargo como "TECNICO MECANICO"
						</td>
					</tr>
				</tfoot>
			</table>
...
<div class="window" id="edit_window2" style="visibility:hidden;min-width:500px;max-width:400px;">
	<div class="title" id="title_window2">
		<span id="subtitle"></span>Duplicar &nbsp;Mec&aacute;nicos	
	</div>
	<div class="content">
		
		<form id="frmOtroEquipo" name="frmOtroEquipo" onSubmit="return false;" style="margin:0px;padding:0px;">
			<input type="hidden" id="id_mecanico_otro" name="id_mecanico_otro" />
			<table class="insert_table" style="width:auto;">
				<tbody>
					<tr>
						<td class="label">
						...
						<td class="field" id="nombre_completo_mecanico">							
						</td>
					</tr>
					<tr>
						<td class="label">
						...
						<td class="field" id="td_otro_equipo">							
						</td>
					</tr>
				</tbody>
				<tfoot>
					<tr>
						<td colspan="2" align="right">
							<button type="submit" id="guardar" onClick="xajax_guardarOtroEquipo(xajax.getFormValues('frmOtroEquipo'));" ><img border="0" src="<?php echo getUrl('img/iconos/add.png'); ?>" />Duplicar</button>
						</td>
					</tr>
					<tr>
						<td colspan="2">
							Esta sección permite duplicar un mec&aacute;nico para otro equipo, incluso para otro equipo de otra empresa (PANAMA)
						</td>
					</tr>
				</tfoot>
			</table>
	...
	<img class="close_window" src="<?php echo getUrl('img/iconos/close_dialog.png'); ?>" alt="cerrar" title="Cerrar" style="cursor:pointer;" onClick="close_window('edit_window2');" border="0" />
</div>
