...
//implementando xajax;
require_once("control/main_control.inc.php");//usar once porque se autoincluye en otros y funciones solo pueden ser definidas 1 vez
...
require("../connections/conex.php");//lo necesita ac_iv_general
require("controladores/ac_iv_general.php");//tiene la funcion listado de empresas final
...
//                                                          )->getAssoc('id_tipo_orden','nombre_tipo_orden');
	...
	if($taller == "1"){
		$filtroTaller = "id_filtro_orden NOT IN(9,10,11,12) AND orden_generica = 0 AND ";
		$objFiltroTaller = new criteria(sqlNOTIN,'id_filtro_orden','(9,10,11,12)');
	}elseif($taller == "2"){
		$filtroTaller="";
		$objFiltroTaller = NULL;
	}
	$queryTipoOrden = sprintf("SELECT id_tipo_orden, nombre_tipo_orden 
								FROM sa_tipo_orden 
...
//volcando los Totales TOTALES
	...
	if($taller == "1"){
		$q_ordenes= $c->sa_v_informe_final_ordenes->doQuery($c, new criteria(sqlAND, array(
																	...
																	//var_dump(get_class_methods($q_ordenes));
	}elseif($taller == "2"){
		$q_ordenes= $c->sa_v_informe_final_ordenes->doQuery($c, new criteria(sqlAND, array(
																	new criteria(sqlEQUAL,'id_empresa',$id_empresa)
																)));
