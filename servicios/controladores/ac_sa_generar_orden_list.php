...
function actualizarStatusOrden($idOrden, $idEstadoOrden) {
	$objResponse = new xajaxResponse();
	
	$sql = sprintf("UPDATE sa_orden SET id_estado_orden = %s WHERE id_orden = %s ",
				valTpDato($idEstadoOrden, "int"),
...
function aprobarOrden($valForm){
	...
	if ($row['contrasena'] != md5($valForm['txtClaveAprobacion'])) {
		...
		return $objResponse->script("$('txtClaveAprobacion').focus();");
	}
	...
	if ($row['cont_aprob'] == 0) {
		...
		return $objResponse->alert("La orden ha sido aprobada solamente, vuelva a intentar.");	
	}
	...
	if ($modoFacturaOrden == 'FACTURA') {                            
		...
		//13 es la que manda a facturacion
		...
		$queryNumeracion = sprintf("SELECT * FROM pg_empresa_numeracion
									WHERE id_numeracion = (SELECT clave_mov.id_numeracion_documento 
															FROM pg_clave_movimiento clave_mov
															WHERE clave_mov.id_clave_movimiento = %s)
									AND (id_empresa = %s OR (aplica_sucursales = 1 AND id_empresa = 
															(SELECT suc.id_empresa_padre 
																FROM pg_empresa suc
		...
		if(usaLotes()){//USA LOTES
			$sqlCosto = sprintf("(SELECT
									if((SELECT valor 
										...
										(SELECT vw_iv_articulos_almacen_costo.costo 
										FROM vw_iv_articulos_almacen_costo 
										WHERE vw_iv_articulos_almacen_costo.id_articulo_costo = sa_det_orden_articulo.id_articulo_costo LIMIT 1), 
										
										(SELECT vw_iv_articulos_almacen_costo.costo_promedio 
										FROM vw_iv_articulos_almacen_costo 
										WHERE vw_iv_articulos_almacen_costo.id_articulo_costo = sa_det_orden_articulo.id_articulo_costo LIMIT 1)
								))",
			valTpDato($idEmpresa, "int"));
			...
			$sqlCosto = sprintf("(SELECT
									if((SELECT valor 
										...
										WHERE config.id_configuracion = 12 
										AND config_emp.status = 1 
										AND config_emp.id_empresa = %s) = 1, 
										
										iv_articulos_costos.costo,
										
										iv_articulos_costos.costo_promedio) as costo
									FROM iv_articulos_costos 
									WHERE iv_articulos_costos.id_articulo = sa_det_orden_articulo.id_articulo 
									AND iv_articulos_costos.id_empresa = %s 
									ORDER BY id_articulo_costo DESC LIMIT 1)",
			valTpDato($idEmpresa, "int"),					
			valTpDato($idEmpresa, "int"));
		}
		...
		while ($rowArt = mysql_fetch_array($rsArt)) {
			...
			if ($rowCont['nro_rpto_desp'] > 0) {					
				$insertSQLDetFact = sprintf("INSERT INTO sa_det_vale_salida_articulo (id_vale_salida, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, porcentaje_pmu, base_pmu, pmu_unitario, id_iva, iva, aprobado, tiempo_asignacion,  tiempo_aprobacion, id_empleado_aprobacion, estado_articulo)
			VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, '%s', '%s', %s, %s);",
				...
				while($rowArtIvaOrden = mysql_fetch_assoc($rs3)){
					$query = sprintf("INSERT INTO sa_det_vale_salida_articulo_iva (id_det_vale_salida_articulo, id_iva, iva, lujo)
											VALUES(%s, %s, %s, %s)",
						valTpDato($idDetValeArticulo, "int"),
						valTpDato($rowArtIvaOrden["id_iva"], "int"),
						valTpDato($rowArtIvaOrden["iva"], "real_inglesa"),
				...
				while ($rowContCas = mysql_fetch_assoc($rsContCas)) {
					...
					$insertSQLKardex = sprintf("INSERT INTO iv_kardex (id_modulo, id_documento, id_articulo, id_casilla, tipo_movimiento, tipo_documento_movimiento, cantidad, precio, pmu_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, costo_cargo, porcentaje_descuento, subtotal_descuento, id_clave_movimiento, estado, fecha_movimiento, hora_movimiento)
		...
		while($rowTemparioOrden = mysql_fetch_assoc($rs)){		
			$query = sprintf("INSERT INTO sa_det_vale_salida_tempario (id_vale_salida, id_paquete, id_tempario, precio, costo, costo_orden, id_modo, base_ut_precio, operador, aprobado, ut, tiempo_aprobacion, tiempo_asignacion, tiempo_inicio, tiempo_fin, id_mecanico, id_empleado_aprobacion, origen_tempario, estado_tempario, precio_tempario_tipo_orden) 
				...
				valTpDato($rowTemparioOrden["origen_tempario"], "int"),// orden
			...
			$idDetValeSalidaTempario = mysql_insert_id();			
			...
			while($rowTempIvaValeSalida = mysql_fetch_assoc($rs3)){
				$query = sprintf("INSERT INTO sa_det_vale_salida_tempario_iva (id_det_vale_salida_tempario, id_iva, iva, lujo)
										VALUES(%s, %s, %s, %s)",
					valTpDato($idDetValeSalidaTempario, "int"),
					valTpDato($rowTempIvaValeSalida["id_iva"], "int"),
					valTpDato($rowTempIvaValeSalida["iva"], "real_inglesa"),
					valTpDato($rowTempIvaValeSalida["lujo"], "int"));
				$rs4 = mysql_query($query);
				if(!$rs4) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
			}		
		}
		...
		WHERE 
			...
			valTpDato($idOrdenAprobar, "int"));
		$rsTot = mysql_query($query);
		if (!$rs) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
		
		while($rowTotOrden = mysql_fetch_assoc($rs)) {
			$query = sprintf("INSERT INTO sa_det_vale_salida_tot(id_vale_salida, id_orden_tot, id_porcentaje_tot, porcentaje_tot, aprobado)
			VALUE (%s, %s, %s, %s, %s)",
				valTpDato($id_vale_salida, "int"),
				valTpDato($rowTotOrden['id_orden_tot'], "int"),
				valTpDato($rowTotOrden['id_porcentaje_tot'], "int"),
				valTpDato($rowTotOrden['porcentaje_tot'], "real_inglesa"),
				valTpDato($rowTotOrden['aprobado'], "int"));
			$rs2 = mysql_query($query);
			if (!$rs2) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
			$idDetValeSalidaTot = mysql_insert_id();
			...
			while($rowTotIvaOrden = mysql_fetch_assoc($rs3)){
				$query = sprintf("INSERT INTO sa_det_vale_salida_tot_iva (id_det_vale_salida_tot, id_iva, iva, lujo)
										VALUES(%s, %s, %s, %s)",
					valTpDato($idDetValeSalidaTot, "int"),
		...
		while($rowNotaOrden = mysql_fetch_assoc($rs)){		
			$query = sprintf("INSERT INTO sa_det_vale_salida_notas(id_vale_salida, descripcion_nota, precio, aprobado) 
				VALUES(%s, %s, %s, %s)",
				valTpDato($id_vale_salida, "int"),
			...
			$rs2 = mysql_query($query);
			if (!$rs2) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
			$idDetValeSalidaNota = mysql_insert_id();
			...
			while($rowNotaIvaOrden = mysql_fetch_assoc($rs3)){
				$query = sprintf("INSERT INTO sa_det_vale_salida_notas_iva (id_det_vale_salida_nota, id_iva, iva, lujo)
										VALUES(%s, %s, %s, %s)",
					valTpDato($idDetValeSalidaNota, "int"),
					valTpDato($rowNotaIvaOrden["id_iva"], "int"),
					valTpDato($rowNotaIvaOrden["iva"], "real_inglesa"),
					valTpDato($rowNotaIvaOrden["lujo"], "int"));
				$rs4 = mysql_query($query);
				if(!$rs4) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
			}		
		}
...
function formAprobarOrden($idOrden, $sobregiro, $claveModulo, $idEmpleadoAprobFactura, $pageNum, $campOrd, $tpOrd, $valBusq, $maxRows, $soloRepuestos = false) {
	...
	/*if ($rowValidarSiExistenTemparioSinMecanico['cant'] > 0) {
		...
		$objResponse->script("document.forms['frmClaveAprobacionOrden'].reset();");
...
function listadoOrdenes($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "5%", $pageNum, "numeracion_recepcion", $campOrd, $tpOrd, $valBusq, $maxRows, ("N° Recepci&oacute;n"));
		...
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "5%", $pageNum, "total_sin_aprobar", $campOrd, $tpOrd, $valBusq, $maxRows, ("Total Sin Despachar"));
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "5%", $pageNum, "total_orden", $campOrd, $tpOrd, $valBusq, $maxRows, ("Total a Facturar"));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\" height=\"22\">";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('reportes/sa_imprimir_presupuesto_pdf.php?valBusq=%s|2|', 1000, 500);\" src=\"../img/iconos/ico_print.png\" title=\"Imprimir Orden\"/>",
					$row['id_orden']);
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
	}
...
function selectEstadoOrden($id_orden) {
	...
	$query2 = "SELECT * FROM sa_estado_orden WHERE activo = 1 ORDER BY sa_estado_orden.orden";
...
function accesoSoloRepuesto($clave){
	...
	if($num_rows){
            while($row = mysql_fetch_assoc($rs)){//NOTA EN LAS MULTIEMPRESAS PUEDE HABER MULTIPLES CLAVES Y DISTINTAS
