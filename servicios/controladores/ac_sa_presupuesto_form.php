...
function asignarArticulo($idArticulo, $valFormDcto, $hddNumeroArt = "", $valFormListaArticulo = "") {
	...
	$objResponse->script("
	$('txtPrecioArtRepuesto').className = 'inputInicial';
	...
	//es por precio no costo calcular despues de que asigne el precio y debe asignar el porcentaje antes del precio para calcular
	$htmlPmu = "<select id=\"selPorcentajePmu\" onChange=\"cambiarPorcentajePmu(this.value);\" class=\"inputHabilitado\">";
	$htmlPmu .= "<option value='".$rowTipoOrden['porcentaje_pmu']."'>ORDEN ".$rowTipoOrden['porcentaje_pmu']."%</option>";
	$htmlPmu .= "<option value=''>MANUAL</option>";
	$htmlPmu .= "</select>";

	$objResponse->assign("txtPorcentajePmu","value",$rowTipoOrden['porcentaje_pmu']);	
	$objResponse->assign("tdSelPorcentajePmu","innerHTML",$htmlPmu);
	if($rowTipoOrden['porcentaje_pmu'] > 0){//solo mostrar si tiene, deberia ser solo garantia
		$objResponse->script("$('tablaPmu').style.display='';");
	}
...
//            $objResponse->assign("txtIvaRepuesto","value",$rowArticulo['iva']."%");
	...
	if(!clienteExento($idCliente) && tipoOrdenPoseeIva($idTipoOrden)){// proviene del ac_iv_general servicios
		$arrayIvasMultiple = ivaServicios(); //proviene de ac_iv_general devuelve array de arrays
		$arrayIvasExentoCliente = clienteImpuestoExento($idCliente);
		//$arrayIvasMultiple = array_diff_key($arrayIvasMultiple, $arrayIvasExentoCliente);//para quitar
		
		$onClick = "onclick=\"return false;\"";//bloqueado por defecto
		if(edicionImpuesto(2, $idTipoOrden)){//desbloqueo si es true
			$onClick = "";
		}
		...
		$arrayIvasArticulo = array();
		while($rowIvasArticulo = mysql_fetch_assoc($rsIvasArticulo)){
			$arrayIvasArticulo[$rowIvasArticulo["id_impuesto"]] = $rowIvasArticulo["id_impuesto"];
		}
		
		$tablaImpuesto = "";		
		foreach($arrayIvasMultiple as $idIva => $arrayIva){
			$checked = ($arrayIva['activo'] == 1 && !array_key_exists($idIva, $arrayIvasExentoCliente) && array_key_exists($idIva, $arrayIvasArticulo)) ? "checked=\"checked\"" : "";
			$cbxIva = sprintf("<input type=\"checkbox\" name=\"cbxIdIvaArticulo[]\" %s %s value=\"%s\" />",
				$checked,
				$onClick,
				$idIva);
			
			$tablaImpuesto .= "<tr>";
				$tablaImpuesto .= "<td align=\"right\" class=\"tituloCampo\" width=\"120\">";
					$tablaImpuesto .= utf8_encode($arrayIva["observacion"]);
				$tablaImpuesto .= "</td>";
				$tablaImpuesto .= "<td>";
					$tablaImpuesto .= sprintf("<input size=\"6\" type=\"text\" readonly=\"readonly\" value=\"%s\" >",
							$arrayIva["iva"]."%");
					$tablaImpuesto .= $cbxIva;
				$tablaImpuesto .= "</td>";
			$tablaImpuesto .= "</tr>";
		}
	}	
	...
	if ($hddNumeroArt == "") { // NO EXISTE EL ARTICULO EN LA LISTA DEL PRESUPUESTO
		...
		$objResponse->alert("NO DEBIO ENTRAR AQUI2");
		...
		xajax_asignarPrecio('%s',this.value,'%s','',%s);",
			$idPrecioSelec,
			$seleccionAuto,
			$idArticulo,
			$precioUnitario,
...
function asignarPrecio($idArticulo, $idPrecio, $precioPredet = false, $precio = "", $idEmpresa) {
	...
	if(usaLotes()){//USA LOTES
		...
		$query = sprintf("SELECT * FROM iv_articulos_precios
							WHERE id_articulo = %s 
							AND id_precio = %s 
							AND id_articulo_costo = (SELECT vw_iv_articulos_almacen_costo.id_articulo_costo 
														FROM vw_iv_articulos_almacen_costo
														WHERE id_articulo = %s 
														AND id_empresa = %s 
														AND estatus_almacen_venta = 1 
														AND cantidad_disponible_logica > 0 
														AND id_articulo_costo IS NOT NULL 
														ORDER BY id_articulo_costo LIMIT 1)",
			valTpDato($idArticulo,"int"),		
	...
	$objResponse->script("byId('txtBasePmu').value = byId('txtPrecioArtRepuesto').value; 
							calcularPmu();");
...
function asignarTempario($idTemparioDet, $valFormDcto) {
	...
	if(!clienteExento($idCliente) && tipoOrdenPoseeIva($idTipoOrden)){// proviene del ac_iv_general servicios		
		...
		$onClick = "onclick=\"return false;\"";//bloqueado por defecto
		if(edicionImpuesto(3, $idTipoOrden)){//desbloqueo si es true
			$onClick = "";
		}

		$tablaImpuesto = "";		
		foreach($arrayIvasMultiple as $idIva => $arrayIva){
			$checked = ($arrayIva['activo'] == 1 && !array_key_exists($idIva, $arrayIvasExentoCliente)) ? "checked=\"checked\"" : "";
			$cbxIva = sprintf("<input type=\"checkbox\" name=\"cbxIdIvaTempario[]\" %s %s value=\"%s\" />",
				$checked,
				$onClick,
				$idIva);
	...
	$objResponse->script("document.getElementById('impuestoPorTempario').innerHTML = '".$tablaImpuesto."';");
...
function asignarTot($id_tot, $frmPresupuesto) {
	$objResponse = new xajaxResponse();

	$idCliente = $frmPresupuesto["txtIdCliente"];
	...
	if($idTipoOrden == false || $idTipoOrden == ""){
	...
	if(!clienteExento($idCliente) && tipoOrdenPoseeIva($idTipoOrden)){// proviene del ac_iv_general servicios		
		...
		$onClick = "onclick=\"return false;\"";//bloqueado por defecto
		if(edicionImpuesto(4, $idTipoOrden)){//desbloqueo si es true
			$onClick = "";
		}
		...
		foreach($arrayIvasMultiple as $idIva => $arrayIva){
			$checked = ($arrayIva['activo'] == 1 && !array_key_exists($idIva, $arrayIvasExentoCliente)) ? "checked=\"checked\"" : "";
			$cbxIva = sprintf("<input type=\"checkbox\" name=\"cbxIdIvaTot[]\" %s %s value=\"%s\" />",
				$checked,
				$onClick,
				$idIva);
	...
	$objResponse->script("document.getElementById('impuestoPorTot').innerHTML = '".$tablaImpuesto."';");
	
	$listadoPorcentajesTot = "<select id=\"listadoPorcentajesTot\" onChange=\"$('txtPorcentaje').className = 'inputInicial'; cambiarPrecioTot(this.value);\" class=\"inputHabilitado\">";
	...
	$objResponse->script("$('divFlotante2').style.display='none';
							$('txtPorcentaje').className = 'inputInicial';");
...
function buscar_mano_obra_repuestos_por_paquete($idPaquete,$codigoPaquete,$descripcionPaquete,$precio_paquete,$origen, $itemManoObraAprob, $itemRepAprob, $accionVistaPaquete){
	...
	$('tblListadoTemparioPorPaquete').style.display='';
	$('tblListadoRepuestosPorPaquete').style.display='';
	$('txtDescripcionPaquete').value='".addslashes($descripcionPaquete)."';
	$('txtCodigoPaquete').value='".addslashes($codigoPaquete)."';
...
function cargaLstSeccionTemp($selId = "") {
	...
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
	$html = "<select id=\"lstSeccionTemp\" name=\"lstSeccionTemp\" onchange=\"xajax_cargaLstSubseccionTemp(this.value); $('btnBuscarTempario').click();\" class=\"inputHabilitado\">";
...
function cargaLstSubseccionTemp($selId = "") {
	...
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
	$html = "<select id=\"lstSubseccionTemp\" name=\"lstSubseccionTemp\" onchange=\"$('btnBuscarTempario').click();\" class=\"inputHabilitado\">";
		$html .= "<option value=\"\">[ Todos ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function cargarDcto($idDocumento, $valFormTotalDcto) {
	...
	$ResultConfig104 = valorConfiguracion(104, $_SESSION['idEmpresaUsuarioSysGts'], $_SESSION['idUsuarioSysGts']);
	if ($ResultConfig104[0] != true && strlen($ResultConfig104[1]) > 0) {
		return $objResponse->alert($ResultConfig104[1]);
	} else if ($ResultConfig104[0] == true) {
		$ResultConfig104 = $ResultConfig104[1];
	}
	
	if ($ResultConfig104 == "") {
		// VERIFICA VALORES DE CONFIGURACION (Porcentaje Máximo de Descuento) GENERAL
		$ResultConfig104 = valorConfiguracion(104, $_SESSION['idEmpresaUsuarioSysGts']);
		if ($ResultConfig104[0] != true && strlen($ResultConfig104[1]) > 0) {
			return $objResponse->alert($ResultConfig104[1]);
		} else if ($ResultConfig104[0] == true) {
			$ResultConfig104 = $ResultConfig104[1];
	...
	$objResponse->assign("hddMaximoDescuento","value", $ResultConfig104);
	...
	if($_GET['acc_ret'] != 1) { //CUANDO SE GENERA LA ORDEN RETRABAJO
		...
		while ($rowDetRep = mysql_fetch_assoc($rsDetRep)) {
			...
			$sigValor, $clase, $sigValor,
				...
				number_format($rowDetRep['pmu_unitario'] * $cantidad_art,2,".",","),
				...
				number_format($montoMultipleIva + ($rowDetRep['pmu_unitario'] * $cantidad_art),2,".",","),
...
//		}
		...
		while ($rowDetPaq = mysql_fetch_assoc($rsDetPaq)) {//de cada paquete se consulta sus items
			...
			if($rowNroManoObraPaq['numeroMo'] > 0 || $tieneRepuestos > 0) {
				...
				//$itemsNoChecados = 1;//error si el paquete solo tiene repuestos y fue devuelto
				$checkedPaq = " ";
				$value_checkedPaq = 0;
			...
			while ($valorManoObraPaq = mysql_fetch_array($rsManoObraPaq)) {
				...
				$sqlIvas = sprintf("SELECT id_iva, iva
									FROM sa_det_presup_tempario_iva
									WHERE id_det_presup_tempario = %s",
							valTpDato($valorManoObraPaq['id_det_presup_tempario'],"int"));
			...
			if ($value_checkedPaq == 1) {
				...
				$sqlIvasIdPorcPaq = sprintf("SELECT 
												GROUP_CONCAT(sa_det_presup_articulo_iva.id_iva) AS id_iva_paquete, 
												GROUP_CONCAT(sa_det_presup_articulo_iva.iva) AS porc_iva_paquete, 
												GROUP_CONCAT(sa_det_presup_articulo.precio_unitario*sa_det_presup_articulo.cantidad) AS base_imponible_paquete
				...
				$sigValor, $sigValor, $bloqueaEliminacionRepuestos,
					$rowDetPaq['idPaq'], utf8_encode(addslashes($rowDetPaq['codigo_paquete'])), utf8_encode(addslashes($rowDetPaq['descripcion_paquete'])),$rowDetPaq['precio_paquete'], 1, '0',''));
...
//				}
		...
		$queryDetalleTot = sprintf("SELECT *,
			%s.%s,
			(SELECT GROUP_CONCAT(id_iva) FROM sa_det_presup_tot_iva WHERE sa_det_presup_tot_iva.id_det_presup_tot = %s.%s) as id_iva,
			(SELECT GROUP_CONCAT(iva) FROM sa_det_presup_tot_iva WHERE sa_det_presup_tot_iva.id_det_presup_tot = %s.%s) as iva
		FROM sa_orden_tot
...
//				}
		...
		$queryDetTemp = sprintf("SELECT 
			%s.%s,
			(SELECT GROUP_CONCAT(id_iva) FROM sa_det_presup_tempario_iva WHERE sa_det_presup_tempario_iva.id_det_presup_tempario = %s.%s) as id_iva,
			(SELECT GROUP_CONCAT(iva) FROM sa_det_presup_tempario_iva WHERE sa_det_presup_tempario_iva.id_det_presup_tempario = %s.%s) as iva,
		...
		while ($rowDetTemp = mysql_fetch_assoc($rsDetTemp)) {
			...
			$sigValor, $clase, $sigValor,
				...
				utf8_encode($rowDetTemp['codigo_tempario']),
		...
		$queryDetTipoDocNotas = sprintf("SELECT
			%s.%s,
			(SELECT GROUP_CONCAT(id_iva) FROM sa_det_presup_notas_iva WHERE sa_det_presup_notas_iva.id_det_presup_nota = %s.%s) as id_iva,
			(SELECT GROUP_CONCAT(iva) FROM sa_det_presup_notas_iva WHERE sa_det_presup_notas_iva.id_det_presup_nota = %s.%s) as iva,
			%s AS idDetNota,
			%s AS idDoc,
			descripcion_nota,
			precio, 
		...
		while ($rowDetTipoDocNotas = mysql_fetch_assoc($rsDetTipoDocNotas)) {
			...
			var elemento = new Element('tr', {'id':'trItmNota:%s', 'class':'textoGris_11px %s', 'title':'trItmNota:%s'}).adopt([
				...
				new Element('td', {'align':'right'}).setHTML(\"<b>%s</b>".
					...
					"<input type='hidden' id='hddIdIvaTot%s' name='hddIdIvaNota%s' value='%s'/>".
					"<input type='hidden' id='hddIvaTot%s' name='hddIvaNota%s' value='%s'/>".
...
//carga listado de impuestos en NOTA ADICIONAL, es el unico que lo carga asi
function cargarImpuestoNota($frmPresupuesto){
	$objResponse = new xajaxResponse();
	...
	if(!clienteExento($idCliente) && tipoOrdenPoseeIva($idTipoOrden)){// proviene del ac_iv_general servicios		
		...
		$onClick = "onclick=\"return false;\"";//bloqueado por defecto
		if(edicionImpuesto(5, $idTipoOrden)){//desbloqueo si es true
			$onClick = "";
		}
		...
		foreach($arrayIvasMultiple as $idIva => $arrayIva){
			$checked = ($arrayIva['activo'] == 1 && !array_key_exists($idIva, $arrayIvasExentoCliente)) ? "checked=\"checked\"" : "";
			$cbxIva = sprintf("<input type=\"checkbox\" name=\"cbxIdIvaNota[]\" %s %s value=\"%s\" />",
				$checked,
				$onClick,
				$idIva);
	...
	$objResponse->script("document.getElementById('impuestoPorNota').innerHTML = '".$tablaImpuesto."';");
...
function formValidarPermisoEdicion($hddModulo) {
	...
	$('txtContrasena').className = 'inputHabilitado';");
...
//		}
		...
		$queryOrdenDetTot = sprintf("SELECT * FROM sa_det_presup_tot WHERE id_presupuesto = %s", valTpDato($idDocumentoVenta,"int"));
...
//                                                }//FIN PAQUETES DUPLICADO
		...
		foreach($valFormTotalDcto["ivaActivo"] as $key => $idIvas){
			...
			if($valFormTotalDcto["txtTotalIva".$idIvas] > 0){
				$idIvaSimple = $idIvas;
				$porcentajeIvaSimple = $valFormTotalDcto["txtIvaVenta".$idIvas];
				$baseImponibleSimple = $valFormTotalDcto["txtBaseImponibleIva".$idIvas];
			}
		}
		...
		//BORRO TODOS LOS IVAS DE LA ORDEN
		$ivasSql = sprintf("DELETE FROM sa_presupuesto_iva WHERE id_presupuesto = %s",
					valTpDato($idDocumentoVenta,"int"));
		$rsIvas = mysql_query($ivasSql);
		if (!$rsIvas) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nQuery: ".$ivasSql); }

		//INSERTO LOS IVA SI NO ES CERO
		foreach($valFormTotalDcto["ivaActivo"] as $key => $idIvas){			
			if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
                ...
                if($valFormTotalDcto['hddAccionTipoDocumento'] == 4){//aprobacion de presupuesto
                    $genero = generarPresupuestoOrden(valTpDato($idDocumentoVenta,"int"));
                    if(is_array($genero)){
		...
		foreach($arrayObjTemp as $indiceTemp => $valorTemp) {
			if (strlen($valFormListTemp['hddIdTemp'.$valorTemp]) > 0) {
				if ($valFormListTemp['hddIdPedDetTemp'.$valorTemp] == "") {
					...
					//nuevo gregor ivas por tempario
					foreach($valFormTotalDcto["ivaActivo"] as $key => $idIvas){
						if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
							
							$arrayIvasTempario = explode(",", $valFormListTemp['hddIdIvaTemp'.$valorTemp]);
							$arrayPorcIvasTempario = explode(",", $valFormListTemp['hddIvaTemp'.$valorTemp]);
							
							foreach($arrayIvasTempario as $indice => $idIvaTempario){							   
								if($idIvaTempario == $idIvas){//si el iva del tempario lo tiene la orden, guardar
									$ivasSql = sprintf("INSERT INTO sa_det_presup_tempario_iva (id_det_presup_tempario, id_iva, iva) 
													VALUES (%s, %s, %s)",
											valTpDato($idDocumentoDetalle,"int"),
											valTpDato($idIvaTempario,"int"),
											valTpDato($arrayPorcIvasTempario[$indice],"real_inglesa")
										);
									$rsIvas = mysql_query($ivasSql);
									if (!$rsIvas) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nQuery: ".$ivasSql); }
								}
		...
		foreach($arrayObj as $indice => $valor) {
			if (strlen($valFormListaArt['hddIdArt'.$valor]) > 0) {
				if ($valFormListaArt['hddIdPedDet'.$valor] == "") {
					...
					$insertSQL = sprintf("INSERT INTO sa_det_presup_articulo (id_presupuesto, id_articulo, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, porcentaje_pmu, base_pmu, pmu_unitario, id_iva, iva, tiempo_asignacion, id_empleado_aprobacion, aprobado)
					VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), %s, %s);",
						valTpDato($idDocumentoVenta, "int"),
						valTpDato($valFormListaArt['hddIdArt'.$valor], "int"),
						valTpDato($valFormListaArt['hddCantArt'.$valor], "int"),
						valTpDato($valFormListaArt['hddIdPrecioArt'.$valor], "int"),
						...
						valTpDato($valFormListaArt['hddIdArtAlmacenCosto'.$valor], "int"),
						valTpDato($valFormListaArt['hddCostoArt'.$valor], "real_inglesa"),
						valTpDato($valFormListaArt['hddPorcentajePmu'.$valor], "real_inglesa"),
						valTpDato($valFormListaArt['hddBasePmu'.$valor], "real_inglesa"),
						valTpDato($valFormListaArt['hddMontoPmu'.$valor], "real_inglesa"),
					...
					foreach($valFormTotalDcto["ivaActivo"] as $key => $idIvas){
						if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
							...
							foreach($arrayIvasArticulo as $indice => $idIvaRepuesto){							   
								if($idIvaRepuesto == $idIvas){//si el iva del repuesto lo tiene la orden, guardar
									$ivasSql = sprintf("INSERT INTO sa_det_presup_articulo_iva (id_det_presup_articulo, id_iva, iva) 
													VALUES (%s, %s, %s)",
											valTpDato($idDocumentoDetalle,"int"),
											valTpDato($idIvaRepuesto,"int"),
											valTpDato($arrayPorcIvasArticulo[$indice],"real_inglesa")
		...
		foreach($arrayObjNota as $indiceNota => $valorNota) {
			if (strlen($valFormListNota['hddIdNota'.$valorNota]) > 0) {
				if ($valFormListNota['hddIdPedDetNota'.$valorNota] == "") {
					...
					$objResponse->assign("hddIdPedDetNota".$valorNota,"value",$idDocumentoDetalle);
					
					//nuevo gregor ivas por nota
					foreach($valFormTotalDcto["ivaActivo"] as $key => $idIvas){
						if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
							
							$arrayIvasNota = explode(",", $valFormListNota['hddIdIvaNota'.$valorNota]);
							$arrayPorcIvasNota = explode(",", $valFormListNota['hddIvaNota'.$valorNota]);
							
							foreach($arrayIvasNota as $indice => $idIvaNota){							   
								if($idIvaNota == $idIvas){//si el iva del tempario lo tiene la orden, guardar
									$ivasSql = sprintf("INSERT INTO sa_det_presup_notas_iva (id_det_presup_nota, id_iva, iva) 
													VALUES (%s, %s, %s)",
											valTpDato($idDocumentoDetalle,"int"),
											valTpDato($idIvaNota,"int"),
											valTpDato($arrayPorcIvasNota[$indice],"real_inglesa")
										);
									$rsIvas = mysql_query($ivasSql);
									if (!$rsIvas) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nQuery: ".$ivasSql); }
								}
		...
		foreach($arrayObjTot as $indiceTot => $valorTot) {
			if (strlen($valFormListTot['hddIdTot'.$valorTot]) > 0) {
				if ($valFormListTot['hddIdPedDetTot'.$valorTot] == "") {
					...
					$objResponse->assign("hddIdPedDetTot".$valorTot,"value",$idDocumentoDetalle);
					
					//nuevo gregor ivas por tot
					foreach($valFormTotalDcto["ivaActivo"] as $key => $idIvas){
						if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
							
							$arrayIvasTot = explode(",", $valFormListTot['hddIdIvaTot'.$valorTot]);
							$arrayPorcIvasTot = explode(",", $valFormListTot['hddIvaTot'.$valorTot]);
							
							foreach($arrayIvasTot as $indice => $idIvaTot){							   
								if($idIvaTot == $idIvas){//si el iva del tempario lo tiene la orden, guardar
									$ivasSql = sprintf("INSERT INTO sa_det_presup_tot_iva (id_det_presup_tot, id_iva, iva) 
													VALUES (%s, %s, %s)",
											valTpDato($idDocumentoDetalle,"int"),
											valTpDato($idIvaTot,"int"),
											valTpDato($arrayPorcIvasTot[$indice],"real_inglesa")
										);
									$rsIvas = mysql_query($ivasSql);
									if (!$rsIvas) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nQuery: ".$ivasSql); }
								}
		...
		foreach($arrayObjPaq as $indicePaq => $valorPaq) {
			...
			if ($idPaquete > 0) {
				if ($valFormPaq['hddIdPedDetPaq'.$valorPaq] == "") {
					...
					while ($row = mysql_fetch_assoc($rs)) {
						...
						$insertSQL = sprintf("INSERT INTO sa_det_presup_tempario (
							...
							VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), NULL, %s, %s, %s, %s)", 
								...
								$row['costoCombo'],
								$costoTemparioOrden,
								2, //COMBO MANEJA PRECIOS YA CARGADOS //valTpDato($row['id_modo'], "int"),
								$rowBaseUt['valor_parametro'],
								valTpDato($row['id_operador'], "int"),
								valTpDato($row['utCombo'], "int"),
								valTpDato($valFormDcto['hddIdEmpleado'], "int"),
						...
						$arrayIdIvaTempPaq = array_filter(explode(",", $valFormPaq['hddIdIvasTemparioPaquete'.$valorPaq]));
						$arrayPorcIvaTemPaq = array_filter(explode(",", $valFormPaq['hddPorcentajesIvasTemparioPaquete'.$valorPaq]));
						foreach($arrayIdIvaTempPaq as $indice => $idIvaTempPaq){
							$ivasSqlPaq = sprintf("INSERT INTO sa_det_presup_tempario_iva (id_det_presup_tempario, id_iva, iva) 
													VALUES (%s, %s, %s)",
											valTpDato($idDocumentoDetalleTemp,"int"),
											valTpDato($idIvaTempPaq,"int"),
											valTpDato($arrayPorcIvaTemPaq[$indice],"real_inglesa")
										);
							$rsIvasPaq = mysql_query($ivasSqlPaq);
							if (!$rsIvasPaq) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nQuery: ".$ivasSqlPaq); }
						}
					...
					if($usaLotes){//USA LOTES
						$queryPrecio = " (SELECT iv_articulos_precios.precio 
											FROM iv_articulos_precios 
											WHERE iv_articulos_precios.id_articulo_costo = 											
														(IFNULL((SELECT almacen.id_articulo_costo 
																FROM vw_iv_articulos_almacen_costo almacen 
																WHERE almacen.id_articulo = iv_articulos.id_articulo 
																AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
																AND almacen.estatus_almacen_venta = 1 
																AND cantidad_disponible_logica > 0 
																ORDER BY almacen.id_articulo_costo ASC LIMIT 1), 
            											(SELECT almacen.id_articulo_costo 
															FROM vw_iv_articulos_almacen_costo almacen 
															WHERE almacen.id_articulo = iv_articulos.id_articulo 
															AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
															AND almacen.estatus_almacen_venta = 1 
															ORDER BY almacen.id_articulo_costo DESC LIMIT 1)))
                                            AND iv_articulos_precios.id_precio = 1 LIMIT 1) ";
						
						$queryIdArticuloCosto = " IFNULL((SELECT almacen.id_articulo_costo 
														FROM vw_iv_articulos_almacen_costo almacen 
														WHERE almacen.id_articulo = iv_articulos.id_articulo 
														AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
														AND almacen.estatus_almacen_venta = 1 
						...
						$queryIdArticuloAlmacenCosto = " IFNULL((SELECT almacen.id_articulo_almacen_costo 
																FROM vw_iv_articulos_almacen_costo almacen 
																WHERE almacen.id_articulo = iv_articulos.id_articulo 
																AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
																AND almacen.estatus_almacen_venta = 1 
																AND cantidad_disponible_logica > 0 
																ORDER BY almacen.id_articulo_costo ASC LIMIT 1), 
															(SELECT almacen.id_articulo_almacen_costo 
															FROM vw_iv_articulos_almacen_costo almacen 
															WHERE almacen.id_articulo = iv_articulos.id_articulo 
															AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
															AND almacen.estatus_almacen_venta = 1 
						...
						$queryCostoArticulo = sprintf(" (SELECT
															if((SELECT valor 
																	FROM pg_configuracion_empresa config_emp
																	INNER JOIN pg_configuracion config ON config_emp.id_configuracion = config.id_configuracion
																	WHERE config.id_configuracion = 12 
																	AND config_emp.status = 1 
																	AND config_emp.id_empresa = %s) IN(1,3),
																	
																		#este if, null sino tiene disponible, busco el ultimo
																		(IFNULL((SELECT almacen.costo 
																					FROM vw_iv_articulos_almacen_costo almacen 
																					WHERE almacen.id_articulo = iv_articulos.id_articulo 
																					AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
																					AND almacen.estatus_almacen_venta = 1 
																					AND cantidad_disponible_logica > 0 ORDER BY almacen.id_articulo_costo ASC LIMIT 1), 
																			(SELECT almacen.costo 
																				FROM vw_iv_articulos_almacen_costo almacen 
																				WHERE almacen.id_articulo = iv_articulos.id_articulo 
																				AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
																				AND almacen.estatus_almacen_venta = 1 
															...
															#este else
															(IFNULL((SELECT almacen.costo_promedio 
																		FROM vw_iv_articulos_almacen_costo almacen 
																		WHERE almacen.id_articulo = iv_articulos.id_articulo 
																		AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
																		AND almacen.estatus_almacen_venta = 1 
																...
																(SELECT almacen.costo_promedio 
																	FROM vw_iv_articulos_almacen_costo almacen 
																	WHERE almacen.id_articulo = iv_articulos.id_articulo 
																	AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
																	AND almacen.estatus_almacen_venta = 1 
																	AND almacen.id_articulo_costo IS NOT NULL 
																	ORDER BY almacen.id_articulo_costo DESC LIMIT 1)
															)) #fin iif principal
								)) ",								
						valTpDato($valFormDcto['txtIdEmpresa'], "int"));
						...
						$queryPrecio = " iv_articulos_precios.precio ";
						$queryIdArticuloCosto = " NULL ";
						$queryIdArticuloAlmacenCosto = " NULL ";
						...
						$queryCostoArticulo = sprintf(" (SELECT
								...
								WHERE iv_articulos_costos.id_articulo = iv_articulos.id_articulo AND iv_articulos_costos.id_empresa = %s ORDER BY id_articulo_costo DESC LIMIT 1) ",
						valTpDato($valFormDcto['txtIdEmpresa'], "int"),
						valTpDato($valFormDcto['txtIdEmpresa'], "int"));
						...
						$queryFiltroSinLote = sprintf("
								AND sa_tipo_orden.id_tipo_orden = %s
								AND iv_articulos_precios.id_empresa = %s",							
							valTpDato($valFormDcto['lstTipoOrden'], "int"),
							valTpDato($valFormDcto['txtIdEmpresa'], "int"));
					}
					...
					$insertSQL = sprintf("INSERT INTO sa_det_presup_articulo (id_presupuesto, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, tiempo_asignacion, id_empleado_aprobacion, aprobado) 		
						SELECT 
							...
							sa_paquete_repuestos.cantidad,
							1, #PVM venta publico buscar precio actual
							".$queryPrecio." AS precio,
	
							".$queryIdArticuloCosto." AS id_articulo_costo,
							".$queryIdArticuloAlmacenCosto." AS id_articulo_almacen_costo,
							".$queryCostoArticulo." AS costo,
					...
					if($guardarIva == "1" && $ivasExistentesOrden != ""){
						...
						while($rowDetRepPaquete = mysql_fetch_assoc($rsIvasRepPaquete)){
							...
							while($rowIvasPorRepuestoPaquete = mysql_fetch_assoc($rsIvasPorRepuestoPaquete)){
								...
								$ivasSqlPaq = sprintf("INSERT INTO sa_det_presup_articulo_iva (id_det_presup_articulo, id_iva, iva) 
														VALUES (%s, %s, %s)",
												valTpDato($rowDetRepPaquete["id_det_presup_articulo"],"int"),
												valTpDato($rowIvasPorRepuestoPaquete["id_impuesto"],"int"),
												valTpDato($porcIvaGuardar,"real_inglesa")
...
function insertarArticulo($valForm, $valFormListaArticulo, $valFormTotalDcto) {
	...
	if($valForm['txtPorcentajePmu'] > 0 && $valForm['txtBasePmu'] > 0 && $valForm['txtMontoPmu'] > 0){
		$txtPorcentajePmu = $valForm['txtPorcentajePmu'];
		$txtBasePmu = $valForm['txtBasePmu'];
		$txtMontoPmu = $valForm['txtMontoPmu'];
	}
	
	$idIvas = implode(",",$valForm['cbxIdIvaArticulo']);//ahora con comas
	...
	if ($hddNumeroArt == "") {
		...
		if($usaLotes){//USA LOTES
			...
			$costoPromedioArt = round($rowCostoArt['costo_promedio'],2);
			
			//MANUAL PORQUE NO USA LOTES
			$arrayDistribucionAlmacen[0] = array(
										"id_articulo_costo" => NULL,
										"id_articulo_almacen_costo" => NULL,
										"costo" => $costoArt,
										"costo_promedio" => $costoPromedioArt,
										"precio" => $precioArt,
										"cantidad_articulos" => $cantidadArt
										);
		}
		...
		foreach($arrayDistribucionAlmacen as $key => $arrayAlmacenUsar){
			...
			if ($valForm['hddBajarPrecio'] == "subir" && round($valForm['hddPrecioArtAsig'],2) > round($precioArt,2)) {
				...
				$sigValor, $clase,
					...
					number_format($txtMontoPmu,2,".",","),
					number_format($cantidadArt * $txtMontoPmu,2,".",","),
					...
					$sigValor, number_format($montoMultipleIva + ($cantidadArt * $txtMontoPmu),2,".",","),
					'-',
						$sigValor, $sigValor, "",
						...
						$sigValor, $sigValor, $txtPorcentajePmu,
						$sigValor, $sigValor, $txtBasePmu,
						$sigValor, $sigValor, $txtMontoPmu,
...
//      }
	...
	} else {//fin if cantidad articulo
		$objResponse->alert("NO DEBIO ENTRAR AQUI");
		// DESCONCATENA PARA SABER CUANTOS ITEMS HAY AGREGADOS
		$arrayVal = explode("|",$valFormTotalDcto['hddObj']);
		foreach ($arrayVal as $indice => $valor) {
			if ($valor > 0)
	...
	$objResponse->script("$('txtPrecioArtRepuesto').className = 'inputInicial';");
...
function insertarNota($valForm, $valFormTotalDcto) {
	...
	$idIvas = implode(",",$valForm['cbxIdIvaNota']);//ahora con comas
	
	if($idIvas != "" && $idIvas != " " && $valForm['txtPrecioNota'] > 0){//ojo la nota puede ser en cero
		$queryIva = sprintf("SELECT idIva, iva, observacion FROM pg_iva WHERE idIva IN (%s);", $idIvas);
		$rsIva = mysql_query($queryIva);
		if (!$rsIva) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
	...
	$totalIvasCalculados = 0;
	foreach($arrayIvasDisponibles as $key => $arrayIvaUsar){                                    
		$totalIvasCalculados += ($valForm['txtPrecioNota'] * $arrayIvaUsar["iva"] / 100);
	}
	
	$montoMultipleIva = $totalIvasCalculados + $valForm['txtPrecioNota'];
...
function insertarPaquete($valForm, $valFormTotalDcto, $valFormListPaq, $valFormDcto) {
	...
	if ($sw == 1) {
		...
		if(!clienteExento($idCliente) && tipoOrdenPoseeIva($idTipoOrden)){//los repuestos se calculan en el calcular paquete
			// para calcular montos paquetes
			$arrayIvas = ivaServicios(); //proviene de ac_iv_general devuelve array de arrays
			foreach($arrayIvas as $arrayIva){
				if($arrayIva["activo"] == 1){//solo ivas predeterminados para manos de obra
					$arrayIvasActivosTempario[$arrayIva["idIva"]] = $arrayIva["iva"];
				}
			}
			$idIvaTempario = implode(",", array_keys($arrayIvasActivosTempario));
			$ivaPorcTempario = implode(",", $arrayIvasActivosTempario);
		}
		...
		$sigValor, $clase, $sigValor,
			$sigValor, $sigValor,
			addslashes($valForm['txtCodigoPaquete']),
			addslashes($valForm['txtDescripcionPaquete']),
			...
			$sigValor, $sigValor,
				...
				$sigValor, $sigValor, $idIvaTempario,//id iva separado por comas
				$sigValor, $sigValor, $ivaPorcTempario,//porc iva separado por comas
				$sigValor, $sigValor, "",
		...
		$sigValor,
			$valForm['hddEscogioPaquete'], addslashes($valForm['txtCodigoPaquete']), addslashes($valForm['txtDescripcionPaquete']), $valForm['txtPrecioPaquete'], 0, $valForm['hddManObraAproXpaq'], $valForm['hddRepAproXpaq']));
...
function insertarTempario($valForm, $valFormManoObraAgreg, $valFormTotalDcto) {
	...
	$idIvas = implode(",",$valForm['cbxIdIvaTempario']);//ahora con comas
	...
	if ($sw == 1 && $valFormTotalDcto['hddDuplicarManoObra'] != 1) {	
		...
		$totalIvasCalculados = 0;
		foreach($arrayIvasDisponibles as $key => $arrayIvaUsar){                                    
			$totalIvasCalculados += ($valForm['txtPrecioTemp'] * $arrayIvaUsar["iva"] / 100);
		}
		
		$montoMultipleIva = $totalIvasCalculados + $valForm['txtPrecioTemp'];
		...
		$sigValor, $clase, $sigValor,
			...
			$caracterIva,
			number_format(str_replace(",","", $valForm['txtPrecioTemp']),2,".",","),
			number_format($montoMultipleIva,2,".",","),
				$sigValor, $sigValor, "",
				$sigValor, $sigValor, $valForm['hddIdTemp'],//hddIdDetTemp
				$sigValor, $sigValor, $rowMecanico['id_mecanico'],
				$sigValor, $sigValor, $idIvas,//id iva
				$sigValor, $sigValor, $montosIvas,//iva
...
function insertarTot($valForm, $valFormTotAgreg, $valFormTotalDcto) {
	...
	$idIvas = implode(",",$valForm['cbxIdIvaTot']);//ahora con comas
	...
	if($sw == 1) {
		...
		$totalIvasCalculados = 0;
		foreach($arrayIvasDisponibles as $key => $arrayIvaUsar){                                    
			$totalIvasCalculados += (str_replace(",","", $valForm['txtMontoTotalTot']) * $arrayIvaUsar["iva"] / 100);
		}
		
		$montoMultipleIva = $totalIvasCalculados + str_replace(",","", $valForm['txtMontoTotalTot']);
		...
		$sigValor, $clase, $sigValor,
			...
			$caracterIva,
			$valForm['txtMontoTotalTot'],//ya usa formato
			number_format($montoMultipleIva,2,".",","),
				$sigValor, $sigValor, "",
	...
	if ($rowEmpresa['paquete_combo']==0){//PAQUETE
		...
		if(usaLotes()){//USA LOTES
			$query = sprintf("SELECT 
				...
				(IFNULL((SELECT
						SUM(
							(SELECT iv_articulos_precios.precio * sa_paquete_repuestos.cantidad 
								FROM iv_articulos
								INNER JOIN iv_articulos_precios ON iv_articulos.id_articulo = iv_articulos_precios.id_articulo
								INNER JOIN vw_iv_articulos_empresa ON iv_articulos.id_articulo = vw_iv_articulos_empresa.id_articulo
								WHERE iv_articulos_precios.id_articulo_costo = (IFNULL(
																		(SELECT almacen.id_articulo_costo 
																		FROM vw_iv_articulos_almacen_costo almacen 
																		WHERE almacen.id_articulo = iv_articulos.id_articulo 
																		AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
		...
		$query = sprintf("SELECT 
			...
			(IFNULL((SELECT SUM(sa_paq_tempario.costo) AS total_paquete 
				FROM sa_paq_tempario
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\" >";
			$htmlTb .= "<td>";
				...
				$htmlTb .= "<button id=\"btnMostrarInfoPaq\" type=\"button\" onclick=\"limpiarPaquetes(); xajax_buscar_mano_obra_repuestos_por_paquete('".$row['id_paq']."','".addslashes(utf8_encode($row['codigo_paquete']))."','".addslashes(utf8_encode($row['descripcion_paquete']))."','".$row['total_paquete']."', 0, '".""."', '".""."',0);\" title=\"Seleccionar Paquete\"><img src=\"".$srcIcono."\"/></button>";
			$htmlTb .= "</td>";
...
function listado_repuestos_por_paquetes($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 50, $totalRows = NULL) {
	...
	//$objResponse->alert("Prcio del query:".$row['precioRepuesto']);//alerta() este es el precio del combo
		if ($valCadBusq[4] == 0) { //NUEVO
			...
			if($usaLotes){//USA LOTES
				$query = sprintf("SELECT iv_articulos_precios.precio 
									FROM iv_articulos
									INNER JOIN iv_articulos_precios ON iv_articulos.id_articulo = iv_articulos_precios.id_articulo
									INNER JOIN vw_iv_articulos_empresa ON iv_articulos.id_articulo = vw_iv_articulos_empresa.id_articulo
									WHERE iv_articulos_precios.id_articulo_costo = (IFNULL(
																		...
																		AND almacen.estatus_almacen_venta = 1 
																		AND cantidad_disponible_logica > 0 
																		ORDER BY almacen.id_articulo_costo ASC 
																		LIMIT 1), 
																		...
																		AND almacen.id_empresa = vw_iv_articulos_empresa.id_empresa 
																		AND almacen.estatus_almacen_venta = 1 
																		ORDER BY almacen.id_articulo_costo DESC 
																		LIMIT 1)))
				...
				WHERE sa_tipo_orden.id_tipo_orden = %s
					...
					AND vw_iv_articulos_precios.id_empresa = %s",
					valTpDato($valCadBusq[2],"int"),
					valTpDato($row['id_articulo'],"int"),
					valTpDato($valCadBusq[0],"int"));
			}
...
function listado_tempario_por_paquetes($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 50, $totalRows = NULL) {
	...
	$totalMo = 0;
	if ($valCadBusq[4] == 0) {//nuevo agregando
		$queryTipoOrden = sprintf("SELECT 
			sa_tipo_orden.id_tipo_orden,
			sa_tipo_orden.precio_tempario
		FROM sa_tipo_orden
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		if ($valCadBusq[4]==1) { //CARGADO EN BD
			...
			$base_ut = $row['base_ut_precio'];
			$importe_por_tipo_orden = $row['importe_por_tipo_orden'];//IMPORTANTE
			
			if($row['id_modo'] == 1){//UT
				if($rowEmpresa['paquete_combo']==1){//COMBO
					$precio = $row['precio'];//todo combo se llevara por precio
					$ut = $row['ut'];
				}else{//PAQUETE
					$precio = "";
					$ut = $row['ut'];
			...
			//CUANDO ES NUEVO BUSCO UT, combo usa su propio ut
			$query = sprintf("SELECT * FROM sa_tempario_det
			...
			if($row['id_modo']==1) {//MODO UT 
			
				if ($rowEmpresa['paquete_combo']==1){//COMBO					
					//$precio = "";
					$precio = $row['costoCombo'];
					//$importe_por_tipo_orden = $row['utCombo'] * $row['costoCombo'] / $rowBaseUt['valor_parametro'];
					$importe_por_tipo_orden = $row['costoCombo'];//CAMBIADO GREGOR COMBO
				...
				if ($row['id_modo'] == 2) { //PRECIO	
					...
					if($rowEmpresa['paquete_combo']==1){//COMBO
						...
						$precio = number_format($row['precio'],2,".",",");
						$base_ut = $rowBaseUt['valor_parametro'];
						$ut = $rowValor['ut'];//NO ES MODO UT LE COLOCO VACIO
					}					
					...
					if ($rowEmpresa['paquete_combo']==1){//COMBO
						...
						$precio = number_format($row['costoCombo'],2,".",",");
						$base_ut = $rowBaseUt['valor_parametro'];
						$ut = $row['utCombo'];//NO ES MODO UT LE COLOCO VACIO
					}else{//PAQUETE
			...
			//validar precio tempario sea cual sea, debe tener UT asi no lo use
			if($ut == '' || $ut == 0 || $importe_por_tipo_orden == '' || $importe_por_tipo_orden == 0){
				$paqConTempSinPrecio = 1;
			}
			$asignacion_precio_tempario = ($paqConTempSinPrecio == 1) ? "<img src='../img/iconos/50.png' width='16' height='16' />" : "";
		}
		
		$htmlTb.= "<tr class=\"".$clase."\" height=\"22\">";
			$htmlTb .= "<td>".utf8_encode($row['codigo_tempario'])."</td>";
...
function listado_tot($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 6, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\" >";
			$htmlTb .= "<td>"."<button type=\"button\" onclick=\"xajax_asignarTot('".$row['id_orden_tot']."', xajax.getFormValues('frmPresupuesto'));\" title=\"Seleccionar T.O.T\"><img src=\"".$srcIcono."\"/></button>"."</td>";
...
function validarPermiso($valForm, $valFormDatosArticulo) {
	...
	if ($totalRowsPermiso > 0) {
		if ($valForm['hddModulo'] == "edc_dcto_ord") {
			...
			$('txtDescuento').focus();
			$('txtDescuento').select();
			$('txtDescuento').className = 'inputHabilitado';");
			...
			$('txtPrecioArtRepuesto').className = 'inputHabilitado';");
			...
			$('txtPrecioTemp').focus();
			$('txtPrecioTemp').select();
			$('txtPrecioTemp').className = 'inputHabilitado';");
			...
			$('txtPorcentaje').className = 'inputHabilitado';");
		}else if($valForm['hddModulo'] == "sa_porcentaje_pmu"){
			$objResponse->script("$('txtPorcentajePmu').readOnly = false;");
			$objResponse->script("$('txtBasePmu').readOnly = false;");
			$objResponse->script("$('divFlotante').style.display = 'none';");
			$objResponse->script("$('tblPermiso').style.display = 'none';");
			$objResponse->script("$('tblArticulo').style.display = '';");
			$objResponse->script("$('divFlotante').style.display = '';");
			$objResponse->script("centrarDiv($('divFlotante'));");
			
			$objResponse->script("
			$('txtPorcentajePmu').focus();
			$('txtPorcentajePmu').select();
			$('txtPorcentajePmu').className = 'inputHabilitado';
			$('txtBasePmu').className = 'inputHabilitado';");
		}
	} else {
...
function generarPresupuestoOrden($idPresupuesto){//ya validado antes
    ...
    $query = sprintf("SELECT sa_orden.id_tipo_orden, id_filtro_orden 
						...
						LEFT JOIN sa_tipo_orden ON sa_orden.id_tipo_orden = sa_tipo_orden.id_tipo_orden
						WHERE id_orden = %s LIMIT 1",
            valTpDato($rowPresupuesto["id_orden"], "int"));
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }    
    ...
    if($rowOrden["id_filtro_orden"] == "5"){//si esta SIN ASIGNAR colocar el tipo de orden del presupuesto
        $query = sprintf("UPDATE sa_orden SET id_tipo_orden = %s
							WHERE id_orden = %s",
            valTpDato($rowPresupuesto["id_tipo_orden"], "int"),
            valTpDato($rowPresupuesto["id_orden"], "int"));
        $rs = mysql_query($query);
        if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }    
        $rowOrden = mysql_fetch_assoc($rs);
	...
	$query = sprintf("UPDATE sa_orden SET
							...
							total_orden = %s
						WHERE id_orden = %s",
	valTpDato($rowPresupuesto["porcentaje_descuento"], "real_inglesa"),
	valTpDato($rowPresupuesto["subtotal_descuento"], "real_inglesa"),
	valTpDato($rowPresupuesto["idIva"], "real_inglesa"),
	valTpDato($rowPresupuesto["iva"], "int"),
	valTpDato($rowPresupuesto["subtotal"], "real_inglesa"),
	valTpDato($rowPresupuesto["base_imponible"], "real_inglesa"),
	valTpDato($rowPresupuesto["subtotal_iva"], "real_inglesa"),
	valTpDato(19, "int"), //19 = presupuesto aprobado
	valTpDato($rowPresupuesto["monto_exento"], "real_inglesa"),
	valTpDato($rowPresupuesto["total_presupuesto"], "real_inglesa"),
	valTpDato($rowPresupuesto["id_orden"], "int"));
    ...
    $query = sprintf("SELECT id_det_presup_articulo, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, porcentaje_pmu, base_pmu, pmu_unitario, id_iva, iva, aprobado, tiempo_asignacion, tiempo_aprobacion, id_empleado_aprobacion, estado_articulo
						FROM sa_det_presup_articulo 
						WHERE id_presupuesto = %s 
						AND id_det_orden_articulo IS NULL",
	            valTpDato($idPresupuesto, "int"));
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
	
    while($rowArticuloPresup = mysql_fetch_assoc($rs)){
        $query = sprintf("INSERT INTO sa_det_orden_articulo (id_orden, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, porcentaje_pmu, base_pmu, pmu_unitario, id_iva, iva, aprobado, tiempo_asignacion, tiempo_aprobacion, id_empleado_aprobacion, estado_articulo)
						 VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            valTpDato($rowPresupuesto["id_orden"], "int"),
            valTpDato($rowArticuloPresup["id_articulo"], "int"),
            valTpDato($rowArticuloPresup["id_paquete"], "int"),
            valTpDato($rowArticuloPresup["cantidad"], "int"),
            valTpDato($rowArticuloPresup["id_precio"], "int"),
            valTpDato($rowArticuloPresup["precio_unitario"], "real_inglesa"), 
            valTpDato($rowArticuloPresup["id_articulo_costo"], "int"), 
            valTpDato($rowArticuloPresup["id_articulo_almacen_costo"], "int"), 
            valTpDato($rowArticuloPresup["costo"], "real_inglesa"),
            valTpDato($rowArticuloPresup["porcentaje_pmu"], "real_inglesa"),
            valTpDato($rowArticuloPresup["base_pmu"], "real_inglesa"),
            valTpDato($rowArticuloPresup["pmu_unitario"], "real_inglesa"),
            valTpDato($rowArticuloPresup["id_iva"], "int"), 
            valTpDato($rowArticuloPresup["iva"], "real_inglesa"), 
            valTpDato($rowArticuloPresup["aprobado"], "int"), 
            valTpDato($rowArticuloPresup["tiempo_asignacion"], "date"), 
            valTpDato($rowArticuloPresup["tiempo_aprobacion"], "date"),  
            valTpDato($rowArticuloPresup["id_empleado_aprobacion"], "int"),
            valTpDato($rowArticuloPresup["estado_articulo"], "text"));
		...
		$query = sprintf("SELECT id_iva, iva, lujo 
							...
							WHERE id_det_presup_articulo = %s",
					valTpDato($rowArticuloPresup["id_det_presup_articulo"], "int"));
		$rs3 = mysql_query($query);
		if(!$rs3) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        
        while($rowArtIvaPresup = mysql_fetch_assoc($rs3)){
            $query = sprintf("INSERT INTO sa_det_orden_articulo_iva (id_det_orden_articulo, id_iva, iva, lujo)
                                    VALUES(%s, %s, %s, %s)",
						valTpDato($idDetOrdenArticulo, "int"),
						valTpDato($rowArtIvaPresup["id_iva"], "int"),
						valTpDato($rowArtIvaPresup["iva"], "real_inglesa"),
    ...
    $query = sprintf("SELECT id_det_presup_tempario, id_paquete, id_tempario, precio, costo, costo_orden, id_modo, base_ut_precio, operador, aprobado, ut, tiempo_aprobacion, tiempo_asignacion, id_empleado_aprobacion, origen_tempario, estado_tempario,precio_tempario_tipo_orden
						FROM sa_det_presup_tempario 
						WHERE id_presupuesto = %s 
						AND id_det_orden_tempario IS NULL",
				valTpDato($idPresupuesto, "int"));
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); } 
	
	while($rowTemparioPresup = mysql_fetch_assoc($rs)){
		$query = sprintf("INSERT INTO sa_det_orden_tempario (id_orden, id_paquete, id_tempario, precio, costo, costo_orden, id_modo, base_ut_precio, operador, aprobado, ut, tiempo_aprobacion, tiempo_asignacion, id_empleado_aprobacion, origen_tempario, estado_tempario,precio_tempario_tipo_orden)
				...
				valTpDato($rowPresupuesto["id_orden"], "int"),
				valTpDato($rowTemparioPresup["id_paquete"], "int"),
				valTpDato($rowTemparioPresup["id_tempario"], "int"),
				valTpDato($rowTemparioPresup["precio"], "real_inglesa"),
				valTpDato($rowTemparioPresup["costo"], "real_inglesa"),
				valTpDato($rowTemparioPresup["costo_orden"], "real_inglesa"),
				valTpDato($rowTemparioPresup["id_modo"], "int"),
				valTpDato($rowTemparioPresup["base_ut_precio"], "int"),
				valTpDato($rowTemparioPresup["operador"], "int"),
				valTpDato($rowTemparioPresup["aprobado"], "int"),
				valTpDato($rowTemparioPresup["ut"], "real_inglesa"),
				valTpDato($rowTemparioPresup["tiempo_aprobacion"], "date"), 
				valTpDato($rowTemparioPresup["tiempo_asignacion"], "date"),
				valTpDato($rowTemparioPresup["id_empleado_aprobacion"], "int"),
				valTpDato(0, "int"),// orden
				valTpDato($rowTemparioPresup["estado_tempario"], "text"),
				valTpDato($rowTemparioPresup["precio_tempario_tipo_orden"], "real_inglesa"));		
        $rs2 = mysql_query($query);
        if(!$rs2) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        $idDetOrdenTempario = mysql_insert_id();
		
        $query = sprintf("SELECT id_iva, iva, lujo
							FROM sa_det_presup_tempario_iva 
							WHERE id_det_presup_tempario = %s",
            		valTpDato($rowTemparioPresup["id_det_presup_tempario"], "int"));
        $rs3 = mysql_query($query);
        if(!$rs3) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        
		while($rowTempIvaPresup = mysql_fetch_assoc($rs3)){
            $query = sprintf("INSERT INTO sa_det_orden_tempario_iva (id_det_orden_tempario, id_iva, iva, lujo)
                                    VALUES(%s, %s, %s, %s)",
				valTpDato($idDetOrdenTempario, "int"),
				valTpDato($rowTempIvaPresup["id_iva"], "int"),
				valTpDato($rowTempIvaPresup["iva"], "real_inglesa"),
				valTpDato($rowTempIvaPresup["lujo"], "int"));
            $rs4 = mysql_query($query);
            if(!$rs4) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        }
	}
    ...
    $query = sprintf("SELECT id_det_presup_nota, descripcion_nota, precio, aprobado
						FROM sa_det_presup_notas 
						WHERE id_presupuesto = %s 
						AND id_det_orden_nota IS NULL",
				valTpDato($idPresupuesto, "int"));
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
	
	while($rowNotaPresup = mysql_fetch_assoc($rs)){
		$query = sprintf("INSERT INTO sa_det_orden_notas (id_orden, descripcion_nota, precio, aprobado)
							VALUES(%s, %s, %s, %s)", 
					valTpDato($rowPresupuesto["id_orden"], "int"),
					valTpDato($rowNotaPresup["descripcion_nota"], "text"),
					valTpDato($rowNotaPresup["precio"], "real_inglesa"),
					valTpDato($rowNotaPresup["aprobado"], "int"));
        $rs2 = mysql_query($query);
        if(!$rs2) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        $idDetOrdenNota = mysql_insert_id();
		
        $query = sprintf("SELECT id_iva, iva, lujo 
                        FROM sa_det_presup_notas_iva 
                        WHERE id_det_presup_nota = %s",
            valTpDato($rowNotaPresup["id_det_presup_nota"], "int"));
        $rs3 = mysql_query($query);
        if(!$rs3) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }

		while($rowNotaIvaPresup = mysql_fetch_assoc($rs3)){
            $query = sprintf("INSERT INTO sa_det_orden_notas_iva (id_det_orden_nota, id_iva, iva, lujo)
                                    VALUES(%s, %s, %s, %s)",
				valTpDato($idDetOrdenNota, "int"),
				valTpDato($rowNotaIvaPresup["id_iva"], "int"),
				valTpDato($rowNotaIvaPresup["iva"], "real_inglesa"),
				valTpDato($rowNotaIvaPresup["lujo"], "int"));
            $rs4 = mysql_query($query);
            if(!$rs4) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        }
	}
    ...
    $query = sprintf("SELECT id_det_presup_tot, id_orden_tot, id_porcentaje_tot, porcentaje_tot, aprobado
						FROM sa_det_presup_tot 
						WHERE id_presupuesto = %s 
						AND id_det_orden_tot IS NULL",
					valTpDato($idPresupuesto, "int"));
    $rs = mysql_query($query);
    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
    
	while($rowTotPresup = mysql_fetch_assoc($rs)){
		$query = sprintf("INSERT INTO sa_det_orden_tot (id_orden, id_orden_tot, id_porcentaje_tot, porcentaje_tot, aprobado)", 
					valTpDato($rowPresupuesto["id_orden"], "int"),
					valTpDato($rowTotPresup["id_orden_tot"], "int"),
					valTpDato($rowTotPresup["id_porcentaje_tot"], "int"),
					valTpDato($rowTotPresup["porcentaje_tot"], "real_inglesa"),
					valTpDato($rowTotPresup["aprobado"], "int"));
        $rs2 = mysql_query($query);
        if(!$rs2) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        $idDetOrdenTot = mysql_insert_id();
	
        $query = sprintf("SELECT id_iva, iva, lujo 
                        FROM sa_det_presup_tot_iva 
                        WHERE id_det_presup_tot = %s",
            valTpDato($rowTotPresup["id_det_presup_tot"], "int"));
        $rs3 = mysql_query($query);
        if(!$rs3) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
	
		while($rowTotIvaPresup = mysql_fetch_assoc($rs3)){
            $query = sprintf("INSERT INTO sa_det_orden_tot_iva (id_det_orden_tot, id_iva, iva, lujo)
                                    VALUES(%s, %s, %s, %s)",
				valTpDato($idDetOrdenTot, "int"),
				valTpDato($rowTotIvaPresup["id_iva"], "int"),
				valTpDato($rowTotIvaPresup["iva"], "real_inglesa"),
				valTpDato($rowTotIvaPresup["lujo"], "int"));
            $rs4 = mysql_query($query);
            if(!$rs4) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); }
        }
	}
    ...
    $query = sprintf("UPDATE sa_orden_tot SET id_orden_servicio = %s, estatus = 2 
						WHERE id_orden_tot IN(SELECT id_orden_tot 
												FROM sa_det_presup_tot 
												WHERE id_presupuesto = %s 
												AND id_det_orden_tot IS NULL)",
                            valTpDato($rowPresupuesto["id_orden"], "int"),
...
$xajax->register(XAJAX_FUNCTION,"cargarImpuestoNota");
