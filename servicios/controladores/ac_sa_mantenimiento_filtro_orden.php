...
function listadoFiltroOrden($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL) {
	...
	$query = sprintf("SELECT 
						...
						edicion_impuesto,
						tot_accesorio
					FROM sa_filtro_orden %s", $sqlBusq);					
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";            
		...
		$htmlTh .= ordenarCampo("xajax_listadoFiltroOrden", "20%", $pageNum, "bloqueo_items", $campOrd, $tpOrd, $valBusq, $maxRows, ("Bloqueo Items"));
		$htmlTh .= ordenarCampo("xajax_listadoFiltroOrden", "20%", $pageNum, "edicion_impuesto", $campOrd, $tpOrd, $valBusq, $maxRows, ("Edici&oacute;n de Impuesto"));
		$htmlTh .= ordenarCampo("xajax_listadoFiltroOrden", "15%", $pageNum, "tot_accesorio", $campOrd, $tpOrd, $valBusq, $maxRows, ("TOT con accesorios"));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<td align=\"center\" ".$title.">".$row['edicion_impuesto']."</td>";
...
function guardarFiltroOrden($form){
	...
	$edicionImpuestoItems = $form["edicionImpuestoItems"];
    ...
    if($idFiltroOrden == ""){//nuevo
        ...
        $sql = sprintf("INSERT INTO sa_filtro_orden (descripcion, bloqueo_items, edicion_impuesto, tot_accesorio) 
						VALUES (%s, %s, %s, %s)",
					valTpdato($descripcion,"text"),                     
					valTpdato($bloqueoItems,"text"),
					valTpdato($edicionImpuestoItems,"text"),
        ...
        $sql = sprintf("UPDATE sa_filtro_orden SET descripcion = %s, bloqueo_items = %s, edicion_impuesto = %s, tot_accesorio = %s
...
function cargarFiltroOrden($idFiltroOrden){
    ...
    $sql = sprintf("SELECT descripcion, bloqueo_items, edicion_impuesto, tot_accesorio 
					FROM sa_filtro_orden 
					WHERE id_filtro_orden = %s LIMIT 1",
				valTpDato($idFiltroOrden,"int"));
	...
	$objResponse->assign("edicionImpuestoItems","value",$row["edicion_impuesto"]);
