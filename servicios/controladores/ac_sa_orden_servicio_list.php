...
function abrirCambioAsesor($idOrden){
	$objResponse = new xajaxResponse();

	if($idOrden == ""){ return $objResponse->alert("No se envio id de la orden"); }

	$sql = sprintf("SELECT
					sa_orden.numero_orden, 
					CONCAT_WS(' ', pg_empleado.nombre_empleado, apellido) AS nombre_empleado
					FROM sa_orden
	...
	$objResponse->assign("spanNumeroOrdenAsesor","innerHTML",$row["numero_orden"]);
	$objResponse->assign("nombreAsesorActual","innerHTML",utf8_encode($row["nombre_empleado"]));
	$objResponse->script("$('asesor_window').style.display = '';
						centrarDiv($('asesor_window'));");
...
function asignarEmpleado($idEmpleado, $nombreEmpleado){
    ...
    $objResponse->assign("idEmpleadoNuevo","value",$idEmpleado);
    $objResponse->assign("nombreEmpleadoNuevo","value",$nombreEmpleado);
...
function asociarFallas($idOrden){	
	$objResponse = new xajaxResponse();
	
	$sql = sprintf("SELECT numero_orden, id_recepcion_falla, descripcion_falla
					FROM sa_orden
					INNER JOIN sa_recepcion ON sa_orden.id_recepcion =  sa_recepcion.id_recepcion
					INNER JOIN sa_recepcion_falla ON sa_recepcion.id_recepcion = sa_recepcion_falla.id_recepcion
					WHERE id_orden = %s",
			valTpDato($idOrden, "int"));
	$rs = mysql_query($sql);
	if(!$rs){ return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__); }
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		$arrayFallas[$row["id_recepcion_falla"]] = $row["descripcion_falla"];
	}
	
	$sql = sprintf("SELECT id_det_orden_articulo, codigo_articulo, descripcion, id_falla
					FROM sa_det_orden_articulo 
					INNER JOIN iv_articulos ON sa_det_orden_articulo.id_articulo = iv_articulos.id_articulo
					WHERE id_orden = %s
	...
	if(mysql_num_rows($rs)){
		$html .= "<tr><td colspan=\"3\" style=\"border:0px;\" align=\"center\" class=\"tituloCampo\"><b>Repuestos</b></td></tr>";	
	}
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		$html .= "<tr class=\"".$clase."\">";
			$html .= "<td>";
				$html .= "<select name=\"selFalla[]\" style=\"width:250px;\">";
					$html .= "<option value=\"REPUESTO|-1|".$row["id_det_orden_articulo"]."\">[ SELECCIONE ]</option>";
					foreach($arrayFallas as $idFalla => $descripcionFalla){
						if($idFalla == $row["id_falla"]){ $selected = "selected=\"selected\""; } else { $selected = ""; }
						$value = "REPUESTO|".$idFalla."|".$row["id_det_orden_articulo"];
						$html .= "<option ".$selected." value=\"".$value."\">".utf8_encode($descripcionFalla)."</option>";
					}
				$html .= "</select>";
			$html .= "</td>";
			$html .= "<td>".utf8_encode($row["codigo_articulo"])."</td>";
			$html .= "<td>".utf8_encode($row["descripcion"])."</td>";
		$html .= "</tr>";
	}

	$sql = sprintf("SELECT id_det_orden_tempario, codigo_tempario, descripcion_tempario, id_falla
	...
	if(mysql_num_rows($rs)){
		$html .= "<tr><td colspan=\"3\" style=\"border:0px;\" align=\"center\" class=\"tituloCampo\"><b>Manos de Obra</b></td></tr>";	
	}
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		$html .= "<tr class=\"".$clase."\">";
			$html .= "<td>";
				$html .= "<select name=\"selFalla[]\" style=\"width:250px;\">";
					$html .= "<option value=\"MO|-1|".$row["id_det_orden_tempario"]."\">[ SELECCIONE ]</option>";
					foreach($arrayFallas as $idFalla => $descripcionFalla){
						if($idFalla == $row["id_falla"]){ $selected = "selected=\"selected\""; } else { $selected = ""; }
						$value = "MO|".$idFalla."|".$row["id_det_orden_tempario"];
						$html .= "<option ".$selected." value=\"".$value."\">".utf8_encode($descripcionFalla)."</option>";
					}
				$html .= "</select>";
			$html .= "</td>";
			$html .= "<td>".utf8_encode($row["codigo_tempario"])."</td>";
			$html .= "<td>".utf8_encode($row["descripcion_tempario"])."</td>";
		$html .= "</tr>";
	}
	
	$sql = sprintf("SELECT sa_det_orden_tot.id_orden_tot, numero_tot, observacion_factura, id_falla
	...
	if(mysql_num_rows($rs)){
		$html .= "<tr><td colspan=\"3\" style=\"border:0px;\" align=\"center\" class=\"tituloCampo\"><b>TOT</b></td></tr>";	
	}
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		$html .= "<tr class=\"".$clase."\">";
			$html .= "<td>";
				$html .= "<select name=\"selFalla[]\" style=\"width:250px;\">";
					$html .= "<option value=\"TOT|-1|".$row["id_orden_tot"]."\">[ SELECCIONE ]</option>";
					foreach($arrayFallas as $idFalla => $descripcionFalla){
						if($idFalla == $row["id_falla"]){ $selected = "selected=\"selected\""; } else { $selected = ""; }
						$value = "TOT|".$idFalla."|".$row["id_orden_tot"];
						$html .= "<option ".$selected." value=\"".$value."\">".utf8_encode($descripcionFalla)."</option>";
					}
				$html .= "</select>";
			$html .= "</td>";
			$html .= "<td>".utf8_encode($row["numero_tot"])."</td>";
			$html .= "<td>".utf8_encode($row["observacion_factura"])."</td>";
		$html .= "</tr>";
	}
	
	$sql = sprintf("SELECT id_det_orden_nota, descripcion_nota, id_falla
	...
	if(mysql_num_rows($rs)){
		$html .= "<tr><td colspan=\"3\" style=\"border:0px;\" align=\"center\" class=\"tituloCampo\"><b>Notas / Cargo Adicional</b></td></tr>";	
	}
	...
	while($row = mysql_fetch_assoc($rs)){
		...
		$html .= "<tr class=\"".$clase."\">";
			$html .= "<td>";
				$html .= "<select name=\"selFalla[]\" style=\"width:250px;\">";
					$html .= "<option value=\"NOTA|-1|".$row["id_det_orden_nota"]."\">[ SELECCIONE ]</option>";
					foreach($arrayFallas as $idFalla => $descripcionFalla){
						if($idFalla == $row["id_falla"]){ $selected = "selected=\"selected\""; } else { $selected = ""; }
						$value = "NOTA|".$idFalla."|".$row["id_det_orden_nota"];
						$html .= "<option ".$selected." value=\"".$value."\">".utf8_encode($descripcionFalla)."</option>";
					}
				$html .= "</select>";
			$html .= "</td>";
			$html .= "<td>".utf8_encode($row["id_det_orden_nota"])."</td>";
			$html .= "<td>".utf8_encode($row["descripcion_nota"])."</td>";
		$html .= "</tr>";
	}
	
	$tabla .= "<table width=\"100%\">";
		$tabla .= "<tr class=\"tituloColumna\">";
			$tabla .= "<td align=\"center\">Falla</td>";
			$tabla .= "<td align=\"center\">C&oacute;digo</td>";
			$tabla .= "<td align=\"center\">Descripci&oacute;n</td>";
		$tabla .= "</tr>";
		$tabla .= $html;
	$tabla .= "</table>";
	
	$objResponse->assign("divFallas","innerHTML",$tabla);
	$objResponse->assign("spanNumeroOrdenFalla","innerHTML",$numeroOrden);
	$objResponse->script("$('fallas_window').style.display = '';
						centrarDiv($('fallas_window'));");
...
function guardarAsociarFallas($valForm){
	$objResponse = new xajaxResponse();
	
	$arrayFallas = $valForm["selFalla"];	
	
	if($arrayFallas == ""){
		return $objResponse->alert("No se envio nada a guardar");
	}
	
	mysql_query("START TRANSACTION;");
	
	foreach($arrayFallas as $fallasAsociadas){
		$fallas = explode("|", $fallasAsociadas);
		
		$tipo = $fallas[0];
		$idFalla = $fallas[1];
		$idItem = $fallas[2];
		
		if($idFalla == "-1"){ $idFalla = "NULL"; }
		
		$sql = "";//reinicio para evitar errores
		if($tipo == "REPUESTO"){
			$sql = sprintf("UPDATE sa_det_orden_articulo SET id_falla = %s 
							WHERE id_det_orden_articulo = %s",
							$idFalla,
				valTpDato($idItem, "int"));
		}else if($tipo == "MO"){
			$sql = sprintf("UPDATE sa_det_orden_tempario SET id_falla = %s 
							WHERE id_det_orden_tempario = %s",
							$idFalla,
				valTpDato($idItem, "int"));
		}else if($tipo == "TOT"){
			$sql = sprintf("UPDATE sa_det_orden_tot SET id_falla = %s 
							WHERE id_det_orden_tot = %s",
							$idFalla,
				valTpDato($idItem, "int"));
		}else if($tipo == "NOTA"){
			$sql = sprintf("UPDATE sa_det_orden_notas SET id_falla = %s 
							WHERE id_det_orden_nota = %s",
							$idFalla,
				valTpDato($idItem, "int"));
		}else{
			return $objResponse->alert("No se envio el tipo de item");
		}
	...
	$objResponse->script("$('fallas_window').style.display='none';");
	
	return $objResponse;
}

function guardarCambioAsesor($valForm){
	$objResponse = new xajaxResponse();
	
	if($valForm['idEmpleadoNuevo'] == ""){
		return $objResponse->alert("No se ha seleccionado asesor");
	}
	
	$sql = sprintf("UPDATE sa_orden SET id_empleado = %s 
					WHERE id_orden = %s",
		valTpDato($valForm['idEmpleadoNuevo'], "int"),
		valTpDato($valForm['idOrdenAsesor'], "int"));

	$rs = mysql_query($sql);
	if(!$rs){ return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nSql:".$sql); }

	$objResponse->alert("Cambiado con Exito");
	$objResponse->script("byId('asesor_window').style.display = 'none';
						$('btnBuscar').click();");	
...
function listadoEmpleado($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
    $objResponse = new xajaxResponse();
	...
	$sqlBusq .= $cond.sprintf(" pg_cargo_departamento.clave_filtro IN (4,5,6,70,80)");
	//4 = GERENTE SERVICIOS (POSTVENTA), 5 = ASESOR SERVICIOS, 6 = JEFE TALLER, 70 = CONTROL DE CALIDAD, 80 = COORDINADOR TECNICO
	
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf(" pg_empleado.activo = 1");
...
function listadoOrdenes($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "5%", $pageNum, "numeracion_recepcion", $campOrd, $tpOrd, $valBusq, $maxRows, ("N° Recepción"));
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "10%", $pageNum, "nombre_empleado", $campOrd, $tpOrd, $valBusq, $maxRows, ("Asesor"));
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "10%", $pageNum, "nombre_tecnico", $campOrd, $tpOrd, $valBusq, $maxRows, ("T&eacute;cnico"));
		...
		$htmlTh .= ordenarCampo("xajax_listadoOrdenes", "6%", $pageNum, "nom_uni_bas", $campOrd, $tpOrd, $valBusq, $maxRows, ("Cat&aacute;logo"));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\" height=\"22\">";
			...
			$htmlTb .= "<td align=\"left\">".utf8_encode($row['nombre_tecnico'])."</td>";
			...
			$htmlTb .= "</td>";
			//boton asociar fallas
			if($pais == 3){
				$htmlTb .= "<td class=\"noprint\">";
				$htmlTb .= sprintf("<img class=\"puntero\" title=\"Asociar Fallas\" onclick=\"xajax_asociarFallas(%s);\" src=\"../img/iconos/chart_organisation.png\"/>",
					$row['id_orden']);
				$htmlTb .= "</td>";	
			}
			$htmlTb .= "<td class=\"noprint\">";
			if ($row['id_tipo_estado'] != 5) { // TERMINADO
				$htmlTb .= sprintf("<img class=\"puntero\" title=\"Cambiar Asesor (Apertura Orden)\" onclick=\"abrirClaveCambioAsesor(%s);\" src=\"../img/iconos/user_suit.png\"/>",
					$row['id_orden']);
			}
			$htmlTb .= "</td>";
			...
			$htmlTb .= "<td class=\"noprint\">";
				$pdfGarantia = "";//si es puerto rico y garantia
				if($pais == 3 && $row['id_filtro_orden'] == 3){ 
					$pdfGarantia = sprintf("verVentana('reportes/sa_imprimir_presupuesto_pdf.php?valBusq=%s|2|0&garantia=1', 700, 700);",
									$row['id_orden']); 
				}
				$htmlTb .= sprintf("<img class=\"puntero\" title=\"Imprimir Orden PDF\" onclick=\"verVentana('reportes/sa_imprimir_presupuesto_pdf.php?valBusq=%s|2|0', 1000, 500); %s\" src=\"../img/iconos/ico_print.png\"/>",
					$row['id_orden'],
					$pdfGarantia);
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
...
function verificarClaveCambioAsesor($pass, $idOrden){
	$objResponse = new xajaxResponse();

	$sql = sprintf("SELECT clave 
					FROM sa_claves 
					WHERE modulo = 'sa_cambio_asesor_orden' 
					AND id_empleado = %s 
					AND id_empresa = %s",
			valTpDato($_SESSION['idEmpleadoSysGts'],"int"),
			valTpDato($_SESSION['idEmpresaUsuarioSysGts'],"int"));
	$rsClaves = mysql_query($sql);
	...
	if($rowClaves){
		...
		if($pass == $rowClaves['clave']){
			...
			$objResponse->script("$('claveCambioAsesorCorrecta').value = 1;");
			$objResponse->script("xajax_abrirCambioAsesor(".$idOrden.");"); 
		}else{
...
$xajax->register(XAJAX_FUNCTION,"abrirCambioAsesor");
$xajax->register(XAJAX_FUNCTION,"asignarEmpleado");
$xajax->register(XAJAX_FUNCTION,"asignarMecanico");
$xajax->register(XAJAX_FUNCTION,"asociarFallas");
...
$xajax->register(XAJAX_FUNCTION,"guardarAsociarFallas");
$xajax->register(XAJAX_FUNCTION,"guardarCambioAsesor");
...
$xajax->register(XAJAX_FUNCTION,"verificarClaveCambioAsesor");
