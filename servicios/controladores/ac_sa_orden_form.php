...
//	if (!$consultaActualizarVale) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__.$consultaActualizarVale);
	...
	$usaLotes = usaLotes();//usa lotes afuera del while para evitar tantas consultas
		...
		if($rowArt['nro_rpto_desp'] > 0) {
			...
			if($rowArt['id_articulo_costo'] > 0){
				$sqlIndividual = sprintf(" AND sa_det_vale_salida_articulo.id_articulo_costo = %s 
								AND fact_vent_detsa_det_vale_salida_articuloid_articulo_almacen_costo = %s ",
			...
			$queryArtValeSalida = sprintf("SELECT 
				...
				sa_det_vale_salida_articulo.porcentaje_pmu,
				sa_det_vale_salida_articulo.base_pmu,
				sa_det_vale_salida_articulo.pmu_unitario,
				...
				(SELECT valor FROM pg_configuracion_empresa config_emp
					...
					(SELECT costo_promedio FROM iv_articulos_costos 
							WHERE id_articulo = %s AND id_empresa = %s ORDER BY id_articulo_costo DESC LIMIT 1) AS costo_promedio
			...
			if($usaLotes){//USA LOTES			
				...
				if($rowArtValeSalida["modo_costo"] == 1){
					$costoPromedioCompra = $rowArtValeSalida["costo_compra"];
				}else{
					$costoPromedioCompra = $rowArtValeSalida["costo_promedio"];
				}
			}
			...
			$insertSQLKardex = sprintf("INSERT INTO iv_kardex(id_modulo, id_documento, id_articulo, id_casilla, id_articulo_almacen_costo, id_articulo_costo, tipo_movimiento, tipo_documento_movimiento, cantidad, precio, pmu_unitario, costo, costo_cargo, porcentaje_descuento, subtotal_descuento, id_clave_movimiento, estado, fecha_movimiento, hora_movimiento) 
			VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), NOW())",
				...
				valTpDato($rowArtValeSalida['pmu_unitario'], "real_inglesa"),
			...
			$insertSQL = sprintf("INSERT INTO iv_movimiento_detalle (id_movimiento, id_articulo, id_kardex, id_articulo_almacen_costo, id_articulo_costo, cantidad, precio, pmu_unitario, costo, costo_cargo, porcentaje_descuento, subtotal_descuento, correlativo1, correlativo2, tipo_costo, llave_costo_identificado, promocion, id_moneda_costo, id_moneda_costo_cambio) 
	...
	$objResponse->script("verVentana('reportes/sa_devolucion_vale_salida_pdf.php?valBusq=0|".$idValeSalida."', 1000, 500)");	
	$objResponse->script("window.location.href = 'sa_devolucion_vale_salida_list.php'");	
...
function generarDctoApartirDeOrden($valForm, $valFormTotalDcto) {
	...
	if ($valFormTotalDcto['hddTipoDocumento'] == 4) { //PRESUPUESTO
		...
		SELECT 
			...
			sa_det_orden_articulo.porcentaje_pmu,
			sa_det_orden_articulo.base_pmu,
			sa_det_orden_articulo.pmu_unitario,
		...
		$insertIvaArticulosPresupuesto = sprintf("INSERT INTO sa_det_presup_articulo_iva (id_det_presup_articulo, id_iva, iva) 
		SELECT 
		...
		//iva temparios orden a presupuesto
		$insertIvaTemparioPresupuesto = sprintf("INSERT INTO sa_det_presup_tempario_iva (id_det_presup_tempario, id_iva, iva) 
		SELECT 
			sa_det_presup_tempario.id_det_presup_tempario,
			sa_det_orden_tempario_iva.id_iva,
			sa_det_orden_tempario_iva.iva                        
		FROM sa_det_presup_tempario
		INNER JOIN sa_det_orden_tempario_iva ON sa_det_presup_tempario.id_det_orden_tempario = sa_det_orden_tempario_iva.id_det_orden_tempario
		WHERE sa_det_presup_tempario.id_presupuesto = %s",
			valTpDato($idPresupuesto, "int"));		
		$rsIvaTemparioPresupuesto = mysql_query($insertIvaTemparioPresupuesto);
		if (!$rsIvaTemparioPresupuesto) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nQyery: ".$insertIvaTemparioPresupuesto); }
		...
		//iva tot orden a presupuesto
		$insertIvaTotPresupuesto = sprintf("INSERT INTO sa_det_presup_tot_iva (id_det_presup_tot, id_iva, iva) 
		SELECT 
			sa_det_presup_tot.id_det_presup_tot,
			sa_det_orden_tot_iva.id_iva,
			sa_det_orden_tot_iva.iva                        
		FROM sa_det_presup_tot
		INNER JOIN sa_det_orden_tot_iva ON sa_det_presup_tot.id_det_orden_tot = sa_det_orden_tot_iva.id_det_orden_tot
		WHERE sa_det_presup_tot.id_presupuesto = %s",
			valTpDato($idPresupuesto, "int"));		
		$rsIvaTotPresupuesto = mysql_query($insertIvaTotPresupuesto);
		if (!$rsIvaTotPresupuesto) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nQyery: ".$insertIvaTotPresupuesto); }
		...
		//iva notas orden a presupuesto
		$insertIvaNotaPresupuesto = sprintf("INSERT INTO sa_det_presup_notas_iva (id_det_presup_nota, id_iva, iva) 
		SELECT 
			sa_det_presup_notas.id_det_presup_nota,
			sa_det_orden_notas_iva.id_iva,
			sa_det_orden_notas_iva.iva                        
		FROM sa_det_presup_notas
		INNER JOIN sa_det_orden_notas_iva ON sa_det_presup_notas.id_det_orden_nota = sa_det_orden_notas_iva.id_det_orden_nota
		WHERE sa_det_presup_notas.id_presupuesto = %s",
			valTpDato($idPresupuesto, "int"));		
		$rsIvaNotaPresupuesto = mysql_query($insertIvaNotaPresupuesto);
		if (!$rsIvaNotaPresupuesto) { return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nQyery: ".$insertIvaNotaPresupuesto); }
...
//		}
		...
		$ivasSql = sprintf("DELETE FROM sa_orden_iva WHERE id_orden = %s",
					valTpDato($idDocumentoVenta,"int"));
						...
						if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
							...
							foreach($arrayIvasTempario as $indice => $idIvaTempario){							   
								if($idIvaTempario == $idIvas){//si el iva del tempario lo tiene la orden, guardar
									$ivasSql = sprintf("INSERT INTO sa_det_orden_tempario_iva (id_det_orden_tempario, id_iva, iva) 
													VALUES (%s, %s, %s)",
		...
		foreach($arrayObj as $indice => $valor) {
			if (strlen($valFormListaArt['hddIdArt'.$valor]) > 0) {
				if ($valFormListaArt['hddIdPedDet'.$valor] == "") {
					...
					$insertSQL = sprintf("INSERT INTO sa_det_orden_articulo (id_orden, id_articulo, cantidad, id_precio, precio_unitario, id_articulo_costo, id_articulo_almacen_costo, costo, porcentaje_pmu, base_pmu, pmu_unitario, id_iva, iva, tiempo_asignacion, id_empleado_aprobacion, aprobado)
						...
						if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
							...
							foreach($arrayIvasArticulo as $indice => $idIvaRepuesto){							   
								if($idIvaRepuesto == $idIvas){//si el iva del repuesto lo tiene la orden, guardar
									$ivasSql = sprintf("INSERT INTO sa_det_orden_articulo_iva (id_det_orden_articulo, id_iva, iva) 
													VALUES (%s, %s, %s)",
											valTpDato($idDocumentoDetalle,"int"),
						...
						if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
							...
							foreach($arrayIvasNota as $indice => $idIvaNota){							   
								if($idIvaNota == $idIvas){//si el iva del tempario lo tiene la orden, guardar
									$ivasSql = sprintf("INSERT INTO sa_det_orden_notas_iva (id_det_orden_nota, id_iva, iva) 
													VALUES (%s, %s, %s)",
											valTpDato($idDocumentoDetalle,"int"),
	...
	if (isset($arrayObjTot)) {
		foreach($arrayObjTot as $indiceTot => $valorTot) {
			if (strlen($valFormListTot['hddIdTot'.$valorTot]) > 0) {
				if ($valFormListTot['hddIdPedDetTot'.$valorTot] == "") {
					$insertSQL = sprintf("INSERT INTO sa_det_orden_tot (id_orden, id_orden_tot, id_porcentaje_tot, porcentaje_tot, aprobado)
					VALUE (%s, %s, %s, %s, %s);",
						valTpDato($idDocumentoVenta, "int"),
						...
						if($valFormTotalDcto["txtBaseImponibleIva".$idIvas] != 0 && $valFormTotalDcto["txtTotalIva".$idIvas] != 0 && $valFormTotalDcto["txtIvaVenta".$idIvas] != 0){
							...
							foreach($arrayIvasTot as $indice => $idIvaTot){							   
								if($idIvaTot == $idIvas){//si el iva del tempario lo tiene la orden, guardar
									$ivasSql = sprintf("INSERT INTO sa_det_orden_tot_iva (id_det_orden_tot, id_iva, iva) 
													VALUES (%s, %s, %s)",
											valTpDato($idDocumentoDetalle,"int"),
				...
				if ($valFormPaq['hddIdPedDetPaq'.$valorPaq] == "") {
					...
					while ($row = mysql_fetch_assoc($rs)) {
						...
						foreach($arrayIdIvaTempPaq as $indice => $idIvaTempPaq){
							$ivasSqlPaq = sprintf("INSERT INTO sa_det_orden_tempario_iva (id_det_orden_tempario, id_iva, iva) 
													VALUES (%s, %s, %s)",
					...
					if($guardarIva == "1" && $ivasExistentesOrden != ""){
						...
						while($rowDetRepPaquete = mysql_fetch_assoc($rsIvasRepPaquete)){
							...
							while($rowIvasPorRepuestoPaquete = mysql_fetch_assoc($rsIvasPorRepuestoPaquete)){
								...
								$ivasSqlPaq = sprintf("INSERT INTO sa_det_orden_articulo_iva (id_det_orden_articulo, id_iva, iva) 
														VALUES (%s, %s, %s)",
...
//INICIALIZA LA ORDEN CUANDO ES NUEVO, HABILITA LOS BOTONES, ESTABLECE USUARIO - FECHA
function nuevoDcto() {
	$objResponse = new xajaxResponse();
