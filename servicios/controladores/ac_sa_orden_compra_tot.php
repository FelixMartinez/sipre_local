...
function asignarTot($idOrdenTOT){
	$objResponse = new xajaxResponse();
	...
	$queryTot = sprintf("SELECT *, CONCAT_WS('-',lrif,rif) as lrif_rif FROM vw_sa_orden_tot WHERE id_orden_tot = %s", valTpDato($idOrdenTOT,"int"));
...
function cargarAFactura($idOrdenTOT,$valForm, $frmTotalFactura){
	...
	if($_GET['acc']==0){
		//FACTURA
		$objResponse->script("verVentana('reportes/sa_imprimir_registro_compra_pdf.php?valBusq=".$idOrdenTOT."',900,900);");
...
function comboRetencionISLR(){
	...
	while ($rowRetenciones = mysql_fetch_assoc($rsRetenciones)) {
		$selected = "";
		if($pais == 2){//si es panama, no obligar a que seleccione
...
function guardarOrdenCompra($idEmpresa,$idProveedor,$idOrden,$idUsuario,$formTrabajos){
	$objResponse = new xajaxResponse();
	...
	$query = sprintf("INSERT INTO sa_orden_tot (numero_tot, id_empresa, id_proveedor, id_orden_servicio, id_usuario, fecha_orden_tot) 
				VALUES (%s, %s, %s, %s, %s, %s);",
				valTpDato($numeroTot, "int"),
				valTpDato($idEmpresa, "int"),
	...
	for ($cont = 1; $cont <= strlen($formTrabajos['hddObj']); $cont++) {
		...
		if ($caracter != "|" && $caracter != ""){
			...
			$queryInsertarTrabajos = sprintf("INSERT INTO sa_orden_tot_detalle (id_orden_tot, descripcion_trabajo, id_precio_tot)
                                                  VALUES (%s, %s, %s)",
												  valTpDato($idOrdenTOT, "int"),
												  valTpDato(str_replace("<br>","\n",$formTrabajos["txtDescripcion".$cadena]), "text"),
												  valTpDato($formTrabajos["idAccesorio".$cadena], "int"));
