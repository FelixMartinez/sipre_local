...
function clienteImpuestoExento($idCliente){	
	$arrayImpuestoExento = array();
	$query = sprintf("SELECT id_impuesto FROM cj_cc_cliente_impuesto_exento WHERE id_cliente = %s",
				valTpDato($idCliente,"int"));
	$rs = mysql_query($query);
	if (!$rs) { return die(mysql_error()."\n\nLine: ".__LINE__."\n\nQuery: ".$query); }
	
	while($row = mysql_fetch_assoc($rs)){
		$arrayImpuestoExento[$row["id_impuesto"]] = $row["id_impuesto"];
	}
	return $arrayImpuestoExento;
}
...
function edicionImpuesto($nroItem, $idTipoOrden){//true si posee y false sino
	$query = sprintf("SELECT COUNT(*) AS permitir_editar
						FROM sa_tipo_orden
						INNER JOIN sa_filtro_orden ON sa_tipo_orden.id_filtro_orden = sa_filtro_orden.id_filtro_orden 
						WHERE id_tipo_orden = %s
						AND edicion_impuesto IN (%s) LIMIT 1",
				valTpDato($idTipoOrden,"int"),
				valTpDato($nroItem,"campo"));
	$rs = mysql_query($query);
	if (!$rs) { return die(mysql_error()."\n\nLine: ".__LINE__."\n\nQuery: ".$query); }
	
	$row = mysql_fetch_assoc($rs);
	if($row["permitir_editar"] == "0" ){
		return false;
	}else{
		return true;
	}
...
function ivaServicios(){
    ...
    $query = sprintf("SELECT idIva, iva, observacion, activo
                      FROM pg_iva 
                      WHERE tipo = 6 AND estado = 1 ");//AND activo = 1  es predeterminado
    $rs = mysql_query($query);
    if(!$rs) { return die(mysql_error()."\nLinea: ".__LINE__."\nArchivo: ".__FILE__); }
...
function configPaqueteCombo($idEmpresa){
	$query = sprintf("SELECT paquete_combo
				  FROM pg_empresa
				  WHERE id_empresa = %s",
                $idEmpresa);//no valido para que de error si se envia vacio!
    $rs = mysql_query($query);
    if(!$rs) { return die(mysql_error()."\nLinea: ".__LINE__."\nArchivo: ".__FILE__); }	
	$row = mysql_fetch_assoc($rs);
	
	return $row["paquete_combo"];
}
