...
function cargarLstPosibilidadCierre($idPosibilidadCierre = "", $idObjDestino = "", $idEmpresa = "") {
	...
	$query = sprintf("SELECT * FROM crm_posibilidad_cierre
		WHERE activo = %s AND id_empresa = %s AND fin_trafico = %s ORDER BY posicion_posibilidad_cierre ASC",
	valTpDato(1,"int"),
	valTpDato($idEmpresa, "int"),
	valTpDato(1,"int"));
	$rs = mysql_query($query);
...
function guardarSeguimientoActividad($idSeguimiento, $frmLstSeguimiento){
	...
	$query= sprintf("SELECT crm_seguimiento.id_seguimiento, id_cliente, id_empleado_creador,id_equipo,id_empleado_vendedor, observacion_seguimiento
			...
			$htmlTh .= "<td>";
				$htmlTh .= "<table width=\"100%\" border=\"0\">";
					$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
						...
						$htmlTh .= sprintf("<td width=\"%s\">%s Costo</td>","15%",cAbrevMoneda);
			...
			$htmlTb .= "<td>";
					$htmlTb .= "<table width=\"100%\" border=\"0\" class=\"divGris trResaltar4 \">";
						$htmlTb .= "<tr class=\"".$clase."\">";
							...
							$htmlTb .= "<td style=\"min-width:8%\">".date("d-m-Y	 h.i a", strtotime($row['fecha_registro']))."</td>";
							$htmlTb .= "<td style=\"min-width:15%\"  align=\"left\">".utf8_encode($row['item'])."</td>";
							$htmlTb .= "<td style=\"min-width:14%\" align=\"left\">".utf8_encode($row['nombre_cliente'])."</td>";
							$htmlTb .= "<td style=\"min-width:14%\"  align=\"left\">".str_replace('_',' ',$row['nom_uni_bas'])."</td>";
							$htmlTb .= sprintf("<td style=\"min-width:%s; %s\"  >%s</td>","10%",$colo,$row['disponible_unidad_fisica']);
							$htmlTb .= sprintf("<td id=\"tdActividadLstSelect%s\"  style=\"min-width:%s\" align=\"left\" rowspan=\"2\"></td>",$row['id_seguimiento'],"20%");
							$htmlTb .= sprintf("<td style=\"min-width:%s\" >%s</td>","15%",cAbrevMoneda." ".$row['precio_unidad_basica']);
							$htmlTb .= sprintf("<td style=\"min-width:%s\"  rowspan=\"2\" class=\"modalImg puntero\" id=\"aPosibleCierre\"  rel=\"#divFlotante6\" onclick=\"abrirFrom(this,'frmBusPosibleCierre','tdFlotanteTitulo6', %s, 'tblLstPosibleCierre')\">".
						...
						$htmlTb .= "<tr class=\"".$clase."\">";
							...
							$htmlTb .= "<td  align=\"left\">".utf8_encode($row['observacion_seguimiento'])."</td>";
							//$htmlTb .= "<td rowspan=\"2\"></td>";
						$htmlTb .= "</tr>";
					$htmlTb .= "</table>";
			$htmlTb .= "</td>";
...
function lstaHstPosibleCierre($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 9, $totalRows = NULL) {
	$objResponse = new xajaxResponse();
	...
	if ($valCadBusq[0] != "-1" && $valCadBusq[0] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("id_seguimiento = %s",
		valTpDato($valCadBusq[0], "int"));
	}
	
	$query = sprintf("SELECT  
		seg_cierre.id_seguimiento_cierre, 
		seg_cierre.id_seguimiento, 
		seg_cierre.id_posibilidad_cierre,
		posicion_posibilidad_cierre,
		posibilidad_cierre.nombre_posibilidad_cierre,
		seg_cierre.fecha_actualizacion
	FROM crm_seguimiento_cierre seg_cierre 
	INNER JOIN crm_posibilidad_cierre posibilidad_cierre ON seg_cierre.id_posibilidad_cierre = posibilidad_cierre.id_posibilidad_cierre %s",
	$sqlBusq);
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listaEmpresa", "10%", $pageNum, "id_posibilidad_cierre", $campOrd, $tpOrd, $valBusq, $maxRows, ("Id"));
		$htmlTh .= ordenarCampo("xajax_listaEmpresa", "65%", $pageNum, "nombre_posibilidad_cierre", $campOrd, $tpOrd, $valBusq, $maxRows, "Nombre Posibilidad de Cierre");
		$htmlTh .= ordenarCampo("xajax_listaEmpresa", "25%", $pageNum, "fecha_actualizacion", $campOrd, $tpOrd, $valBusq, $maxRows, ("Fecha"));
	$htmlTh .= "</tr>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td></td>";
			$htmlTb .= sprintf("<td align=\"center\">%s</td>",$contFila);
			$htmlTb .= sprintf("<td align=\"left\">%s</td>",$row['nombre_posibilidad_cierre']);
			$htmlTb .= sprintf("<td align=\"center\">%s</td>",date("d-m-Y	 h.i a", strtotime($row['fecha_actualizacion'])));
		$htmlTb .= "</tr>";
	}
	...
	$objResponse->assign("divHstPosibilidadCierre","innerHTML",$htmlTblIni.$htmlTf.$htmlTh.$htmlTb.$htmlTf.$htmlTblFin);
...
$xajax->register(XAJAX_FUNCTION,"lstaHstPosibleCierre");
