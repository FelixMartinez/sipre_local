...
$styleArrayTotales = array('fill' => array('type' => PHPExcel_Style_Fill::FILL_GRADIENT_LINEAR,'rotation' => 90,'startcolor' => array('argb' => 'FEF7E0'),'endcolor' => array('argb' => 'FFFFFFFF')),'font' => array('bold' => true,'underline'  =>  PHPExcel_Style_Font :: UNDERLINE_SINGLE));
		...
		$objPHPExcel->getActiveSheet()->getStyle('B5:K5')->applyFromArray($styleArrayTitulo);
		...
		$objPHPExcel->setActiveSheetIndex(0)
					...
					->setCellValue('F7', 'Sexo')
					->setCellValue('G7', 'CI /R.I.F')
					->setCellValue('H7', 'Telefono')
					->setCellValue('I7', 'Correo')
					->setCellValue('J7', 'Tipo de cliente')
		...
		$objPHPExcel->getActiveSheet()->getStyle('B7:K7')->applyFromArray($styleArrayHead);
		...
		$totalFilas = ($count + $totalRegistros);
			...
			$objPHPExcel->setActiveSheetIndex(0)
						...
						->setCellValue('E'.$count, trim($fechaNacimiento))
						->setCellValue('F'.$count, trim($rowlListCLient['sexo']))
						->setCellValue('G'.$count, trim($rowlListCLient['ci_cliente']))
						->setCellValue('H'.$count, trim($rowlListCLient['telf']))
						->setCellValue('I'.$count, trim($rowlListCLient['correo']))
						->setCellValue('J'.$count, trim($rowlListCLient['tipo']))
						->setCellValue('K'.$count, trim($modelo));
						
			/*$objPHPExcel->getActiveSheet()->mergeCells('B'.$count.':B'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('E'.$count.':I'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('J'.$count.':K'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('M'.$count.':N'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('P'.$count.':R'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('S'.$count.':T'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('U'.$count.':V'.$count);
			
			$objPHPExcel->getActiveSheet()->mergeCells('B'.$count.':B'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('C'.$count.':C'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('D'.$count.':D'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('E'.$count.':E'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('F'.$count.':F'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('G'.$count.':G'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('H'.$count.':H'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('I'.$count.':I'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('J'.$count.':J'.$count);
			$objPHPExcel->getActiveSheet()->mergeCells('K'.$count.':K'.$count);*/
			...
			(fmod($count, 2) == 0) ? "" : $objPHPExcel->getActiveSheet()->getStyle('B'.$count.':K'.$count)->applyFromArray($styleArrayRelleno);
			
			}	
		
			//COLO EL BORDER A LAS CELDAS
			$objPHPExcel->getActiveSheet()->getStyle('B7:K'.$totalFilas)->applyFromArray($styleArray);//INTERNO
			$objPHPExcel->getActiveSheet()->getStyle('B7:K'.$totalFilas)->applyFromArray($styleArray2);//EXTERNO
			...
			$objPHPExcel->getActiveSheet()->mergeCells('J'.($totalFilas +1).':K'.($totalFilas +1));
