...
function comboPosicion($pos){
	...
	$sqlUltimaPosicion = "SELECT IFNULL(MAX(posicion_estatus),0) as ultima_posicion FROM crm_estatus";
	$rsUltimaPosicion = mysql_query($sqlUltimaPosicion);
	$rowUltimaPosicion = mysql_fetch_array($rsUltimaPosicion);
	
	$html = "<select id='lstPosicion' name='lstPosicion'>
				<option value='-1'>[ Seleccione ]</option>";
                    
	for($i = 1; $i <= ($rowUltimaPosicion['ultima_posicion'] + 1); $i++){
		if ($pos == $i)
			$checked = " selected='selected'";
		else
			$checked = "";
			
		$html .= sprintf("<option value='%s' %s>%s</option>",$i,$checked,$i);
	}
	$html .= "</select>";
