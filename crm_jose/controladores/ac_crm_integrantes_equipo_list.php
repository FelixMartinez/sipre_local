...
function agregarIntegrante($fomrEquipo){
	...
	if(isset($fomrEquipo['checkItemIntegrante'])){
		...
		$objResponse->script("byId('checkItemIntegrante').click();");
	} else {
...
function comboxTipoEquipo($idObj,$tipo = ""){
	...
	$html = sprintf("<select id=\"%s\" name=\"%s\"%s>",$id,$name,$class);
...
function guadarFormEquipo($datosForm){
	...
	$objResponse->script("byId('butCerrarEquipo').click();");	$objResponse->loadCommands(listaEquipo(0,'',''));
...
function listaEquipo($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15, $totalRows = NULL){ 
	...
	$query = sprintf("SELECT id_equipo,nombre_equipo,tipo_equipo,descripcion_equipo,crm_equipo.id_empresa,crm_equipo.id_empleado,
		pg_empresa.id_empresa, nombre_empresa, jefe_equipo,
		pg_empleado.id_empleado,CONCAT_WS(' ',nombre_empleado, apellido) AS nombre_jefe_equipo,crm_equipo.activo
			FROM crm_equipo
		LEFT JOIN pg_empresa ON pg_empresa.id_empresa = crm_equipo.id_empresa
		LEFT JOIN pg_empleado ON pg_empleado.id_empleado = crm_equipo.jefe_equipo %s", 
	$sqlBusq);
...
//$objResponse->alert($queryLimit);
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listaEquipo", "", $pageNum, "id_equipo", $campOrd, $tpOrd, $valBusq, $maxRows, "Id");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) { //AQUI ESPESIFICO EL ESTILO Y EL COLOR AL MOVER EL MOUSE SOBRE LOS REGISTRO
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" onmouseover=\"this.className='trSobre';\" onmouseout=\"this.className='".$clase."';\" height=\"24\">";
			...
			$htmlTb .= "<td>".$row['id_equipo']."</td>";	
			$htmlTb .= "<td>".utf8_encode($row['nombre_empresa'])."</td>";
			$htmlTb .= "<td>".utf8_encode($row['nombre_equipo'])."</td>";
			$htmlTb .= "<td>".$row['tipo_equipo']."</td>";
			$htmlTb .= "<td>".utf8_encode($row['descripcion_equipo'])."</td>";
			$htmlTb .= "<td>".utf8_encode($row['nombre_jefe_equipo'])."</td>";
			$htmlTb .= "<td>";
			$htmlTb .= sprintf("<a class=\"modalImg\" id=\"aEditar%s\" rel=\"#divFlotante\" onclick=\"abrirNuevo(%s); openImg(this); ;\"><img class=\"puntero\" src=\"../img/iconos/ico_edit.png\" title=\"Editar equipo\"/></a>", //EDITAR EQUIPO
...
//$objResponse->alert($queryLimit);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		if($valCadBusq[3] == "asignarJefeEquipo"){
