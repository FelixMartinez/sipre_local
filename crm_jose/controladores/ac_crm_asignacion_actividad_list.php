...
function cargarDatos($fromTipoEquipo,$idIntegrante,$horaAsignacion,$idActividaEjecucion){
	...
	$objResponse->assign("textNombreCliente","value", $rowIntegreante['nombre_apellido_cliente']);
	$objResponse->assign("comboxEstadoActAgenda","value", $rowIntegreante['tipo_finalizacion']);
	$objResponse->assign("textNotaCliente","value", $rowIntegreante['notas']);
...
function cargarLstActivida($tipoEquipo, $idObjDestino, $idActividad = "", $option ="Seleccione"){
	$objResponse = new xajaxResponse();
		...
		if($idObjDestino == "tdListTipoActividaGrafico" || $idObjDestino == "tdListTipoActividaGrafico2"){
			$sqlActividad = sprintf("SELECT  crm_actividades_ejecucion.id_actividad, nombre_actividad,posicion_actividad
				FROM crm_actividades_ejecucion
					LEFT JOIN crm_actividad ON crm_actividad.id_actividad= crm_actividades_ejecucion.id_actividad
				WHERE activo = %s AND tipo = %s AND crm_actividades_ejecucion.id_empresa = %s
					GROUP BY crm_actividades_ejecucion.id_actividad",
			valTpDato(1,"int"),
			valTpDato($tipoEquipo,"text"),
			valTpDato($_SESSION['idEmpresaUsuarioSysGts'],"int"));
		} else {
			$sqlActividad = sprintf("SELECT * FROM crm_actividad 
				WHERE activo = %s AND tipo = %s AND id_empresa = %s
				ORDER BY posicion_actividad",
			valTpDato(1,"int"),
			valTpDato($tipoEquipo,"text"),
			valTpDato($_SESSION['idEmpresaUsuarioSysGts'],"int"));	
		}
		...
		$queryActividad = mysql_query($sqlActividad);
		if (!$queryActividad) return $objResponse->alert("Error ".mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);

		$html = "<select id=\"listActividad\" name=\"listActividad\" class='inputHabilitado'>";
			$html .= sprintf("<option value=\"\">[ %s ]</option>",$option);
			while($rowAct = mysql_fetch_array($queryActividad)){
				$selected = ($idActividad == $rowAct['id_actividad']) ? "selected=\"selected\"" : "";
				$html.= sprintf("<option value=\"%s\" %s >%s. %s</option>",$rowAct['id_actividad'],$selected,$rowAct['posicion_actividad'],$rowAct['nombre_actividad']);
			}
		$html.="</select>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\" onmouseover=\"this.className='trSobre';\" onmouseout=\"this.className='".$clase."';\" height=\"22\">";
			$htmlTb .= sprintf("<td align=\"left\">%s",$row['nombre_equipo']);
				$htmlTb .=sprintf("<img id=\"imgVerActividades%s\" align=\"right\" class=\"puntero\" title=\"Ver Detalle\" src=\"../img/crm/show_crm.gif\" onclick=\"mostrarOcultatActividad(%s)\" name=\"imgVerActividades\">",$row['id_equipo'],$row['id_equipo']);
				$htmlTb .=sprintf("<img id=\"imgOcutarActividades%s\" style=\"display:none\" align=\"right\" class=\"puntero\" title=\"Ver Detalle\" src=\"../img/crm/hide_crm.gif\" onclick=\"mostrarOcultatActividad(%s)\" name=\"imgOcutarActividades\">",$row['id_equipo'],$row['id_equipo']);
			$htmlTb .="</td>";
		$htmlTb .= "</tr>";
		$htmlTb .= "<tr>";
...
//$objResponse->alert($queryLimit);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" onmouseover=\"this.className='trSobre';\" onmouseout=\"this.className='".$clase."';\" height=\"24\">";
			$htmlTb .= sprintf("<td>"."<button  type=\"button\" onclick=\"xajax_asignarCliente(%s);\" title=\"Seleccionar\"><img src=\"../img/iconos/ico_aceptar.gif\"/></button>"."</td>",$row['id']); 
...
//$objResponse->alert($query);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= sprintf("<tr class=\"%s\" onmouseover=\"this.className='trSobre';\" onmouseout=\"this.className='%s';\" height=\"22\">",$clase,$clase);
			...
			$htmlTb .= sprintf("<td align=\"left\"><button type=\"button\" id=\"butMostraVehiculo%s\" name=\"butMostraVehiculo\" class=\"close\" onclick=\"mostrarOcultatVehiculo(%s)\" title=\"Ver Vehículos del Cliente\"><img src=\"../img/crm/show_crm.gif\" align=\"right\" title=\"Ver\" class=\"puntero\"  id=\"imgMostrarCliente\"/></button><button type=\"button\" id=\"butOcultarVehiculo%s\" name=\"butOcultarVehiculo\" class=\"close\" style=\"display:none\" onclick=\"mostrarOcultatVehiculo(%s)\" title=\"Ocultar Vehículos del Cliente\"><img src=\"../img/crm/hide_crm.gif\" align=\"right\" title=\"Oclutar\" class=\"puntero\" id=\"imgOcultarCliente\"/></button></td>",$row['id_cliente_contacto'],$row['id_cliente_contacto'],$row['id_cliente_contacto'],$row['id_cliente_contacto']);
