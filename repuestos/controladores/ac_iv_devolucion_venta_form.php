...
function asignarArticulo($hddNumeroArt, $frmListaArticulo) {
	...
	$queryFactDet = sprintf("SELECT * FROM cj_cc_factura_detalle
	...
	$objResponse->assign("txtCantidadArt","value",($rowFactDet['cantidad'] - $rowFactDet['devuelto']));
...
function guardarDcto($frmDcto, $frmListaArticulo, $frmTotalDcto, $bloquearForm = "") {
	...
	if (isset($arrayObjIva)) {
		foreach ($arrayObjIva as $indice => $valor) {
			if ($frmTotalDcto["hddLujoIva".$valor] == 1) {
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			if ($frmListaArticulo['hddIdFactDet'.$valor] > 0) {
				$queryFactDet = sprintf("SELECT * FROM cj_cc_factura_detalle WHERE id_factura_detalle = %s;",
				...
				$cantPedida = str_replace(",", "", $frmListaArticulo['txtCantFactItm'.$valor]);
				...
				VALUE (%s, %s, %s, %s, %s, %s, %s, %s);",
					...
					valTpDato($cantPendienteNotaCred, "real_inglesa"),
				...
				$objResponse->assign("hddIdOrdDet".$valor,"value",$$idNotaCreditoDetalle);
			}
		}
	}
	...
	WHERE id_pedido_venta = %s",
		valTpDato(4, "int"), // 0 = Pendiente, 1 = Pedido, 2 = Orden, 3 = Facturado, 4 = Devuelto
