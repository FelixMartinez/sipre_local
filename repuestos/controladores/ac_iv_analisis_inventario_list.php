...
function listaAnalisisInventario($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 50, $totalRows = NULL) {
	...
	$sqlBusq = "";
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("id_analisis_inventario = %s",
		valTpDato($idAnalisisInv, "int"));
	
	if ($valCadBusq[2] != "-1" && $valCadBusq[2] != ""
	&& ($valCadBusq[3] == "-1" || $valCadBusq[3] == "")) {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("cantidad_existencia > 0");
	}
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaAnalisisInventario", "6%", $pageNum, "meses_existencia", $campOrd, $tpOrd, $valBusq, $maxRows, "Meses Existencia");
		$htmlTh .= ordenarCampo("xajax_listaAnalisisInventario", "6%", $pageNum, "promedio_diario", $campOrd, $tpOrd, $valBusq, $maxRows, "Prom. Día");
		$htmlTh .= ordenarCampo("xajax_listaAnalisisInventario", "6%", $pageNum, "promedio_mensual", $campOrd, $tpOrd, $valBusq, $maxRows, "Venta Mensual");
		$htmlTh .= ordenarCampo("xajax_listaAnalisisInventario", "4%", $pageNum, "inventario_recomendado", $campOrd, $tpOrd, $valBusq, $maxRows, "Inv. Recom.");
		$htmlTh .= ordenarCampo("xajax_listaAnalisisInventario", "4%", $pageNum, "sobre_stock", $campOrd, $tpOrd, $valBusq, $maxRows, "Sobre Stock");
		$htmlTh .= ordenarCampo("xajax_listaAnalisisInventario", "4%", $pageNum, "sugerido", $campOrd, $tpOrd, $valBusq, $maxRows, "Suger.");
	...
	while($rowMovDet = mysql_fetch_array($rsTipoMov)){
		...
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq2 = $cond.sprintf("id_analisis_inventario = %s
