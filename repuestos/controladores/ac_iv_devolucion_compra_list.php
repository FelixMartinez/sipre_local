...
function formNotaCredito($idFacturaCompra) {
	...
	$objResponse->assign("txtFechaProveedorFactura","value",implode("-", array_reverse(explode("-", $rowFactura['fecha_factura_proveedor']))));
...
function guardarDcto($frmDcto, $frmListaFacturaCompra) {
	...
	$insertSQL = sprintf("INSERT INTO cp_notacredito (id_empresa, id_proveedor, saldo_notacredito, fecha_notacredito, observacion_notacredito, estado_notacredito, id_documento, tipo_documento, subtotal_notacredito, subtotal_descuento, id_departamento_notacredito, monto_exonerado_notacredito, monto_exento_notacredito, aplica_libros_notacredito, numero_nota_credito, numero_control_notacredito, fecha_registro_notacredito, id_motivo, id_empleado_creador)
	SELECT 
		id_empresa,
		id_proveedor,
		%s,
	...
	$insertSQL = sprintf("INSERT INTO iv_kardex (id_modulo, id_documento, id_articulo, id_casilla, tipo_movimiento, id_clave_movimiento, tipo_documento_movimiento, cantidad, precio, costo, costo_cargo, costo_diferencia, porcentaje_descuento, subtotal_descuento, estado, fecha_movimiento, observacion, hora_movimiento)
	SELECT 
		kardex.id_modulo,
		%s,
		kardex.id_articulo,
	...
	while ($row = mysql_fetch_assoc($rs)) {
		...
		// BUSCA EN LA MISMA CASILLA DE LA COMPRA
		...
		if (!(doubleval($rowArtEmp['cantidad_disponible_logica'] - $cantPedida) >= 0)) {
			...
			$idCasilla = ($totalRowsArtEmp > 0 && $rowArtEmp['id_casilla_predeterminada_compra'] > 0) ? $rowArtEmp['id_casilla_predeterminada_compra'] : $idCasilla;
			
			// BUSCA EN LA CASILLA PREDETERMINADA
			$queryArtEmp = sprintf("SELECT * FROM vw_iv_articulos_empresa_ubicacion
			WHERE id_articulo = %s
				AND id_casilla = %s;",
				valTpDato($idArticulo, "int"),
