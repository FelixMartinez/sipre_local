...
function eliminarCasilla($idCasilla) {
	...
	if ($existencia > 0) {
		...
		$queryTramo = "SELECT id_tramo FROM iv_casillas WHERE id_casilla = '".$idCasilla."'";
		$rsTramo = mysql_query($queryTramo);
		if (!$rsTramo) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
		$rowTramo = mysql_fetch_array($rsTramo);
		
		$query = "DELETE FROM iv_casillas WHERE iv_casillas.id_casilla = '".$idCasilla."'";
		$rs = mysql_query($query);
		if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
...
function eliminarTramo($idTramo) {
	...
	if ($existencia > 0) {
		...
		$query = "DELETE FROM iv_tramos WHERE iv_tramos.id_tramo = '".$idTramo."'";
		$rs = mysql_query($query);
		if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
