...
function accionBloque($frmListaSolicitudDetalle, $accionDeshacer = false) {
	...
	if (isset($arrayObjItm)) {
		foreach ($arrayObjItm as $indice => $valor) {
			$idSolicitudDetalle = $valor;
			if (isset($arrayObj)) {
				foreach ($arrayObj as $indice2 => $valor2) {
					if ($frmListaSolicitudDetalle['hddIdSolicitudDetalle'.$valor2] == $idSolicitudDetalle) {
						$valor = $valor2;
					}
				}
			}
			...
			switch ($idAccionSolicitud) {
				case 1 : // 2 = DESAPROBAR
					if ($accionDeshacer == false && $frmListaSolicitudDetalle['hddSolicitudDetalleAccion'.$valor] != 1) {
						 $objResponse->loadCommands(aprobarRepuesto($valor, $frmListaSolicitudDetalle, false));
					} else if ($accionDeshacer == true && $frmListaSolicitudDetalle['hddSolicitudDetalleAccion'.$valor] == 1) {
						 $objResponse->loadCommands(aprobarRepuesto($valor, $frmListaSolicitudDetalle, false));
					}
					break;
				case 2 : // 2 = APROBAR
					if ($accionDeshacer == false && $frmListaSolicitudDetalle['hddSolicitudDetalleAccion'.$valor] != 2) {
						 $objResponse->loadCommands(aprobarRepuesto($valor, $frmListaSolicitudDetalle, false));
					} else if ($accionDeshacer == true && $frmListaSolicitudDetalle['hddSolicitudDetalleAccion'.$valor] == 2) {
						 $objResponse->loadCommands(aprobarRepuesto($valor, $frmListaSolicitudDetalle, false));
					}
					break;
				case 3 : // 3 = DESPACHAR
					if ($accionDeshacer == false) {
						$objResponse->loadCommands(despacharRepuesto($valor, $frmListaSolicitudDetalle, false));
					} else if ($accionDeshacer == true && $frmListaSolicitudDetalle['hddSolicitudDetalleAccion'.$valor] == 3) {
						$objResponse->loadCommands(aprobarRepuesto($valor, $frmListaSolicitudDetalle, false));
					}
					break;
				case 4 : // 4 = DEVOLVER
					if ($accionDeshacer == false) {
						$objResponse->loadCommands(devolverRepuesto($valor, $frmListaSolicitudDetalle, false));
					} else if ($accionDeshacer == true && $frmListaSolicitudDetalle['hddSolicitudDetalleAccion'.$valor] == 4) {
						$objResponse->loadCommands(despacharRepuesto($valor, $frmListaSolicitudDetalle, false));
					}
					break;
				case 6 : // 6 = ANULAR
					if ($accionDeshacer == false) {
						$objResponse->loadCommands(anularRepuesto($valor, $frmListaSolicitudDetalle, false));
					} else if ($accionDeshacer == true && $frmListaSolicitudDetalle['hddSolicitudDetalleAccion'.$valor] == 6) {
						$objResponse->loadCommands(aprobarRepuesto($valor, $frmListaSolicitudDetalle, false));
					}
					break;
			}
...
function formSolicitudRepuesto($idSolicitud, $idAccionSolicitud) {
	...
	$objResponse->assign("txtFechaSolicitud","value",date("d-m-Y",strtotime($rowSolicitud['tiempo_solicitud'])));
	...
	if ((strlen($idAccionSolicitud) > 0 && !($rowSolicitud['id_empleado_entrega'] > 0))) {
		...
		$idEmpleadoEntrega = $rowUsuario['id_empleado'];
	...
	$objResponse->assign("txtFechaOrden","value",date("d-m-Y",strtotime($rowSolicitud['tiempo_orden'])));
...
function guardarSolicitud($frmSolicitud, $frmListaSolicitudDetalle, $frmListaSolicitud) {
	...
	if ($hddIdAccionRealizada == 1) { // 1 = SOLICITADO
		...
		$insertSQL = sprintf("INSERT INTO iv_kardex_surtido (id_solicitud, id_articulo, id_casilla, cantidad, fecha_movimiento, id_estado_solicitud, id_empleado_despacha, id_empleado_recibe) 
		SELECT
...
function listaComprobanteSurtido($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 100, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".date("d-m-Y h:i:sa", strtotime($row['fecha_movimiento']))."</td>";
