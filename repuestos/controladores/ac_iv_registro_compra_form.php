...
function aprobarDcto($frmDcto, $frmListaArticulo, $frmTotalDcto) {
	...
	$fechaVencimiento = ($rowProv['credito'] == "Si") ? suma_fechas("d-m-Y",$txtFechaProveedor,$rowProv['diascredito']) : $txtFechaProveedor;
	...
	VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
		...
		valTpDato(implode("-",array_reverse(explode("-",$txtFechaProveedor))), "date"),
		valTpDato($frmDcto['txtIdProv'], "int"),
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			...
			if ($idModoCompra == 1) { // 1 = Nacional
				...
				// REGISTRA EL DETALLE DE LA FACTURA
				$insertSQL = sprintf("INSERT INTO cp_factura_detalle_importacion (id_factura_detalle, id_arancel_familia, costo_unitario, gasto_unitario, costo_unitario_cambio, gasto_unitario_cambio, gastos_import_nac_unitario, gastos_import_unitario, porcentaje_grupo)
				VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s);",
					valTpDato($hddIdFacturaDetalleItm, "int"),
			...
			$insertSQL = sprintf("INSERT INTO iv_articulos_costos (id_empresa, id_proveedor, id_articulo, fecha, costo, fecha_registro)
			VALUE (%s, %s, %s, %s, %s, %s);",
				valTpDato($idEmpresa, "int"),
				valTpDato($frmDcto['txtIdProv'], "int"),
				valTpDato($idArticulo, "int"),
			...
			$arrayEtiqueta[] = implode(",", array($idArticulo, $idCasilla, $cantRecibida));
		}
	}
		...
		foreach ($arrayObjGasto as $indice => $valor) {
			...
			if (round($txtMontoGasto, 2) != 0) {
				...
				SELECT %s, id_gasto, %s, %s, %s, %s, id_modo_gasto, %s, afecta_documento FROM pg_gastos WHERE id_gasto = %s;",
					valTpDato($idFactura, "int"),
					valTpDato($frmTotalDcto['hddTipoGasto'.$valor], "int"), // 0 = Porcentaje, 1 = Monto Fijo
	...
	switch ($idModoCompra) {
		...
		case 2 : // 2 = Importacion
			...
			if ($rowMonedaLocal['incluir_impuestos'] == 1 && $frmDcto['lstNacionalizar'] == 1) { // 0 = No, 1 = Si
				...
				$queryIva = sprintf("SELECT iva.*, IF (iva.tipo IN (3,2), 1, NULL) AS lujo FROM pg_iva iva WHERE iva.tipo = 1 AND iva.estado = 1 AND iva.activo = 1 ORDER BY iva;");
...
function cargaLstIva($nombreObjeto, $selId = "", $selVal = "") {
	...
	$queryIva = sprintf("SELECT iva.*, IF (iva.tipo IN (3,2), 1, NULL) AS lujo FROM pg_iva iva WHERE iva.estado = 1 AND iva.tipo IN (1,3)
	ORDER BY iva;");
	$rsIva = mysql_query($queryIva);
	if (!$rsIva) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	...
	while ($rowIva = mysql_fetch_assoc($rsIva)) {
		$html .= "<optgroup label=\"".utf8_encode($rowIva['observacion'])."\">";
			...
			if (in_array($rowIva['iva'],explode(",",$selVal)) && in_array($rowIva['idIva'],explode(",",$selId))) {
				...
				$selected = "selected=\"selected\"";
			} else if ($selId == "" && $rowIva['tipo'] == 1 && $rowIva['activo'] == 1) { // IMPUESTO PREDETERMINADO
				$selected = "selected=\"selected\"";
			}
...
function cargarDcto($idFacturaCompra, $frmListaArticulo, $frmTotalDcto) {
	...
	if ($idFacturaCompra > 0) {
		...
		$queryFacturaDetalle = sprintf("SELECT * FROM iv_factura_compra_gasto fact_comp_gasto
		WHERE id_factura_compra = %s
			AND id_modo_gasto IN (1,3)
			...
			foreach ($arrayObj as $indice => $valor) {
				...
				if ($rowFacturaDetalle['id_factura_compra_detalle'] == $hddIdFacturaDetItm) {
					...
					WHERE id_factura_compra_detalle = %s;",
						...
						valTpDato($txtPesoItm, "real_inglesa"),
						valTpDato($frmListaArticulo['hddTipoDescuentoItm'.$valor], "boolean"),
						...
						valTpDato($frmListaArticulo['hddTipoItm'.$valor], "int"),
						valTpDato($frmListaArticulo['hddIdClienteItm'.$valor], "int"),
						valTpDato($estatusDet, "boolean"), // 0 = En Espera, 1 = Recibido
						valTpDato($cantRecibida, "int"),
		...
		if (isset($arrayObjGasto)) {
			foreach ($arrayObjGasto as $indice => $valor) {
				...
				if (round($txtMontoGasto, 2) != 0 && $rowFacturaGasto['id_factura_compra_gasto'] == $hddIdFacturaCompraGasto) {
					...
					$updateSQL = sprintf("UPDATE iv_factura_compra_gasto fact_comp_gasto, pg_gastos gasto SET
		...
		foreach ($arrayFilaImportar as $indice => $valor) {
			...
			if (isset($arrayObj)) {
				foreach ($arrayObj as $indice2 => $valor2) {
					if ($frmListaArticulo['hddIdPedidoDetItm'.$valor2] == $idPedidoDetalle
					&& str_replace(",", "", $frmListaArticulo['txtCostoItm'.$valor2]) == $arrayFilaImportar[$indice][6]) {
						$existe = true;
					}
				}
			}
			...
			if ($existe == false) {
				if (count($arrayObj) < $rowConfig6['valor']) {
					...
					$Result1 = insertarItemArticulo($objResponse, $arrayObj, $arrayObjUbicacion, $frmDcto, $contFila, $idPedidoDetalle, "", $idMonedaLocal, $idMonedaOrigen, $cantPedida, $cantRecibida, $costoUnitario, $almacen, $ubicacion, $idClienteArt);
					$arrayObjUbicacion = $Result1[3];
					if ($Result1[0] != true && strlen($Result1[1]) > 0) {
						return $objResponse->alert($Result1[1]);
					} else if ($Result1[0] == true) {
...
function insertarArticulo($idPedidoDetalle, $frmDcto, $frmListaArticulo, $frmTotalDcto) {
	...
	if ($existe == false) {
		if (count($arrayObj) < $rowConfig6['valor']) {
			$Result1 = insertarItemArticulo($objResponse, $arrayObj, $arrayObjUbicacion, $frmDcto, $contFila, $idPedidoDetalle, "", $idMonedaLocal, $idMonedaOrigen);
			$arrayObjUbicacion = $Result1[3];
			if ($Result1[0] != true && strlen($Result1[1]) > 0) {
				return $objResponse->alert($Result1[1]);
			} else if ($Result1[0] == true) {
				...
				if ($frmArticuloMultiple['txtCantEntregadaArtMult'.$cont] > 0) {
					...
					if ($existe == false) {
						if (count($arrayObj) + $cantDetAgregados < $rowConfig6['valor']) {
							$Result1 = insertarItemArticulo($objResponse, $arrayObj, $arrayObjUbicacion, $frmDcto, $contFila, $idPedidoDetalle, "", $idMonedaLocal, $idMonedaOrigen, $frmArticuloMultiple['txtCantArtMult'.$cont], $frmArticuloMultiple['txtCantEntregadaArtMult'.$cont], $frmArticuloMultiple['txtCostoArtMult'.$cont]);
							$arrayObjUbicacion = $Result1[3];
							if ($Result1[0] != true && strlen($Result1[1]) > 0) {
								return $objResponse->alert($Result1[1]);
							} else if ($Result1[0] == true) {
...
function insertarItemArticulo($objResponse, $arrayObj, $arrayObjUbicacion, $frmDcto, $contFila, $idPedidoDetalle, $idFacturaDetalle, $idMonedaLocal, $idMonedaOrigen, $cantPedida = "", $cantRecibida = "", $costoUnitario = "", $almacen = "", $ubicacion = "", $hddIdClienteItm = "") {
	$contFila++;
	...
	if ($idPedidoDetalle > 0) {
		...
		$queryPedidoDet = sprintf("SELECT
			...
			fact_comp_det.tipo_descuento,
			fact_comp_det.porcentaje_descuento,
			fact_comp_det.subtotal_descuento,
...
function insertarItemGasto($contFila, $hddIdGasto, $hddIdFacturaCompraGasto = "") {
	...
	if ($hddIdFacturaCompraGasto > 0) {
		...
		FROM pg_iva iva
			...
			INNER JOIN iv_factura_compra_gasto fact_comp_gasto ON (gasto.id_gasto = fact_comp_gasto.id_gasto)
...
function insertarItemOtroCargo($contFila, $hddIdGastoCargo = "", $idFacturaCompraGasto = "") {
		...
		$query = sprintf("SELECT
			...
			WHERE expediente_det_cargo.id_gasto = gasto.id_gasto
				AND expediente_det_cargo.id_expediente = (SELECT exp_det_fact.id_expediente FROM iv_expediente_detalle_factura exp_det_fact
															WHERE exp_det_fact.id_factura_compra = fact_comp_gasto.id_factura_compra)
	...
	$txtFechaFacturaGasto = ($txtFechaFacturaGasto == "" && $totalRows > 0 && $row['fecha_origen'] != "") ? date("d-m-Y", strtotime($row['fecha_origen'])) : $txtFechaFacturaGasto;
