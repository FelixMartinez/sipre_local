...
function cargarDcto($idPedido, $frmListaArticulo, $frmTotalDcto) {
	...
	$queryPedidoDet = sprintf("SELECT
		...
		vw_iv_art_datos_bas.vw_iv_art_datos_bas.descripcion,
		unidad
	...
	while ($rowPedidoDet = mysql_fetch_assoc($rsPedidoDet)) {
		...
		$objResponse->script(sprintf("$('#trItmPie').before('".
			"<tr id=\"trItm:%s\" align=\"left\" height=\"24\">".
				...
				"<td><input type=\"text\" id=\"txtCostoItm%s\" name=\"txtCostoItm%s\" class=\"inputSinFondo\" readonly=\"readonly\" value=\"%s\"/></td>".
				...
				"<td><input type=\"text\" id=\"txtTotalItm%s\" name=\"txtTotalItm%s\" class=\"inputSinFondo\" readonly=\"readonly\" value=\"%s\"/>".
					"<input type=\"hidden\" id=\"hddIdPedidoDetItm%s\" name=\"hddIdPedidoDetItm%s\" readonly=\"readonly\" value=\"%s\"/>".
			...
			$contFila,
				...
				$contFila, $contFila,
				$imgCliente,
				$contFila, $contFila, elimCaracter(utf8_encode($rowPedidoDet['codigo_articulo']),";"),
				$contFila, preg_replace("/[\"?]/","''",preg_replace("/[\r?|\n?]/","<br>",utf8_encode($rowPedidoDet['descripcion']))), $arancelArticulo,
				$contFila, $contFila, number_format($rowPedidoDet['cantidad'], 2, ".", ","),
				$contFila, $contFila, number_format($rowPedidoDet['precio_unitario'], 2, ".", ","),
				$contFila, $ivaUnidad,
				$contFila, $contFila, number_format(($rowPedidoDet['cantidad'] * $rowPedidoDet['precio_unitario']), 2, ".", ","),
					$contFila, $contFila, $idPedidoDetalle,
					$contFila, $contFila, $idArticulo,
					$contFila, $contFila, 0,
					$contFila, $contFila, 0,
					$contFila, $contFila, $rowPedidoDet['tipo'],
		...
		if ($rowPedidoDet['id_cliente'] > 0) {
			...
			byId('aClienteItem:%s').onclick = function() { abrirDivFlotante1(this, 'tblCliente', '%s'); }",
				$contFila, $rowPedidoDet['id_cliente']));
		}
	...
	$objResponse->assign("txtFechaOrdenCompra","value",date("d-m-Y"));
	...
	$objResponse->assign("txtFechaPreparado","value",date("d-m-Y", strtotime($rowPedido['fecha'])));
