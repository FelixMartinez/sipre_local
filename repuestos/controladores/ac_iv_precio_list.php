...
function listaPrecio($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("porcentaje NOT IN (0)");
	...
	if ($valCadBusq[2] != "-1" && $valCadBusq[2] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("descripcion_precio LIKE %s",
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listadoClavesUsuarios", "34%", $pageNum, "descripcion_precio", $campOrd, $tpOrd, $valBusq, $maxRows, "Descripción");
		$htmlTh .= ordenarCampo("xajax_listadoClavesUsuarios", "12%", $pageNum, "porcentaje", $campOrd, $tpOrd, $valBusq, $maxRows, "Porcentaje");
		...
		$htmlTh .= "<td width=\"10%\">Actualizar con Costo</td>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			if ($row['id_precio'] != 6 && $row['id_precio'] != 7) {
