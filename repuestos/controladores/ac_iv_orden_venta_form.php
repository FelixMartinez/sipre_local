...
function asignarPrecio($idArticulo, $idPrecio, $frmDcto, $precioPredet = "false", $precioUnitario = "") {
	...
	// BUSCA EL PRECIO
	$queryArtPrecio = sprintf("SELECT art_precio.*,
	...
	$precioUnitario = ($rowArtPrecio['precio'] > 0 && $precioUnitario == "" && !in_array($rowArtPrecio['id_precio'], array(6,7,12,13))) ? $rowArtPrecio['precio'] : $precioUnitario;
...
function cargarDcto($idDocumento, $frmTotalDcto) {
	...
	if ($rowPedido['id_pedido_venta'] > 0) {
		...
		// DATOS DE LA ORDEN
		$objResponse->assign("txtFechaAprobacion","value",date("d-m-Y"));
		...
		$objResponse->assign("txtFechaAprobado","value",date("d-m-Y"));
...
function guardarDcto($frmDcto, $frmListaArticulo, $frmTotalDcto, $bloquearForm = "false") {
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			...
			$cantPedida = $frmListaArticulo['hddCantItm'.$valor];
			$cantPendiente = $frmListaArticulo['hddCantItm'.$valor];
			$precioUnitario = $frmListaArticulo['hddPrecioItm'.$valor];
...
function insertarArticulo($frmDatosArticulo, $frmDcto, $frmListaArticulo, $frmTotalDcto) {
	...
	if ($frmListaArticulo['hddIdCasilla'.$hddNumeroArt] == $idCasilla) {
		if (($cantPedida - $hddCantItm) <= doubleval($rowArtEmp['cantidad_disponible_logica']) && doubleval($rowArtEmp['cantidad_disponible_logica']) >= 0) {
			$permitir = true;
		}
	...
	if ($permitir == true) {
		...
		while ($cantFaltante != 0 || ($hddNumeroArt > 0 && $precioUnitario != $hddPrecioItm)) {
			...
			while ($rowArtCosto = mysql_fetch_assoc($rsArtCosto)) {
				...
				$loteSinExistencia = ($rowArtEmp['cantidad_disponible_logica'] == $cantFaltante) ? true : false;
				$cantPedida = ($rowArtEmp['cantidad_disponible_logica'] > $cantFaltante) ? $cantFaltante : $rowArtEmp['cantidad_disponible_logica'];
...
function errorDesaprobarDcto($objResponse) {
	$objResponse->script("
	byId('btnGuardarDesaprobarDcto').disabled = false;
	byId('btnCancelarDesaprobarDcto').disabled = false;");
}

function errorInsertarArticulo($objResponse) {
	$objResponse->script("
	byId('btnGuardarArticulo').disabled = false;
	byId('btnCancelarArticulo').disabled = false;");
}

function errorGuardarDcto($objResponse) {
	$objResponse->script("
	byId('btnGuardar').disabled = false;
	byId('aDesaprobar').style.display = '';
