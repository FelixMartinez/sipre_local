...
function buscarMovInterAlmacen($frmBuscar) {
	...
	$objResponse->loadCommands(listaMovInterAlmacen(0, "CONCAT(descripcion_almacen, ubicacion)", "ASC", $valBusq));
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			...
			if ($idArticulo > 0 && $idCasilla > 0 && $idCasillaDestino > 0) {
				...
				$queryAlmacenOrigen = sprintf("SELECT * FROM vw_iv_articulos_empresa_ubicacion
				WHERE id_articulo = %s
					AND id_casilla = %s
					AND (estatus_articulo_almacen = 1 OR (estatus_articulo_almacen IS NULL AND cantidad_disponible_logica > 0));",
				...
				if (($rowArticulo || !$rowArticulo) && $cantTransferir <= $rowAlmacenOrigen['cantidad_disponible_logica']) {
					if (($idArticulo == $rowArticulo['id_articulo'] && $idCasillaDestino != $idCasilla)
					|| !$rowArticulo) {
...
function formMovimientoInterAlmacen($idArticulo, $idCasilla, $frmBuscar) {
	$objResponse = new xajaxResponse();
	...
	if ($totalRowsInvFis == 0) {
		$queryArtEmpUbic = sprintf("SELECT
			art.id_articulo,
			art.codigo_articulo,
			art.descripcion,
		...
		FROM vw_iv_articulos_empresa_ubicacion vw_iv_art_emp_ubic
			INNER JOIN iv_articulos art ON (vw_iv_art_emp_ubic.id_articulo = art.id_articulo)
		WHERE art.id_articulo = %s
			AND vw_iv_art_emp_ubic.id_casilla = %s
			AND (vw_iv_art_emp_ubic.estatus_articulo_almacen = 1
				OR (vw_iv_art_emp_ubic.estatus_articulo_almacen IS NULL AND vw_iv_art_emp_ubic.cantidad_disponible_logica > 0));",
			valTpDato($idArticulo, "int"),
			valTpDato($idCasilla, "int"));
		$rsArtEmpUbic = mysql_query($queryArtEmpUbic);
		if (!$rsArtEmpUbic) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
		$rowArtEmpUbic = mysql_fetch_assoc($rsArtEmpUbic);
		
		$objResponse->assign("hddIdCasilla","value",$idCasilla);
		$objResponse->assign("hddIdArticulo","value",$idArticulo);
		$objResponse->assign("txtCodigoArt","value",elimCaracter(utf8_encode($rowArtEmpUbic['codigo_articulo']),";"));
		$objResponse->assign("txtDescripcionArt","innerHTML",utf8_encode($rowArtEmpUbic['descripcion']));
		$objResponse->assign("txtUnidadArt","value",utf8_encode($rowArtEmpUbic['unidad']));
		$objResponse->assign("txtSeccionArt","value",utf8_encode($rowArtEmpUbic['descripcion_seccion']));
		$objResponse->assign("txtTipoPiezaArt","value",utf8_encode($rowArtEmpUbic['tipo_articulo']));
		$objResponse->assign("txtCantDisponible","value",$rowArtEmpUbic['cantidad_disponible_logica']);
		
		$objResponse->script("byId('txtCantDisponible').className = '".(($rowArtEmpUbic['cantidad_disponible_logica'] > 0) ? "inputCantidadDisponible" : "inputCantidadNoDisponible")."'");
	...
	if (isset($arrayObj1)) {
		foreach ($arrayObj1 as $indice => $valor) {
			...
			WHERE id_articulo = %s
				AND id_casilla = %s
				AND (estatus_articulo_almacen = 1
					OR (estatus_articulo_almacen IS NULL AND cantidad_disponible_logica > 0));",
				valTpDato($idArticulo, "int"),
				valTpDato($idCasilla, "int"));
			$rsAlmacenOrigen = mysql_query($queryAlmacenOrigen);
			if (!$rsAlmacenOrigen) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			...
			// VERIFICA SI ALGUN OTRO ARTICULO TIENE LA UBICACION OCUPADA
			...
			if ($cantTransferir <= $rowAlmacenOrigen['cantidad_disponible_logica']) {
				if ($totalRowsArticulo == 0 && $idCasillaDestino != $idCasilla) {
					// BUSCA EL ULTIMO COSTO DEL ARTICULO EN LA EMPRESA ORIGEN
					$queryArtCosto = sprintf("SELECT * FROM iv_articulos_costos WHERE id_articulo = %s AND id_empresa = %s ORDER BY fecha_registro DESC LIMIT 1;",
						valTpDato($idArticulo, "int"),
						valTpDato($idEmpresaOrigen, "int"));
					$rsArtCosto = mysql_query($queryArtCosto);
					...
					if ($idValeEntrada > 0) {
						...
						if ($idEmpresaOrigen != $idEmpresaDestino) {
							// BUSCA EL ULTIMO COSTO DEL ARTICULO EN LA EMPRESA DESTINO
							$queryArtCosto = sprintf("SELECT * FROM iv_articulos_costos WHERE id_articulo = %s AND id_empresa = %s ORDER BY fecha_registro DESC LIMIT 1;",
								valTpDato($idArticulo, "int"),
								valTpDato($idEmpresaDestino, "int"));
							$rsArtCosto = mysql_query($queryArtCosto);
							...
							if (!($totalRowsArtCosto > 0) || $costoUnitario != str_replace(",", ",", $frmVale['txtCostoTransferir'.$valor])) {
								...
								SELECT %s, id_proveedor, id_articulo, NOW(), costo, costo_promedio, id_moneda, NOW() FROM iv_articulos_costos art_costo
						...
						// VERIFICA SI LA ARTICULO TIENE AGREGADA LA UBICACION
						...
						if ($totalRows > 0) {
							...
							if (!$Result1) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
							mysql_query("SET NAMES 'latin1';");
						} else { // SI EL ARTICULO NO TENIA AGREGADA LA UBICACION DE DESTINO
							// VERIFICA SI LA EMPRESA TIENE AGREGADA LA PIEZA
							$query = sprintf("SELECT * FROM iv_articulos_empresa art_emp
							...
							if ($totalRows > 0) {
								...
								$insertSQL = sprintf("INSERT INTO iv_articulos_empresa (id_empresa, id_articulo, sustituido, id_casilla_predeterminada, id_casilla_predeterminada_compra, clasificacion, estatus)
								VALUE (%s, %s, %s, %s, %s, %s, %s);",
									valTpDato($idEmpresaDestino, "int"),
									valTpDato($idArticulo, "int"),
									valTpDato(0, "boolean"), // 0 = No, 1 = Si
...
function listaMovInterAlmacen($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	&& ($valCadBusq[2] == "-1" || $valCadBusq[2] == "")) {
		...
		WHERE art_alm2.id_articulo = (SELECT iv_articulos.id_articulo
							...
							WHERE iv_articulos_almacen.id_casilla = vw_iv_art_emp_ubic.id_casilla
	...
	if ($valCadBusq[7] != "-1" && $valCadBusq[7] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("vw_iv_art_emp_ubic.estatus_casilla = %s",
			valTpDato($valCadBusq[7], "int"));
	}
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaMovInterAlmacen", "64%", $pageNum, "descripcion", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Descripción"));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			if ($row['cantidad_disponible_logica'] > 0) {
					$htmlTb .= sprintf("<a class=\"modalImg\" id=\"aTransferencia%s\" rel=\"#divFlotante1\" onclick=\"abrirDivFlotante1(this, 'tblAlmacen', '%s', '%s');\"><img class=\"puntero\" src=\"../img/iconos/ico_movimiento_almacen.gif\" title=\"%s\"/></a>",
						$contFila,
						$row['id_articulo'],
						$row['id_casilla'],
	...
	if ($contFila > 0) {
		$htmlTb .= "<tr align=\"right\" class=\"trResaltarTotal\" height=\"22\">";
			$htmlTb .= "<td class=\"tituloCampo\" colspan=\"5\">".utf8_encode("Total Página:")."</td>";
		...
		if ($pageNum == $totalPages) {
			...
			$htmlTb .= "<tr align=\"right\" class=\"trResaltarTotal3\" height=\"22\">";
				$htmlTb .= "<td class=\"tituloCampo\" colspan=\"5\">".utf8_encode("Total de Totales:")."</td>";
