...
function asignarPrecio($idArticulo, $idPrecio, $frmDcto, $precioPredet = "false", $precioUnitario = "") {
	...
	$precioUnitario = ($rowArtPrecio['precio'] > 0 && $precioUnitario == "" && !in_array($rowArtPrecio['id_precio'], array(6,7,12))) ? $rowArtPrecio['precio'] : $precioUnitario;
	...
	if ($idDocumento > 0) {
		...
		if ($rowPedido['id_pedido_venta'] > 0) {
			...
			while ($rowPedidoDet = mysql_fetch_assoc($rsPedidoDet)) {
				$Result1 = insertarItemArticulo($contFila, $rowPedidoDet['id_pedido_venta_detalle'], "", $idCliente, $rowPedidoDet['id_articulo'], $rowPedidoDet['id_casilla'], $rowPedidoDet['cantidad'], $rowPedidoDet['pendiente'], $rowPedidoDet['id_precio'], $rowPedidoDet['precio_unitario'], "", "", $rowPedidoDet['id_iva']);
			...
			$objResponse->assign("txtFechaPedido","value",date("d-m-Y",strtotime($rowPedido['fecha'])));
		...
		if ($totalRowsInvFis == 0) {
			...
			if ($totalRowsCierreMensual > 0) {
				...
				$objResponse->assign("txtFechaPedido","value",date("d-m-Y"));
				
				if ($totalRowsEmpleado > 0) {
					$objResponse->loadCommands(asignarEmpleado($rowUsuario['id_empleado'], "false"));
...
function cargarPresupuesto($idPresupuesto) {
	...
	if ($rowPresupuesto['estatus_presupuesto_venta'] == 0) {
		...
		$objResponse->assign("txtFechaVencimientoPres","value",date("d-m-Y",strtotime($rowPresupuesto['fecha_vencimiento'])));
		...
		while ($rowPresupuestoDet = mysql_fetch_assoc($rsPresupuestoDet)) {
			...
			$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
				...
				if (strtotime(date("d-m-Y")) > strtotime($rowPresupuesto['fecha_vencimiento'])
				&& !in_array($rowPresupuestoDet['id_precio'], array(6,7,12))) {
				...
				while ($rowUbic = mysql_fetch_assoc($rsUbic)) {
					...
					$cantPendiente = ($selId == $rowUbic['id_casilla'] || $rowPresupuestoDet['id_casilla_predeterminada'] == $rowUbic['id_casilla']) ? $rowPresupuestoDet['pendiente'] : 0;
					...
					$htmlTb .= "<tr align=\"center\">";
						$htmlTb .= "<td class=\"texto_9px ".(($rowUbic['estatus_articulo_almacen'] == 1) ? "" : "divMsjError")."\" nowrap=\"nowrap\" width=\"44%\">";
							$htmlTb .= "<span class=\"textoNegrita_10px\">".utf8_encode($rowUbic['descripcion_almacen'])."</span>";
							$htmlTb .= "<br>";
							$htmlTb .= utf8_encode(str_replace("-[]", "", $rowUbic['ubicacion']));
							$htmlTb .= ($rowUbic['estatus_articulo_almacen'] == 1) ? "" : "<br>(Inactiva)";
							$htmlTb .= sprintf("<input id=\"cbxPresupuestoDet\" name=\"cbxPresupuestoDet[]\" type=\"checkbox\" checked=\"checked\" style=\"display:none\" value=\"%s\">".
								...
								$contFilaUbic, $contFilaUbic, $rowUbic['id_casilla']);
						$htmlTb .= "</td>";
						$htmlTb .= "<td align=\"right\" width=\"12%\"></td>";
						$htmlTb .= "<td width=\"22%\">";
							$htmlTb .= "<input type=\"text\" id=\"txtCantUbicacion".$contFilaUbic."\" name=\"txtCantUbicacion".$contFilaUbic."\" class=\"inputCompleto\" readonly=\"readonly\" style=\"text-align:right\" value=\"".$rowUbic['cantidad_disponible_logica']."\"/>";
						$htmlTb .= "</td>";
						$htmlTb .= "<td width=\"22%\">";
...
function guardarDcto($frmDcto, $frmListaArticulo, $frmTotalDcto, $frmNotaEntrega, $bloquearForm = "false", $txtIdPedido = "") {
	...
	WHERE inv_fis.id_empresa = %s
		...
		valTpDato($idEmpresa, "int"));
	$rsInvFis = mysql_query($queryInvFis);
	if (!$rsInvFis) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	$totalRowsInvFis = mysql_num_rows($rsInvFis);
	...
	if ($totalRowsInvFis == 0) {
		...
		if ($idDocumentoVenta > 0) {
			if (!xvalidaAcceso($objResponse,"iv_pedido_venta_list","editar")) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse; }
			
			// BUSCA LOS DATOS DEL PEDIDO
			$queryPedido = sprintf("SELECT * FROM iv_pedido_venta WHERE id_pedido_venta = %s;",
				valTpDato($idDocumentoVenta, "int"));
			$rsPedido = mysql_query($queryPedido);
			if (!$rsPedido) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			$rowPedido = mysql_fetch_assoc($rsPedido);
			...
			if (!$Result1) {
				errorPedidoVenta($arrayObj, $objResponse);
				if (mysql_errno() == 1452) {
			...
			$queryPedidoDet = sprintf("SELECT * FROM iv_pedido_venta_detalle WHERE id_pedido_venta = %s;",
				valTpDato($idDocumentoVenta, "int"));
			$rsPedidoDet = mysql_query($queryPedidoDet);
			if (!$rsPedidoDet) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			while ($rowPedidoDet = mysql_fetch_assoc($rsPedidoDet)) {
				$existRegDet = false;
				if (isset($arrayObj)) {
					foreach ($arrayObj as $indice => $valor) {
				...
				if ($existRegDet == false) {
					...
					$Result1 = mysql_query($deleteSQL);
					if (!$Result1) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
					...
					// SE CONECTA CON EL SISTEMA DE SOLICITUDES
					$Result1 = actualizarCantidadSistemaSolicitud($frmDcto['txtNumeroReferencia'], $idArticulo, $cantPendiente);
					if ($Result1[0] != true && strlen($Result1[1]) > 0) {
						errorPedidoVenta($arrayObj, $objResponse); 
						return $objResponse->alert($Result1[1]);
					}
			...
			if (!xvalidaAcceso($objResponse,"iv_pedido_venta_list","insertar")) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse; }
			...
			if (!$rsNumeracion) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		...
		if ($idPresupuesto > 0 && !($idDocumentoVenta > 0)) {
			$queryPresupuesto = sprintf("SELECT * FROM iv_presupuesto_venta WHERE id_presupuesto_venta = %s;",
				valTpDato($idPresupuesto, "int"));
			$rsPresupuesto = mysql_query($queryPresupuesto);
			if (!$rsPresupuesto) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			$rowPresupuesto = mysql_fetch_assoc($rsPresupuesto);
			...
			if ($rowPresupuesto['estatus_presupuesto_venta'] == 0) {
				...
				if (isset($arrayObj)) {
					foreach ($arrayObj as $indice => $valor) {
						if (strlen($frmListaArticulo['hddIdArticuloItm'.$valor]) > 0) {
							$queryPresupuestoDet = sprintf("SELECT * FROM iv_presupuesto_venta_detalle WHERE id_presupuesto_venta_detalle = %s",
								valTpDato($frmListaArticulo['hddIdPresupuestoDet'.$valor], "int"));
							$rsPresupuestoDet = mysql_query($queryPresupuestoDet);
							if (!$rsPresupuestoDet) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
							while ($rowPresupuestoDet = mysql_fetch_assoc($rsPresupuestoDet)) {
								$cantPendiente = $rowPresupuestoDet['pendiente'] - str_replace(",", "", $frmListaArticulo['hddCantItm'.$valor]);
			...
			foreach ($arrayObj as $indice => $valor) {
				...
				if ($cantPedida > 0) {
					if ($idDocumentoDetalle > 0) {
						...
						$insertSQL = sprintf("INSERT INTO iv_pedido_venta_detalle (id_pedido_venta, id_articulo, id_casilla, cantidad, pendiente, id_precio, precio_unitario, id_iva, iva)
			...
			for ($indice = 0; $indice < count($arrayVentasPerdidas); $indice += 2) {
				...
				if ($cantPerdida > 0) {
					...
					WHERE id_empresa = %s
						...
						valTpDato($idArticulo, "int"));
					$rsArticulo = mysql_query($queryArticulo);
					if (!$rsArticulo) { errorPedidoVenta($arrayObj, $objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
					$rowArticulo = mysql_fetch_assoc($rsArticulo);
...
function insertarArticulo($frmDatosArticulo, $frmDcto, $frmListaArticulo, $frmTotalDcto) {
	...
	if (($cantPedida - str_replace(",","",$frmListaArticulo['hddCantItm'.$hddNumeroArt])) <= $rowArtEmp['cantidad_disponible_logica']
		...
		if ($hddNumeroArt > 0) {
			...
			if (count($arrayObj) < $rowConfig5['valor']) {
				if (in_array($lstPrecioArt, array(6,7)) && $hddBajarPrecio == false && round($hddPrecioArtPredet,2) > round($precioUnitario,2)) {
					...
					$Result1 = insertarItemArticulo($contFila, "", "", $idCliente, $idArticulo, $rowArtEmp['id_casilla'], $cantPedida, $cantPedida, $lstPrecioArt, $precioUnitario, $costoUnitario, $abrevMonedaCostoUnitario, $idIva);
					if ($Result1[0] != true && strlen($Result1[1]) > 0) {
						return $objResponse->alert($Result1[1]);
					} else if ($Result1[0] == true) {
						$contFila = $Result1[2];
		...
		if (isset($arrayObjPresupuestoDet)) {
			foreach ($arrayObjPresupuestoDet as $indice => $valor) {
				if ($frmPresupuesto['txtCantidad'.$valor] > 0) {
					...
					$Result1 = insertarItemArticulo($contFila, "", $rowPresupuestoDet['id_presupuesto_venta_detalle'], $idCliente, $idArticulo, $idCasilla, $cantPedida, $cantPedida, $lstPrecioArt, $precioUnitario, "", "", $rowPresupuestoDet['id_iva']);
					if ($Result1[0] != true && strlen($Result1[1]) > 0) {
						return $objResponse->alert($Result1[1]);
					} else if ($Result1[0] == true) {
						$contFila = $Result1[2];
			...
			$htmlTf .= "<tr class=\"tituloCampo\">";
				...
				$htmlTf .= "<td align=\"center\" class=\"tituloColumna\" width=\"210\">";
					...
					$htmlTf .= "<tr align=\"center\">";
						...
						$htmlTf .= "<td width=\"25\">";
						if ($pageNum < $totalPages) {
							$htmlTf .= sprintkf("<a class=\"puntero\" onclick=\"xajax_listaEmpleado(%s,'%s','%s','%s',%s);\">%s</a>",
								min($totalPages, $pageNum + 1), $campOrd, $tpOrd, $valBusq, $maxRows, "<img src=\"../img/iconos/ico_reg_sig.gif\"/>");
						}
						$htmlTf .= "</td>";
						$htmlTf .= "<td width=\"25\">";
...
function insertarItemArticulo($contFila, $hddIdPedidoDet = "", $hddIdPresupuestoDet = "", $idCliente = "", $idArticulo = "", $idCasilla = "", $cantPedida = "", $cantPendiente = "", $hddIdPrecioItm = "", $precioUnitario = "", $costoUnitario = "", $abrevMonedaCostoUnitario = "", $idIva = "") {
	$contFila++;
	...
	if ($hddIdPresupuestoDet > 0 || $hddIdPedidoDet > 0) {
		...
		if ($hddIdPresupuestoDet > 0) {
			...
			FROM iv_presupuesto_venta_detalle pres_vent_det
				INNER JOIN iv_pedido_venta pres_vent ON (pres_vent_det.id_presupuesto_venta = pres_vent.id_presupuesto_venta)
...
function errorPedidoVenta($arrayObj, $objResponse, $calcularDcto = true) {
	if (isset($arrayObj)) {
		$objResponse->script("
