...
function generarAnalisisInv($idCierreMensual, $frmListaCierreMensual) {
	...
	WHERE cierre_mensual.id_cierre_mensual = %s;",
		valTpDato($idCierreMensual, "int"));
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$totalRows = mysql_num_rows($rs);
	...
	// VERIFICA VALORES DE CONFIGURACION (Cantidad de Meses de Consumo Promedio)
	$queryConfig9 = sprintf("SELECT * FROM pg_configuracion_empresa config_emp
		INNER JOIN pg_configuracion config ON (config_emp.id_configuracion = config.id_configuracion)
	WHERE config.id_configuracion = 9 AND config_emp.status = 1 AND config_emp.id_empresa = %s;",
		valTpDato($idEmpresa, "int"));
	$rsConfig9 = mysql_query($queryConfig9);
	if (!$rsConfig9) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$rowConfig9 = mysql_fetch_assoc($rsConfig9);
	...
	while ($row = mysql_fetch_assoc($rs)) {
		...
		$costoUnitario = ($rowConfig12['valor'] == 1) ? round($rowArtCosto['costo'],3) : round($rowArtCosto['costo_promedio'],3);
		
		if ($rowConfig9['valor'] > 0) {
			$arrayPromedioVentas = calcularConsumoPromedioVentas($idEmpresa, $idArticulo, $anoCierre, $rowConfig9['valor'], 20);
...
function generarClasificacionInv($idCierreMensual, $frmListaCierreMensual) {
	...
	$queryArtEmp = sprintf("SELECT
		id_articulo,
		id_articulo_empresa,
		(cantidad_compra + cantidad_entrada - cantidad_venta - cantidad_salida) AS cantidad_saldo,
		clasificacion,
		(SELECT art.fecha_registro FROM iv_articulos art
		WHERE art.id_articulo = iv_articulos_empresa.id_articulo) AS fecha_registro
	FROM iv_articulos_empresa
	WHERE id_empresa = %s
	...
	while ($rowArtEmp = mysql_fetch_assoc($rsArtEmp)) {
		...
		$queryArtPasado = sprintf("SELECT * FROM iv_cierre_anual
			WHERE id_empresa = %s
				AND id_articulo = %s
				AND ano = %s;",
			valTpDato($idEmpresa, "int"),
		...
		$rowArtPasado = mysql_fetch_assoc($rsArtPasado);
		
		$queryArt = sprintf("SELECT * FROM iv_cierre_anual
			WHERE id_empresa = %s
				AND id_articulo = %s
				AND ano = %s;",
			valTpDato($idEmpresa, "int"),
		...
		$detalleVenta[0] = $rowArtEmp['id_articulo_empresa'];
		...
		$detalleVenta[37] = $rowArt['cantidad_saldo'];
		$detalleVenta[38] = $idArticulo;
		$detalleVenta[39] = $rowArtEmp['clasificacion'];
		$detalleVenta[40] = $rowArtEmp['fecha_registro'];
	...
	if (isset($arrayArticuloVenta)) {
		foreach ($arrayArticuloVenta as $indice => $valor) {
			$idArticulo = $arrayArticuloVenta[$indice][38];
			$cantidadSaldo = $arrayArticuloVenta[$indice][37];
			$claseAsignada = false;
			
			if (restaFechas("Y-m-d", date("Y-m-d",strtotime($arrayArticuloVenta[$indice][40])), date("Y-m-d")) <= 5) { // SI LA PIEZA TIENE IGUAL O MENOS DE 5 MESES DE HABER SIDO CREADA
				$claseAsignada = "F";
			} else {
				$ano = $anoCierre;
				$mes = $mesCierre + 24;
				...
				if ($mesesLlenos == 6 && $claseAsignada == false) // VENDIDAS EN TODOS LOS ULTIMOS 6 MESES
					$claseAsignada = "A";
				else if($mesesLlenos == 5 && $claseAsignada == false) // VENDIDAS EN 5 DE LOS ULTIMOS 6 MESES
					$claseAsignada = "B";
				else if(($mesesLlenos == 4 || $mesesLlenos == 3) && $claseAsignada == false) // VENDIDAS EN 3 o 4 DE LOS ULTIMOS 6 MESES
					$claseAsignada = "C";
				else if (($mesesLlenos == 1 || $mesesLlenos == 2) && $claseAsignada == false) // VENDIDAS EN 1 o 2 DE LOS ULTIMOS 6 MESES
					$claseAsignada = "D";
				else if ($arrayArticuloVenta[$indice][37] > 0 && $mesesLlenos == 0 && $claseAsignada == false) // CON DISPONIBILIDAD Y NO VENDIDAS EN LOS ULTIMOS 6 MESES
					$claseAsignada = "E";
				...
				if ($mesesLlenos >= 1 && $claseAsignada == false) { // VENDIDAS EN CUALQUIER MES DEL ULTIMO AÑO
					$claseAsignada = "D";
				} else if ($claseAsignada == false) {
					$claseAsignada = "E";
				}
			}
			
			if (!($cantidadSaldo > 0 || ($cantidadSaldo == 0 && $arrayArticuloVenta[$indice][$mesCierre + 24] > 0))) {
				$claseAsignada = $arrayArticuloVenta[$indice][39];
			}
			...
			WHERE id_articulo_empresa = %s;",
				...
				valTpDato($arrayArticuloVenta[$indice][0], "int"));
			$Result1 = mysql_query($updateSQL);
			if (!$Result1) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			...
			if ($totalRows == 0) {
				...
				VALUE (%s, %s, %s, %s);",
					valTpDato($idCierreMensual, "int"),
					valTpDato($idArticulo, "int"),
					valTpDato($arrayArticuloVenta[$indice][39], "text"),
					valTpDato($claseAsignada, "text"));
				$Result1 = mysql_query($insertSQL);
				if (!$Result1) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			} else {
...
function generarMaximoMinimo($idCierreMensual, $frmListaCierreMensual) {
	...
	$queryConfigMesesInv = sprintf("SELECT * FROM pg_configuracion_empresa config_emp
		INNER JOIN pg_configuracion config ON (config_emp.id_configuracion = config.id_configuracion)
	WHERE config.id_configuracion = 1 AND config_emp.status = 1 AND config_emp.id_empresa = %s;",
		valTpDato($idEmpresa, "int"));
	$rsConfigMesesInv = mysql_query($queryConfigMesesInv);
	if (!$rsConfigMesesInv) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$rowConfigMesesInv = mysql_fetch_assoc($rsConfigMesesInv);
	
	// VERIFICA VALORES DE CONFIGURACION
	$queryConfigMesesMin = sprintf("SELECT * FROM pg_configuracion_empresa config_emp
		INNER JOIN pg_configuracion config ON (config_emp.id_configuracion = config.id_configuracion)
	WHERE config.id_configuracion = 2 AND config_emp.status = 1 AND config_emp.id_empresa = %s;",
		valTpDato($idEmpresa, "int"));
	$rsConfigMesesMin = mysql_query($queryConfigMesesMin);
	if (!$rsConfigMesesMin) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$rowConfigMesesMin = mysql_fetch_assoc($rsConfigMesesMin);
	
	// VERIFICA VALORES DE CONFIGURACION
	$queryConfigMesesProteccion = sprintf("SELECT * FROM pg_configuracion_empresa config_emp
		INNER JOIN pg_configuracion config ON (config_emp.id_configuracion = config.id_configuracion)
	WHERE config.id_configuracion = 3 AND config_emp.status = 1 AND config_emp.id_empresa = %s;",
		valTpDato($idEmpresa, "int"));
	$rsConfigMesesProteccion = mysql_query($queryConfigMesesProteccion);
	if (!$rsConfigMesesProteccion) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$rowConfigMesesProteccion = mysql_fetch_assoc($rsConfigMesesProteccion);
	
	$mesesInventario = $rowConfigMesesInv['valor'];
	$mesesMinimo = $rowConfigMesesMin['valor'];
	$mesesProteccion = $rowConfigMesesProteccion['valor'];
...
function guardarCierreMensual($frmCierreMensual, $frmListaCierreMensual) {
	...
	if ($totalRows == 0) {
		...
		$insertSQL = sprintf("INSERT INTO iv_cierre_mensual (id_empresa, mes, ano, id_empleado_creador, id_tipo_costo)
		SELECT
