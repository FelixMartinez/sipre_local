	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			...
			if ($idModoCompra == 2 && $incluirIvaMonedaLocal == 1) { // 2 = Importacion
				...
				$queryIva = sprintf("SELECT iva.*, IF (iva.tipo IN (3,2), 1, NULL) AS lujo FROM pg_iva iva WHERE iva.estado = 1 AND iva.tipo IN (1) AND iva.activo = 1;");
			} else {
...
function cargarDcto($idDocumento, $frmListaArticulo, $frmTotalDcto) {
	...
	if ($idDocumento > 0) {
		...
		if ($totalRowsPedido > 0) {
			...
			$objResponse->assign("txtFecha","value",date("d-m-Y",strtotime($rowPedido['fecha'])));
	...
	while ($rowPedidoDet = mysql_fetch_assoc($rsPedidoDet)) {
		...
		if (isset($arrayObj)) {
			foreach ($arrayObj as $indice => $valor) {
				...
				$gastoUnitario = $frmListaArticulo['hddGastosItm'.$valor] / str_replace(",","",$frmListaArticulo['txtCantItm'.$valor]);
				...
				if ($rowPedidoDet['id_pedido_compra_detalle'] == $hddIdPedidoDetItm) {
					...
					WHERE id_pedido_compra_detalle = %s;",
						valTpDato($gastoUnitario, "real_inglesa"),
						valTpDato($hddIdIvaItm, "int"),
						valTpDato($hddIvaItm, "real_inglesa"),
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			...
			if ($idArticulo > 0 && !($hddIdPedidoDetItm > 0)) {
				$insertSQL = sprintf("INSERT INTO iv_pedido_compra_detalle (id_pedido_compra, id_articulo, cantidad, pendiente, precio_unitario, gasto_unitario, tipo, id_cliente)
				VALUE (%s, %s, %s, %s, %s, %s, %s, %s)",
					...
					valTpDato($gastoUnitario, "real_inglesa"),
					valTpDato($frmListaArticulo['hddTipoItm'.$valor], "boolean"),
...
function importarDcto($frmImportarPedido, $frmDcto, $frmListaArticulo, $frmTotalDcto) {
	...
	while (strlen($archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue()) > 0) {
		...
		if ($itemExcel == true && doubleval($cantPedida) > 0) {
			$arrayAlmacen[] = array(
				$archivoExcel->getActiveSheet()->getCell('A'.$i)->getValue(), // Código Artículo
				$archivoExcel->getActiveSheet()->getCell('B'.$i)->getValue(), // Ped.
				$archivoExcel->getActiveSheet()->getCell('C'.$i)->getValue(), // Costo Unit.
				$archivoExcel->getActiveSheet()->getCell('D'.$i)->getValue()); // Id Cliente
		}
	...
	if (isset($arrayAlmacen)) {
		foreach ($arrayAlmacen as $indice => $valor) {
			...
			WHERE art.codigo_articulo LIKE %s;",
				valTpDato($arrayAlmacen[$indice][0], "text"));
			...
			if (isset($arrayObj)) {
				foreach ($arrayObj as $indice2 => $valor2) {
					if ($frmListaArticulo['hddIdArticuloItm'.$valor2] == $idArticulo && $idArticulo > 0) {
						$existe = true;
					}
				}
			}
			...
			if ($existe == false) {
				if (count($arrayObj) < $rowConfig6['valor']) {
					if ($totalRowsArt > 0) {
						...
						$cantPedida = $arrayAlmacen[$indice][1];
						$costoUnitario = $arrayAlmacen[$indice][2];
						$hddIdIvaItm = implode(",",$arrayIdIvaItm);
						$gastoUnitario = 0;
						$hddTipoItm = 0;
						$idClienteItm = $arrayAlmacen[$indice][3];
						
						$Result1 = insertarItemArticulo($contFila, "", $idArticulo, $cantPedida, $costoUnitario, $hddIdIvaItm, $gastoUnitario, $hddTipoItm, $idClienteItm);
						if ($Result1[0] != true && strlen($Result1[1]) > 0) {
							return $objResponse->alert($Result1[1]);
						} else if ($Result1[0] == true) {
							$contFila = $Result1[2];
...
function insertarArticulo($frmArticulo, $frmListaArticulo, $frmTotalDcto) {
	...
	$gastoUnitario = str_replace(",","",$frmListaArticulo['hddGastosItm'.$contFila]);
...
function insertarItemArticulo($contFila, $idPedidoDetalle = "", $idArticulo = "", $cantPedida = "", $costoUnitario = "", $hddIdIvaItm = "", $gastoUnitario = 0, $hddTipoItm = "0", $idClienteItm = "") {
	$contFila++;
	...
	if ($rowArticulo['id_arancel_familia'] > 0) {
	...
	$htmlItmPie = sprintf("$('#trItmPie').before('".
		"<tr id=\"trItm:%s\" align=\"left\">".
			...
			"<td><input type=\"text\" id=\"txtTotalItm%s\" name=\"txtTotalItm%s\" class=\"inputSinFondo\" readonly=\"readonly\" value=\"%s\">".
				...
				"<input type=\"hidden\" id=\"hddGastosItm%s\" name=\"hddGastosItm%s\" value=\"%s\">".
