...
function guardarInventario($frmInventario, $cancelarInvAbierto = false) {
	...
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("((id_casilla IS NOT NULL AND estatus_articulo_almacen = 1)
			OR (id_casilla IS NOT NULL AND estatus_articulo_almacen IS NULL AND cantidad_disponible_logica > 0))");
	...
	switch($frmInventario['lstFiltroArticulos']) {
		...
		case 2 : // Con Movimientos
			...
			WHERE kardex.id_articulo = vw_iv_articulos_empresa_ubicacion.id_articulo
				AND kardex.id_casilla = vw_iv_articulos_empresa_ubicacion.id_casilla) > 0");
			break;
		case 3 : // Con Saldo
			$filtroArticulos = "Con Disponibilidad";
			
			$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
			$sqlBusq .= $cond.sprintf("vw_iv_articulos_empresa_ubicacion.cantidad_disponible_logica > 0");
			break;
	}
	...
	switch($frmInventario['lstOrdenArticulos']) {
		case 1: // Código Artículo
			...
			$sqlOrd = "ORDER BY vw_iv_articulos_empresa_ubicacion.codigo_articulo ASC";
			break;
		case 2 : // Código Proveedor
			$ordenArticulos = "Código de Proveedor";
			$sqlOrd = "ORDER BY vw_iv_articulos_empresa_ubicacion.codigo_articulo_prov ASC";
			break;
		case 3 : // Localización
			$ordenArticulos = "Por Localización";
			$sqlOrd = "ORDER BY CONCAT(vw_iv_articulos_empresa_ubicacion.descripcion_almacen, vw_iv_articulos_empresa_ubicacion.ubicacion) ASC";
			break;
	}
	
	// BUSCA LOS ARTICULOS DE LA EMPRESA DEPENDIENDO DE LOS FILTROS
	$queryArtEmp = sprintf("SELECT * FROM vw_iv_articulos_empresa_ubicacion %s %s", $sqlBusq, $sqlOrd);
	...
	while ($rowArtEmp = mysql_fetch_assoc($rsArtEmp)) {
		...
		if ($costoUnitario > 0) {
			$insertSQL = sprintf("INSERT INTO iv_inventario_fisico_detalle (numero, id_inventario_fisico, id_articulo, id_casilla, existencia_kardex, costo_proveedor, habilitado)
			VALUE (%s, %s, %s, %s, %s, %s, true);",
