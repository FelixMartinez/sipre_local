	...
	if (isset($arrayArticulo)) {
		...
		foreach ($arrayArticulo as $indice => $valor) {
			...
			if ($totalRowsArticulo == 0) {
				...
				SELECT %s, iva.idIva FROM pg_iva iva WHERE iva.estado = 1 AND iva.tipo IN (1,6) AND iva.activo = 1;",
					valTpDato($idArticulo, "int")); 
				mysql_query("SET NAMES 'utf8';");
				$Result1 = mysql_query($insertSQL);
				if (!$Result1) { return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
...
function listaArticulo($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a class=\"modalImg\" rel=\"#divFlotante1\" onmouseover=\"Tip('<div id=divPrecios></div><div id=divCostos></div>', TITLE, '%s) Lista de Precios (Artículo Código: %s - %s)', WIDTH, 800); xajax_listaPrecios('%s', xajax.getFormValues('frmBuscar')); xajax_listaCostoProveedor('%s', '%s');\" onmouseout=\"UnTip();\"><img class=\"puntero\" src=\"../img/iconos/money.png\" title=\"Ver Precios\"/></a>",
					$row['id_articulo'],
...
function listaBloqueoVenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 5, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td align=\"center\">".implode("-",array_reverse(explode("-",$row['fecha_bloqueo'])))."</td>";
...
function listaCostoProveedor($idEmpresa, $idArticulo) {
	...
	$sqlBusq = " ";
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("id_proveedor IS NOT NULL");
	
	$sqlBusq2 = " ";
	$cond = (strlen($sqlBusq2) > 0) ? " AND " : " WHERE ";
	$sqlBusq2 .= $cond.sprintf("id_proveedor IS NOT NULL");
	...
	$htmlTblIni = "<table border=\"0\" class=\"texto_9px\" width=\"100%\">";
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= "<td width=\"14%\">".utf8_encode("Fecha")."</td>";
		$htmlTh .= "<td width=\"54%\">".utf8_encode("Proveedor")."</td>";
		$htmlTh .= "<td width=\"16%\">".utf8_encode("Costo")."</td>";
		$htmlTh .= "<td width=\"16%\">".utf8_encode("Costo Promedio")."</td>";
	$htmlTh .= "</tr>";
	...
	while ($row = mysql_fetch_assoc($rs)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"right\" class=\"".$classCosto."\">".$row['abreviacion_moneda'].number_format($row['costo'],2,".",",")."</td>";
			$htmlTb .= "<td align=\"right\" class=\"".$classCostoProm."\">".$row['abreviacion_moneda'].number_format($row['costo_promedio'],2,".",",")."</td>";
		$htmlTb .= "</tr>";
	}
	$htmlTblFin .= "</table>";
...
function listaPrecios($idArticulo, $frmBuscar) {
	...
	AND (id_empresa = %s
		OR (%s IS NULL
			AND id_empresa IN (SELECT suc.id_empresa FROM pg_empresa suc
								WHERE suc.id_empresa_padre IS NULL)))
	AND estatus = 1",
		valTpDato($idArticulo, "int"),
		valTpDato($idEmpresa, "int"),
		valTpDato($idEmpresa, "int"));
	
	$queryArtPrecio = sprintf("SELECT * FROM vw_iv_articulos_precios %s ORDER BY porcentaje DESC;", $sqlBusq);
	...
	$htmlTblIni = "<table border=\"0\" width=\"798\">";
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= "<td width=\"49%\">Descripción</td>";
		$htmlTh .= "<td width=\"17%\">".$spanPrecioUnitario."</td>";
		$htmlTh .= "<td width=\"17%\">Impuesto</td>";
		$htmlTh .= "<td width=\"17%\">Total</td>";
	$htmlTh .= "</tr>";
	...
	while ($rowArtPrecio = mysql_fetch_assoc($rsArtPrecio)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td>".utf8_encode($rowArtPrecio['descripcion_precio'])."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($rowArtPrecio['precio'], 2, ".", ",")."</td>";
			$htmlTb .= "<td align=\"right\">".number_format($rowArtPrecio['monto_iva'], 2, ".", ",")."</td>";
			$htmlTb .= "<td align=\"right\" class=\"divMsjInfo\">".number_format($rowArtPrecio['precio_con_iva'], 2, ".", ",")."</td>";
		$htmlTb .= "</tr>";
	}
	$htmlTblFin .= "</table>";
...
function listaSaldos($idEmpresa, $idArticulo) {
	...
	// BUSCA LOS ALMACENES DE LA EMPRESA Y EL ARTICULO
	$query = sprintf("SELECT id_almacen, descripcion_almacen FROM vw_iv_articulos_almacen
	WHERE id_articulo = %s
		AND (estatus_articulo_almacen = 1 OR estatus_articulo_almacen IS NULL) %s
	GROUP BY id_almacen, descripcion_almacen",
	...
	$htmlTh .= "<tr align=\"center\">";
		...
		$htmlTh .= "<td colspan=\"2\">".(implode("-",array_reverse(explode("-",$rowArticulo['fecha_ultima_compra']))))."</td>";
		$htmlTh .= "<td align=\"right\" class=\"tituloCampo\">"."Ult. Venta:"."</td>";
		$htmlTh .= "<td colspan=\"2\">".(implode("-",array_reverse(explode("-",$rowArticulo['fecha_ultima_venta']))))."</td>";
	$htmlTh .= "</tr>";
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= "<td width=\"30%\"></td>";
		$htmlTh .= "<td width=\"10%\">"."Saldo"."</td>";
		...
		while ($rowUbic = mysql_fetch_assoc($rsUbic)) {
			...
			&& ($rowDispUbic['id_casilla'] > 0 || $rowDispUbic['cantidad_reservada'] > 0 || $rowDispUbic['cantidad_pedida'] > 0)) {
				...
				$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
					...
					$htmlTb .= "</td>";
					$htmlTb .= "<td align=\"right\">".valTpDato(number_format($rowDispUbic['existencia'], 2, ".", ","),"cero_por_vacio")."</td>";
					$htmlTb .= "<td align=\"right\">".valTpDato(number_format($rowDispUbic['cantidad_reservada'], 2, ".", ","),"cero_por_vacio")."</td>";
					$htmlTb .= "<td align=\"right\">".valTpDato(number_format($rowDispUbic['cantidad_espera'], 2, ".", ","),"cero_por_vacio")."</td>";
					$htmlTb .= "<td align=\"right\">".valTpDato(number_format($rowDispUbic['cantidad_bloqueada'], 2, ".", ","),"cero_por_vacio")."</td>";
					$htmlTb .= ($rowDispUbic['estatus_almacen_venta'] == 1 && $rowDispUbic['cantidad_disponible_logica'] > 0) ? "<td align=\"right\" class=\"divMsjInfo\">" : "<td align=\"right\">";
						$htmlTb .= valTpDato(number_format($rowDispUbic['cantidad_disponible_logica'], 2, ".", ","),"cero_por_vacio");
					$htmlTb .= "</td>";
					$htmlTb .= "<td align=\"right\">".valTpDato(number_format($rowDispUbic['cantidad_pedida'], 2, ".", ","),"cero_por_vacio")."</td>";
					$htmlTb .= "<td align=\"right\">".valTpDato(number_format($rowDispUbic['cantidad_futura'], 2, ".", ","),"cero_por_vacio")."</td>";
				$htmlTb .= "</tr>";
			}
		}
...
function listaSolicitudServicio($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 5, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td>".$imgEstatusPedido."</td>";
			$htmlTb .= "<td align=\"center\">".date("d-m-Y", strtotime($row['tiempo_solicitud']))."</td>";
...
(Serv.)");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		if ($row['estatus_articulo_almacen'] == 1) {
			...
			$aEtiqueta = "<a class=\"modalImg\" id=\"aEtiqueta".$contFila."\" rel=\"#divFlotante1\" onclick=\"verVentana('reportes/iv_articulo_etiqueta_pdf.php?valBusq=".$row['id_articulo'].",".$row['id_casilla'].",1', 400, 300);\"><img class=\"puntero\" src=\"../img/iconos/tag_blue.png\" title=\"Etiqueta\"/></a>";
		} else {
			// VERIFICA SI ALGUN ARTICULO TIENE LA UBICACION OCUPADA
			$queryCasilla = sprintf("SELECT
				casilla.*,
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\" class=\"".$classUbic."\">".utf8_encode($row['descripcion_almacen'])."<br>".utf8_encode(str_replace("-[]", "", $row['ubicacion'].$ocupada))."</td>";
