...
function cargaLstArticulo ($idMarca = "", $idTipoArticulo = "", $idTipoUnidad = "", $idSeccion = "", $idSubSeccion = "", $idPrecioPredet = "", $hddTipoVista = "", $bloquearObj = false) {
	...
	$queryPrecio = sprintf("SELECT * FROM pg_precios precio WHERE precio.id_precio NOT IN (6,7) AND precio.estatus IN (1) ORDER BY precio.id_precio ASC;");
...
function cargarArticulo($idArticulo, $idEmpresa, $hddTipoVista = "", $bloquearObj = false) {
	...
	if ($idArticulo > 0) {
		...
		$objResponse->assign("spnFechaUltCompraArt","innerHTML",implode("-",array_reverse(explode("-",$rowArticulo['fecha_ultima_compra']))));
		$objResponse->assign("spnFechaUltVentaArt","innerHTML",implode("-",array_reverse(explode("-",$rowArticulo['fecha_ultima_venta']))));
		...
		$objResponse->assign("spnFechaRegistro","innerHTML",date("d-m-Y H:i:s",strtotime($rowArticulo['fecha_registro'])));
		...
		$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
			$htmlTh .= "<td width=\"55%\">Descripción</td>";
			$htmlTh .= "<td width=\"15%\">Precio</td>";
...
function guardarArticulo($frmArticulo, $frmListaArtSust, $frmListaArtAlt) {
	...
	$updateSQL = sprintf("UPDATE iv_articulos SET
		posee_iva = IF((SELECT COUNT(id_articulo)
							...
							WHERE iva.estado = 1 AND iva.tipo IN (1,6) AND iva.activo = 1
...
function listaImpuesto($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("iva.tipo IN (1,3,6,2)");
