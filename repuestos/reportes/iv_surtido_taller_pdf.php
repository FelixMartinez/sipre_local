...
while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
	...
	if (fmod($contFilaY, $maxRows) == 1) {
		...
		imagestring($img,1,430,$posY,": ".date("d-m-Y",strtotime($rowEncabezado['tiempo_solicitud'])),$textColor);
	...
	imagestring($img,1,480,$posY,str_pad(date("d-m-Y", strtotime($fechaMovimiento)), 10, " ", STR_PAD_BOTH),$textColor);
	
	$posY += 9;
	(fmod($contFila, 2) == 0) ? "" : imagefilledrectangle($img, 0, $posY, 569, $posY+9, $backgroundAzul);
	imagestring($img,1,0,$posY,strtoupper(substr($rowDetalle['descripcion_articulo'],0,32)),$textColor);
	imagestring($img,1,480,$posY,str_pad(date("h:i:sa", strtotime($fechaMovimiento)), 10, " ", STR_PAD_BOTH),$textColor);
