...
WHERE cxc_nc.idNotaCredito = %s
	AND cxc_nc.idDepartamentoNotaCredito IN (3);",
	...
	if (fmod($contFilaY, $maxRows) == 0 || $contFila == $totalRowsDetalle) {
		if ($contFila == $totalRowsDetalle) {
			...
			if ($rowConfig406['valor'] == 2) {
				...
				imagestring($img,1,410,$posY,str_pad(number_format($subtotalNotaCredito, 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				...
				while ($rowIvaNotaCred = mysql_fetch_assoc($rsIvaNotaCred)) {
					...
					imagestring($img,1,410,$posY,str_pad(number_format($rowIvaNotaCred['base_imponible'], 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
					...
					imagestring($img,1,410,$posY,str_pad(number_format($rowIvaNotaCred['subtotal_iva'], 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				...
				imagestring($img,2,398,$posY-3,str_pad(number_format($rowEncabezado['montoNetoNotaCredito'], 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
			} else {
				$posY = 460;
