...
while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
	...
	if (fmod($contFila, $maxRows) == 1) {
		...
		imagestring($img,1,390,$posY,": ".date("d-m-Y", strtotime($rowEncabezado['fecha_orden'])),$textColor);
		...
		imagestring($img,1,265,$posY,": ".date("d-m-Y",strtotime($rowEncabezado['fecha_entrega'])),$textColor);
	...
	if (fmod($contFila, $maxRows) == 0 || $contFila == $totalRowsDetalle) {
		if ($contFila == $totalRowsDetalle) {
			...
			imagestring($img,1,275,$posY,": ".implode("-",array_reverse(explode("-",$rowEncabezado['fecha_cotizacion']))),$textColor);
...
imagestring($img,1,70,$posY,": ".implode("-",array_reverse(explode("-",$rowEncabezado['fecha']))),$textColor);
imagestring($img,1,255,$posY,"FECHA",$textColor);
imagestring($img,1,325,$posY,": ".implode("-",array_reverse(explode("-",$rowEncabezado['fecha_orden']))),$textColor);
