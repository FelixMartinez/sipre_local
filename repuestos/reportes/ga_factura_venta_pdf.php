...
foreach ($arrayDetalle as $indice => $valor) {
	...
	if (!isset($tieneDetalle)) {
		...
		if ($rowConfig406['valor'] == 2) {
			imagestring($img,1,65,$posY,strtoupper(substr($arrayDetalle[$indice]['descripcion_articulo'],0,38)),$textColor);
		} else {
			imagestring($img,1,35,$posY,strtoupper(substr($arrayDetalle[$indice]['descripcion_articulo'],0,44)),$textColor);
		}
	...
	if (fmod($contFilaY, $maxRows) == 0 || $contFila == $totalRowsDetalle) {
		if ($contFila == $totalRowsDetalle) {
			...
			if (!$verObservacion) {
				$arrayObservacionDcto = str_split(strtoupper($rowEncabezado['observacionFactura']), 50);
				if (isset($arrayObservacionDcto)) {
					foreach ($arrayObservacionDcto as $indice => $valor) {
						$posY += 9;
						imagestring($img,1,0,$posY,strtoupper(trim($valor)),$textColor);
			...
			if ($rowConfig406['valor'] == 2) {
				...
				imagestring($img,1,410,$posY,str_pad(number_format($subtotalFactura, 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				...
				if ($subtotalDescuento > 0) {
					...
					imagestring($img,1,410,$posY,str_pad(number_format($subtotalDescuento, 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				}
				
				if ($totalGastosConIva != 0) {
					$posY += 9;
					imagestring($img,1,330,$posY,str_pad("CARGOS C/IMPTO", 12, " ", STR_PAD_RIGHT).":",$textColor);
					imagestring($img,1,410,$posY,str_pad(number_format($totalGastosConIva, 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				}
				...
				while ($rowIvaFact = mysql_fetch_assoc($rsIvaFact)) {
					...
					imagestring($img,1,410,$posY,str_pad(number_format($rowIvaFact['base_imponible'], 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
					...
					imagestring($img,1,410,$posY,str_pad(number_format($rowIvaFact['subtotal_iva'], 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				...
				if ($totalGastosSinIva != 0) {
					...
					imagestring($img,1,410,$posY,str_pad(number_format($totalGastosSinIva, 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				}
				
				if ($rowEncabezado['montoExento'] > 0) {
					$posY += 9;
					imagestring($img,1,330,$posY,str_pad("EXENTO", 12, " ", STR_PAD_RIGHT).":",$textColor);
					imagestring($img,1,410,$posY,str_pad(number_format($rowEncabezado['montoExento'], 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				}
				
				if ($rowEncabezado['montoExonerado'] > 0) {
					$posY += 9;
					imagestring($img,1,330,$posY,str_pad("EXONERADO", 12, " ", STR_PAD_RIGHT).":",$textColor);
					imagestring($img,1,410,$posY,str_pad(number_format($rowEncabezado['montoExonerado'], 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
				}
				...
				imagestring($img,2,398,$posY-3,str_pad(number_format($rowEncabezado['montoTotalFactura'], 2, ".", ","), 12, " ", STR_PAD_LEFT),$textColor);
			} else {
				$posY = 460;
