...
$objPHPExcel->getActiveSheet()->setCellValue("D".$contFila, "Ped.");
$objPHPExcel->getActiveSheet()->setCellValue("E".$contFila, "Recib.");
$objPHPExcel->getActiveSheet()->setCellValue("F".$contFila, "Pend.");
$objPHPExcel->getActiveSheet()->setCellValue("G".$contFila, "Costo Unit.");
$objPHPExcel->getActiveSheet()->setCellValue("H".$contFila, "% Impuesto");
$objPHPExcel->getActiveSheet()->setCellValue("I".$contFila, "Total");
$objPHPExcel->getActiveSheet()->setCellValue("J".$contFila, "Almacen");
$objPHPExcel->getActiveSheet()->setCellValue("K".$contFila, "Ubicación");
$objPHPExcel->getActiveSheet()->setCellValue("L".$contFila, "Id Cliente");
...
while ($row = mysql_fetch_assoc($rs)) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D".$contFila, $row['cantidad']);
	$objPHPExcel->getActiveSheet()->setCellValue("E".$contFila, ($row['cantidad'] - $row['pendiente']));
	$objPHPExcel->getActiveSheet()->setCellValue("F".$contFila, "=D".$contFila."-E".$contFila);
	$objPHPExcel->getActiveSheet()->setCellValue("G".$contFila, $row['precio_unitario']);
	$objPHPExcel->getActiveSheet()->setCellValue("H".$contFila, $porcIva);
	$objPHPExcel->getActiveSheet()->setCellValue("I".$contFila, ($row['cantidad'] * $row['precio_unitario']));
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("J".$contFila, $rowUbicPredet['descripcion_almacen'], PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValueExplicit("K".$contFila, str_replace("-[]", "", $rowUbicPredet['ubicacion']), PHPExcel_Cell_DataType::TYPE_STRING);
	$objPHPExcel->getActiveSheet()->setCellValue("L".$contFila, $row['id_cliente']);
	...
	$objPHPExcel->getActiveSheet()->getStyle("G".$contFila)->getNumberFormat()->setFormatCode('"'.$rowPedido['abreviacion_moneda_origen'].'"#,##0.00');
