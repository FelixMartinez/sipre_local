...
while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
	...
	if (fmod($contFila, $maxRows) == 1) {
		...
		imagestring($img,1,190,$posY,str_pad("PED.", 6, " ", STR_PAD_BOTH),$textColor);
		imagestring($img,1,225,$posY,str_pad("ENTREG", 6, " ", STR_PAD_BOTH),$textColor);
		imagestring($img,1,260,$posY,str_pad("PEND.", 6, " ", STR_PAD_BOTH),$textColor);
	...
	imagestring($img,1,190,$posY,str_pad(number_format($cantPedida, 2, ".", ","), 6, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,225,$posY,str_pad(number_format($cantEntregada, 2, ".", ","), 6, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,260,$posY,str_pad(number_format($cantPendiente, 2, ".", ","), 6, " ", STR_PAD_LEFT),$textColor);
...
imagestring($img,1,100,$posY,": ".date("d-m-Y",strtotime($row['fecha'])),$textColor);
imagestring($img,1,235,$posY,"FECHA",$textColor);
imagestring($img,1,335,$posY,": ".date("d-m-Y",strtotime($row['fecha_aprobacion'])),$textColor);
