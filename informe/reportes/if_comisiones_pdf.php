		...
		while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
			...
			if (fmod($contFila, $maxRowsDetalle) == 1) {
				...
				imagestring($img,1,600,$posY+10,
					...
					str_pad(utf8_decode(strtoupper($mes[$rowComision['mes_documento']]." ".$rowComision['ano_documento'])), 20, " ", STR_PAD_BOTH),$textColor);
...
if (isset($arrayImg)) {
	foreach ($arrayImg as $indice => $valor) {
		...
		$pdf->Cell(780,8,"Impreso: ".date(spanDateFormat." h:i a"),0,0,'R');
		$pdf->SetY(-35);
		$pdf->SetTextColor(0,0,0);
