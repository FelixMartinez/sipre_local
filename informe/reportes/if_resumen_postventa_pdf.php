...
while ($row = mysql_fetch_assoc($rs)) {
	...
	$htmlMsj = "(".utf8_encode("Cierre generado el ".date(spanDateFormat, strtotime($row['fecha_creacion']))." a las ".date("h:ia", strtotime($row['fecha_creacion']))." por ".$row['nombre_empleado']).")";
}
...
if (isset($arrayOperador)) {
	foreach ($arrayOperador as $idOperador => $operador) {
		...
		if (isset($arrayProdTipoOrden)) {
			foreach ($arrayProdTipoOrden as $idTipo => $tipo) {
				imagestring($img,1,$posX,$posY,str_pad(formatoNumero(((isset($arrayMO[$idOperador][$idTipo])) ? $arrayMO[$idOperador][$idTipo] : 0), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
				$posX += 75;
			}
		}
...
if (isset($arrayProdTipoOrden)) {
	foreach ($arrayProdTipoOrden as $idTipo => $tipo) {
		...
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($arrayTotalMOTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayTipoArticulo)) {
	foreach ($arrayTipoArticulo as $idTipoArticulo => $tipoArticulo) {
		if ($arrayTotalRepuestoTipo[$idTipoArticulo] != 0) {
			...
			if (isset($arrayProdTipoOrden)) {
				foreach ($arrayProdTipoOrden as $idTipo => $tipo) {
					imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($arrayRepuesto[$idTipoArticulo][$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
					$posX += 75;
				}
			}
...
if (isset($arrayProdTipoOrden)) {
	foreach ($arrayProdTipoOrden as $idTipo => $tipo) {
		...
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($arrayTotalRepuestoTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayProdTipoOrden)) {
	foreach ($arrayProdTipoOrden as $idTipo => $tipo) {
		...
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero((-1)*round($arrayTotalDescuentoRepuestoTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayProdTipoOrden)) {
	foreach ($arrayProdTipoOrden as $idTipo => $tipo) {
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($arrayTotalRepuestoTipoOrden[$idTipo] - $arrayTotalDescuentoRepuestoTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayProdTipoOrden)) {
	foreach ($arrayProdTipoOrden as $idTipo => $tipo) {
		...
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($totalTipoOrden, 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayProdTipoOrden)) {
	foreach ($arrayProdTipoOrden as $idTipo => $tipo) {
		...
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($porcTotalTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayOtroOperador)) {
	foreach ($arrayOtroOperador as $idOperador => $operador) {
		...
		if (isset($arrayOtroProdTipoOrden)) {
			foreach ($arrayOtroProdTipoOrden as $idTipo => $tipo) {
				imagestring($img,1,$posX,$posY,str_pad(formatoNumero(((isset($arrayOtroMO[$idOperador][$idTipo])) ? $arrayOtroMO[$idOperador][$idTipo] : 0), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
				$posX += 75;
			}
		}
...
if (isset($arrayOtroProdTipoOrden)) {
	foreach ($arrayOtroProdTipoOrden as $idTipo => $tipo) {
		...
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($arrayOtroTotalMOTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayOtroTipoArticulo)) {
	foreach ($arrayOtroTipoArticulo as $idTipoArticulo => $tipoArticulo) {
		if ($arrayOtroTotalRepuestoTipo[$idTipoArticulo] != 0) {
			...
			if (isset($arrayOtroProdTipoOrden)) {
				foreach ($arrayOtroProdTipoOrden as $idTipo => $tipo) {
					imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($arrayOtroRepuesto[$idTipoArticulo][$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
					$posX += 75;
				}
			}
...
if (isset($arrayOtroProdTipoOrden)) {
	foreach ($arrayOtroProdTipoOrden as $idTipo => $tipo) {
		...
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($arrayOtroTotalRepuestoTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayOtroProdTipoOrden)) {
	foreach ($arrayOtroProdTipoOrden as $idTipo => $tipo) {
		...
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero((-1)*round($arrayOtroTotalDescuentoRepuestoTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
if (isset($arrayOtroProdTipoOrden)) {
	foreach ($arrayOtroProdTipoOrden as $idTipo => $tipo) {
		imagestring($img,1,$posX,$posY,str_pad(formatoNumero(round($arrayOtroTotalRepuestoTipoOrden[$idTipo] - $arrayOtroTotalDescuentoRepuestoTipoOrden[$idTipo], 2), 1, $nroDecimales), 15, " ", STR_PAD_LEFT),$textColor);
		$posX += 75;
	}
}
...
imagestring($img,1,660,$posY,str_pad(formatoNumero(round($totalExistNroArt, 2), 1, $nroDecimales), 20, " ", STR_PAD_LEFT),$textColor);
...
if (isset($arrayEquipo)) {
	foreach ($arrayEquipo as $indice => $valor) {
		...
		if (isset($arrayTecnico)) {
			foreach ($arrayTecnico as $indice2 => $valor2) {
				...
				imagestring($img,1,360,$posY,str_pad(formatoNumero($arrayFila[2], 1, $nroDecimales), 20, " ", STR_PAD_LEFT),$textColor);
				imagestring($img,1,460,$posY,str_pad(formatoNumero($arrayFila[3], 1, $nroDecimales), 8, " ", STR_PAD_LEFT),$textColor);
		...
		imagestring($img,1,260,$posY,str_pad(formatoNumero($arrayEquipo[$indice][2], 1, $nroDecimales), 20, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,360,$posY,str_pad(formatoNumero($arrayEquipo[$indice][3], 1, $nroDecimales), 20, " ", STR_PAD_LEFT),$textColor);
		imagestring($img,1,460,$posY,str_pad(formatoNumero($porcTotalEquipo, 1, $nroDecimales), 8, " ", STR_PAD_LEFT),$textColor);
	...
	imagestring($img,1,260,$posY,str_pad(formatoNumero($arrayTotalFactTecnicos[0], 1, $nroDecimales), 20, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,360,$posY,str_pad(formatoNumero($arrayTotalFactTecnicos[1], 1, $nroDecimales), 20, " ", STR_PAD_LEFT),$textColor);
	imagestring($img,1,460,$posY,str_pad(formatoNumero($arrayTotalFactTecnicos[2], 1, $nroDecimales), 8, " ", STR_PAD_LEFT),$textColor);
}
