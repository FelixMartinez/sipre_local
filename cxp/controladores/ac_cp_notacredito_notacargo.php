...
function cargarDatosNotaCredito($idDocumento,$valForm){
	...
	$clase = "trResaltar4";
		$queryTipoIva = sprintf("SELECT * FROM pg_iva WHERE idIva = '%s'",$rowIvas['id_iva']);
	...
	$clase = "trResaltar4";
		...
		$queryTipoIva = sprintf("SELECT * FROM pg_iva WHERE idIva = '%s'",$rowGasto['id_iva']);
		$rsTipoIva = mysql_query($queryTipoIva);
		$rowTipoIva = mysql_fetch_array($rsTipoIva);
...
function guardarDatos($valFormFactura,$valFormNotaCredito,$valFormProveedor,$valFormTotalNotaCredito){
	...
	$insertNotaCredito = sprintf("INSERT INTO cp_notacredito (id_proveedor, saldo_notacredito, fecha_notacredito, observacion_notacredito, estado_notacredito, id_documento, tipo_documento, subtotal_notacredito, id_departamento_notacredito, monto_exonerado_notacredito, monto_exento_notacredito, aplica_libros_notacredito, numero_nota_credito, numero_control_notacredito, fecha_registro_notacredito, id_empresa, id_motivo, id_empleado_creador)
	VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%s,%s,%s)",
				...
				$rowFactura['id_motivo'],
				valTpDato($_SESSION['idEmpleadoSysGts'], "int"));
	$queryNotaCredito=mysql_query($insertNotaCredito) or die(mysql_error());
	...
	$queryEstadoCuenta = sprintf("INSERT INTO cp_estado_cuenta(tipoDocumento, idDocumento, fecha, tipoDocumentoN)VALUES('%s','%s','%s','%s')",
				'NC',
				$idNotaCredito,
				$fechaOrigenNotaCredito,
				'4');
	$rsEstadoCuenta = mysql_query($queryEstadoCuenta) or die(mysql_error());
