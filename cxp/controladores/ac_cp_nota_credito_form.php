...
function asignarFacturaCompra($idFacturaCompra, $cerrarVentana = "true") {
	...
	$objResponse->assign("txtFechaFactura","value",implode("-",array_reverse(explode("-",$rowFactura['fecha_factura_proveedor']))));
	$objResponse->assign("txtFechaRegistroFactura","value",implode("-",array_reverse(explode("-",$rowFactura['fecha_origen']))));
...
function formNotaCredito($idNotaCredito, $frmListaPagoDcto, $hddTipo) {
	...
	if ($idNotaCredito > 0) {
		...
		$objResponse->assign("txtFechaRegistroCompra","value",date("d-m-Y",strtotime($rowNotaCredito['fecha_registro_notacredito'])));
		...
		$objResponse->assign("txtFechaProveedor","value",date("d-m-Y",strtotime($rowNotaCredito['fecha_notacredito'])));
		$objResponse->call("selectedOption","lstTipoPago",0);
		...
		byId('btnNotaCreditoPDF').onclick = function() {
			verVentana('reportes/cp_nota_credito_pdf.php?valBusq=".$rowNotaCredito['id_notacredito']."', 960, 550);
		}");
...
function guardarDcto($frmDcto, $frmListaPagoDcto, $frmTotalDcto) {
	...
	if ($idNotaCredito > 0) {
		...
		$insertSQL = sprintf("INSERT INTO cp_notacredito (id_empresa, numero_nota_credito, numero_control_notacredito, fecha_notacredito, id_proveedor, fecha_registro_notacredito, id_departamento_notacredito, estado_notacredito, observacion_notacredito, id_documento, tipo_documento, monto_exento_notacredito, monto_exonerado_notacredito, subtotal_notacredito, subtotal_descuento, total_cuenta_pagar, saldo_notacredito, aplica_libros_notacredito, id_motivo, id_empleado_creador)
		...
		if ($idFacturaCompra > 0) {
			...
			&& ((date("d", strtotime($rowRetencionDetalle['fechaComprobante'])) <= 15 && date("d",strtotime($frmDcto['txtFechaRegistroCompra'])) <= 15)
				|| (date("d", strtotime($rowRetencionDetalle['fechaComprobante'])) > 15 && date("d",strtotime($frmDcto['txtFechaRegistroCompra'])) > 15))
			&& date("m-Y", strtotime($rowRetencionDetalle['fechaComprobante'])) == date("m-Y",strtotime($frmDcto['txtFechaRegistroCompra']))) {
...
function listaFacturaCompra($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\" nowrap=\"nowrap\">".implode("-",array_reverse(explode("-",$row['fecha_origen'])))."</td>";
...
function insertarItemMetodoPago($contFila, $idPago = "", $txtFechaPago = "", $txtMetodoPago = "", $txtIdNumeroDctoPago = "", $txtNumeroDctoPago = "", $txtBancoCompaniaPago = "", $txtCuentaCompaniaPago = "", $txtBancoProveedorPago = "", $txtCuentaProveedorPago = "", $txtMontoPago = "") {
	...
	if ($idPago > 0) {
		...
		$query = sprintf("SELECT *,
			...
			(CASE 
				...
				WHEN tipo_documento_pago LIKE 'NC' THEN
					pago_dcto.tipo_pago
				...
				WHEN tipo_pago LIKE 'NC' THEN
					(CASE tipo_documento_pago
						WHEN 'FA' THEN
							...
							WHERE cxp_fact.id_factura = pago_dcto.id_documento_pago)
						WHEN 'ND' THEN
			...
			(CASE 
				...
				WHEN tipo_pago LIKE 'NC' THEN
					(CASE tipo_documento_pago
						WHEN 'FA' THEN
							(SELECT cxp_fact.observacion_factura FROM cp_factura cxp_fact
							WHERE cxp_fact.id_factura = pago_dcto.id_documento_pago)
						WHEN 'ND' THEN
							(SELECT cxp_nd.observacion_notacargo FROM cp_notadecargo cxp_nd
							WHERE cxp_nd.id_notacargo = pago_dcto.id_documento_pago)
					END)
				WHEN tipo_documento_pago LIKE 'NC' THEN
					(CASE tipo_pago
						WHEN 'AN' THEN
							(SELECT anticipo.observaciones FROM cp_anticipo anticipo
							WHERE anticipo.id_anticipo = pago_dcto.id_documento)
						WHEN 'NC' THEN
	...
	$classMontoPago = ($row['estatus'] == 1) ? "" : "class=\"divMsjError\"";
	...
	switch ($txtMetodoPago) {
		case "RETENCION" :
			$aVerNotaCredito = "<a  href=\"javascript:verVentana(\'../cxp/reportes/an_comprobante_retencion_compra_pdf.php?valBusq=".$row['id_documento']."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Comprobante de Retención\"/><a>"; break;
		case "FA" :
			$aVerNotaCredito = "<a id=\"aVerNotaCredito\" href=\"cp_factura_form.php?id=".$txtIdNumeroDctoPago."&vw=v\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\" title=\"Ver Factura\"/><a>";
			switch ($row['id_modulo']) {
				case 0 : // REPUESTOS
					$aVerNotaCredito .= "<a href=\"javascript:verVentana(\'../repuestos/reportes/iv_registro_compra_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Registro Compra PDF\"/></a>";
					break;
				case 1 : // SERVICIOS
					$aVerNotaCredito .= "<a href=\"javascript:verVentana(\'../servicios/sa_imprimir_registro_compra_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Registro Compra PDF\"/></a>";
					break;
				case 2 : // VEHICULOS
					$aVerNotaCredito .= "<a href=\"javascript:verVentana(\'../vehiculos/reportes/an_registro_compra_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Registro Compra PDF\"/></a>";
					break;
				case 3 : // ADMINISTRACION
					$aVerNotaCredito .= "<a href=\"javascript:verVentana(\'../compras/reportes/ga_registro_compra_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Registro Compra PDF\"/></a>";
					break;
			}
			break;
		case "ND" :
			$aVerNotaCredito = "<a id=\"aVerNotaCredito\" href=\"cp_nota_cargo_form.php?id=".$txtIdNumeroDctoPago."&vw=v\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\" title=\"Ver Nota de Débito\"/><a>";
			$aVerNotaCredito .= "<a href=\"javascript:verVentana(\'../cxp/reportes/cp_nota_cargo_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota de Débito PDF\"/><a>"; break;
	}
