...
function asignarProveedor($idProveedor, $objDestino, $asigDescuento = true) {
	...
	$objResponse->assign("txtNombre".$objDestino,"value",htmlentities($rowProv['nombre']));
...
function formNotaCredito($idNotaCredito, $frmListaPagoDcto) {
	...
	if ($idNotaCredito > 0) {
		...
		$objResponse->assign("txtFecha","value",date("d-m-Y",strtotime($rowNotaCredito['fecha_registro_notacredito'])));
		...
		$objResponse->assign("txtFechaNotaCredito","value",date("d-m-Y",strtotime($rowNotaCredito['fecha_notacredito'])));
		$objResponse->call("selectedOption","lstTipoPago",-1);
		...
		window.onload = function(){
			jQuery(function($){
				$(\"#txtFechaNotaCredito\").maskInput(\"99-99-9999\",{placeholder:\" \"});
			});
