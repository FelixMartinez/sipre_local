...
function calcularDcto($frmDcto, $frmListaPagoDcto, $frmTotalDcto) {
	...
	$txtTotalOrden = $txtSubTotal - $txtSubTotalDescuento + round($totalSubtotalIva, 2) + round($txtGastosConIva, 2) + round($txtGastosSinIva, 2);
...
function formFactura($idFactura, $frmListaPagoDcto) {
	...
	if ($idFactura > 0) {
		...
		$updateSQL = sprintf("UPDATE cp_factura cxp_fact SET
			cxp_fact.saldo_factura = (%s - IFNULL((SELECT SUM(pago_dcto.monto_cancelado) FROM cp_pagos_documentos pago_dcto
		...
		$updateSQL = sprintf("UPDATE cp_factura cxp_fact SET
			estatus_factura = (CASE
								WHEN (ROUND(saldo_factura, 2) <= 0) THEN
									1
								WHEN (ROUND(saldo_factura, 2) > 0 AND ROUND(saldo_factura, 2) < (%s)) THEN
									2
								ELSE
									0
							END)
		WHERE cxp_fact.id_modo_compra IN (2)
			AND cxp_fact.id_factura = %s
			AND (SELECT COUNT(cxp_fact.id_factura) FROM cp_pagos_documentos pago_dcto
				WHERE pago_dcto.tipo_documento_pago LIKE 'FA'
		...
		switch ($rowFactura['id_modulo']) {
			case 0 : // REPUESTOS
				$aVerDcto = sprintf("verVentana('../repuestos/reportes/iv_registro_compra_pdf.php?valBusq=%s', 960, 550);",
					$rowFactura['id_factura']);
				break;
			case 1 : // SERVICIOS
				$aVerDcto = sprintf("verVentana('../servicios/sa_imprimir_registro_compra_pdf.php?valBusq=%s', 960, 550);",
					$rowFactura['id_orden_tot']);
				break;
			case 2 : // VEHICULOS
				$aVerDcto = sprintf("verVentana('../vehiculos/reportes/an_registro_compra_pdf.php?valBusq=%s', 960, 550);",
					$rowFactura['id_factura']);
				break;
			case 3 : // ADMINISTRACION
				$aVerDcto = sprintf("verVentana('../compras/reportes/ga_registro_compra_pdf.php?valBusq=%s', 960, 550);",
					$rowFactura['id_factura']);
				break;
		}
		...
		$objResponse->assign("txtFechaRegistroNotaCargo","value",date("d-m-Y", strtotime($rowNotaCargo['fecha_origen_notacargo'])));
		$objResponse->assign("txtFechaNotaCargo","value",date("d-m-Y", strtotime($rowNotaCargo['fecha_notacargo'])));
		...
		WHERE tipo_documento_pago LIKE 'FA'
		...
		} else if ((date("m-Y",strtotime($rowFactura['fecha_origen'])) == date("m-Y") || in_array($row['estatus_factura'],array(0))) && $_GET['vw'] == "e") {
			$objResponse->script("
...
function guardarDcto($frmDcto, $frmListaPagoDcto, $frmTotalDcto) {
	...
	if ($idFactura > 0) {
		if ((date("m-Y",strtotime($row['fecha_origen'])) == date("m-Y") || in_array($row['estatus_factura'],array(0))) && $_GET['vw'] == "e") {
