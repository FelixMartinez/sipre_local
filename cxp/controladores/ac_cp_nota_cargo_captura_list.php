...
function listaNotaCargo($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	if ($valCadBusq[6] != "-1" && $valCadBusq[6] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("cxp_nd.id_motivo = %s",
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaNotaCargo", "6%", $pageNum, "numero_notacargo", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Nota de Débito");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\" nowrap=\"nowrap\"".((strlen($row['nombre_empleado']) > 0) ? "title=\"Nro. Nota de Débito: ".$row['numero_notacargo'].". Registrado por: ".$row['nombre_empleado']."\"" : "").">".implode("-", array_reverse(explode("-", $row['fecha_origen_notacargo'])))."</td>";
			...
			$htmlTb .= "<td>";
				...
				$htmlTb .= ((strlen($row['observacion_notacargo']) > 0) ? "<tr><td><span class=\"textoNegritaCursiva_9px\">".utf8_encode($row['observacion_notacargo'])."</span></td></tr>" : "");
				$htmlTb .= "</table>";
			$htmlTb .= "</td>";
			...
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"cp_nota_cargo_form.php?id=%s&vw=v\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/></a>",
					$row['id_notacargo']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"javascript:verVentana('reportes/cp_nota_cargo_pdf.php?valBusq=%s', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota de Débito PDF\"/></a>",
					$row['id_notacargo']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
