		...
		WHEN (vw_cxp_as.tipoDocumento IN ('FA','ND')) THEN
			(vw_cxp_as.total_cuenta_pagar - IFNULL((SELECT SUM(cxp_pago.monto_cancelado) FROM cp_pagos_documentos cxp_pago
													WHERE cxp_pago.id_documento_pago = vw_cxp_as.idFactura
														...
														AND (cxp_pago.estatus = 1 OR (cxp_pago.estatus IS NULL AND cxp_pago.fecha_pago <> DATE(fecha_anulado) AND DATE(fecha_anulado) >= %s))),0))
	...
	END) > 0
		AND NOT ((CASE
				WHEN (vw_cxp_as.tipoDocumento IN ('FA','ND')) THEN
					...
					WHERE cxp_pago.id_documento_pago = vw_cxp_as.idFactura
						AND CONVERT(cxp_pago.tipo_documento_pago, CHAR) = CONVERT(vw_cxp_as.tipoDocumento, CHAR)
						AND (cxp_pago.estatus = 1 OR (cxp_pago.estatus IS NULL AND cxp_pago.fecha_pago <> DATE(fecha_anulado) AND DATE(fecha_anulado) >= %s))) 
				WHEN (vw_cxp_as.tipoDocumento IN ('AN','NC')) THEN
					(SELECT MAX(cxp_pago.fecha_pago) FROM cp_pagos_documentos cxp_pago
					WHERE cxp_pago.id_documento = vw_cxp_as.idFactura
						AND CONVERT(cxp_pago.tipo_pago, CHAR) = CONVERT(vw_cxp_as.tipoDocumento, CHAR)
...
if ($valCadBusq[6] != "-1" && $valCadBusq[6] != "") {
	$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
	$sqlBusq .= $cond.sprintf("vw_cxp_as.tipoDocumentoN IN (%s)",
...
if (in_array($valCadBusq[3],array(1))) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("I".$contFilaY, "Proveedor");
...
while ($row = mysql_fetch_assoc($rs)) {
	...
	if (in_array($valCadBusq[3],array(1))) {
		...
		} else if (in_array($row['tipoDocumentoN'],array(3,4))){ // 3 = Anticipo, 4 = Nota de Crédito
			$totalSaldo -= $row['total_cuenta_pagar'] - $row['total_pagos'];
		}
		...
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("D".$contFilaY, date("d-m-Y",strtotime($row['fecha_origen'])), PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("E".$contFilaY, date("d-m-Y",strtotime($row['fecha_vencimiento'])), PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("H".$contFilaY, $row['id_proveedor'], PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("I".$contFilaY, utf8_encode($row['nombre_proveedor']), PHPExcel_Cell_DataType::TYPE_STRING);
		...
		if ($valCadBusq[6] != "-1" && $valCadBusq[6] != "") {
			$cond = (strlen($sqlBusq2) > 0) ? " AND " : " WHERE ";
			$sqlBusq2 .= $cond.sprintf("vw_cxp_as.tipoDocumentoN IN (%s)",
				valTpDato($valCadBusq[6], "campo"));
		}
		...
		while ($rowEstado = mysql_fetch_array($rsEstado)) {
			...
			} else if (in_array($rowEstado['tipoDocumentoN'],array(3,4))){ // 3 = Anticipo, 4 = Nota de Crédito
				$totalSaldo -= $rowEstado['total_cuenta_pagar'] - $rowEstado['total_pagos'];
			}
			...
			if (in_array($valCadBusq[3],array(3,4))) { // 3 = General por Proveedor, 4 = General por Dcto.
				...
				$objPHPExcel->getActiveSheet()->setCellValueExplicit(($contColum++).$contFilaY, date("d-m-Y",strtotime($rowEstado['fecha_origen'])), PHPExcel_Cell_DataType::TYPE_STRING);
				$objPHPExcel->getActiveSheet()->setCellValueExplicit(($contColum++).$contFilaY, date("d-m-Y",strtotime($rowEstado['fecha_vencimiento'])), PHPExcel_Cell_DataType::TYPE_STRING);
...
cabeceraExcel($objPHPExcel, $idEmpresa, (in_array($valCadBusq[3],array(1))) ? "O" : ($contColumUlt));
