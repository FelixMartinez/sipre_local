...
while ($row = mysql_fetch_assoc($rs)){
	...
	while ($rowDet = mysql_fetch_assoc($rsDet)) {
		...
		imagestring($img,1,65,$posYDet,str_pad(implode("-",array_reverse(explode("-",$rowDet['fechaFactura']))), 12, " ", STR_PAD_BOTH),$textColor); // <----
	...
	imageline($img,0,$posY,1040,$posY,$textColor);
	imageline($img,1050,$posY,1130,$posY,$textColor);
	imagestring($img,1,800,$posY+5,str_pad(number_format($totBaseImponible, 2, ".", ","), 16, " ", STR_PAD_LEFT),$textColor); // <----
	...
	//Nombre admin en Firma y sello
	$queryAdmin = sprintf("SELECT
		id_empleado,
		nombre_empleado,
		apellido
	FROM pg_empleado
	WHERE id_cargo_departamento = 122 AND activo = 1");
	...
	$posY += 10;
	/*$nombreAdmin = strtoupper($rowAdmin['nombre_empleado']." ".$rowAdmin['apellido']);
	imagestring($img,2,70,$posY-30,$nombreAdmin,$textColor);
	imagestring($img,2,70,$posY-30,$nombreAdmin,$textColor);*/
...
if (isset($arrayImg)) {
	foreach ($arrayImg as $indice => $valor) {
		...
		//$pdf->Image("../img/logos/firma_sello_admin.png",40,470,75);
	}
}
