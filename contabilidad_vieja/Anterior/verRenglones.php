...
for($i = 0;$i <= $NumEle - 1; $i++){
		...
		$sCampos = "codigo";
		$j = $i + 1;
		$sql = "Select " .$sCampos. " from oriomka_inavi.cuenta where " .$sCondicion ;
		...
		$sCampos = "codigo";
		$j = $i + 1;
		$sql = "Select " .$sCampos. " from oriomka_inavi.transacciones where " .$sCondicion ;
		$rs1 = EjecutarExec($con,$sql) or die($sql) ; 
		if (NumeroFilas($rs1) == 0){
		...
		$sCampos = "codigo";
		$j = $i + 1;
		$sql = "Select " .$sCampos. " from oriomka_inavi.documentos where " .$sCondicion ;
		$rs1 = EjecutarExec($con,$sql) or die($sql) ; 
		if (NumeroFilas($rs1) == 0){
		...
		$sCampos = "codigo";
		$j = $i + 1;
		$sql = "Select " .$sCampos. " from oriomka_inavi.centrocosto where " .$sCondicion ;
		$rs1 = EjecutarExec($con,$sql) or die($sql) ; 
		if (NumeroFilas($rs1) == 0){
