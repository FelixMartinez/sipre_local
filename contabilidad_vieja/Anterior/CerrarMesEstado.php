...
/* para verificar si es comporbante de cierre */
 $con = ConectarBD();
   $SqlStr="Select fec_proceso,cierrea from oriomka_inavi.parametros ";
        $exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
        if ( NumeroFilas($exc)>0){
...
echo "paso1";
$SqlStr = "insert into oriomka_inavi.$TablaPrincipalEnc
   select * from $Cod.enc_diario";
$exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
echo "paso2";

$SqlStr = "insert into oriomka_inavi.$TablaPrincipalDet
select * from $Cod.movimien";
$exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
...
/*asignar que el mes ya esta cerrado*/
$SqlStr = " Insert Into adcontabilidad.comcerrada (mes,ano,codigo,debe,haber) values ($Mes,$Ano,'$Cod',$TDebe,$THaber)";
$exc = EjecutarExec($con,$SqlStr) or die($SqlStr);
/*fin asignar que el mes ya esta cerrado*/
