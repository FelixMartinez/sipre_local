...
$conAd = ConectarBD();
					...
					oriomka_inavi.movimien  group by comprobant,cc,fecha having round(sum(Debe),2) <> round(Sum(Haber),2)
					union all
					select comprobant,cc,fecha,sum(Debe),Sum(Haber) from 
					oriomka_inavi.movimiendif group by comprobant,cc,fecha having round(sum(Debe),2) <> round(Sum(Haber),2)
					 order by fecha" ; 
