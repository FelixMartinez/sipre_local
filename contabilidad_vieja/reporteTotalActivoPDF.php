...
//Cabecera de página
	function Header(){   
		...
		$SqlEmp = "SELECT id_empresa, nombre_empresa,logo_empresa,rif,web FROM erp_automotriz.pg_empresa WHERE id_empresa = 1";
		//echo $SqlEmp."<br>";
...
//Pie de página
	function Footer(){
		...
		$sqlUser = "SELECT * FROM adcontabilidad.usuario WHERe nombre = '".$_SESSION["UsuarioSistema"]."' ;";
...
//CREO EL OBJETO SEGUN LA ORIENTACION 
	...
	switch($_GET['orientacion']){
		case 'P'://LISTADO DE ACTIVO
			...
			$sqlListAct = "SELECT * FROM oriomka_inavi.deprecactivos ORDER BY Codigo ASC;";
		...
		case 'L': //DEPRECIACION ACTIVO
			...
			deprecactivos.MesesDepre AS Vida_Util,
			  #CALCULO DE LA VUP_ANTERIOR
						  (SELECT COUNT(a.id) FROM oriomka_inavi.con_depreciacion a
						  WHERE a.anomes <= DATE_FORMAT(
							(SELECT DATE_SUB(Fechacomp_cierr, INTERVAL 1 YEAR) FROM oriomka_inavi.parametros  limit 1), '%Y%m')
							AND a.anomes >= DATE_FORMAT(
							  (SELECT DATE_SUB(Fechacomp_cierr, INTERVAL 2 YEAR) FROM oriomka_inavi.parametros limit 1), '%Y%m')
							  AND a.codigoactivos = deprecactivos.Codigo) AS VUP_ANTERIOR,
			  #CALCULO DE LA VUP_ACTUAL
						(SELECT COUNT(b.anomes) FROM oriomka_inavi.con_depreciacion b
						  WHERE b.anomes >= DATE_FORMAT(FechaDepre,'%Y%m')
						 AND b.anomes <= DATE_FORMAT(
						   (SELECT Fechacomp_cierr FROM oriomka_inavi.parametros limit 1), '%Y%m')
			  ...
			  #CALCULO DE LA VU_RESTANTE = (Vida_Util - VUP_ANTERIOR - VIDA UTIL P ACTUAL)
					   (deprecactivos.MesesDepre - (SELECT COUNT(b.anomes) FROM oriomka_inavi.con_depreciacion b
										 WHERE b.anomes >= DATE_FORMAT(FechaDepre,'%Y%m')
										 AND b.anomes <= DATE_FORMAT(
										   (SELECT Fechacomp_cierr FROM oriomka_inavi.parametros limit 1), '%Y%m')
			 ...
			 #CALCULO DE GSTO ANUAL
					  ...
					  (SELECT DATE_FORMAT(Fechacomp_cierr, '%Y')
						FROM oriomka_inavi.parametros LIMIT 1), 12,COUNT(SUBSTRING(anomes,5))) AS GSTO_ANUAL,
			 ...
			 #CALCULO DEL PERIODO = (GSTO_ANUAL * DEP_MENSUAL)
					  ROUND(((IF(DATE_FORMAT(Fecha, '%Y') < (
						SELECT DATE_FORMAT(Fechacomp_cierr, '%Y')
						  FROM oriomka_inavi.parametros LIMIT 1), 12,COUNT(SUBSTRING(anomes,5))))*DepreMensual),2) AS DE_PERIODO
					FROM oriomka_inavi.deprecactivos
						LEFT JOIN oriomka_inavi.con_depreciacion ON deprecactivos.Codigo = con_depreciacion.codigoactivos
						LEFT JOIN  oriomka_inavi.tipoactivo ON deprecactivos.tipo = tipoactivo.Codigo
						WHERE estatus = 0
					AND SUBSTRING(anomes,1,4) <= YEAR((SELECT Fechacomp_cierr FROM oriomka_inavi.parametros LIMIT 1))
					AND SUBSTRING(anomes,5) <= MONTH((SELECT Fechacomp_cierr FROM oriomka_inavi.parametros LIMIT 1))
