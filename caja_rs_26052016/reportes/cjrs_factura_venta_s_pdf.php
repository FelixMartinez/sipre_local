	...
	while ($contFila<1) {
		...
		$query = sprintf("SELECT 
			...
			FROM cj_cc_cliente
				...
				LEFT OUTER JOIN sa_retrabajo_orden ON (vw_sa_orden.id_orden = sa_retrabajo_orden.id_orden) %s ORDER BY numero_orden DESC", $sqlBusq);
		...
		while ($rowDetalle = mysql_fetch_assoc($rsDetalle)){
			...
			$pdf->Cell($arrayTamCol[0],12,date("d-m-Y", strtotime($rowDetalle['tiempo_orden_registro'])),'LR',0,'C',true);
