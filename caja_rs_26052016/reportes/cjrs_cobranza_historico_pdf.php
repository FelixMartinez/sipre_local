...
if ($totalRows > 0) {
	...
	while ($row = mysql_fetch_assoc($rs)) {
		...
		$pdf->Cell($arrayCol[3]['tamano'],14,date("d-m-Y", strtotime($row['fechaRegistroFactura'])),'LR',0,'C',true);
		...
		$pdf->Cell($arrayCol[8]['tamano'],14,date("d-m-Y", strtotime($row['fechapago'])),'LR',0,'C',true);
