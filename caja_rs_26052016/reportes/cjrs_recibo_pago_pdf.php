...
while ($rowRecibo = mysql_fetch_assoc($rsRecibo)) {
			...
			$queryPago = sprintf("SELECT q.*,
				...
				(CASE q.formaPago
					WHEN 8 THEN
						...
						WHERE cxc_nc.idNotaCredito = q.numeroDocumento)
	...
	$pdf->Cell(90,14,date("d-m-Y",strtotime($rowRecibo['fechaComprobante'])),0,0,'C');
	$pdf->Ln();
	...
	$pdf->Cell(370,14,utf8_decode("ID: ".$rowFactura['id']),0,0,'L');
	...
	while ($rowPago = mysql_fetch_assoc($rsPago)) {
		...
		$pdf->SetAligns(array(
			...
			"R"));
		$pdf->Row(array(
			date("d-m-Y", strtotime($rowPago['fechaPago'])),
		...
		/*$pdf->Cell($arrayCol[0]['tamano'],14,date("d-m-Y", strtotime($rowPago['fechaPago'])),'LR',0,'C',true);
	...
	$pdf->Cell(20,5,"",0,0,'L');
	$pdf->MultiCell(570, 15, "LA CANTIDAD DE: ".utf8_decode(strtoupper(num2letras($arrayTotal[9],false,true,$rowMoneda['descripcion']))), 0, 'L');
	$pdf->Ln();
