...
function eliminarPago($idPago, $tablaPago, $frmListaPago) {
	...
	if (!xvalidaAcceso($objResponse,"cjrs_eliminar_pago_list","eliminar")) { return $objResponse; }
			...
			$htmlTb .= "<td>";
				switch ($row['tabla']) {
					...
					case "cj_det_nota_cargo" : 
						...
						break;
					case "cj_cc_detalleanticipo" : 
						$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('reportes/cjrs_recibo_impresion_pdf.php?idRecibo=%s', 960, 550);\" src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/>",
							$row['id_recibo_pago']);
						break;
				}
			$htmlTb .= "</td>";
...
function validarPermiso($frmPermiso, $frmDatosArticulo) {
	...
	if ($totalRowsPermiso > 0) {
		if ($frmPermiso['hddModulo'] == "cj_rs_eliminar_pagos") {
