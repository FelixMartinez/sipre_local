...
function cargarDcto($idNotaCargo){
	...
	$sqlFactura = sprintf("SELECT cxc_nd.* FROM cj_cc_notadecargo cxc_nd
	WHERE cxc_nd.idNotaCargo = %s", $idNotaCargo);
	...
	$objResponse->assign("txtFecha","value",date("d-m-Y",strtotime($rowFactura['fechaRegistroNotaCargo'])));
	$objResponse->assign("txtFechaVencimiento","value",date("d-m-Y",strtotime($rowFactura['fechaVencimientoNotaCargo'])));
...
function guardarDcto($frmDcto, $frmDetallePago, $frmListaPagos){
	...
	if (!xvalidaAcceso($objResponse,"cjrs_nota_cargo_por_pagar_list","insertar")) { return $objResponse; }
