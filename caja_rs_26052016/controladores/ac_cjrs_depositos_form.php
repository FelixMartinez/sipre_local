...
function depositarPlanilla($frmPlanilla){
	...
	if (!xvalidaAcceso($objResponse,"cjrs_depositos_form")){ return $objResponse; }
	...
	VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
		...
		valTpDato(0, "real_inglesa"),
		valTpDato(0, "real_inglesa"),
		valTpDato("Deposito Cierre ".$nombreCajaPpal." ".$fecha, "text"),
	...
	if (in_array($idCajaPpal,array(1))) { // 1 = CAJA DE VEHICULOS, 2 = CAJA DE REPUESTOS Y SERVICIOS
		if (function_exists("generarDepositosTeVe")) { generarDepositosTeVe($idCabeceraPlanillaTesoreria,"",""); } 
	} else {
		...
		foreach($frmPagos['cbxItm'] as $indice => $valor){
			...
			$objResponse->script(sprintf("
				...
				$contFila, $clase, $contFila,
					$contFila, $contFila,
					date("d-m-Y",strtotime($rowDetallePago['fechaPago'])),
