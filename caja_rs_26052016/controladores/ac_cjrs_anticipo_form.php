...
function calcularPagos($frmListaPagos, $frmListaDctoPagado, $frmDcto){
	...
	if (isset($arrayObj2)) {
		...
		foreach ($arrayObj2 as $indice => $valor) {
			...
			if (!in_array($frmListaPagos['txtIdConceptoPago'.$valor], array(6,7,8))) {
				$txtMontoPagadoAnticipo += str_replace(",", "", $frmListaPagos['txtMonto'.$valor]);
			}
		}
	}
    ...
    $objResponse->assign("txtTotalDctoPagados","value",number_format($txtTotalDctoPagadosAnticipo, 2, ".", ","));
...
function cargaLstConceptoPago($nombreObjeto){
	...
	WHERE id_formapago = 11
		AND id_concepto NOT IN (2)
		AND estatus = 1
...
function cargarDcto($idAnticipo, $vw) {
	...
	if ($idAnticipo > 0) {
		...
		switch($rowAnticipo['estadoAnticipo']) { // 0 = No Cancelado, 1 = Cancelado (No Asignado), 2 = Parcialmente Asignado, 3 = Asignado, 4 = No Cancelado (Asignado)
			...
			default : $objResponse->loadCommands(cargaLstTipoPago("1,2,3,4,5,6","1")); break;
		}
...
function guardarAnticipo($frmDcto, $frmDetallePago, $frmListaPagos, $frmListaDctoPagado){
	...
	if (!xvalidaAcceso($objResponse,"cjrs_anticipo_list","insertar")) { return $objResponse; }
				...
				if ($rowPago['idCierre'] > 0) {
					if ($rowPago['tipo_documento'] == 1) { // 1 = FA, 2 = ND, 3 = NC, 4 = AN, 5 = CH, 6 = TB (Relacion Tabla tipodedocumentos)
						...
						if (in_array($idCaja,array(1))) { // 1 = CAJA DE VEHICULOS, 2 = CAJA DE REPUESTOS Y SERVICIOS
							...
							$insertSQL = sprintf("INSERT INTO an_pagos (id_factura, fechaPago, formaPago, numeroDocumento, bancoOrigen, bancoDestino, cuentaEmpresa, montoPagado, numeroFactura, tipoCheque, tomadoEnComprobante, tomadoEnCierre, idCaja, idCierre, estatus, id_encabezado_v)
						...
						// INSERTA EL NUEVO PAGO CON LA APERTURA DE CAJA ACTUAL
						$insertSQL = sprintf("INSERT INTO cj_det_nota_cargo (idNotaCargo, fechaPago, idFormaPago, numeroDocumento, bancoOrigen, bancoDestino, cuentaEmpresa, monto_pago, tipoCheque, tomadoEnComprobante, tomadoEnCierre, idCaja, idCierre, estatus, id_encabezado_nc)
...
function insertarPago($frmListaPagos, $frmDetallePago, $frmDeposito, $frmLista, $frmDcto){
	...
	$Result1 = insertarItemMetodoPago($contFila, "", $idFormaPago, $txtIdNumeroDctoPago, $txtNumeroDctoPago, $txtIdBancoCliente, $txtCuentaClientePago, $txtIdBancoCompania, $txtIdCuentaCompaniaPago, $txtFechaDeposito, $lstTipoTarjeta, $txtIdConceptoPago, $txtMontoPago);
	if ($Result1[0] != true && strlen($Result1[1]) > 0) {
		return $objResponse->alert($Result1[1]);
	} else if ($Result1[0] == true) {
		$contFila = $Result1[2];
...
function insertarItemMetodoPago($contFila, $hddIdPago, $idFormaPago = "", $txtIdNumeroDctoPago = "", $txtNumeroDctoPago = "", $txtIdBancoCliente = "", $txtCuentaClientePago = "", $txtIdBancoCompania = "", $txtIdCuentaCompaniaPago = "", $txtFechaDeposito = "", $lstTipoTarjeta = "", $txtIdConceptoPago = "", $txtMontoPago = "") {
	$clase = (fmod($contFila, 2) == 0) ? "trResaltar4" : "trResaltar5";
	$contFila++;
	...
	$htmlItmPie = sprintf("$('#trItmPiePago').before('".
		"<tr align=\"left\" id=\"trItmPago:%s\" class=\"textoGris_11px %s\">".
			...
			"<td>%s".
				...
				"<input type=\"hidden\" id=\"txtIdConceptoPago%s\" name=\"txtIdConceptoPago%s\" readonly=\"readonly\" value=\"%s\" title=\"txtIdConceptoPago\"/></td>".
		"</tr>');",
		$contFila, $clase,
			$contFila, $contFila,
				$contFila,
