...
<head>
	...
	<title>.: SIPRE <?php echo cVERSION; ?> :. Caja de Repuestos y Servicios - Histórico de Cierre</title>
	...
	function validarGuardarAprobacion() {
		if (confirm('¿Seguro desea aprobar esta caja?') == true) {
			byId('btnGuardarAprobacion').disabled = true;
			byId('btnCancelar').disabled = true;
			
			xajax_guardarAprobacion(xajax.getFormValues('frmVerificacion'));
		}
	}
	
	function validarGuardarValidacion() {
		if (confirm('¿Seguro desea validar esta caja?') == true) {
			byId('btnGuardarValidacion').disabled = true;
			byId('btnCancelar').disabled = true;
			
			xajax_guardarValidacion(xajax.getFormValues('frmVerificacion'));
		}
	}
	
	function validarGuardarVerificacionPortadaCaja() {
	...
	<div id="divInfo" class="print" style="vertical-align:middle">
		...
		<tr>
			<td align="right">
				...
				<tr>
					<td>
						<button type="button" onclick="xajax_exportarListadoCierre(xajax.getFormValues('frmBuscar'));" style="cursor:default"><table align="center" cellpadding="0" cellspacing="0"><tr><td>&nbsp;</td><td><img src="../img/iconos/page_excel.png"/></td><td>&nbsp;</td><td>XLS</td></tr></table></button>
					</td>
				</tr>
				</table>
				...
				<tr align="left">
					...
					<td align="left" id="tdVerificacion"><label>
						<select id="slctVerificacion" name="slctVerificacion" class="inputHabilitado" onchange="byId('btnBuscar').click();">
							...
							<option value="1">Caja Verificada</option>
						</select></label>
					</td>
					<td>
...
<form id="frmVerificacionPortadaCaja" name="frmVerificacionPortadaCaja" onsubmit="return false;" style="margin:0">
	...
	<tr>
		<td align="right" colspan="4"><hr>
			...
			<button type="button" id="btnGuardarVerificacionPortadaCaja" name="btnGuardarVerificacionPortadaCaja" onclick="validarGuardarVerificacionPortadaCaja();"><table align="center" cellpadding="0" cellspacing="0"><tr><td>Aceptar</td></tr></table></button>
...
xajax_listaCierre(0,'apertura.fechaAperturaCaja','DESC','<?php echo $_SESSION['idEmpresaUsuarioSysGts']; ?>|' + byId('txtFechaDesde').value + '|' + byId('txtFechaHasta').value);
</script>