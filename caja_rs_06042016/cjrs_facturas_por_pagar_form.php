...
<head>
	...
	<title>.: SIPRE <?php echo cVERSION; ?> :. Caja de Repuestos y Servicios - Pago de Facturas</title>
	...
	function anticipo(){
		...
		$('btnAgregarDetDeposito').style.display = 'none';
		$('agregar').style.display = 'none';
		$('btnAgregarDetAnticipoNotaCredito').style.display = '';
	}
	...
	function validar(){
		error = false;
		if ($('selTipoPago').value == 1){/*EFECTIVO*/
			if (!(validarCampo('txtSaldoFactura','t','monto') == true
			&& validarCampo('montoPago','t','monto') == true)){
				validarCampo('txtSaldoFactura','t','monto');
			...
			if (validarCampo('txtSaldoFactura','t','monto') == true
			 ...
			 && validarCampo('numeroControl','t','') == true){
				...
				validarCampo('txtSaldoFactura','t','monto') == true
				validarCampo('montoPago','t','monto');
			...
			&& validarCampo('numeroControl','t','') == true
			&& validarCampo('porcentajeRetencion','t','monto') == true
			...
			&& validarCampo('montoTotalComision','t','numPositivo') == true)){
				...
				validarCampo('numeroControl','t','');
				validarCampo('porcentajeRetencion','t','monto');
				validarCampo('montoTotalRetencion','t','monto');
	...
	function calcularTotal(){
		...
		$('txtMontoPagadoFactura').value = formato(parsenum(pagos));
	...
	function validarGuardar(){
		...
		if (error == true) {
			...
			byId('btnGuardarPago').disabled = true;
			byId('btnCancelar').disabled = true;
			xajax_guardarPago(xajax.getFormValues('frmDetallePago'),xajax.getFormValues('frmListadoPagos'),xajax.getFormValues('frmDcto'));
		}		
	}
						...
						<tr>
							...
							<fieldset><legend class="legend">Datos de la Factura</legend>
								...
								<tr>
									...
									<td align="left"><input type="text" id="txtNumeroControlFactura" name="txtNumeroControlFactura" size="20" style="color:#F00; font-weight:bold; text-align:center;" readonly="readonly"/></td>
								</tr>
								<tr>
				...
				<table border="0" width="100%">
					...
					<tr>
						...
						<td width="28" id="tdImgAgregarFormaAnticipoNotaCredito">
							<button style="display:none" type="button" id="btnAgregarDetAnticipoNotaCredito" name="btnAgregarDetAnticipoNotaCredito" onClick="$('btnBuscarAnticipoNotaCredito').click();" title="Buscar Documento"><img src="../img/iconos/find.png" width="16" height="16"/></button>
						</td>
					</tr>
					<tr>
...
<div id="divFlotante" class="root" style="cursor:auto; display:none; left:0px; position:absolute; top:0px; z-index:0;">
	...
	<tr id="trBuscarAnticipoNotaCredito">
		<td>
			<form id="frmBuscarAnticipoNotaCredito" name="frmBuscarAnticipoNotaCredito" onsubmit="$('btnBuscarAnticipoNotaCredito').click(); return false;" style="margin:0">
			<table align="right">
			<tr>
				<td align="right" class="tituloCampo" width="120">Criterio:</td>
				<td><input type="text" id="txtCriterioAnticipoNotaCredito" name="txtCriterioAnticipoNotaCredito" onkeyup="$('btnBuscarAnticipoNotaCredito').click();"/></td>
				<td>
					<button type="submit" id="btnBuscarAnticipoNotaCredito" name="btnBuscarAnticipoNotaCredito" onclick="xajax_buscarAnticipoNotaCredito(xajax.getFormValues('frmBuscarAnticipoNotaCredito'),$('txtIdCliente').value,$('selTipoPago').value,xajax.getFormValues('frmDetallePago'),xajax.getFormValues('frmListadoPagos'));">Buscar</button>
...
xajax_cargarFactura(<?php echo $_GET['id_factura'] ?>);
