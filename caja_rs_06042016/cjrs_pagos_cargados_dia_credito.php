...
<head>
	...
	<title>.: SIPRE 2.0 :. Caja de Repuestos y Servicios - Histórico de Recibos por Medio de Pago/Crédito</title>
...
<div id="divGeneralPorcentaje">
	...
	<tr>
		...
		if ($_GET['acc'] == 2)
			...
			echo "<td align='center' colspan='2'><strong>PAGOS CARGADOS DEL DÍA - CRÉDITO (REPUESTOS Y SERVICIOS)</strong></td>";
		?>
	</tr>
	</table>	
	<br>
...
if ($rowConfig400['valor'] == 0) { // MOSTRAR POR SUCURSALES
	$andEmpresa1 = sprintf(" AND sa_iv_apertura.id_empresa = %s",
		valTpDato($idEmpresa,"int"));
	$andEmpresa2 = sprintf(" AND vw_pagos_del_dia_rs_credito.id_empresa = %s",
		valTpDato($idEmpresa,"int"));
...
$sqlFormaPago = "SELECT 
		vw_pagos_del_dia_rs_credito.formaPago,
		vw_pagos_del_dia_rs_credito.tipoDoc,
		vw_pagos_del_dia_rs_credito.id_empresa,
		formapagos.nombreFormaPago
	FROM vw_pagos_del_dia_rs_credito,
		formapagos
	WHERE vw_pagos_del_dia_rs_credito.formaPago = formapagos.idFormaPago
	".$andEmpresa2."
	GROUP BY formaPago, vw_pagos_del_dia_rs_credito.id_empresa
	ORDER BY vw_pagos_del_dia_rs_credito.formaPago";
...
while ($rowFormaPago = mysql_fetch_array($consultaFormaPago)) {
		...
		if ($numReg > 0) {
			...
			while ($lafila = mysql_fetch_array($consultaMostrarPorFormaPago)) {
				...
				<tr class="<?php echo $clase; ?>" onmouseover="this.className = 'trSobre';" onmouseout="this.className = '<?php echo $clase; ?>';" height="22">
					<td align="center"><?php echo $lafila["tipoDoc"]; ?></td>
	...
	<tr align="right" height="22">
		<td class="tituloColumna" width="94%">Total Crédito:</td>
