...
<head>
	...
	<title>.: SIPRE 2.0 :. Caja de Repuestos y Servicios - Histórico de Recibos por Medio de Pago</title>
...
<div id="divGeneralPorcentaje">
	...
	<tr>
		...
		if ($_GET['acc'] == 2)
			...
			echo "<td align='center' colspan='2'><strong>PAGOS CARGADOS DEL DÍA (REPUESTOS Y SERVICIOS)</strong></td>";
		?>
	</tr>
	</table>	
	<br>
...
if ($rowConfig400['valor'] == 0) { // MOSTRAR POR SUCURSALES
	$andEmpresa = sprintf(" AND vw_forma_pagos_formato_corte_caja_rs.id_empresa = %s",
		valTpDato($idEmpresa,"int"));
	$andEmpresa2 = sprintf(" AND sa_iv_apertura.id_empresa = %s",
		valTpDato($idEmpresa,"int"));
	$andEmpresa3 = sprintf(" AND vw_pagos_del_dia_rs.id_empresa = %s",
		valTpDato($idEmpresa,"int"));
...
if ($_GET['acc'] == 2){
	...
	WHERE idCaja = 2
		AND statusAperturaCaja IN(1,2) ".$andEmpresa2."");
	...
	$sqlFormaPago = "SELECT 
		vw_pagos_del_dia_rs.formaPago,
		vw_pagos_del_dia_rs.tipoDoc,
		vw_pagos_del_dia_rs.id_empresa,
		formapagos.nombreFormaPago
	FROM vw_pagos_del_dia_rs,
		formapagos
	WHERE vw_pagos_del_dia_rs.formaPago = formapagos.idFormaPago
	".$andEmpresa3."
	GROUP BY formaPago, vw_pagos_del_dia_rs.id_empresa
	ORDER BY vw_pagos_del_dia_rs.formaPago";
