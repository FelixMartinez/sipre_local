...
<head>
	...
	<title>.: SIPRE 2.0 :. Caja de Repuestos y Servicios - Corte de Caja</title>
				...
				<form id="form1" name="form1">
					...
					<tr>
						<td colspan="5" class="noprint" bgcolor="#FFFFFF" scope="row" style="text-align:right">
							...
							<button type="button" id="btnCierreTotal" name="btnCierreTotal" onclick="window.location.href = 'cjrs_pagos_cargados_dia.php?acc=2'"><table align="center" cellpadding="0" cellspacing="0"><tr><td>&nbsp;</td><td><img src="../img/iconos/key_go.png"/></td><td>&nbsp;</td><td>Cierre Total</td></tr></table></button>
	...
	while($registro = mysql_fetch_array($consultaFormaPago)){
		...
		WHERE
		 ...
		 $sqlMostrarPorFormaPago = "SELECT
			...
			SELECT
				sum(pa.monto_pago) AS montoPagado
			FROM
				cj_det_nota_cargo pa,
				cj_cc_notadecargo fv
			WHERE
				...
				AND fv.idDepartamentoOrigenNotaCargo IN (0,1,3)
				".$andEmpresaSql2."
				AND pa.idFormaPago NOT IN (7,8)
				AND fv.fechaRegistroNotaCargo <> ".$fechaApertura."";
		$consultaMostrarPorFormaPago = mysql_query($sqlMostrarPorFormaPago)or die(mysql_error().$sqlMostrarPorFormaPago);
