...
function guardarDcto($frmDcto, $frmListaArticulo, $frmTotalDcto, $bloquearForm = "") {
	...
	if (!xvalidaAcceso($objResponse,"cjrs_devolucion_admon_form","insertar")) { errorGuardarDcto($objResponse); return $objResponse; }
	...
	if (isset($arrayObj)) {
		foreach ($arrayObj as $indice => $valor) {
			if (strlen($frmListaArticulo['hddIdFactDet'.$valor]) > 0) {
				...
				if (!$rsFactDet) { errorGuardarDcto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	if (isset($arrayObjGasto)) {
		foreach ($arrayObjGasto as $indice => $valor) {
			...
			if (!$rsGastoFact) { errorGuardarDcto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
			...
			if (!$rsGastoNotaCred) { errorGuardarDcto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
	...
	if (in_array($rowFact['estadoFactura'],array(0,2))) { // 0 = No Cancelado, 2 = Parcialmente Cancelado
		...
		if ($rowFact['estadoFactura'] == 0) { // 0 = No Cancelado
			if ($rowFact['saldoFactura'] == str_replace(",","",$frmTotalDcto['txtTotalFactura'])) {
				$saldoFactura = str_replace(",","",$frmTotalDcto['txtTotalFactura']);
...
function insertarItemArticulo($contFila, $hddIdFacturaVentaDetalle = "", $idConcepto = "", $txtCantItm = "", $txtPrecioItm = "", $txtCostoItm = "", $abrevMonedaCostoUnitario = "", $idIva = "") {
	...
	$htmlItmPie = sprintf("$('#trItmPie').before('".
		"<tr align=\"left\" id=\"trItmArticulo:%s\" class=\"textoGris_11px %s\">".
			...
			"<td>%s</td>".
			"<td><input type=\"text\" id=\"txtDescItm%s\" name=\"txtDescItm%s\" class=\"inputSinFondo\" maxlength=\"44\" readonly=\"readonly\" style=\"text-align:left\" value=\"%s\"/></td>".
