...
function guardarNotaCargo($numeroCheque,$monto,$idCliente,$idBanco){
	...
	if ($totalRowsCheque > 0) {
		...
		// NUMERACION DEL DOCUMENTO (Nota Cargo Repuestos y Servicios)
		...
		VALUES (%s, NOW(), %s, NOW(), %s, %s, 0, %s, 0, %s, 0, 0, 0, 0, %s, 0, 0, 0, 0, 0, 0, %s, 0, 0, %s, %s, 364)",
			valTpDato($numeroCheque,'text'),
		...
		// ACTUALIZA LA NUMERACIÓN DEL DOCUMENTO (Nota Cargo Repuestos y Servicios)
		...
		$sqlInsertEstadoCuenta = sprintf("INSERT INTO cj_cc_estadocuenta (tipoDocumento, idDocumento, fecha, tipoDocumentoN) VALUES ('ND', %s, NOW(), 2)",$idNotaCargo);
		...
		$updateNumeroFolio = sprintf("UPDATE te_folios SET numero_actual = %s WHERE id_folios = 1",$rowNumeroFolio['numero_actual'] + 1);
		...
		VALUES (%s, NOW(), %s, 0, %s, NOW(), NOW(), 0, 2, 2, %s, %s, 0, %s, 1, %s, 515)",
...
function listadoCheques($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	WHERE
		...
		AND cj_cc_detalleanticipo.numeroControlDetalleAnticipo NOT IN (SELECT cj_cc_notadecargo.numeroControlNotaCargo
											...
											AND cj_cc_notadecargo.montoTotalNotaCargo = cj_cc_detalleanticipo.montoDetalleAnticipo)
	...
	WHERE
		...
		AND cj_det_nota_cargo.numeroDocumento NOT IN (SELECT cj_cc_notadecargo.numeroControlNotaCargo
											...
											AND cj_cc_notadecargo.montoTotalNotaCargo = cj_det_nota_cargo.monto_pago)
		%s
	...
	WHERE
		...
		AND sa_iv_pagos.numeroDocumento NOT IN (SELECT cj_cc_notadecargo.numeroControlNotaCargo
											...
											AND cj_cc_notadecargo.montoTotalNotaCargo = sa_iv_pagos.montoPagado)
		%s",
			$sqlBusqAnticipo, $sqlBusqNotaCargo, $sqlBusqFactura);
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" onmouseover=\"this.className='trSobre';\" onmouseout=\"this.className='".$clase."';\" height=\"24\">";
				...
				$htmlTb .= "<td align=\"center\">".date("d-m-Y", strtotime($row['fechaCheque']))."</td>";
