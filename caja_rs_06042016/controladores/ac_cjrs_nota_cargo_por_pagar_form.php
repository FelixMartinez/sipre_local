...
function cargarNotaCargo($idNotaCargo){
	$objResponse = new xajaxResponse();
...
function cargarPago($formDetallePago, $formDetalleDeposito){
	...
	if (str_replace(",","",$formDetallePago['hddMontoFaltaPorPagar']) < str_replace(",","",$formDetallePago['montoPago'])){
		...
		return $objResponse->alert("El monto a pagar no puede ser mayor que el saldo de la Nota de Cargo");
	}
...
function guardarPago($frmDetallePago, $frmListadoPagos, $frmDcto){
	...
	if (!xvalidaAcceso($objResponse,"cjrs_nota_cargo_por_pagar_list","insertar")) { errorCargarPago($objResponse); return $objResponse; }
	...
	VALUES (%s, %s, %s, %s, %s, %s, %s)",
		...
		valTpDato(2,"int"),
		valTpDato(0,"int"),		
		valTpDato($_GET['id_nota_cargo'],"int"),
		valTpDato(1,"int"),
	...
	//FUNCION AGREGADA EL 03/07/2014
	// INSERTA EL ENCABEZADO DEL PAGO (PARA AGRUPAR LOS PAGOS, AFECTA CONTABILIDAD)
	$sqlInsertEncabezadoPagoNC = sprintf("INSERT INTO cj_cc_encabezado_pago_nc_rs (id_nota_cargo, fecha_pago)
	VALUES (%s, %s)",
		valTpDato($_GET['id_nota_cargo'],"int"),
		valTpDato($fechaRegistroPago,"date"));
	$rsInsertEncabezadoPagoNC = mysql_query($sqlInsertEncabezadoPagoNC);
	if (!$rsInsertEncabezadoPagoNC) { errorCargarPago($objResponse); return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nSQL: ".$sqlInsertEncabezadoPagoNC); }
	$idEncabezadoPagoNC = mysql_insert_id();
	...
	foreach($arrayObj as $indice => $valor) {
		if (isset($frmListadoPagos['txtFormaPago'.$valor])){
			...
			$sqlInsertPago = sprintf("INSERT INTO cj_det_nota_cargo (fechaPago, idFormaPago, numeroDocumento, bancoOrigen, bancoDestino, cuentaEmpresa, monto_pago, idNotaCargo, tipoCheque, tomadoEnComprobante, tomadoEnCierre, idCaja, idCierre, id_encabezado_nc)
			VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
				...
				valTpDato($frmDcto['hddIdNotaCargo'],"text"),
				valTpDato($tipoCheque,"text"),
				valTpDato(1,"int"),
				valTpDato(0,"int"),
				valTpDato(2,"int"),
				valTpDato(0,"int"),
				valTpDato($idEncabezadoPagoNC,"int"));
			...
			if ($frmListadoPagos['txtFormaPago'.$valor] == 3){ //DEPOSITO
				...
				foreach($arrayPosiciones as $indiceDeposito => $valorDeposito) {
					if ($valorDeposito == $valor){
						...
						$sqlInsertDetalleDeposito = sprintf("INSERT INTO cj_det_pagos_deposito_nota_cargo (id_det_nota_cargo, fecha_deposito, idFormaPago, idBanco, numero_cuenta, numero_cheque, monto, id_tipo_documento, idCaja)
	...
	if ($frmListadoPagos['txtMontoPorPagar'] == 0) {
		$sqlUpdateNotaCargo = sprintf("UPDATE cj_cc_notadecargo SET
			saldoNotaCargo = 0,
			estadoNotaCargo = 1
		WHERE idNotaCargo = %s",
			valTpDato($_GET['id_nota_cargo'],"int"));
	...
	$rsUpdateNotaCargo = mysql_query($sqlUpdateNotaCargo);
	if (!$rsUpdateNotaCargo) { errorCargarPago($objResponse); return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__."\n\nSQL: ".$sqlUpdateNotaCargo); }
	
	errorCargarPago($objResponse);
	
	$objResponse->alert("Pago cargado correctamente");
	
	$objResponse->script(sprintf("window.location.href='cjrs_nota_cargo_por_pagar_list.php';"));
	
	$objResponse->script(sprintf("verVentana('reportes/cjrs_comprobante_pago_nota_cargo_pdf.php?valBusq=%s|%s|%s',960,550)", $idEmpresa, $_GET['id_nota_cargo'],$numeroActual));
...
function validarAperturaCaja(){
	...
	if (mysql_num_rows($rsCierreCaja) > 0){
		...
		$objResponse->script("location.href='cjrs_nota_cargo_por_pagar_list.php'");
...
$xajax->register(XAJAX_FUNCTION,"cargarNotaCargo");
