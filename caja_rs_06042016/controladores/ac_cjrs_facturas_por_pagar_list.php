...
function cargaLstVendedor($selId = "") {
	...
	WHERE cxc_fact.idDepartamentoOrigenFactura IN (0,1,3)
...
function imprimirFacturaVenta($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjrs_factura_cobrar_pdf.php?valBusq=%s',890,550)", $valBusq));
...
function listaFacturaVenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaFacturaVenta", "6%", $pageNum, "numero_pedido", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Pedido");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"cjrs_facturas_por_pagar_form.php?id_factura=%s\" target=\"_self\"><img src=\"../img/iconos/money_add.png\" title=\"Pagar Factura\"/></a>",
					$row['idFactura']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('reportes/cjrs_comprobante_pago_factura_pdf.php?valBusq=%s', 960, 550);\" src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/>",
					$row['idFactura']);
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
		
		$arrayTotal[11] += $row['cant_items'];
	...
	if ($contFila > 0) {
		...
		if ($pageNum == $totalPages) {
			$rs = mysql_query($query);
			while ($row = mysql_fetch_assoc($rs)) {
				$arrayTotalFinal[11] += $row['cant_items'];
