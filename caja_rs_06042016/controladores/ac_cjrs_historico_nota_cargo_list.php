...
function listaNotaCargo($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	$sqlBusq .= $cond.sprintf("idDepartamentoOrigenNotaCargo IN (0,1,3)");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			if (in_array($row['idDepartamentoOrigenNotaCargo'], array(0,1))){ // RESPUESTOS O SERVICIOS
				$sPar .= "&ct=15";
				$sPar .= "&dt=08";
				$sPar .= "&cc=04";
			} else if ($row['idDepartamentoOrigenNotaCargo'] == 2){ // VEHICULOS
				$sPar .= "&ct=15";
				$sPar .= "&dt=08";
				$sPar .= "&cc=02";			
			}
			$htmlTb .= "<td>";
			if (in_array($row['idDepartamentoOrigenNotaCargo'], array(0,1,2))){
