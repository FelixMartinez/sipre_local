...
//		}
		...
		while ($rowDetPaq = mysql_fetch_assoc($rsDetPaq)) {//de cada paquete se consulta sus items
			...
			if ($value_checkedPaq == 1) {
                            ...
                            $sqlIvasIdPorcPaq = sprintf("SELECT GROUP_CONCAT(sa_det_orden_articulo_iva.id_iva) AS id_iva_paquete, GROUP_CONCAT(sa_det_orden_articulo_iva.iva) AS porc_iva_paquete, GROUP_CONCAT(sa_det_orden_articulo_iva.base_imponible*sa_det_orden_articulo.cantidad) AS base_imponible_paquete
                                                        FROM sa_det_orden_articulo
...
function devolverFacturaVenta($valForm, $valFormTotalDcto){
	...
	$saldoNotaCredito = str_replace(",","", $rowFact['montoTotalFactura']) - str_replace(",","", $rowFact['saldoFactura']);
	...
	//INSERTA EL PAGO
	$insertSQL = sprintf("INSERT INTO sa_iv_pagos (fechaPago, formaPago, numeroDocumento, bancoOrigen, bancoDestino, montoPagado, numeroFactura, tomadoEnComprobante, tomadoEnCierre, idCaja, idCierre)
		VALUES ('%s', 8, %s, 1, 1, %s, %s, 1, 0, 2, 0);",
						date("Y-m-d"),
						$idNotaCredito,
						valTpDato($rowFact['montoTotalFactura'], "real_inglesa"),
	...
	while ($rowArt = mysql_fetch_array($rsArt)) {
		...
		if ($rowArt['nro_rpto_desp'] > 0) {
			...
			if ($idCasilla == NULL || $idCasilla == ""){
				...
				WHERE fact_vent.idFactura = %s AND fact_vent_det.id_articulo = %s LIMIT 1",
					valTpDato($idFactura, "int"),
				...
				$insertSQLKardex = sprintf("INSERT INTO iv_kardex (id_documento, id_modulo, id_articulo, id_casilla, tipo_movimiento, tipo_documento_movimiento, cantidad, precio, costo, costo_cargo, porcentaje_descuento, subtotal_descuento, id_clave_movimiento, estado, fecha_movimiento, hora_movimiento)
				VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), NOW())",
					valTpDato($idNotaCredito, "int"),
				...
				$insertSQL = sprintf("INSERT INTO iv_movimiento_detalle (id_movimiento, id_articulo, id_kardex, cantidad, precio, costo, costo_cargo, porcentaje_descuento, subtotal_descuento, correlativo1, correlativo2, tipo_costo, llave_costo_identificado, promocion, id_moneda_costo, id_moneda_costo_cambio)
					VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
						...
						valTpDato($rowFactDet['precio_unitario'], "real_inglesa"),
						valTpDato($rowFactDet['costo_compra'], "real_inglesa"),
						valTpDato(0, "int"),//costo cargo
						valTpDato($valFormTotalDcto['txtDescuento'], "text"),
						valTpDato((($valFormTotalDcto['txtDescuento'] * $rowFactDet['precio_unitario']) / 100), "real_inglesa"),
						valTpDato("", "int"),
	...
	$objResponse->script(sprintf("window.location.href='cjrs_devolucion_servicios_list.php';"));
...
function guardarFactura($frmPresupuesto, $frmTotalPresupuesto, $frmDetallePago, $frmListadoPagos){
	...
	if ($totalRows > 0) {
		...
		if($frmTotalPresupuesto['txtIvaVenta'] == NULL){
			$frmTotalPresupuesto['txtIvaVenta'] = 0;
                ...
                foreach ($frmTotalPresupuesto["ivaActivo"] as $key => $idIvaActivo){
                    $totalIvas += $frmTotalPresupuesto["txtTotalIva".$idIvaActivo];
                }
		...
		VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
			...
			valTpDato(str_replace(",","", $frmTotalPresupuesto['txtIvaVenta']),"double"),
			str_replace(",","", $totalIvas),
			str_replace(",","", $frmTotalPresupuesto['txtSubTotal']),
			valTpDato($tipoPago, "int"),
			str_replace(",","", $frmTotalPresupuesto['txtBaseImponible']),
		...
		$queryArt = sprintf("SELECT
			...
			WHERE iv_articulos_costos.id_articulo = det_orden_art.id_articulo ORDER BY id_articulo_costo DESC LIMIT 1) AS costo,
			...
			while($rowArt = mysql_fetch_array($rsArt)) {
				...
				if ($rowCont['nro_rpto_desp'] > 0) {
					...
					while($rowContCas = mysql_fetch_assoc($rsContCas)) {
						...
						// REGISTRA EL MOVIMIENTO DEL ARTICULO 
						$insertSQLKardex = sprintf("INSERT INTO iv_kardex (id_documento, id_modulo, id_articulo, id_casilla, tipo_movimiento, cantidad, precio, costo, costo_cargo, porcentaje_descuento, subtotal_descuento, id_clave_movimiento, estado, fecha_movimiento, hora_movimiento)
						VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, NOW(), NOW())",
							valTpDato($idFactura, "int"),
							valTpDato("1", "int"), // 0 = Repuestos, 1 = Servicios, 2 = Vehiculos, 3 = Administracion
							valTpDato($idArticulo, "int"),
							valTpDato($idCasilla, "int"),
						...
						// INSERTA EL DETALLE DEL MOVIMIENTO
						$insertMovimientoDetalleSQL = sprintf("INSERT INTO iv_movimiento_detalle (id_movimiento, id_articulo, id_kardex, cantidad, precio, costo, costo_cargo, porcentaje_descuento, subtotal_descuento, correlativo1, correlativo2, tipo_costo, llave_costo_identificado, promocion, id_moneda_costo, id_moneda_costo_cambio)
						VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
							valTpDato($idMovimiento, "int"),
							valTpDato($idArticulo, "int"),
							valTpDato($idKardex, "int"),
