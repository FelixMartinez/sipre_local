...
function buscarNotaCredito($frmBuscar){
	...
	$objResponse->loadCommands(listaNotaCreditoVenta(0, "idNotaCredito", "DESC", $valBusq));
...
function cargaLstEmpleado($selId = "", $nombreObjeto = "", $objetoDestino = "") {
	...
	WHERE nota_cred.idDepartamentoNotaCredito IN (0,1,3)
...
function imprimirNotaCredito($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjrs_historico_nota_credito_pdf.php?valBusq=%s',890,550)", $valBusq));
...
function listaNotaCreditoVenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "14%", $pageNum, "id_empresa", $campOrd, $tpOrd, $valBusq, $maxRows, "Empresa");
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "6%", $pageNum, "fechaNotaCredito", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Registro");
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "6%", $pageNum, "numeracion_nota_credito", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Nota de Crédito");
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "6%", $pageNum, "cxc_nc.numeroControl", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Control");
		...
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "22%", $pageNum, "nombre_cliente", $campOrd, $tpOrd, $valBusq, $maxRows, "Cliente");
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "6%", $pageNum, "condicion_pago", $campOrd, $tpOrd, $valBusq, $maxRows, "Tipo de Pago");
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "8%", $pageNum, "descripcion_estado_nota_credito", $campOrd, $tpOrd, $valBusq, $maxRows, "Estado Nota de Crédito");
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "4%", $pageNum, "cant_items", $campOrd, $tpOrd, $valBusq, $maxRows, "Items");
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "8%", $pageNum, "saldoNotaCredito", $campOrd, $tpOrd, $valBusq, $maxRows, "Saldo Nota de Crédito");
		$htmlTh .= ordenarCampo("xajax_listaNotaCreditoVenta", "8%", $pageNum, "total", $campOrd, $tpOrd, $valBusq, $maxRows, "Total Nota de Crédito");
	...
	while ($row = mysql_fetch_assoc($rsLimit)){
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".implode("-",array_reverse(explode("-",$row['fechaNotaCredito'])))."</td>";
			$htmlTb .= "<td align=\"right\">".$row['numeracion_nota_credito']."</td>";
			$htmlTb .= "<td align=\"right\">".$row['numeroControl']."</td>";
			$htmlTb .= "<td align=\"center\">".implode("-",array_reverse(explode("-",$row['fechaRegistroFactura'])))."</td>";
			...
			$htmlTb .= "<td>";
				...
				$htmlTb .= ((strlen($row['observacionesNotaCredito']) > 0) ? "<tr><td>".utf8_encode($row['observacionesNotaCredito'])."</td></tr>" : "");
				$htmlTb .= "</table>";
			$htmlTb .= "</td>";
			...
			switch ($row['idDepartamentoNotaCredito']) {
				case 0 : // REPUESTOS
					$htmlTb .= sprintf("<a href=\"javascript:verVentana('../repuestos/reportes/iv_devolucion_venta_pdf.php?valBusq=%s', 960, 550);\" target=\"_self\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota Crédito PDF\"/></a>",
						$row['idNotaCredito']);
					break;
				case 1 : // SERVICIOS
					if ($row['aplicaLibros'] == 1) {
						$htmlTb .= sprintf("<a href=\"javascript:verVentana('../servicios/reportes/sa_devolucion_venta_pdf.php?valBusq=%s', 960, 550);\" target=\"_self\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota Crédito PDF\"/></a>",
							$row['idNotaCredito']);
					}
					break;
				case 2 : // VEHICULOS
					$htmlTb .= sprintf("<a href=\"javascript:verVentana('../vehiculos/reportes/an_devolucion_venta_pdf.php?valBusq=%s', 960, 550);\" target=\"_self\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota Crédito PDF\"/></a>",
						$row['idNotaCredito']);
					break;
				case 3 : // ADMINISTRACION
					$htmlTb .= sprintf("<a href=\"javascript:verVentana('../repuestos/reportes/ga_devolucion_venta_pdf.php?valBusq=%s', 960, 550);\" target=\"_self\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota Crédito PDF\"/></a>",
						$row['idNotaCredito']);
					break;
			}
			$htmlTb .= "</td>";
			...
			if ($row['idDepartamentoNotaCredito'] == 0) {
				...
				$sPar .= "&dt=01";
				$sPar .= "&cc=04";
			} else if ($row['idDepartamentoNotaCredito'] == 2){
				$sPar .= "&ct=02";
				$sPar .= "&dt=01";
				$sPar .= "&cc=02";
			}
