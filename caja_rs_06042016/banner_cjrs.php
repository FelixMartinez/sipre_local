...
$queryCaja = sprintf("SELECT * FROM caja WHERE caja.idCaja = 2;");
$rsCaja = mysql_query($queryCaja);
if (!$rsCaja) die(mysql_error()."\n\nLine: ".__LINE__."\n\nSQL: ".$queryCaja);
$rowCaja = mysql_fetch_assoc($rsCaja);

$queryApertura = sprintf("SELECT
	apert.fechaAperturaCaja,
	apert.statusAperturaCaja,
	(CASE apert.statusAperturaCaja
		WHEN 0 THEN 'CERRADA TOTALMENTE'
		WHEN 1 THEN CONCAT_WS(' EL ', 'ABIERTA', DATE_FORMAT(apert.fechaAperturaCaja,'%s'))
		WHEN 2 THEN 'CERRADA PARCIALMENTE'
		ELSE 'CERRADA TOTALMENTE'
	END) AS estatus_apertura_caja,
	IF (vw_iv_emp_suc.id_empresa_suc > 0, CONCAT_WS(' - ', vw_iv_emp_suc.nombre_empresa, vw_iv_emp_suc.nombre_empresa_suc), vw_iv_emp_suc.nombre_empresa) AS nombre_empresa
FROM sa_iv_apertura apert
	RIGHT JOIN vw_iv_empresas_sucursales vw_iv_emp_suc ON (apert.id_empresa = vw_iv_emp_suc.id_empresa_reg)
WHERE ((apert.statusAperturaCaja IN (0,1,2) AND apert.idCaja = %s)
		OR statusAperturaCaja IS NULL)
	AND vw_iv_emp_suc.id_empresa_reg = %s
ORDER BY apert.id DESC
LIMIT 1;",
	valTpDato("%d-%m-%Y", "campo"),
	valTpDato($rowCaja['idCaja'], "int"),
	valTpDato($idEmpresa, "int"));
$rsApertura = mysql_query($queryApertura);
if (!$rsApertura) die(mysql_error()."\n\nLine: ".__LINE__."\n\nSQL: ".$queryApertura);
