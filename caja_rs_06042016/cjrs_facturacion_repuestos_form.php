...
<head>
	...
	<title>.: SIPRE <?php echo cVERSION; ?> :. Caja de Repuestos y Servicios - Pago y Facturación de Repuestos</title>
	...
	function abrirDivFlotante1(nomObjeto, verTabla, valor) {
		...
		if (verTabla == "tblDeposito") {
			if (valor == "Deposito") {
				...
				&& validarCampo('txtNumeroDctoPago','t','') == true) {
					...
					byId('tdFlotanteTitulo1').innerHTML = 'Detalle Deposito';
				} else {
			...
			byId('trBuscarAnticipoNotaCredito').style.display = '';
			
			byId('txtCriterioAnticipoNotaCredito').className = 'inputHabilitado';
		
			xajax_buscarAnticipoNotaCredito(xajax.getFormValues('frmBuscarAnticipoNotaCredito'), xajax.getFormValues('frmDcto'), xajax.getFormValues('frmDetallePago'), xajax.getFormValues('frmListaPagos'));
		...
		if (verTabla == "tblLista") {
			byId('txtCriterioAnticipoNotaCredito').focus();
			byId('txtCriterioAnticipoNotaCredito').select();
		}
	}
	
	function abrirDivFlotante2(nomObjeto, verTabla, valor, valor2) {
		byId('tblAnticipoNotaCredito').style.display = 'none';
		
		if (verTabla == "tblAnticipoNotaCredito") {
			byId('txtMontoDocumento').className = 'inputHabilitado';
			
			xajax_cargarSaldoDocumento(valor, valor2);
	...
	function validarFrmAnticipoNotaCredito() {
		...
		if (parseFloat(saldo) < parseFloat(monto)) {
			...
			if (parseFloat(montoFaltaPorPagar) >= parseFloat(monto)) {
				if (confirm("Desea cargar el pago?")) {
					byId('hddIdAnticipoNotaCredito').value = byId('hddIdDocumento').value;
					...
					if (error == true) {
						...
						byId('btnGuardarDetallePago').disabled = true;
						byId('btnGuardar').disabled = true;
						byId('btnCancelar').disabled = true;
						
						xajax_insertarPago(xajax.getFormValues('frmListaPagos'), xajax.getFormValues('frmDetallePago'), xajax.getFormValues('frmDeposito'), xajax.getFormValues('frmTotalDcto'));
					}
				}
			} else {
	...
	function validarFrmDetalleDeposito() {
		...
		if (byId('lstTipoPago').value == 1) { // EFECTIVO
			...
			if (error == true) {
				...
				byId('btnGuardarDeposito').disabled = true;
				byId('btnCancelarDeposito').disabled = true;
...
<div id="divGeneralPorcentaje">
	...
	<div id="divInfo" class="print">
            ...
            <form id="frmDetallePago" name="frmDetallePago" style="margin:0">
                ...
                <tr align="left">
                    ...
                    <td id="tdselTipoPago" width="26%">
                        <select id="selTipoPago" name="selTipoPago" onchange="asignarTipoPago(this.value)" style="width:200px">
                            <option value="-1">[ Seleccione ]</option>
                        </select>
                    </td>
                    <td rowspan="6" valign="top" width="62%">
                    	<table width="100%">
                        <tr>
                        	<td width="20%"></td>
                            <td width="16%"></td>
                        ...
                        <tr id="trTipoTarjeta">
                            <td align="right" class="tituloCampo">Tipo Tarjeta:</td>
                            <td id="tdtarjeta" colspan="3">
                                <select id="tarjeta" name="tarjeta" style="width:200px">
                                    <option value="-1">[ Seleccione ]</option>
                                </select>
                            </td>
                        </tr>
                        <tr id="trPorcentajeRetencion">
                            <td align="right" class="tituloCampo">Porcentaje Retenci&oacute;n:</td>
                            <td><input type="text" id="porcentajeRetencion" name="porcentajeRetencion" readonly="readonly" size="10" style="text-align:right;"/></td>
                            <td align="right" class="tituloCampo">Monto:</td>
                            <td><input type="text" id="montoTotalRetencion" name="montoTotalRetencion" readonly="readonly" size="19" style="text-align:right;"/></td>
                        </tr>
                        <tr id="trPorcentajeComision">
                            <td align="right" class="tituloCampo">Porcentaje Comisi&oacute;n:</td>
                            <td><input type="text" id="porcentajeComision" name="porcentajeComision" readonly="readonly" size="10" style="text-align:right;" value="0.00"/></td>
                            <td align="right" class="tituloCampo">Monto:</td>
                            <td><input type="text" id="montoTotalComision" name="montoTotalComision" readonly="readonly" size="19" style="text-align:right;" value="0.00"/></td>
                        </tr>
                        </table>
                    </td>
                </tr>
                ...
                <tr id="trNumeroDocumento" align="left">
                    <td align="right" class="tituloCampo">Nro.:</td>
                    <td>
                    	<table border="0" cellpadding="0" cellspacing="0">
                        <tr>
                        	<td><input type="text" id="txtNumeroDctoPago" name="txtNumeroDctoPago"/></td>
                        	<td>
                            <a class="modalImg" id="btnAgregarDetAnticipoNotaCredito" rel="#divFlotante1" onclick="abrirDivFlotante1(this, 'tblLista');">
                        ...
                        <input type="hidden" id="hddIdAnticipoNotaCredito" name="hddIdAnticipoNotaCredito"/>
                    </td>
                </tr>
                <tr align="left">
                    <td class="tituloCampo" align="right">Monto:</td>
                    <td>
                    	<table border="0" cellpadding="0" cellspacing="0">
                        <tr>
            ...
            <form id="frmListaPagos" name="frmListaPagos" style="margin:0">
				<fieldset><legend class="legend">Desglose de Pagos</legend>
                    ...
                    <tr align="center" class="tituloColumna">
                        ...
                        <td width="20%">Banco Cliente</td>
                        <td width="20%">Banco Compañia</td>
                        <td width="20%">Cuenta</td>
                        <td width="18%">Nro. Control</td>
                        <td width="12%">Monto</td>
                        <td></td>
                    </tr>
                    <tr id="trItmPiePago" class="trResaltarTotal">
                    	<td align="right" class="tituloCampo" colspan="6">Total Pagos:</td>
                    ...
                    <tr class="trResaltarTotal3">
                    	<td align="right" class="tituloCampo" colspan="6">Total Faltante:</td>
		...
		<button type="button" id="btnCancelar" name="btnCancelar" onclick="window.open('cjrs_factura_venta_list.php','_self');" style="cursor:default"><table align="center" cellpadding="0" cellspacing="0"><tr><td>&nbsp;</td><td><img src="../img/iconos/cancel.png"/></td><td>&nbsp;</td><td>Cancelar</td></tr></table></button>
	     </td>
        </tr>
        </table>
    </div>
...
<form id="frmDeposito" name="frmDeposito" style="margin:0">
	<table border="0" id="tblDeposito" style="display:none" width="760">
    <tr>
    	<td width="20%"></td>
    	<td width="80%"></td>
    </tr>
	...
	<tr id="trBuscarAnticipoNotaCredito">
        ...
        <form id="frmBuscarAnticipoNotaCredito" name="frmBuscarAnticipoNotaCredito" onsubmit="byId('btnBuscarAnticipoNotaCredito').click(); return false;" style="margin:0">
			...
			<tr>
				...
				<td>
					<button type="submit" id="btnBuscarAnticipoNotaCredito" name="btnBuscarAnticipoNotaCredito" onclick="xajax_buscarAnticipoNotaCredito(xajax.getFormValues('frmBuscarAnticipoNotaCredito'), xajax.getFormValues('frmDcto'), xajax.getFormValues('frmDetallePago'), xajax.getFormValues('frmListaPagos'));">Buscar</button>
...
<form id="frmAnticipoNotaCredito" name="frmAnticipoNotaCredito" onsubmit="return false;" style="margin:0">
	<table border="0" id="tblAnticipoNotaCredito" width="360">
	<tr align="left">
	...
	<tr align="left">
		<td align="right" colspan="2"><hr/>
            <input type="hidden" name="hddIdDocumento" id="hddIdDocumento"/>
			<button type="submit" id="btnAceptarAnticipoNotaCredito" name="btnAceptarAnticipoNotaCredito" onclick="validarFrmAnticipoNotaCredito();" style="cursor:default"><table align="center" cellpadding="0" cellspacing="0"><tr><td>Guardar</td></tr></table></button>
			<button type="button" id="btnCancelarAnticipoNotaCredito" name="btnCancelarAnticipoNotaCredito" class="close">Cancelar</button>
		</td>
	</tr>
	</table>
</form>
