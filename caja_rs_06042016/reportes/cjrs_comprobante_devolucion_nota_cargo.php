...
if ($totalRows > 0) {
	...
	while ($contFila < 1) {
		...
		if ($contFila % 45 == 1) {
			...
			if (!$rsDetalleCab) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			...
			$pdf->Ln();
			$pdf->Cell(562,15,"CAJA DE REPUESTOS Y SERVICIOS",0,0,'C');
			$pdf->Ln();$pdf->Ln();$pdf->Ln();
			$pdf->SetFont('Arial','',9);
			$pdf->Cell(562,5,"COMPROBANTE DE DEVOLUCION DE CHEQUE - NOTA DE CARGO",0,0,'C');
			$pdf->Ln();$pdf->Ln();$pdf->Ln();
			$pdf->SetFont('Arial','',9);
			$pdf->Cell(562,5,"Nro. Nota de Cargo: ".$rowDetalleCab['numeroNotaCargo']."",0,0,'C');
		...
		//DETALLE DE LOS PAGOS
			...
			while ($rowDetalle = mysql_fetch_assoc($rsDetalle)){
				...
				$montoPagado = $rowDetalle['montoTotalNotaCargo'];
					...
					$pdf->Cell($arrayTamCol[0],12,date("d-m-Y", strtotime($fechaPago)),'LR',0,'L',true);
			...
			$pdf->Ln();
			$pdf->Cell(20,5,"",0,0,'L');
			$pdf->Cell(150,15,"Nota de Cargo por la Cantidad de: ".utf8_decode(trim(substr(strtoupper(num2letras($montoTotal,false,true,$rowMoneda['descripcion'])),0,75)))."",0,0,'L');
			$pdf->Ln();
			$pdf->Cell(20,5,"",0,0,'L');
