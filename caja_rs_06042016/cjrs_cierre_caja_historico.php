...
if(!(validaAcceso("cjrs_cierre_caja"))) {
...
<head>
	...
	<title>.: SIPRE 2.0 :. Caja de Repuestos y Servicios - Histórico de Cierre</title>
	...
	<body>
		<div id="divGeneralPorcentaje">
			...
			<div id="divInfo">
				...
				<tr>
					<td align="center" class="tituloPaginaCajaRS">Corte de Caja (Repuestos y Servicios)<br/><span class="textoNegroNegrita_10px">(Resumen de Saldos)</span></td>
				</tr>
				<tr>
					<td>&nbsp;</td>
				</tr>
				...
				<?php
					...
					WHERE
						sa_iv_cierredecaja.idCierre = (SELECT
														...
														INNER JOIN sa_iv_cierredecaja ON (sa_iv_apertura.id = sa_iv_cierredecaja.id)
														WHERE
															sa_iv_apertura.id = ".$_GET['idApertura'].")
						%s", $andEmpresa);
					...
					if($lafila = mysql_fetch_array($consultaConsultaFecha)){
						...
						$fechaActual = date("Y-m-d",strtotime($_GET['fecha']));
				...
				<form id="form1" name="form1">
					<?php
						...
						if ($rowConfig400['valor'] == 0) { // MOSTRAR POR SUCURSALES
							$andEmpresa = sprintf(" AND ape.id_empresa = %s",
								valTpDato($idEmpresa,"int"));
							$andEmpresa2 = sprintf(" AND cierre.id_empresa = %s",
								valTpDato($idEmpresa,"int"));
						...
						//FECHA EJECUCION DE CIERRE
						$sqlCierre = sprintf("SELECT
							cierre.fechaCierre,
							cierre.fechaEjecucionCierre,
							cierre.horaEjecucionCierre
						FROM
							sa_iv_cierredecaja cierre
						WHERE
							cierre.id = ".$_GET['idApertura']."
							%s",$andEmpresa2);
						$sqlCierre = mysql_query($sqlCierre);
						list($fechaCierreCaja,$fechaEjecucionCierre,$horaEjecucionCierre) = mysql_fetch_array($sqlCierre);
					?>
					...
					<tr>
						...
						<td style="text-align:left"><input name="nombreCaja2" type="text" id="nombreCaja2" style="text-align:center;"
							value="<?php
										...
										$hora = substr($horaApertura,0,2);
										$minuto = substr($horaApertura,3,2);
										$segundo = substr($horaApertura,6,2);
										
										$fechaAperturaCaja2 = $dia."-".$mes."-".$anio;
										$horaAperturaCaja2 = $hora.":".$minuto.":".$segundo;
										
										echo $fechaAperturaCaja2."   ".$horaAperturaCaja2;
									?>"
						...
						<td style="text-align:left">
							<?php $fechaPago = date("d-m-Y",strtotime($_GET['fecha'])); ?>
					...
					<tr>
						...
						<td style="text-align:left"><input id="txtFechaCierre" name="txtFechaCierre" type="text" style="text-align:center;"
							value="<?php
										$anio = substr($fechaCierreCaja,0,4);
										$mes = substr($fechaCierreCaja,5,2);
										$dia = substr($fechaCierreCaja,8,2);
										
										$hora = substr($fechaCierreCaja,0,2);
										$minuto = substr($fechaCierreCaja,3,2);
										$segundo = substr($fechaCierreCaja,6,2);
										
										$fechaCierreCaja2 = $dia."-".$mes."-".$anio;
										$horaCierreCaja2 = $hora.":".$minuto.":".$segundo;
										
										echo $fechaCierreCaja2;
									?>" 
							size="25" readonly="readonly" align="top"/>
						</td>
						<td>&nbsp;</td>
						...
						<td style="text-align:left"><input id="txtFechaCierre" name="txtFechaCierre" type="text" style="text-align:center;"
							value="<?php
										$anio = substr($fechaEjecucionCierre,0,4);
										$mes = substr($fechaEjecucionCierre,5,2);
										$dia = substr($fechaEjecucionCierre,8,2);
										
										$hora = substr($horaEjecucionCierre,0,2);
										$minuto = substr($horaEjecucionCierre,3,2);
										$segundo = substr($horaEjecucionCierre,6,2);
										
										$fechaEjecucionCierre2 = $dia."-".$mes."-".$anio;
										$horaEjecucionCierre2 = $hora.":".$minuto.":".$segundo;
										
										echo $fechaEjecucionCierre2."   ".$horaEjecucionCierre2;
									?>" 
							size="25" readonly="readonly" align="top"/>
						</td>
					</tr>
					...
					<tr>
						<td colspan="5" class="noprint" bgcolor="#FFFFFF" scope="row" style="text-align:right"><hr>
							<button type="button" id="button" name="button" onclick="window.print();"><table align="center" cellpadding="0" cellspacing="0"><tr><td>&nbsp;</td><td><img src="../img/iconos/ico_print.png"/></td><td>&nbsp;</td><td>Imprimir</td></tr></table></button>
...
function saldosDeCierre($montoApertura){
	...
	if ($rowConfig400['valor'] == 0) { // MOSTRAR POR SUCURSALES
		...
		$andEmpresa4 = sprintf(" AND cj_cc_notadecargo.id_empresa = %s",
			valTpDato($idEmpresa,"int"));	
		$andEmpresa5 = sprintf(" AND cj_cc_notacredito.id_empresa = %s",
			valTpDato($idEmpresa,"int"));	
	...
	FROM cj_cc_encabezadofactura
		INNER JOIN sa_iv_pagos pa ON (cj_cc_encabezadofactura.idFactura = pa.id_factura)
		INNER JOIN formapagos ON (pa.formaPago = formapagos.idFormaPago)	
	WHERE pa.idCierre = ".$_GET['idCierre']."
		AND pa.idCaja = 2 ".$andEmpresa2."
	GROUP BY pa.formaPago
	
	UNION
	
	SELECT
		formapagos.idFormaPago AS formaPago,
		'4' AS tipoDoc,
		formapagos.nombreFormaPago,
		cj_cc_anticipo.id_empresa AS id_empresa
	FROM cj_cc_anticipo
		INNER JOIN cj_cc_detalleanticipo pa ON (cj_cc_anticipo.idAnticipo = pa.idAnticipo)
		INNER JOIN formapagos ON (pa.tipoPagoDetalleAnticipo = formapagos.aliasFormaPago)
	WHERE pa.idCierre = ".$_GET['idCierre']."
		AND cj_cc_anticipo.idDepartamento IN (0,1,3)
		AND cj_cc_anticipo.estatus = 1 ".$andEmpresa3."
	...
	FROM cj_cc_notadecargo
		INNER JOIN cj_det_nota_cargo nc ON (cj_cc_notadecargo.idNotaCargo = nc.idNotaCargo)
		INNER JOIN formapagos ON (nc.idFormaPago = formapagos.idFormaPago)
	WHERE nc.idCierre = ".$_GET['idCierre']."
		AND cj_cc_notadecargo.idDepartamentoOrigenNotaCargo IN (0,1,3) ".$andEmpresa4."
	GROUP BY nc.idFormaPago");
	$consultaFormaPago = mysql_query($sqlFormaPago)or die(mysql_error().$sqlFormaPago);
	$sw='';
	...
	while($registro = mysql_fetch_array($consultaFormaPago)){
		...
		if ($sw != $varFormaPago) {
			if ($varFormaPago == 1) {
				...
				$registro = "EF";
			} else if ($varFormaPago == 2) {
				$existePagoEnEfectivoOcheque = 1;
				$registro = "CH";
			} else if ($varFormaPago == 3) {
				$registro = "DP";
			} else if ($varFormaPago == 4) {
				$registro = "TB";
			} else if ($varFormaPago == 5) {
				$registro = "TC";
			} else if ($varFormaPago == 6) {
				$registro = "TD";
			} else if ($varFormaPago == 11) {
				$registro = "CB";
			}
			...
			FROM cj_cc_anticipo an
				INNER JOIN cj_cc_detalleanticipo pa ON (an.idAnticipo = pa.idAnticipo)
				INNER JOIN formapagos ON (pa.tipoPagoDetalleAnticipo = formapagos.aliasFormaPago)
			WHERE pa.fechaPagoAnticipo = ".$fechaApertura."
				AND an.idDepartamento IN (0,1,3)
				AND pa.idCierre = ".$_GET['idCierre']."
				".$andEmpresaSql1."
			...
			FROM cj_cc_encabezadofactura fv
				...
				INNER JOIN formapagos ON (pa.formaPago = formapagos.idFormaPago)
			WHERE pa.fechaPago = ".$fechaApertura."
			...
			FROM cj_cc_notadecargo fv
				...
				INNER JOIN formapagos ON (pa.idFormaPago = formapagos.idFormaPago)
			WHERE pa.fechaPago = ".$fechaApertura."
	...
	FROM cj_cc_encabezadofactura
	WHERE cj_cc_encabezadofactura.idDepartamentoOrigenFactura IN (0,1,3)
		AND cj_cc_encabezadofactura.aplicaLibros = 1
		AND cj_cc_encabezadofactura.fechaRegistroFactura = (SELECT ape.fechaAperturaCaja
															FROM caja ca
																INNER JOIN sa_iv_apertura ape ON (ca.idCaja = ape.idCaja)
															WHERE ape.id = ".$_GET['idApertura']."
																%s)
		%s",$andEmpresa1,$andEmpresa2);
	$rsNumeros = mysql_query($queryNumeros)or die(mysql_error().$queryNumeros);
	$rowNumero = mysql_fetch_assoc($rsNumeros);
	
	$queryNumerosNotaCredito = sprintf("SELECT
	...
	WHERE cj_cc_notacredito.idDepartamentoNotaCredito IN (0,1,3)
		AND cj_cc_notacredito.aplicaLibros = 1
		AND cj_cc_notacredito.fechaNotaCredito = (SELECT ape.fechaAperturaCaja
													FROM caja ca
														INNER JOIN sa_iv_apertura ape ON (ca.idCaja = ape.idCaja)
													WHERE ape.id = ".$_GET['idApertura']."
														AND ape.id_empresa = ".$idEmpresa."
														%s)
		%s",$andEmpresa1,$andEmpresa5);
