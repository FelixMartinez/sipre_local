...
// VERIFICA VALORES DE CONFIGURACION (Mostrar Numero Control en Impresi�n de Factura)
...
imagestring($img,1,350,10,str_pad("FACTURA", 36, " ", STR_PAD_BOTH),$textColor);

imagestring($img,1,350,20,"ORDEN DE TRABAJO",$textColor);
imagestring($img,1,440,20,":",$textColor);
imagestring($img,1,450,20,$row['numero_orden']." ".$row['nombre_tipo_orden'],$textColor); // <---- antes 'numeroPedido' ahora numero_orden

imagestring($img,1,350,30,"SERIE-SR",$textColor);
...
imagestring($img,1,450,$posY,date("d-m-Y", strtotime($row['fechaRegistroFactura'])),$textColor); // <----
$posY += 10;
imagestring($img,1,350,$posY,"FECHA VENCIMIENTO",$textColor);
imagestring($img,1,440,$posY,":",$textColor);
imagestring($img,1,450,$posY,date("d-m-Y", strtotime($row['fechaVencimientoFactura'])),$textColor); // <----
$posY += 10;
...
while ($rowIvas = mysql_fetch_assoc($rsIvas)){
    ...
    imagestring($img,1,370,$posY,$rowIvas['iva'],$textColor);
    imagestring($img,1,400,$posY,":",$textColor);
...
imagestring($img,1,400,$posY,":",$textColor);
imagestring($img,2,440,$posY,strtoupper(str_pad(number_format($row['montoTotalFactura'], 2, ".", ","), 15, " ", STR_PAD_LEFT)),$textColor); // <----
...
if($rowConfig403['valor'] == "3"){//puerto rico
    ...
    imagepng($img2,"tmp/factura_venta_encabezado.png");
}
...
if (isset($arrayImg)) {
	foreach ($arrayImg as $indice => $valor) {
		...
		$pdf->Image($valor, 15, $rowConfig2['valor'], 580, 680);//$rowConfig2['valor'] = 80
		
                if($rowConfig403['valor'] == "3"){                    
                    $pdf->Image("tmp/factura_venta_encabezado.png", 55, 25, 530+20, 40+5);
