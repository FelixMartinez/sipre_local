...
if ($valCadBusq[0] != "-1" && $valCadBusq[0] != "") {
	...
	$sqlBusq .= $cond.sprintf("((CASE
									WHEN (id_factura IS NOT NULL AND id_nota_credito IS NULL) THEN
										(SELECT fact_vent.id_empresa FROM cj_cc_encabezadofactura fact_vent
										WHERE fact_vent.idFactura = comision_emp.id_factura)
									WHEN (id_nota_credito IS NOT NULL) THEN
										(SELECT nota_cred.id_empresa FROM cj_cc_notacredito nota_cred
...
if ($valCadBusq[1] != "-1" && $valCadBusq[1] != "") {
	...
	(CASE
		WHEN (id_factura IS NOT NULL AND id_nota_credito IS NULL) THEN
			MONTH((SELECT fact_vent.fechaRegistroFactura FROM cj_cc_encabezadofactura fact_vent
					WHERE fact_vent.idFactura = comision_emp.id_factura))
		WHEN (id_nota_credito IS NOT NULL) THEN
			MONTH((SELECT nota_cred.fechaNotaCredito FROM cj_cc_notacredito nota_cred
					WHERE nota_cred.idNotaCredito = comision_emp.id_nota_credito))
		WHEN (id_vale_salida IS NOT NULL AND id_vale_entrada IS NULL) THEN
...
if ($valCadBusq[1] != "-1" && $valCadBusq[1] != "") {
	...
	(CASE
		WHEN (id_factura IS NOT NULL AND id_nota_credito IS NULL) THEN
			YEAR((SELECT fact_vent.fechaRegistroFactura FROM cj_cc_encabezadofactura fact_vent
					WHERE fact_vent.idFactura = comision_emp.id_factura))
		WHEN (id_nota_credito IS NOT NULL) THEN
			YEAR((SELECT nota_cred.fechaNotaCredito FROM cj_cc_notacredito nota_cred
					WHERE nota_cred.idNotaCredito = comision_emp.id_nota_credito))
		WHEN (id_vale_salida IS NOT NULL AND id_vale_entrada IS NULL) THEN
...
if ($valCadBusq[4] != "-1" && $valCadBusq[4] != "") {
	...
	(CASE
		WHEN (id_factura IS NOT NULL AND id_nota_credito IS NULL) THEN
			(SELECT fact_vent.idDepartamentoOrigenFactura FROM cj_cc_encabezadofactura fact_vent
			WHERE fact_vent.idFactura = comision_emp.id_factura)
		WHEN (id_nota_credito IS NOT NULL) THEN
			(SELECT nota_cred.idDepartamentoNotaCredito FROM cj_cc_notacredito nota_cred
			WHERE nota_cred.idNotaCredito = comision_emp.id_nota_credito)
		WHEN (id_vale_salida IS NOT NULL AND id_vale_entrada IS NULL) THEN
...
for ($pageNum = 0; $pageNum <= $totalPages; $pageNum++) {
	...
	while ($rowComision = mysql_fetch_assoc($rsLimitComision)) {
		...
		$queryDetalle = sprintf("SELECT
			...
			(CASE
				WHEN (id_factura IS NOT NULL AND id_nota_credito IS NULL) THEN
					(SELECT fact_vent.numeroFactura FROM cj_cc_encabezadofactura fact_vent
					WHERE fact_vent.idFactura = comision_emp.id_factura)
				WHEN (id_nota_credito IS NOT NULL) THEN
					(SELECT nota_cred.numeracion_nota_credito FROM cj_cc_notacredito nota_cred
					WHERE nota_cred.idNotaCredito = comision_emp.id_nota_credito)
				WHEN (id_vale_salida IS NOT NULL AND id_vale_entrada IS NULL) THEN
			...
			(CASE
				WHEN (id_factura IS NOT NULL AND id_nota_credito IS NULL) THEN
					(SELECT fact_vent.condicionDePago FROM cj_cc_encabezadofactura fact_vent
					WHERE fact_vent.idFactura = comision_emp.id_factura)
				WHEN (id_nota_credito IS NOT NULL) THEN
					(SELECT fact_venta.condicionDePago AS condicionDePago
					FROM cj_cc_encabezadofactura fact_venta
						INNER JOIN cj_cc_notacredito nota_cred on (fact_venta.idFactura = nota_cred.idDocumento)
					WHERE nota_cred.idNotaCredito = comision_emp.id_nota_credito)
				WHEN (id_vale_salida IS NOT NULL AND id_vale_entrada IS NULL) THEN
			...
			(CASE
				WHEN (id_factura IS NOT NULL AND id_nota_credito IS NULL) THEN
					...
					FROM sa_orden orden_serv
						...
						INNER JOIN cj_cc_encabezadofactura fact_vent ON (orden_serv.id_orden = fact_vent.numeroPedido)
					WHERE fact_vent.idFactura = comision_emp.id_factura
				...
				WHEN (id_nota_credito IS NOT NULL) THEN
					...
					FROM sa_orden orden_serv
						INNER JOIN sa_tipo_orden tipo_orden ON (orden_serv.id_tipo_orden = tipo_orden.id_tipo_orden)
						INNER JOIN cj_cc_encabezadofactura fact_vent ON (orden_serv.id_orden = fact_vent.numeroPedido)
						INNER JOIN cj_cc_notacredito nota_cred ON (fact_vent.idFactura = nota_cred.idDocumento)
					WHERE nota_cred.idNotaCredito = comision_emp.id_nota_credito
						AND fact_vent.idDepartamentoOrigenFactura = 1
						AND nota_cred.idDepartamentoNotaCredito = fact_vent.idDepartamentoOrigenFactura
			...
			(CASE
				WHEN (id_factura IS NOT NULL AND id_nota_credito IS NULL) THEN
					(SELECT fact_vent.fechaRegistroFactura FROM cj_cc_encabezadofactura fact_vent
					WHERE fact_vent.idFactura = comision_emp.id_factura)
				WHEN (id_nota_credito IS NOT NULL) THEN
					(SELECT nota_cred.fechaNotaCredito FROM cj_cc_notacredito nota_cred
					WHERE nota_cred.idNotaCredito = comision_emp.id_nota_credito)
				WHEN (id_vale_salida IS NOT NULL AND id_vale_entrada IS NULL) THEN
		...
		while ($rowDetalle = mysql_fetch_assoc($rsDetalle)) {
			...
			if (fmod($contFila, $maxRowsDetalle) == 1) {
				...
				$posY += 10;
				imagestring($img,1,0,$posY,str_pad(utf8_decode("FOLIO FACT."), 14, " ", STR_PAD_BOTH),$textColor); // <----
				imagestring($img,1,75,$posY,str_pad(utf8_decode("COD. CLTE"), 13, " ", STR_PAD_BOTH),$textColor); // <----
				imagestring($img,1,145,$posY,str_pad(utf8_decode("CLIENTE"), 20, " ", STR_PAD_BOTH),$textColor); // <----
				...
				imagestring($img,1,650,$posY,str_pad(utf8_decode("FEC. DCTO."), 10, " ", STR_PAD_BOTH),$textColor); // <----
