	...
	function load_page($args=''){//$page,$maxrows,$order,$ordertype,$capa,
		...
		if($rec){
			foreach($rec as $v){
				...
				$html.='<table class="order_table"><col width="20%" /><col width="40%" /><col width="20%" /><thead><tr><td>Codigo</td><td>Descripci&oacute;n</td><td>Unidad</td><td>Cantidad</td>';
				if($esCombo){ $html.= '<td>Precio</td><td>Importe</td><td>Costo</td>'; }
				$html.= '</tr></thead><tbody>';
	...
	<head>
		...
		<script>
            ...
            function exportarExcel(){
				window.open('reportes/sa_informe_paquetes_excel.php?valBusq='+toPaginator(datos),'_self');				
			}
			
            function exportarExcelSinUnidad(){
				window.open('reportes/sa_informe_paquetes_sin_unidad_excel.php?valBusq='+toPaginator(datos),'_self');				
			}
		</script>
	</head>
	<body>
...
//echo ' - '.inputSelect('iva',$ivas,0,'onchange=cambio_iva(this)',false); ?>
            ...
            <button type="button" onClick="exportarExcelSinUnidad();" class="noprint" style="cursor:default"><img src="../img/iconos/page_excel.png">Exportar Sin Unidad</button>
			<hr />
		</div>
