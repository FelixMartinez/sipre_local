...
if ($valCadBusq[1] == 1){//presupuesto y cotizacion
	...
	if($valCadBusq[2] == 3){//vale de salida
		$queryCliente = sprintf("SELECT
				...
				INNER JOIN an_uni_bas ON (vw_sa_vales_recepcion.id_uni_bas = an_uni_bas.id_uni_bas)				INNER JOIN an_transmision ON (an_uni_bas.trs_uni_bas = an_transmision.id_transmision)
...
$queryEmpresa = sprintf("SELECT logo_familia, IF (nombre_empresa_suc = '-', nombre_empresa, CONCAT_WS('-',nombre_empresa, nombre_empresa_suc)) as nombre_empresa, rif, direccion, telefono1, fax FROM vw_iv_empresas_sucursales WHERE id_empresa_reg = %s",
						$idEmpresa);
...
//NRO DE LLAVES
imageline($img,480,50,480,60,$textColor);
...
if ($valCadBusq[1] == 1){//presupuesto y cotizacion
	...
	WHERE
	  sa_presupuesto.id_presupuesto = %s AND sa_det_presup_articulo.estado_articulo <> 'DEVUELTO'",
		valTpDato($idDocumento,"int"));
}
else{
	if($valCadBusq[2] == 3) //vale de salida
	{
		$queryRepuestosGenerales = sprintf("SELECT
			...
			INNER JOIN sa_vale_salida ON (sa_orden.id_orden = sa_vale_salida.id_orden)
			WHERE
			sa_vale_salida.id_vale_salida = %s AND sa_det_orden_articulo.estado_articulo <> 'DEVUELTO'",
		valTpDato($idDocumento,"int"));
	}
	else //orden
	{
...
if (mysql_num_rows($rsOrdenDetRep) > 0){
	imagestring($img,1,300,$posY,"REPUESTOS",$textColor);
        ...
        imagestring($img,1,365,$posY,"DISPO.",$textColor);
        if ($valCadBusq[1] != 1){//presupuesto / cotizacion
            //imagestring($img,1,400,$posY,"CANT.",$textColor);            
            imagestring($img,1,400,$posY,"CANT.",$textColor);
            imagestring($img,1,440,$posY,"SOL.",$textColor);
            imagestring($img,1,465,$posY,"RESV.",$textColor);
        }        
	...
	while ($rowOrdenDetRep = mysql_fetch_assoc($rsOrdenDetRep)) {
		...
		imagestring($img,1,100,$posY,str_pad(strtoupper(substr($solicitudRep.$provienePaquete." ".$rowOrdenDetRep['descripcion'],0,50).$anex), 53, " ", STR_PAD_RIGHT),$textColor);
                ...
                imagestring($img,1,298,$posY,str_pad(cantidadDisponibleActual($rowOrdenDetRep['id_articulo'],$rowCliente['id_empresa']), 17, " ", STR_PAD_LEFT),$textColor);
                imagestring($img,1,330,$posY,str_pad($rowOrdenDetRep['cantidad'], 17, " ", STR_PAD_LEFT),$textColor);                
                if ($valCadBusq[1] != 1){//presupuesto
                    imagestring($img,1,367,$posY,str_pad(cantidadSolicitada($rowOrdenDetRep['id_det_orden_articulo']), 17, " ", STR_PAD_LEFT),$textColor);
                    imagestring($img,1,393,$posY,str_pad(cantidadDespachada($rowOrdenDetRep['id_det_orden_articulo']), 17, " ", STR_PAD_LEFT),$textColor);
                }
...
//imagestring($img2,1,415,$posY,str_pad("DESCUENTO X MANO DE OBRA:", 25, " ", STR_PAD_LEFT),$textColor);
////DESCUENTO
//imagestring($img2,1,540,$posY,str_pad(number_format($descuentoDetallesManoDeObra,2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
//$posY += 8;
//imageline($img2,412,$posY,602,$posY,$textColor);
//$posY += 2;
//
////TEXTO "DESCUENTO X REPUESTOS"
//imagestring($img2,1,415,$posY,str_pad("DESCUENTO X REPUESTOS:", 25, " ", STR_PAD_LEFT),$textColor);
////DESCUENTO
//imagestring($img2,1,540,$posY,str_pad(number_format($descuentoDetallesRepuestos,2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
//$posY += 8;
//imageline($img2,412,$posY,602,$posY,$textColor);
//$posY += 2;

//TEXTO "BASE IMPONIBLE"
//imagestring($img2,1,415,$posY,str_pad("BASE IMPONIBLE:", 25, " ", STR_PAD_LEFT),$textColor);
...
//ya no se usa
//if ($rowCliente['iva'] != 0 && $rowCliente['iva'] != "") {
//    $montoExento = $montoExento - ($montoExento*($rowCliente['porcentaje_descuento']/100));    
//}else{
//    $montoExento = 0;
//}

//$baseImponible = $rowCliente['base_imponible'];



//$baseImponible = $rowCliente['subtotal'] - $montoExento - round($nuevoSubtotalDescuento,2);
//$baseImponible = $rowCliente['subtotal'] - $rowCliente['subtotal_descuento'] + $rowCliente['subtotal_iva'];

//imagestring($img2,1,540,$posY,str_pad(number_format($baseImponible,2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
//$posY += 8;
//imageline($img2,412,$posY,602,$posY,$textColor);
//$posY += 2;
...
//MONTO EXENTO //anterior $montoExento
imagestring($img2,1,540,$posY,str_pad(number_format($rowCliente["monto_exento"],2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
$posY += 8;
imageline($img2,395,$posY,602,$posY,$textColor);
$posY += 2;

//TEXTO "IVA" // IVA ANTERIOR
//imagestring($img2,1,415,$posY,str_pad(nombreIva($rowCliente['idIva']).":" /*"IVA:"*/, 25, " ", STR_PAD_LEFT),$textColor);
////IVA
//$iva = round($baseImponible * $rowCliente['iva'] / 100,2);
//imagestring($img2,1,540,$posY,str_pad(number_format($iva,2,".",","), 12, " ", STR_PAD_LEFT),$textColor);
//$posY += 8;
//imageline($img2,412,$posY,602,$posY,$textColor);
//$posY += 2;
