		...
		if ($row['cont_aprob'] > 0) {
			...
			if ($modoFacturaOrden == 'FACTURA') {
				...
				$queryArt = sprintf("SELECT 
					...
					WHERE iv_articulos_costos.id_articulo = sa_det_orden_articulo.id_articulo ORDER BY id_articulo_costo DESC LIMIT 1) as costo,
				...
				while ($rowArt = mysql_fetch_array($rsArt)) {
					...
					if ($rowCont['nro_rpto_desp'] > 0) {
						$insertSQLDetFact = sprintf("INSERT INTO sa_det_vale_salida_articulo (id_vale_salida, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, costo, id_iva, iva, aprobado, tiempo_asignacion,  tiempo_aprobacion, id_empleado_aprobacion, estado_articulo)
...
function formAprobarOrden($idOrden, $sobregiro, $claveModulo, $idEmpleadoAprobFactura, $pageNum, $campOrd, $tpOrd, $valBusq, $maxRows, $soloRepuestos = false) {
	...
	if ($num_rows > 0) {
		...
		if ($rowTotalRptoDespachados['total_rpto'] > 0) {
			$objResponse->alert("La Orden no puede terminar, debido a que tiene Solicitud(es) de Repuestos pendiente(s).\nVerifique las Solicitudes de la Orden.");
		} else {
			if ($rowFechaEntrega['tiempo_entrega'] == NULL) {
				$objResponse->alert("Debe actualizar la fecha y hora de entrega del vehiculo");
			} else {
		...
		$objResponse->alert(utf8_encode("Usted no tiene acceso para realizar esta accion."));
	}
