...
function actualizarTOT($frmDatosFactura, $frmDetallesTrabajoRequeridos, $frmTotalFactura, $idOrdenTOT){
	...
	tipo_pago = '%s',
	aplica_libros = '%s',
	estatus = '1' WHERE id_orden_tot = '%s' ;",
...
//	return $objResponse->alert(validaTextArea($frmDatosFactura['txtObservacionFactura']));
	...
	if ($frmTotalFactura['hddObjTotalDetalles'] == 1){
		...
		for ($cont = 1; $cont <= strlen($frmDetallesTrabajoRequeridos['hddObj']); $cont++) {
			...
			if ($caracter != "|" && $caracter != "")
				...
				$queryActualizarTrabajos = "UPDATE sa_orden_tot_detalle
                                        SET monto = '".$frmDetallesTrabajoRequeridos["txtMonto".$cadena]."',
                                            cantidad = ".valTpDato($frmDetallesTrabajoRequeridos["txtCantidad".$cadena],"int")."
                                                            WHERE id_orden_tot_detalle = '".$frmDetallesTrabajoRequeridos["txtIdDescripcion".$cadena]."';";
		...
		foreach ($frmTotalFactura['cbx'] as $indice => $valor){
                        ...
                        if(!mysql_num_rows($rsVerificarIva)){//sino tiene el tipo de iva ya insertado, verificacion extra
                            ...
                            $queryInsertIva = sprintf("INSERT INTO sa_orden_tot_iva (id_orden_tot, base_imponible, subtotal_iva, id_iva, iva) 
                                                               VALUES (%s, '%s', '%s', '%s', '%s');", //eliminadi id_orden_tot_iva = '' para que genere automatico
                                                        $idOrdenTOT,
	...
	$arrayRespuesta = cargarAFactura($idOrdenTOT, $frmDatosFactura);//este tiene el commit, usado adentro porque tiene que ser antes de la funcion de ernesto        
        //mysql_query("COMMIT;");
		   ...
		   <table border='0' width='100%'><tr>
			   <td align='left' class='textoNegrita_12px' colspan='6'>
					...
					<table width='100%' >
						<tr>
							<td width='50%'>
								<button type='button' ".$displayTrabajo." ".$disabledTrabajo." id='btnInsertarTrabajoRequerido' name='btnInsertarTrabajoRequerido' onclick=\" $('tblTrabajoRequeridos').style.display = ''; $('divFlotante').style.display = ''; $('divFlotanteTitulo').innerHTML = 'Agregar Detalle'; $('tblListados').style.display = 'none'; centrarDiv($('divFlotante')); document.forms['frmTrabajoRequerido'].reset();\" title='Agregar Detalle'><img src='../img/iconos/ico_agregar.gif'/></button>
                                                                <button type='button' ".$displayAccesorio." ".$disabledAccesorio." id='btnInsertarAccesorios' name='btnInsertarAccesorios' onclick='abrirAccesorios();' title='Agregar Detalle'><img src='../img/iconos/ico_agregar.gif'/></button>
			&nbsp;
			<button type='button' id='btnEliminarTrabajoRequerido' name='btnEliminarTrabajoRequerido' disabled='disabled' onclick=\" xajax_eliminarTrabajoRequerido(xajax.getFormValues('frmDetallesTrabajoRequeridos'));\" title='Eliminar Detalle'><img src='../img/iconos/ico_quitar.gif'/></button>
							</td>
...
function asignarProveedor($idProveedor, $editar = 2){
	...
	$queryProveedor = sprintf("SELECT *, CONCAT_WS('-',lrif, rif) as lrif_rif FROM cp_proveedor WHERE id_proveedor = %s", valTpDato($idProveedor,"text"));
	...
	$objResponse = new xajaxResponse();
        ...
        if ($cantidadTot > 0 && $verificar == false){//si tiene mas de 1 pedir confirmacion, si es 0 o verificar true dejar pasar
            ...
            $queryRecepcion = sprintf("SELECT vw_sa_orden.*,  sa_filtro_orden.tot_accesorio
                       ...
                       INNER JOIN sa_filtro_orden ON sa_tipo_orden.id_filtro_orden = sa_filtro_orden.id_filtro_orden
                        WHERE id_orden = %s LIMIT 1", valTpDato($idOrden ,"text"));
...
function calcularTotal($frmTotalFactura,$frmDetallesTrabajoRequeridos,$accion){
	...
	if($subTotal < 0){
		...
		$objResponse->script("$('txtMontoExento').className = 'inputInicial';
							  $('txtSubTotal').className = 'inputInicial';
							  $('btnGuardar').disabled = '';");
...
function cargarAFactura($idOrdenTOT,$valForm){
	$objResponse = new xajaxResponse();
	...
	if($proveedor['credito'] == 'Si'){
		...
		//$date_result1=substr($rowOrdenTOT['fecha_factura_proveedor'],6,4).substr($rowOrdenTOT['fecha_factura_proveedor'],2,4).substr($rowOrdenTOT['fecha_factura_proveedor'],0,2);//anterior dañado
	...
	if ($rbtPorcentajeRetencion){
		...
		while($rowIva = mysql_fetch_assoc($rsIva)){//vuelvo a recorrer los registros
                        ...
                        $insertPago = sprintf("INSERT INTO cp_pagos_documentos (id_documento_pago, tipo_documento_pago, tipo_pago, id_documento, fecha_pago, numero_documento, banco_proveedor, banco_compania, cuenta_proveedor, cuenta_compania, monto_cancelado) VALUES (%s, 'FA', 'RETENCION', %s, NOW(), '%s', '-', '-', '-', '-', '%s')",
                                                $idFactura,
                                                $idRetencionCabezera,
                                                date("Ym",strtotime($rowOrdenTOT['fecha_origen'])).str_pad($numeroActual, 8, "0", STR_PAD_LEFT),
                                                $porcentajeRetencion);
	...
	if($_GET['acc']==0){
                $objResponse->script("verVentana('sa_imprimir_registro_compra_pdf.php?valBusq=".$idOrdenTOT."',900,900);");//mostrar factura
                if($idRetencionCabezera){
                    $objResponse->script("verVentana('../cxp/reportes/an_comprobante_retencion_compra_pdf.php?valBusq=".$idRetencionCabezera."',900,900);");//mostrar retencion
                }
...
function comboEmpresa($selId){
	...
	$query = sprintf("SELECT * FROM vw_iv_usuario_empresa WHERE id_usuario = %s ORDER BY nombre_empresa",
		...
		$html = "<select id=\"selEmpresa\" name=\"selEmpresa\">";
...
function contribuyente($idOrden){//muestra la seccion contribuyente, NO - 75% - 100%, sino es contribuyente se mancar radio NO
	$objResponse = new xajaxResponse();
	
	$queryEmpresa = sprintf("SELECT contribuyente_especial FROM pg_empresa WHERE id_empresa = (SELECT id_empresa FROM sa_orden_tot WHERE id_orden_tot = %s)",$idOrden);
...
function guardarOrdenCompra($idEmpresa,$idProveedor,$idValeRecepcion,$idUsuario,$formTrabajos){
	...
	//INSERTANDO NUMERO DE TOT gregor numeracion
	/*
	$sqlNumeroTOT = sprintf("SELECT IFNULL(MAX(numero_tot)+1, 1) as numero_tot from sa_orden_tot WHERE
			id_empresa = %s LIMIT 1", 
				valTpDato($_SESSION["idEmpresaUsuarioSysGts"],"int"));
	$rsSql = mysql_query($sqlNumeroTOT);
	if (!$rsSql) return $objResponse->alert(mysql_error()." Error generando numero de tot \n\nLine: ".__LINE__."\n\n File: ".__FILE__);
	$dtSql = mysql_fetch_assoc($rsSql);				
	$numeroTOT = $dtSql["numero_tot"];*/
	...
	$sqlNumeroTot = sprintf("SELECT * FROM pg_empresa_numeracion
		...
		ORDER BY aplica_sucursales DESC
		LIMIT 1",
		valTpDato($idEmpresa,"int"), //antes $_SESSION["idEmpresaUsuarioSysGts"] tomaba las numeraciones que no eran
		...
		$objResponse->script(sprintf("
			var elemento = new Element('tr', {'id':'trItm:%s', 'class':'textoGris_11px', 'title':'trItm:%s'}).adopt([
				%s new Element('td', {'align':'left', 'id':'tdItm:%s'}).setHTML(\"%s <input type='hidden' id='txtIdDescripcion%s' name='txtIdDescripcion%s' value='%s'/><input type='hidden' id='idAccesorio%s' name='idAccesorio%s' value='%s'/> \"),
                                new Element('td',{'align':'right'}).setHTML(\"<input type='text' id='txtCantidad%s' name='txtCantidad%s' ".$readonly." size='10' onkeypress='return numeros(event);' style='text-align:right;' />\"),
				new Element('td',{'align':'right'}).setHTML(\"<input type='text' id='txtMonto%s' name='txtMonto%s' title='txtMonto%s' ".$readonly." size='20' onkeypress='return validarSoloNumerosReales(event);' style='text-align:right' />\")
			]);
			elemento.injectBefore('trPie');
...
function listadoProveedores($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL){
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr class=\"".$clase."\">";
			$htmlTb .= "<td>"."<button type=\"button\" onclick=\"xajax_asignarProveedor('".$row['id_proveedor']."', ".$valCadBusq[1].");\" title=\"Seleccionar Proveedor\"><img src=\"../img/iconos/ico_aceptar.gif\"/></button>"."</td>";
...
function listadoVehiculos($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL){
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr class=\"".$clase."\">";
			$htmlTb .= "<td>"."<button type=\"button\" onclick=\"xajax_asignarVehiculo('".$row['id_orden']."',false,'".$valCadBusq[1]."');\" title=\"Seleccionar Vehiculo\"><img src=\"../img/iconos/ico_aceptar.gif\"/></button>"."</td>";
	...
	//$objResponse->assign("tdCriterioBusquedaInput","innerHTML","<input type='text' id='txtCriterioBusqueda' name='txtCriterioBusqueda' onkeyup='xajax_listadoVehiculos(".$totalPages.",\"".$campOrd."\",\"".$tpOrd."\",\"jojo\",".$maxRows.");' />");
	//$objResponse->assign("tdCriterioBusquedaButton","innerHTML","<button class='noprint' type='button' id='btnBuscar' name='btnBuscar' onclick='alert(\"holaOrden\")' style='cursor:default'><table align='center' cellpadding='0' cellspacing='0'><tr><td>&nbsp;</td><td><img src='../img/iconos/find.png'/></td><td>&nbsp;</td><td>Buscar</td></tr></table></button>");
...
//	}
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\" height=\"22\">";
                ...
                $htmlTb .= "<td align=\"center\" ><button title='Seleccionar Accesorio' onclick='xajax_asignarAccesorio(".$row['id_precio_tot'].",xajax.getFormValues(\"frmTrabajoRequerido\"),xajax.getFormValues(\"frmDetallesTrabajoRequeridos\"));' type='button'><img src='../img/iconos/ico_aceptar.gif'></button></td>";
