...
/*	if ($valCadBusq[0] > 0){
	...
	$rsTOT = mysql_query($queryTOT) or die(mysql_error());
	
	$sqlOrd = ($campOrd != "") ? sprintf(" ORDER BY %s %s", $campOrd, $tpOrd) : "";
	
	$queryLimitTOT = $queryTOT.$sqlOrd." LIMIT ".$maxRows." OFFSET ".$startRow.";";
	$rsLimitTOT = mysql_query($queryLimitTOT) or die(mysql_error());
	...
	/*
	$htmlTh .= sprintf("<tr class=\"tituloColumna\">
	...
	*/
	while ($rowTOT = mysql_fetch_assoc($rsLimitTOT)) {
		...
		$htmlTb .= "<tr class=\"".$clase."\">";
			...
			//ver pdf
			$htmlTb .= sprintf("<td %s class=\"noprint puntero\" ><img src='../img/iconos/ico_print.png' onclick=\"verVentana('sa_imprimir_registro_compra_pdf.php?valBusq=".$rowTOT['id_orden_tot']."',900,900);\" title='Ver PDF Registro de Compra' /></td>", $display2);
