...
function verMensual($fecha_desde,$fecha_hasta){
    ...
    $query_presupuestos_entregados = "SELECT COUNT(id_presupuesto) as presupuesto, MONTH(fecha_presupuesto) as mes FROM sa_presupuesto 
                                       WHERE tipo_presupuesto = 1 AND DATE(fecha_presupuesto) BETWEEN '$fecha_desde' AND '$fecha_hasta' GROUP BY mes";
...
function verSemanal($fecha_desde, $fecha_hasta){
    ...
    foreach ($array_titulo_fechas as $fecha) {
        if ($fecha != "Total") {
         ...
         $query_presupuestos_entregados = "SELECT COUNT(id_presupuesto) as presupuesto, MONTH(fecha_presupuesto) as mes FROM sa_presupuesto 
                                       WHERE tipo_presupuesto = 1 AND DATE(fecha_presupuesto) BETWEEN '$fecha1' AND '$fecha2' GROUP BY mes";
    $presupuestos_entregados = mysql_query($query_presupuestos_entregados, $conex) or die("Error de Selección 'Presupuestos entregados': " . mysql_error() . "<br>Error Nº: " . mysql_errno());
