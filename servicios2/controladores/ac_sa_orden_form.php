...
//            $objResponse->assign("txtIvaRepuesto","value",$rowArticulo['iva']."%");
	...
	if ($hddNumeroArt == "") { // NO EXISTE EL ARTICULO EN LA LISTA DEL PRESUPUESTO
		...
		$objResponse->loadCommands(asignarPrecio($idArticulo, $selId, false, $precioUnitario));
...
//	mysql_query("SET NAMES 'latin1';");
	...
	while($rowArt = mysql_fetch_array($rsArt)) {
		if($rowArt['nro_rpto_desp'] > 0) {
			...
			WHERE id_articulo = %s
				AND id_casilla = %s;",
				valTpDato(doubleval($rowArt['nro_rpto_desp']), "double"),
				valTpDato($rowArt['id_articulo'], "int"),
				valTpDato($rowArt['id_casilla'], "int"));
			mysql_query("SET NAMES 'utf8';");
			$Result1 = mysql_query($updateSQL);
			if (!$Result1) return $objResponse->alert(mysql_error()."\n\nLine: ".__LINE__);
			mysql_query("SET NAMES 'latin1';");*/
			...
			$queryArtValeSalida = sprintf("SELECT 
				...
				FROM sa_det_vale_salida_articulo WHERE id_vale_salida = %s AND id_articulo = %s LIMIT 1",			
				$rowArt["id_empresa"],
				valTpDato($rowArt['id_articulo'], "int"),
				valTpDato($idValeSalida, "int"),
				valTpDato($rowArt['id_articulo'], "int"));		
			...
			$insertSQLKardex = sprintf("INSERT INTO iv_kardex(id_modulo, id_documento, id_articulo, id_casilla, tipo_movimiento, tipo_documento_movimiento, cantidad, precio, costo, costo_cargo, porcentaje_descuento, subtotal_descuento, id_clave_movimiento, estado, fecha_movimiento, hora_movimiento) 
			...
			VALUE (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
				...
				valTpDato($rowArt['nro_rpto_desp'], "int"),
				valTpDato($rowArtValeSalida['precio_unitario'], "real_inglesa"),
				valTpDato($rowArtValeSalida['costo_compra'], "real_inglesa"),
				valTpDato(0, "int"),//costo cargo
				valTpDato(0, "text"),
			...
			// ACTUALIZA LOS MOVIMIENTOS TOTALES DEL ARTICULO - Roger
                        $Result1 = actualizarMovimientoTotal($rowArt['id_articulo'], $idEmpresa);
                        if ($Result1[0] != true && strlen($Result1[1]) > 0) { return $objResponse->alert($Result1[1]); }

                        // ACTUALIZA LOS SALDOS DEL ARTICULO (ENTRADAS, SALIDAS, RESERVADAS Y ESPERA)
                        $Result1 = actualizarSaldos($rowArt['id_articulo'], $rowArt['id_casilla']);
                        if ($Result1[0] != true && strlen($Result1[1]) > 0) { return $objResponse->alert($Result1[1]); }
	...
	if(!$queryIdEmpleado){ $objResponse->alert("Error buscando id empleado para devolucion vale \n".mysql_error()."\n Linea:".__LINE__); }
...
//		}
		...
		WHERE sa_orden.id_orden = %s",
			...
			valTpDato("", "int"),//no se usa
			valTpDato("", "real_inglesa"),//no se usa
		...
		if ($valFormTotalDcto['hddTipoDocumento'] == 1) { // PRESUPUESTO
			...
			VALUE (%s, %s, %s, %s, %s, %s, NOW(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
				...
				valTpDato($valFormTotalDcto['txtDescuento'], "real_inglesa"),
				valTpDato($valFormTotalDcto['txtSubTotalDescuento'], "real_inglesa"),
				valTpDato(0, "int"),//ahora no usado
				valTpDato(0, "real_inglesa"),//ahora no usado
				valTpDato($valFormTotalDcto['txtSubTotal'], "real_inglesa"),
				valTpDato(0, "real_inglesa"),//ahora no usado                                
				valTpDato($totalSumatoriaIvas, "real_inglesa"),//El total subivas
                                valTpDato($valFormTotalDcto['txtMontoExento'], "real_inglesa"),//nuevo
		...
		foreach($arrayObj as $indice => $valor) {
			if (strlen($valFormListaArt['hddIdArt'.$valor]) > 0) {
				if ($valFormListaArt['hddIdPedDet'.$valor] == "") {
					...
					$insertSQL = sprintf("INSERT INTO sa_det_orden_articulo (id_orden, id_articulo, cantidad, id_precio, precio_unitario, costo, id_iva, iva, tiempo_asignacion, id_empleado_aprobacion, aprobado)
					VALUE (%s, %s, %s, %s, %s, %s, %s, %s, NOW(), %s, %s);",
						valTpDato($idDocumentoVenta, "int"),
		...
		foreach($arrayObjPaq as $indicePaq => $valorPaq) {
			...
			if ($idPaquete > 0) {
				if ($valFormPaq['hddIdPedDetPaq'.$valorPaq] == "") {
					...
					$insertSQL = sprintf("INSERT INTO sa_det_orden_articulo (id_orden, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, costo, tiempo_asignacion, id_empleado_aprobacion, aprobado) 		
					SELECT 
						%s,
...
function insertarArticulo($valForm, $valFormListaArticulo, $valFormTotalDcto) {
	...
	if ($hddNumeroArt == "") {
		...
		foreach ($arrayVal as $indice => $valor) {
			...
			if($rowConfig12["valor"] == 1){//es el costo a comprobar en la insercion, 1 costo del articulo
			...
			if($costoComparar == NULL){ return $objResponse->alert("El articulo no posee costo ".$idArticuloCosto." definido"); }//sino existe ningun tipo de costo	
			...
			if ($valForm['hddBajarPrecio'] == "subir" && round($valForm['hddPrecioArtAsig'],2) > round($precioArt,2)) {
				...
				//TODOS valida el costo para poder continuar
			} else if ($valForm['hddBajarPrecio'] !='debajo_costo' && $costoComparar > round($precioArt,2)) {
