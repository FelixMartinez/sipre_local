	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb.= "<tr class=\"".$clase."\" onmouseover=\"this.className = 'trSobre';\" onmouseout=\"this.className = '".$clase."';\" height=\"22\">";
			...
			if($row['id_tipo_estado'] != 5){//FINALIZADO si esta finalizado o terminado 13 no permitir modificar mas
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"window.open('sa_orden_form.php?doc_type=4&id=%s&ide=%s&acc=2','_self');\" src=\"../img/iconos/generarPresupuesto.png\"/>",
					$row['id_orden'],
					$row['id_empresa']);
			}
			$htmlTb .= "</td>";
			...
			&& !(($row['id_tipo_estado'] == 1 || $row['id_tipo_estado'] == 2 || $row['id_tipo_estado'] == 3) //1 ABIERTO //2 EN ESPERA //3 PROCESO
				...
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"xajax_verificarBloqueoSolicitud('%s','%s',4);\" src=\"../img/iconos/aprobar_presup.png\"/></td>",
					$row['id_orden'],
					$row['id_empresa']);
			}
			$htmlTb .= "</td>";
			//Boton ver orden
			$htmlTb .= "<td class=\"noprint\">";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"window.open('sa_orden_form.php?doc_type=2&id=%s&ide=%s&acc=2&cons=0','_self');\" src=\"../img/iconos/ico_view.png\"/>",
					$row['id_orden'],
					$row['id_empresa']);
			$htmlTb .= "</td>";
			//Boton editar orden
			...
			if ($row['id_tipo_estado'] != 5) { // TERMINADO
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"xajax_verificarBloqueoSolicitud('%s','%s',3);\" src=\"../img/iconos/ico_edit.png\"/>",
					$row['id_orden'],
					$row['id_empresa']);
			}
			$htmlTb .= "</td>";
			...
			if ($row['id_tipo_estado'] == 3) { // PROCESO
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"xajax_verificarAprobacion('%s');\" src=\"../img/iconos/time_go.png\"/>",
						$row['id_orden']);
			}
			$htmlTb .= "</td>";
			//Boton imprimir orden pdf
			$htmlTb .= "<td class=\"noprint\">";
				$htmlTb .= sprintf("<img class=\"puntero\" onclick=\"verVentana('sa_imprimir_presupuesto_pdf.php?valBusq=%s|2|0', 1000, 500);\" src=\"../img/iconos/ico_print.png\"/>",
					$row['id_orden']);
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
