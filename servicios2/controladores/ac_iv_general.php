...
function temparioUnico($idEmpresa){
    $query = sprintf("SELECT valor_parametro FROM pg_parametros_empresas "
            . "WHERE descripcion_parametro = 'TEMPARIO UNICO' "
            . "AND (id_empresa = %s OR id_empresa = (SELECT padre.id_empresa_padre "
                                                    . "FROM pg_empresa padre WHERE padre.id_empresa = %s))",
            $idEmpresa,
            $idEmpresa);
...
function empresasVinculadas($idEmpresa){
    $arrayEmpresas = array();
    $queryEmpresas = sprintf("SELECT id_empresa FROM pg_empresa WHERE id_empresa = %s OR id_empresa_padre = %s 
                                    OR id_empresa_padre = (SELECT padre.id_empresa_padre FROM pg_empresa padre WHERE padre.id_empresa = %s)
                                    OR id_empresa = (SELECT padre2.id_empresa_padre FROM pg_empresa padre2 WHERE padre2.id_empresa = %s)",
                          $idEmpresa,
                          $idEmpresa,
                          $idEmpresa,
                          $idEmpresa);
...
function nombreEmpleado($idEmpleado){
    $query = sprintf("SELECT CONCAT_WS(' ', nombre_empleado, apellido) as nombre_completo "
            . "FROM pg_empleado WHERE id_empleado = %s LIMIT 1",
