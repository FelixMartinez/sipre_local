...
//                                                }//FIN PAQUETES DUPLICADO
		...
		WHERE id_presupuesto = %s",
			...
			valTpDato($valFormTotalDcto['hddIdIvaVenta'], "int"),//no se usa
			valTpDato($valFormTotalDcto['txtIvaVenta'], "real_inglesa"),//no se usa
		...
		if ($valFormTotalDcto['hddTipoDocumento'] == 1) { // PRESUPUESTO
			...
			VALUE (%s, now(), %s, %s, %s, %s, %s, %s, NOW(), %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);",
				...
				valTpDato($valFormTotalDcto['txtSubTotal'], "real_inglesa"),
				valTpDato(0, "real_inglesa"),//ahora no usado
				valTpDato($totalSumatoriaIvas, "real_inglesa"),
                                1,
				valTpDato($valFormTotalDcto['txtMontoExento'], "real_inglesa"),//nuevo
                                valTpDato($valFormTotalDcto['txtTotalPresupuesto'], "real_inglesa")//nuevo
		...
		foreach($arrayObj as $indice => $valor) {
			if (strlen($valFormListaArt['hddIdArt'.$valor]) > 0) {
				if ($valFormListaArt['hddIdPedDet'.$valor] == "") {
					...
					$insertSQL = sprintf("INSERT INTO sa_det_presup_articulo (id_presupuesto, id_articulo, cantidad, id_precio, precio_unitario, costo, id_iva, iva, tiempo_asignacion, id_empleado_aprobacion, aprobado)
		...
		foreach($arrayObjPaq as $indicePaq => $valorPaq) {
			...
			if ($idPaquete > 0) {
				if ($valFormPaq['hddIdPedDetPaq'.$valorPaq] == "") {
					...
					if ($rowEmpresa['paquete_combo']==0) { // PAQUETE REPUESTOS costo promedio
					$insertSQL = sprintf("INSERT INTO sa_det_presup_articulo (id_presupuesto, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, costo, tiempo_asignacion, id_empleado_aprobacion, aprobado) 		
					SELECT 
						%s,
						iv_articulos.id_articulo,
						%s,
...
//    if(!$rs) { return array(false, mysql_error()."\n\nLine: ".__LINE__."\n\nQuery:".$query); } 
    ...
    while($rowArticuloPresup = mysql_fetch_assoc($rs)){
        $query = sprintf("INSERT INTO sa_det_orden_articulo (id_orden, id_articulo, id_paquete, cantidad, id_precio, precio_unitario, id_articulo_costo, id_iva, iva, aprobado, tiempo_asignacion, tiempo_aprobacion, id_empleado_aprobacion, estado_articulo)
