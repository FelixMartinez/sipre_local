...
function anularCheque($formAnulacion){
	...
	if(!xvalidaAcceso($objResponse,"cjrs_devolucion_cheque_list","insertar")){ return $objResponse; }
	...
	//ACTUALIZO EL ESTADO DEL CHEQUE A ANULADO
	$query = sprintf("UPDATE cj_cc_cheque SET estatus = 0, id_empleado_anulado = %s, motivo_anulacion = %s WHERE id_cheque = %s",//0 = anulado, 1 = activo
...
function devolverCheque($idCheque){
	...
	//ACTUALIZO EL ESTADO DEL CHEQUE A ANULADO
	$query = sprintf("UPDATE cj_cc_cheque SET saldo_cheque = 0, estatus = 0, id_empleado_anulado = %s, motivo_anulacion = %s WHERE id_cheque = %s",//0 = anulado, 1 = activo
					  valTpDato($_SESSION['idEmpleadoSysGts'], "int"),
					  valTpDato('DEVOLUCION DE CHEQUE '.$numeroCheque, "text"),
					  $idCheque);
	$rs = mysql_query($query);
...
function listaCheque($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaCheque", "7%", $pageNum, "fecha_cheque", $campOrd, $tpOrd, $valBusq, $maxRows, "Fecha Cheque");		
		$htmlTh .= ordenarCampo("xajax_listaCheque", "7%", $pageNum, "tipo_cheque", $campOrd, $tpOrd, $valBusq, $maxRows, "Tipo Cheque");
		$htmlTh .= ordenarCampo("xajax_listaCheque", "8%", $pageNum, "numero_cheque", $campOrd, $tpOrd, $valBusq, $maxRows, "Nro. Cheque");
		$htmlTh .= ordenarCampo("xajax_listaCheque", "38%", $pageNum, "nombre_cliente", $campOrd, $tpOrd, $valBusq, $maxRows, "Cliente");
		$htmlTh .= ordenarCampo("xajax_listaCheque", "10%", $pageNum, "descripcion_estado_cheque", $campOrd, $tpOrd, $valBusq, $maxRows, "Estado");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		if ($row['estatus'] == 0){ // 0 = ANULADO ; 1 = ACTIVO
			$imgEstatus = "<img src=\"../img/iconos/ico_rojo.gif\" title=\"Cheque Anulado\"/>";
		} else if ($row['estatus'] == 1){ // 0 = ANULADO ; 1 = ACTIVO
			$imgEstatus = "<img src=\"../img/iconos/ico_verde.gif\" title=\"Cheque Activo\"/>";
		}
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				...
				$htmlTb .= "</tr>";
				$htmlTb .= ((strlen($row['observacion_cheque']) > 0) ? "<tr><td><span class=\"textoNegritaCursiva_9px\">".utf8_encode($row['observacion_cheque'])."</span></td></tr>" : "");
				$htmlTb .= "</table>";
			$htmlTb .= "</td>";
			...
			$htmlTb .= "<td>";			
				$htmlTb .= sprintf("<a href=\"javascript:verVentana('reportes/cjrs_recibo_impresion_pdf.php?idTpDcto=5&id=%s', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Recibo Cheque PDF\"/></a>",
					$row['id_cheque']);			
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
	...
	if ($contFila > 0) {
		...
		if ($pageNum == $totalPages) {
			$rs = mysql_query($query);
			while ($row = mysql_fetch_assoc($rs)) {
				$arrayTotalFinal[9] += $row['saldo_cheque'];
				$arrayTotalFinal[10] += $row['monto_neto_cheque'];
			}
