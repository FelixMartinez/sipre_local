...
function exportarCheque($frmBuscar) {
	...
	$objResponse->script("window.open('reportes/cjrs_cheque_historico_excel.php?valBusq=".$valBusq."','_self');");
...
function imprimirCheque($frmBuscar){
	...
	$objResponse->script(sprintf("verVentana('reportes/cjrs_cheque_historico_pdf.php?valBusq=%s&lstOrientacionPDF=%s',890,550)", $valBusq, $frmBuscar['lstOrientacionPDF']));
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			if ($row['saldo_cheque'] > 0 && $row['estatus'] == 1) {
				$htmlTb .= sprintf("<a href=\"cjrs_cheque_form.php?id=%s\" target=\"_self\"><img src=\"../img/iconos/application_view_columns_add.png\" title=\"Pagar Dctos.\"/></a>",
					$row['id_cheque']);
			}
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
