...
if(!(validaAcceso("cjrs_transferencia_list","insertar"))) {
...
include("controladores/ac_cjrs_transferencia_form.php");
...
<head>
	...
	function validarFrmAgregarDcto(tipoDcto) {
		...
		&& validarCampo('lstTipoTransferencia', 't', 'lista') == true
		&& validarCampo('txtMontoPago', 't', 'monto') == true)) {
			validarCampo('txtIdCliente', 't', '');
			validarCampo('lstTipoTransferencia', 't', 'lista');
	...
	function validarFrmDcto(){
		...
		/*if(inArray(tipoTransferencia, [1,3])){//tipo cliente y pnd requiere cliente
			if (!(validarCampo('txtIdCliente','t','') == true)){					
				validarCampo('txtIdCliente','t','');
				error = true;
			}
		...
		if (error == true) {
			...
			if (confirm(mensaje)) { 
				xajax_guardarTransferencia(xajax.getFormValues('frmDcto'), xajax.getFormValues('frmDetallePago'), xajax.getFormValues('frmListaDctoPagado'));
			}
		}
	}
	...
	function asignarTipoTransferencia(lstTipoTransferencia){
		if (limpiarTransferencia() != true) {
			selectedOption('lstTipoTransferencia', byId('hddTipoTransferencia').value);
		} else {
			byId('hddTipoTransferencia').value = lstTipoTransferencia;
		}
		
		if (inArray(lstTipoTransferencia, [1,3])) { // 1 = Cliente, 3 = PND
			...
			} else if (lstTipoTransferencia == 3) { // 3 = PND
				byId('btnAgregarFactura').style.display = "";
				byId('btnAgregarNotaDebito').style.display = "";
				byId('btnAgregarAnticipo').style.display = "";
	...
	function limpiarTransferencia(){
		if ((byId('cbx4') != undefined && confirm('Se elimaran todos los documentos a Cobrar. ¿Seguro desea cambiar el tipo de transferencia?'))
		|| byId('cbx4') == undefined) {
