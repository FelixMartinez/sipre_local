...
function buscarComision($frmBuscar) {
	...
	$valBusq = sprintf("%s|%s|%s|%s|%s|%s|%s",
		...
		$frmBuscar['lstTipoPorcentajeBuscar'],
...
function cargaLstTipoComision($selId = "") {
	...
	$html = "<select id=\"lstTipoComision\" name=\"lstTipoComision\" class=\"inputHabilitado\" style=\"width:99%\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
...
function formComision($idComision, $frmComision) {
	...
	if ($idComision > 0) {
		...
		$objResponse->call(asignarTipoPorcentaje,$row['tipo_porcentaje']);
...
function guardarComision($frmComision, $frmListaComision) {
	...
	$objResponse->script("
	byId('btnCancelarComision').click();");
