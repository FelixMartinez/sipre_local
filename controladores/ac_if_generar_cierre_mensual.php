					...
					if (isset($arrayVentaAsesor)) {
						foreach ($arrayVentaAsesor as $indice2 => $valor2) {
							...
							&& $arrayVentaAsesor[$indice2]['id_empleado'] == $rowServ['id_empleado']) {
								...
								$arrayVentaAsesor[$indice2]['total_ut'] += $signo * $rowServ['uts'];
...
function facturacionMostrador($idEmpresa, $mesCierre, $anoCierre) {
	...
	while ($rowEmpleado = mysql_fetch_assoc($rsEmpleado)) {
		...
		FROM cj_cc_notacredito cxc_nc
			INNER JOIN cj_cc_encabezadofactura cxc_fact ON (cxc_nc.idDocumento = cxc_fact.idFactura) %s;", $sqlBusq2);
		$rs = mysql_query($query);
		if (!$rs) return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__, $totalVentaVendedores);
...
function facturacionTecnicos($idEmpresa, $mesCierre, $anoCierre, $rowConfig301 = NULL) {
	global $conex;
		...
		while ($rowEmpleado = mysql_fetch_assoc($rsEmpleado)) {
			...
			$sql5 = sprintf("SELECT
				...
				IFNULL(SUM((CASE a.id_modo
					...
					WHEN 2 THEN -- PRECIO
						a.precio
				END)), 0) AS valor_uts_vale_dev
				
			FROM sa_det_vale_salida_tempario det_orden_temp
				INNER JOIN sa_v_vale_informe_final_tempario_dev a ON (det_orden_temp.id_det_vale_salida_tempario = a.id_det_vale_salida_tempario) %s;", $sqlBusq);
			$rs5 = mysql_query($sql5);
			if (!$rs5) return array(false, mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			$totalRowsValeEnt = mysql_num_rows($rs5);
			$row5 = mysql_fetch_assoc($rs5);
			
			$totalMecanicoUts = $row2['uts'] - $row3['uts_dev'] + $row4['uts_vale'] - $row5['uts_vale_dev'];
			$totalMecanicoBs = $row2['valor_uts'] - $row3['valor_uts_dev'] + $row4['valor_uts_vale'] - $row5['valor_uts_vale_dev'];
