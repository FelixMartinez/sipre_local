...
function formEmpleado($idEmpleado) {
	...
	if ($idEmpleado > 0) {
		...
		$objResponse->assign("txtFechaIngreso","value",date("d-m-Y",strtotime($row['fecha_ingreso'])));
		if ($row['fecha_egreso'] != '')
			$objResponse->assign("txtFechaEgreso","value",date("d-m-Y",strtotime($row['fecha_egreso'])));
	} else {
		...
		$objResponse->assign("txtFechaIngreso","value",date("d-m-Y"));
	}
...
function listaEmpleado($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td align=\"center\">".date("d-m-Y",strtotime($row['fecha_ingreso']))."</td>";
			$htmlTb .= "<td>";
