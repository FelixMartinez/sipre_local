...
function buscarAccesorio($frmBuscar) {
	...
	$objResponse->loadCommands(listAccesorio(0, "nom_accesorio", "ASC", $valBusq));
...
function buscarComision($frmBuscar) {
	...
	$valBusq = sprintf("%s|%s|%s|%s|%s|%s",
		...
		$frmBuscar['lstCargoBuscar'],
		$frmBuscar['lstModuloBuscar'],
		$frmBuscar['lstTipoComisionBuscar'],
		$frmBuscar['lstModoComisionBuscar'],
...
function cargaLstCargoBuscar($idEmpresa, $selId = "") {
	...
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$html = "<select id=\"lstCargoBuscar\" name=\"lstCargoBuscar\" class=\"inputHabilitado\" onchange=\"byId('btnBuscar').click();\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function cargaLstModuloBuscar($selId = ""){
	...
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$html = "<select id=\"lstModuloBuscar\" name=\"lstModuloBuscar\" class=\"inputHabilitado\" onchange=\"byId('btnBuscar').click();\">";
		$html .="<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
		$selected = ($selId == $row['id_enlace_concepto']) ? "selected='selected'" : "";
...
function cargaLstTipoComision($selId = "") {
	...
	$html = "<select id=\"lstTipoComision\" name=\"lstTipoComision\" onchange=\"asignarTipoComision();\" class=\"inputHabilitado\" style=\"width:99%\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
...
function formComision($idComision, $frmComision) {
	...
	if ($idComision > 0) {
		...
		$objResponse->call(asignarTipoComision,$row['tipo_porcentaje']);
...
function guardarComision($frmComision, $frmListaComision) {
	...
	if(isset($arrayObj3)) {
		foreach($arrayObj3 as $indice => $valor) {
			if ($frmComision['hddIdComisionProductividadItm'.$valor] > 0) { // EDITA
...
function insertaNivelProductUnidad($frmComision, $frmNivelPrdoctUnidad){
	$objResponse = new xajaxResponse();
	...
	$Result1 = insertarItemNivelProductividadUnidad($contFila, $frmNivelPrdoctUnidad);
	if ($Result1[0] != true && strlen($Result1[1]) > 0) {
		return $objResponse->alert($Result1[1]);
	} else if ($Result1[0] == true) {
		$contFila = $Result1[2];
...
function listAccesorio($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10, $totalRows = NULL) {
	$objResponse = new xajaxResponse();
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		$htmlTh .= "<td></td>";
		$htmlTh .= ordenarCampo("xajax_listAccesorio", "5%", $pageNum, "id_accesorio", $campOrd, $tpOrd, $valBusq, $maxRows, ("Id"));
		$htmlTh .= ordenarCampo("xajax_listAccesorio", "40%", $pageNum, "nom_accesorio", $campOrd, $tpOrd, $valBusq, $maxRows, ("Articulo"));
		$htmlTh .= ordenarCampo("xajax_listAccesorio", "55%", $pageNum, "des_accesorio", $campOrd, $tpOrd, $valBusq, $maxRows, ("Descripcion"));
	$htmlTh .= "</tr>";
			...
			$htmlTf .= "<tr class=\"tituloCampo\">";
				...
				$htmlTf .= "<td align=\"center\" class=\"tituloColumna\" width=\"210\">";
					...
					$htmlTf .= "<tr align=\"center\">";
						$htmlTf .= "<td width=\"25\">";
						if ($pageNum > 0) {
							$htmlTf .= sprintf("<a class=\"puntero\" onclick=\"xajax_listAccesorio(%s,'%s','%s','%s',%s);\">%s</a>",
						...
						$htmlTf .= "<td width=\"100\">";
							...
							$htmlTf .= sprintf("<select id=\"pageNum\" name=\"pageNum\" onchange=\"xajax_listAccesorio(%s,'%s','%s','%s',%s)\">",
	...
	$objResponse->assign("tdLstArticulo","innerHTML",$htmlTblIni.$htmlTf.$htmlTh.$htmlTb.$htmlTf.$htmlTblFin);
...
function listaComision($pageNum = 0, $campOrd = "", $tpOrd = "ASC", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	if ($valCadBusq[1] != "-1" && $valCadBusq[1] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("cargo.id_cargo = %s",
	...
	if ($valCadBusq[3] != "-1" && $valCadBusq[3] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("comision.tipo_comision = %s",
			valTpDato($valCadBusq[3], "int"));
	}
	
	if ($valCadBusq[4] != "-1" && $valCadBusq[4] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("comision.modo_comision = %s",
	...
	$htmlTh .= "<tr class=\"tituloColumna\">";
		$htmlTh .= ordenarCampo("xajax_listaComision", "", $pageNum, "comision.id_comision", $campOrd, $tpOrd, $valBusq, $maxRows, "id");
		...
		$htmlTh .= ordenarCampo("xajax_listaComision", "10%", $pageNum, "modo_comision", $campOrd, $tpOrd, $valBusq, $maxRows, "Modo Comisión");
		$htmlTh .= ordenarCampo("xajax_listaComision", "8%", $pageNum, "cantidad_tipo_orden", $campOrd, $tpOrd, $valBusq, $maxRows, "Filtro de Orden");
		$htmlTh .= ordenarCampo("xajax_listaComision", "16%", $pageNum, "cantidad_empresas", $campOrd, $tpOrd, $valBusq, $maxRows, "Dctos. de la(s) Empresa(s)");
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			$htmlTb .= "<td>".utf8_encode($row['id_comision'])."</td>";
			...
			$htmlTb .= "<td align=\"right\">".number_format($row['porcentaje_comision'],3)."</td>";
...
$xajax->register(XAJAX_FUNCTION,"listAccesorio");
...
function insertarItemNivelProductividadUnidad($contFila, $arrayItemNivelProductUnidad = "", $idNivelProductividadUnidad = ""){
	...
	$htmlItmPie = sprintf("$('#trItmPieNivelProductividadUnidad').before('".
		"<tr id=\"trItemPieNivelProdUnidad%s\"  align=\"center\" class=\"%s\">".
			...
			"</td>".
			"<td>".
				"<a id=\"aEditarNivelUnidad%s\" class=\"modalImg\" rel=\"#divFlotante4\">".
					"<img id=\"idEditar%s\" name=\"imgEditar%s\" class=\"puntero\" title=\"Editar\" src=\"img/iconos/pencil.png\">".
				"</a>".
		...
		byId('aEditarNivelUnidad%s').onclick = function() {
			abrirDivFlotante2(this, 'tblNivelProducitividadUnidad', 'Editar Nivel Productividad Por Unidad', 'editarNivelProdutividadUnidad');
