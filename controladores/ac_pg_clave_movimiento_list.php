...
function cargaLstDctoGenerado($selId = "") {
	...
	$html = "<select id=\"lstDctoGenerado\" name=\"lstDctoGenerado\" class=\"inputHabilitado\" onchange=\"xajax_asignarTipoMovimiento(xajax.getFormValues('frmClaveMovimiento'))\" style=\"width:150px\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	foreach ($array as $indice => $valor) {
		$selected = ($selId == $indice) ? "selected=\"selected\"" : "";
		
		$html .= "<option ".$selected." value=\"".($indice)."\">".utf8_encode($array[$indice])."</option>";
	}
	$html .= "</select>";
...
function cargaLstModulo($selId = ""){
	...
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$html = "<select id=\"lstModulo\" name=\"lstModulo\">";
		$html .="<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function guardarClaveMovimiento($frmClaveMovimiento, $frmListaClaveMovimiento) {
	...
	if ($idClaveMovimiento > 0) {
		...
		$insertSQL = sprintf("INSERT INTO pg_clave_movimiento (clave, descripcion, id_modulo, tipo, documento_genera, id_clave_movimiento_contra, pago_contado, pago_credito, id_numeracion_documento, id_numeracion_control, afecta_consumo, proveedor_individual, contra_cuenta, estadistica, prefijo_folio_multiple, area)
...
function listaClaveMovimiento($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL) {
	...
	if ($valCadBusq[0] != "-1" && $valCadBusq[0] != "") {
		$cond = (strlen($sqlBusq) > 0) ? " AND " : " WHERE ";
		$sqlBusq .= $cond.sprintf("vw_pg_clave_mov.id_modulo = %s",
	...
	$htmlTh .= "<tr align=\"center\" class=\"tituloColumna\">";
		...
		$htmlTh .= ordenarCampo("xajax_listaClaveMovimiento", "24%", $pageNum, "descripcion", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Descripción"));
		$htmlTh .= ordenarCampo("xajax_listaClaveMovimiento", "8%", $pageNum, "tipo_movimiento", $campOrd, $tpOrd, $valBusq, $maxRows, utf8_encode("Tipo"));
