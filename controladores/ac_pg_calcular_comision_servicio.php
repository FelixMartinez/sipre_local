...
function calcular_comision_factura($idDocumento, $generarDirecto = true) {
	global $conex;
...
function calcular_comision_vale_salida($idDocumento, $generarDirecto = true) {
	global $conex;
	...
	while ($rowEmpleado = mysql_fetch_array($rsEmpleado)) {
		...
		if (!($idComisionEmpleado > 0)) {
			$insertSQL = sprintf("INSERT INTO pg_comision_empleado (id_empleado, id_cargo_departamento, id_vale_salida)
			VALUE (%s, %s, %s);",
				valTpDato($idEmpleado, "int"),
				valTpDato($rowEmpleado['id_cargo_departamento'], "int"),
				valTpDato($idDocumento, "int"));
...
function devolverComisionNC($idDocumento, $idFactura, $generarDirecto = true) {
	global $conex;
