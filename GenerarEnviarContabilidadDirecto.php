...
//**************************COMPRAS ADMINISTRATIVAS********************************
$SqlStr = "select a.id_factura as factura
	...
	,(select cuenta from detalleintegracion x1 where x1.idencabezado = 6 and x1.idobjeto = 3 and x1.sucursal = a.id_empresa) as cuentaiva
...
$SqlStr = "select a.id_factura as factura
	...
	,(select cuenta from detalleintegracion x1 where x1.idencabezado = 22 and x1.idobjeto = 3 and x1.sucursal = a.id_empresa) as cuentaiva
	,a.id_proveedor as proveedor
	,(select concat(nombre) from ".$_SESSION['bdEmpresa'].".cp_proveedor x3 where x3.id_proveedor = a.id_proveedor) as Proveedor
	,a.subtotal_descuento 
	,d.id_modelo
...
$SqlStr = "select a.id_factura as factura
       ...
       ,a.numero_factura_proveedor as documento
       ,b.iva as iva 
       ,(select cuenta from detalleintegracion x1 where x1.idencabezado = 23 and x1.idobjeto = 3 and x1.sucursal = a.id_empresa) as cuentaiva
	   ,a.id_proveedor as proveedor
	   ,(select concat(nombre) from ".$_SESSION['bdEmpresa'].".cp_proveedor x3 where x3.id_proveedor = a.id_proveedor) as Proveedor
	   ,a.subtotal_descuento 
	   ,a.subtotal_factura 
...
where c.id_tipo_articulo = d.id_tipo_articulo
and a.id_factura = b.id_factura
and b.id_articulo = c.id_articulo
and a.id_modulo = 0";
if($idFactura != 0){
...
$exec =  EjecutarExec($con,$SqlStr) or die($SqlStr); 
    ...
    while ($row = ObtenerFetch($exec)) {
	  ...
	  $Debe = $MontoFactura;
	  $Debe1 = bcsub($MontoFactura,$Descuento,2);
	  $montoiva = bcmul($Debe1,$porciva,2);
	  $montoiva = bcdiv($montoiva,100,3);
	  $descripcion  = $descripcion . " " .$Desproveedor;
	  $ct='01';
	  $dt='01';
	  $cc='04';
	  ...
	  if(buscarDoc($id,$cc,$ct,$dt,$fecha)==0){
            ...
            $SqlStr=" select monto,iva,b.id_gasto,b.nombre from ".$_SESSION['bdEmpresa'].".cp_factura_gasto a
...
//			   ingresarRenglon($cuentaiva,$descripcion,$montoiva,0,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
			...
			$cuentaImpor="8.2.01.01.054";
			$cuentaRever="8.2.01.01.057";
			$cuentaDif="9.1.01.01.016";
				if(buscarDoc($id,$cc,$ct,$dt,$fecha)==0){
				  if ($fechaAnt != $fecha){
					 $icomprobant++;
					 $fechaAnt = $fecha;
				...
				$SqlStr=" SELECT a.monto,a.iva,a.id_modo_gasto,a.afecta_documento, b.id_gasto from ".$_SESSION['bdEmpresa'].".cp_factura_gasto a
							,".$_SESSION['bdEmpresa'].".pg_gastos b  
							WHERE a.id_gasto = b.id_gasto
							AND a.id_factura = $id";
				  $exec5 =  EjecutarExec($con,$SqlStr) or die($SqlStr); 
				  ...
				  while($row3 = ObtenerFetch($exec5)){
					...
					if($modoGasto<>1 || $afectaDoc<>1 )
					$gastos = bcadd($gastos,$costo_partida,2);
					
					if($modo==12 || $modo==13 || $modo==17)
					$reverso = bcadd($reverso,$costo_partida,2);
					
					if($modo==21 || $modo==22 || $modo==16)
				...
				$SqlStrDet=" SELECT 
				 ...
				 while($rowDet = ObtenerFetch($execDet)){
					...
					$Debe = bcsub($Debe,$total_tarifa_adv,2);
					$Debe = bcadd($Debe,$costo_partidaSuma,2);
					$gastos2=$gastos-$reverso-$diferencia;
					$gastos = bcadd($gastos,$total_tarifa_adv,2);
				  }
				  ...
				  // Compras sin descuentos
				   ingresarRenglon($cuentacontable,$descripcion,$Debe,$Haber,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
					...
					/* Fin Para insertar los terceros */
				   		...
				   		ingresarRenglon($cuentacontable,$descripcion,$gastos,$Haber,$documento,$ct,$dt,$cc,$icomprobant,$fecha,'D',$cc,$id);
							//NO VA - 28/07/2014
							/* Para insertar los terceros */
				             	/*ingresarEnlacesTerceros($cuentacontable,$fecha,$gastos,$Haber,$idproveedor
					            ,"cp_proveedor","id_proveedor|nombre",$descripcion);*/
					        /* Fin Para insertar los terceros */
...
$SqlStr = "select a.id_factura as factura
       ...
       ,(select cuenta from detalleintegracion x1 where x1.idencabezado = 24 and x1.idobjeto = 3 and x1.sucursal = a.id_empresa) as cuentaiva
	   ,a.id_proveedor as proveedor
	   ,(select concat(nombre) from ".$_SESSION['bdEmpresa'].".cp_proveedor x3 where x3.id_proveedor = a.id_proveedor) as Proveedor
	   ,a.subtotal_descuento
	   ,a.id_empresa 
...
$con = ConectarBD();
	$SqlStr = "select a.idfactura
		   ...
		   ,a.calculoIvaFactura 
		   ,(select cuenta from detalleintegracion x1 where x1.idencabezado = 26 and x1.idobjeto = 15 and x1.sucursal = a.id_empresa) as cuentaiva
...
$SqlStr = "select a.idfactura
	...
	,'02' as centrocosto
	,a.numerofactura as documento
	,(select cuenta from detalleintegracion x1 where x1.idencabezado = 25 and x1.idobjeto = 15 and x1.sucursal = a.id_empresa) as cuentaiva
	,a.idcliente as idcliente
	,(select concat(nombre,' ',apellido) from ".$_SESSION['bdEmpresa'].".cj_cc_cliente x3 where x3.id = a.idcliente) as descliente
	,a.descuentofactura
	,a.id_empresa
...
$execFact =  EjecutarExec($con,$SqlStr) or die($SqlStr);
    $fechaAnt = "";
	$icomprobant = 0; 
...
$cc = $rowFact[2];
				   ...
				   if ($Nota==0){
				          $documento=$rowFact[3];
							...
							$execDoc =  EjecutarExec($con,$SqlStr) or die($SqlStr); 
...
where 
	...
	$execCont =  EjecutarExec($con,$SqlStr) or die($SqlStr); 
					   ...
					   $cuentaRetenido = buscarContable(27,17);// 23 es compras adminstriativas en el encabezado y 5 es en el detalle de los fijos.
						...
						$exec =  EjecutarExec($con,$SqlStr) or die($SqlStr);
							...
							$SqlStr = "select a.idfactura
						       ,a.fecharegistrofactura
						       ,'03' as centrocosto
						       ,(select cuenta from detalleintegracion x1 where x1.idencabezado = 27 and x1.idobjeto = 40 and x1.sucursal = a.id_empresa) as cuentacontable
						       ,b.descripcion_nota as descripcion
						       ,0 as Debe 
						       ,sum(b.precio) as haber 
						       ,a.numerofactura as documento
						       ,sum(round(((b.precio)/100)*a.porcentajeivafactura,2))
						       ,(select cuenta from detalleintegracion x1 where x1.idencabezado = 27 and x1.idobjeto = 15 and x1.sucursal = a.id_empresa) as cuentaiva
						       ,a.idcliente as idcliente
						       ,(select concat(nombre,' ',apellido) from ".$_SESSION['bdEmpresa'].".cj_cc_cliente x3 where x3.id = a.idcliente) as descliente
						       ,a.descuentofactura 
							   ,a.id_empresa
						...
						$exec =  EjecutarExec($con,$SqlStr) or die($SqlStr); 
							...
							$SqlStr = "select a.idfactura
									,a.fecharegistrofactura
									,'03' as centrocosto
									,(select cuenta from detalleintegracion x1 where x1.idencabezado = 27 and x1.idobjeto = 46 and x1.sucursal = a.id_empresa) as cuentacontable
									,'TOT ' as descripcion
									,0 as Debe 
									,round(sum(d.monto_subtotal+(d.monto_subtotal*(e.porcentaje_tot/100))),2) as haber 
									,a.numerofactura as documento
									...
									,a.descuentofactura
									,a.id_empresa  
									 from ".$_SESSION['bdEmpresa'].".sa_orden_tot d	,".$_SESSION['bdEmpresa'].".sa_orden c
									 ,".$_SESSION['bdEmpresa'].".cj_cc_encabezadofactura a,".$_SESSION['bdEmpresa'].".sa_tipo_orden e
									where d.id_orden_servicio = c.id_orden
...
$SqlStr = "select a.idnotacredito
      ...
      ,'N/C' as descripcion
      ,0 as debe
      ,baseimponibleNotaCredito as haber 
      ,a.numeracion_nota_credito as documento
      ,ivaNotaCredito
      ,(select cuenta from detalleintegracion x1 where x1.idencabezado = 26 and x1.idobjeto = 15 and x1.sucursal = a.id_empresa) as cuentaiva
      ,a.idcliente as idcliente
...
$con = ConectarBD();
		$SqlStr = "  select a.id_vale_salida
					...
					,a.id_empresa		
					 from ".$_SESSION['bdEmpresa'].".sa_vale_salida a";
					   ...
					   $cuentaRetenido = buscarContable(27,17);// 23 es compras adminstriativas en el encabezado y 5 es en el detalle de los fijos.
						...
						$exec =  EjecutarExec($con,$SqlStr) or die($SqlStr); 
							...
							$SqlStr = "select a.id_vale_salida
										,a.fecha_vale
										,'03' as centrocosto
										,(select cuenta from detalleintegracion x1 where x1.idencabezado = 27 and x1.idobjeto = 41 and x1.sucursal = a.id_empresa) as cuentacontable
										,b.descripcion_nota as descripcion
										,0 as Debe 
										,sum(b.precio) as haber 
										,a.numero_vale as documento
										...
										where a.id_vale_salida = b.id_vale_salida
						...
						$exec =  EjecutarExec($con,$SqlStr) or die($SqlStr); 
							...
							$SqlStr = "select a.id_vale_salida
										...
										,a.id_empresa
											...
											where a.id_vale_salida = a.id_vale_salida";
							    $SqlStr.=" and a.id_vale_salida= ".$idFactura;
...
$con = ConectarBD();
	...
	while ($row = ObtenerFetch($exec)) {
		...
		$execBan =  EjecutarExec($con,$SqlStr) or die($SqlStr); 
...
$con = ConectarBD();
							...
							// *******************************************************************************************
							  ...
							  $dt='03';
							  $cc='05';	
							$SqlStr = "select a.id_nota_credito
										,a.fecha_registro 
										,'05' as centrocosto
...
$con = ConectarBD();
							...
							// *******************************************************************************************
							  ...
							  $dt='07';
							  $cc='05';
							     $SqlStr = "    select c.id_encabezado_rs
												,c.fecha_pago
												,'05' as centrocosto
...
select a.id_vale_salida
	   ...
	   ,x1.id_clave_movimiento
	   ,a.id_empresa
	   from ".$_SESSION['bdEmpresa'].".iv_vale_salida a
...
select a.id_vale_salida
	   ...
	   ,'' as idmov
	   ,a.id_empresa
	   from ".$_SESSION['bdEmpresa'].".an_vale_salida a
";
// and x.tipo_documento_movimiento = 1  es para entradas y salidas
// and x1.tipo = 2 es para entradas y 4 es para salidas
...
$con = ConectarBD();
							...
							$SqlStr = "select a.id_NotaCargo
							 ...
							 if($idTran != 0){
									...
									$SqlStr.=" and a.fecha_NotaCargo  between '".date('Y-m-d',strtotime($Desde))."' and  '". date('Y-m-d',strtotime($Hasta))."'";
							}
						$icomprobant=0;							
						$exec =  EjecutarExec($con,$SqlStr) or die($SqlStr." " .mysql_error()); 
							 while ($row = ObtenerFetch($exec)) {
...
$con = ConectarBD();
						...
						$icomprobant=0;						
							...
							while ($row = ObtenerFetch($exec)) {
								...
								if($islr != 0){
									if($id == 5){
										$documento="CISLR".$idcuenta."-".$codig;
									}else{
										$documento="DISLR".$idcuenta."-".$codig;
									}
							
									if(buscarDocTe($documento,$cc,$ct,$dt,$fecha)==0){
										$cuentaMov =  buscarContable(25,16,$sucursal);  
										$descripcion = "ISLR ". $descripcion;
...
function ingresarEnlacesTerceros($codigo,$fecha,$debe,$haber,$idobjeto
	            ...
	            if ($inumero == 0){
					$SqlStr ="insert into cuentaterceros(
					  ...
					  where codigo = '$codigo'";		
					  $excTerce = EjecutarExec($con,$SqlStr) or die($SqlStr);
				}
