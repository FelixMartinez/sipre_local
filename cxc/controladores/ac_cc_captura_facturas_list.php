...
function cargaLstVendedor($selId = "") {
	...
	WHERE cxc_fact.idDepartamentoOrigenFactura IN (0,1,2,3,4)
	ORDER BY nombre_empleado");
	$rs = mysql_query($query);
	if (!$rs) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	$totalRows = mysql_num_rows($rs);
	$html = "<select multiple id=\"lstEmpleado\" name=\"lstEmpleado\" class=\"inputHabilitado\" onchange=\"byId('btnBuscar').click();\" style=\"width:99%\">";
		$html .= "<option value=\"-1\">[ Seleccione ]</option>";
	while ($row = mysql_fetch_assoc($rs)) {
...
function listaFactura($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"cc_factura_form.php?id=%s&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/ title=\"Ver\"></a>",
					$row['idFactura']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
