...
function cargarDcto($idCheque){
	...
	if ($idCheque > 0) {
		...
		if ($totalRowsCheque > 0) {
			if (in_array($rowCheque['id_departamento'],array(2,4))) {
				$aVerPago = sprintf("verVentana('../caja_vh/reportes/cjvh_recibo_impresion_pdf.php?idTpDcto=5&id=%s', 960, 550);", $idCheque);
			} else if (in_array($rowCheque['id_departamento'],array(0,1,3))) {
				$aVerPago = sprintf("verVentana('../caja_rs/reportes/cjrs_recibo_impresion_pdf.php?idTpDcto=5&id=%s', 960, 550);", $idCheque);
			}
...
function insertarItemDctoPagado($contFila, $idPago = "", $idFactura = "", $idNotaCargo = "", $idAnticipo = "", $idCaja = "", $txtFechaPago = "", $txtMetodoPago = "", $txtIdNumeroDctoPago = "", $txtNumeroDctoPago = "", $txtBancoCompaniaPago = "", $txtCuentaCompaniaPago = "", $txtBancoClientePago = "", $txtCuentaClientePago = "", $txtMontoPagado = "") {
	...
	if (in_array($row['id_modulo_documento_pagado'],array(2,4))) {
		if ($idFactura > 0 || $idNotaCargo > 0) {
			$aVerReciboAux = sprintf("javascript:verVentana(\'../caja_vh/reportes/cjvh_recibo_pago_pdf.php?idRecibo=%s\', 960, 550);", $row['id_recibo_pago']);
		} else if ($idAnticipo > 0) {
			$aVerReciboAux = sprintf("javascript:verVentana(\'../caja_vh/reportes/cjvh_recibo_impresion_pdf.php?idRecibo=%s\', 960, 550);", $row['id_recibo_pago']);
		}
	} else if (in_array($row['id_modulo_documento_pagado'],array(0,1,3))) {
		if ($idFactura > 0 || $idNotaCargo > 0) {
			$aVerReciboAux = sprintf("javascript:verVentana(\'../caja_rs/reportes/cjrs_recibo_pago_pdf.php?idRecibo=%s\', 960, 550);", $row['id_recibo_pago']);
		} else if ($idAnticipo > 0) {
			$aVerReciboAux = sprintf("javascript:verVentana(\'../caja_rs/reportes/cjrs_recibo_impresion_pdf.php?idRecibo=%s\', 960, 550);", $row['id_recibo_pago']);
		}
	}
	$aVerRecibo = (strlen($aVerReciboAux) > 0) ? "<a href=\"".$aVerReciboAux."\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>" : "";
