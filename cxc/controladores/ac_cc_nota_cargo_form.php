...
function formDcto($idNotaCargo, $acc){
	...
	if ($idNotaCargo > 0) {
		...
		switch($rowNotaCargo['estadoNotaCargo']) {
			case 0 : $claseEstatus = "divMsjError"; $imgEstatus = "<img src=\"../img/iconos/no_cancelado.png\">"; break;
			case 1 : $claseEstatus = "divMsjInfo"; $imgEstatus = "<img src=\"../img/iconos/cancelado.png\">"; break;
			case 2 : $claseEstatus = "divMsjAlerta"; $imgEstatus = "<img src=\"../img/iconos/cancelado_parcial.png\">"; break;
		}
		...
		if ($totalRowsPago > 0) {
			switch ($rowFactura['idDepartamentoOrigenFactura']) {
				case 0 : // REPUESTOS
					$aVerPago = sprintf("verVentana('../caja_rs/reportes/cjrs_comprobante_pago_nota_cargo_pdf.php?valBusq=%s', 960, 550);",
						$idNotaCargo);
					break;
				case 1 : // SERVICIOS
					$aVerPago = sprintf("verVentana('../caja_rs/reportes/cjrs_comprobante_pago_nota_cargo_pdf.php?valBusq=%s', 960, 550);",
					...
					break;
				case 2 : // VEHICULOS
					$aVerPago = sprintf("verVentana('../cajax3/reportes/cj_comprobante_pago_nota_cargo_pdf.php?valBusq=%s', 960, 550);",
						$idNotaCargo);
					break;
		...
		$objResponse->loadCommands(cargaLstVendedor());
...
function cargaLstVendedor($selId = "", $bloquearObj = false){
	...
	WHERE clave_filtro IN (1,5,7,9,10)
...
function guardarDcto($frmDcto, $frmTotalDcto){
	...
	if ($idNotaCargo > 0) {
		...
		if (!$rsNotaCargo) { errorGuardarDcto($objResponse); return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__); }
		$rowNotaCargo = mysql_fetch_array($rsNotaCargo);
...
function insertarItemMetodoPago($contFila, $idPago = "", $idNotaCargo = "", $idCaja = "", $txtFechaPago = "", $txtMetodoPago = "", $txtIdNumeroDctoPago = "", $txtNumeroDctoPago = "", $txtBancoCompaniaPago = "", $txtCuentaCompaniaPago = "", $txtBancoProveedorPago = "", $txtCuentaProveedorPago = "", $txtMontoPago = "") {
	$clase = (fmod($contFila, 2) == 0) ? "trResaltar4" : "trResaltar5";
	$contFila++;
	...
	$txtBancoProveedorPago = ($txtBancoProveedorPago == "" && $totalRows > 0) ? $row['nombre_banco_cliente'] : $txtBancoProveedorPago;
	$txtCuentaProveedorPago = ($txtCuentaProveedorPago == "" && $totalRows > 0) ? $row['nombre_caja'] : $txtCuentaProveedorPago;
	...
	switch ($row['id_modulo']) {
		case 0 : // REPUESTOS
			$aVerRecibo = sprintf("<a href=\"javascript:verVentana(\'../caja_rs/reportes/cjrs_comprobante_pago_nota_cargo_pdf.php?valBusq=%s\', 960, 550);\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>",
				$row['idNotaCargo']);
			break;
		case 1 : // SERVICIOS
			$aVerRecibo = sprintf("<a href=\"javascript:verVentana(\'../caja_rs/reportes/cjrs_comprobante_pago_nota_cargo_pdf.php?valBusq=%s\', 960, 550);\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>",
			...
			break;
		case 2 : // VEHICULOS
			$aVerRecibo = sprintf("<a href=\"javascript:verVentana(\'../cajax3/reportes/cj_comprobante_pago_nota_cargo_pdf.php?valBusq=%s|%s\', 960, 550);\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>",
				$row['idNotaCargo'],
				$row['id_recibo_pago']);
			break;
		case 3 : // ADMINISTRACION
	...
	switch ($row['idFormaPago']) { // 7 = Anticipo, 8 = Nota Crédito
		case 7 : // 7 = Anticipo
			$aVerDcto = "<a id=\"aVerDcto\" href=\"cc_anticipo.php?id=".$txtIdNumeroDctoPago."&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\" title=\"Ver Anticipo\"/><a>";
			if (in_array($row['id_modulo_documento_pago'],array(0,1,3))) {
				$aVerDcto .= "<a href=\"javascript:verVentana(\'../caja_rs/reportes/cjrs_comprobante_pago_anticipo_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Recibo Anticipo PDF\"/></a>";
			} else if (in_array($row['id_modulo_documento_pago'],array(2))) {
				$aVerDcto .= "<a href=\"javascript:verVentana(\'../cajax3/reportes/cj_comprobante_pago_anticipo_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Recibo Anticipo PDF\"/></a>";
			}
			...
			FROM cj_cc_detalleanticipo det_anticipo
				INNER JOIN cj_cc_anticipo cxc_anto ON (det_anticipo.idAnticipo = cxc_anto.idAnticipo)
		...
		case 8 : // 8 = Nota Crédito
			$aVerDcto = "<a id=\"aVerDcto\" href=\"cc_nota_de_credito_xajax.php?id=".$txtIdNumeroDctoPago."&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\" title=\"Ver Nota de Crédito\"/><a>";
			switch ($row['id_modulo_documento_pago']) {
				case 0 : // REPUESTOS
					$aVerDcto .= "<a href=\"javascript:verVentana(\'../repuestos/reportes/iv_devolucion_venta_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota de Crédito PDF\"/></a>";
					break;
				case 1 : // SERVICIOS
					$aVerDcto .= "<a href=\"javascript:verVentana(\'../servicios/reportes/sa_devolucion_venta_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota de Crédito PDF\"/></a>";
					break;
				case 2 : // VEHICULOS
					$aVerDcto .= "<a href=\"javascript:verVentana(\'../vehiculos/reportes/an_devolucion_venta_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota de Crédito PDF\"/></a>";
					break;
				case 3 : // ADMINISTRACION
					$aVerDcto .= "<a href=\"javascript:verVentana(\'../repuestos/reportes/ga_devolucion_venta_pdf.php?valBusq=".$txtIdNumeroDctoPago."\', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota de Crédito PDF\"/></a>";
					break;
			}
			break;
	}
	...
	$htmlItmPie = sprintf("$('#trItmPiePago').before('".
		"<tr id=\"trItm:%s\" align=\"left\" class=\"textoGris_11px %s\" height=\"24\">".
			...
			"<td align=\"center\" %s><input type=\"text\" id=\"txtFechaPago%s\" name=\"txtFechaPago%s\" class=\"inputSinFondo\" readonly=\"readonly\" style=\"text-align:center\" value=\"%s\"/>".
				"%s</td>".
			"<td><table width=\"%s\"><tr align=\"right\"><td>%s</td><td width=\"%s\">%s</td></tr></table></td>".
		...
		$contFila, $clase,
			...
			$classMontoPago, $contFila, $contFila, utf8_encode(date("d-m-Y", strtotime($txtFechaPago))),
				$empleadoCreadorPago,
			"100%", $aVerRecibo, "100%", $txtNumeroRecibo,
			$classMontoPago, $contFila, $contFila, ($txtMetodoPago),
				$txtMetodoPagoConcepto,
				$estatusPago,
			$classMontoPago, "100%", $aVerDcto, $contFila, $contFila, utf8_encode($txtNumeroDctoPago),
