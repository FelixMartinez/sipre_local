...
function listaCheque($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"cc_cheque_form.php?id=%s&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/ title=\"Ver\"></a>",
					$row['id_cheque']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";	
			if (in_array($row['id_departamento'], array(2,4))){ // 2 = Vehiculos
				$htmlTb .= sprintf("<a href=\"javascript:verVentana('../caja_vh/reportes/cjvh_recibo_impresion_pdf.php?idTpDcto=5&id=%s', 960, 550);\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>",
					$row['id_cheque']);	
			} else if (in_array($row['id_departamento'], array(0,1,3))){ // 0 = Repuestos, 1 = Servicios, 3 = Administración
				$htmlTb .= sprintf("<a href=\"javascript:verVentana('../caja_rs/reportes/cjrs_recibo_impresion_pdf.php?idTpDcto=5&id=%s', 960, 550);\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>",
					$row['id_cheque']);	
			}
			$htmlTb .= "</td>";
		$htmlTb .= "</tr>";
