...
function listaNotaCredito($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	while ($row = mysql_fetch_assoc($rsLimit)){
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"cc_nota_credito_form.php?id=%s&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/ title=\"Ver\"></a>",
					$row['idNotaCredito']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			switch ($row['idDepartamentoNotaCredito']) {
				case 0 : // REPUESTOS
					$aVerDctoAux = sprintf("../repuestos/reportes/iv_devolucion_venta_pdf.php?valBusq=%s", $row['idNotaCredito']); break;
				case 1 : // SERVICIOS
					if ($row['aplicaLibros'] == 1) {
						$aVerDctoAux = sprintf("../servicios/reportes/sa_devolucion_venta_pdf.php?valBusq=%s", $row['idNotaCredito']);
					} else {
						$aVerDctoAux = "";
					}
					break;
				case 2 : // VEHICULOS
					$aVerDctoAux = sprintf("../vehiculos/reportes/an_devolucion_venta_pdf.php?valBusq=%s", $row['idNotaCredito']); break;
				case 3 : // ADMINISTRACION
					$aVerDctoAux = sprintf("../repuestos/reportes/ga_devolucion_venta_pdf.php?valBusq=%s", $row['idNotaCredito']); break;
				default : $aVerDctoAux = "";
			}
				$htmlTb .= (strlen($aVerDctoAux) > 0) ? "<a href=\"javascript:verVentana('".$aVerDctoAux."', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota de Crédito PDF\"/></a>" : "";
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			// MODIFICADO ERNESTO
			...
			if ($row['idDepartamentoNotaCredito'] == 0) {
				...
				$sPar .= "&dt=01";
				$sPar .= "&cc=04";
			} else if ($row['idDepartamentoNotaCredito'] == 1) {
				$sPar .= "&ct=02";
				$sPar .= "&dt=01";
				$sPar .= "&cc=03";
				...
				$sPar .= "&dt=01";
				$sPar .= "&cc=02";
			} else if ($row['idDepartamentoNotaCredito'] == 3){
				$sPar .= "&ct=02";
				$sPar .= "&dt=01";
				$sPar .= "&cc=01";
			}
