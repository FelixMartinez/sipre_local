...
function listaECGeneral($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 10000, $totalRows = NULL) {
	...
	$query = sprintf("SELECT q.*,
		CONCAT_WS(' ', cliente.lci,	cliente.ci) AS ci_cliente,
		CONCAT_WS(' ', cliente.nombre, cliente.apellido) AS nombre_cliente,
		SUM(IF(q.tipoDocumentoN IN (1,2), 1, -1) * q.saldoFactura) AS saldoFactura,
		SUM(IF(q.tipoDocumentoN IN (1,2), 1, -1) * q.montoTotalFactura) AS montoTotalFactura,
	...
	FROM (SELECT 
		...
		INNER JOIN vw_iv_empresas_sucursales vw_iv_emp_suc ON (q.id_empresa = vw_iv_emp_suc.id_empresa_reg) %s %s", $sqlBusq, $sqlBusq1, $sqlBusq2, $sqlBusq3, $sqlBusq4, $sqlBusq5, $sqlBusq6, $groupBy);
	...
	while ($row = mysql_fetch_array($rsLimit)) {
		...
		// SE GUARDA EL ULTIMO ID DEL CLIENTE PARA ELIMINARLO EN LA CONSULTA QUE GENERARA LOS TOTALES EN CASO DE QUE EXISTAN MAS DE UNA PAGINA
		$cond = (strlen($sqlBusq6) > 0) ? " AND " : " WHERE ";
		$ultIdCliente = $cond.sprintf("q.idCliente = %s",
			valTpDato($row['idCliente'], "int"));
	...
	FROM (SELECT 
		...
		while ($rowEstado = mysql_fetch_array($rsEstado)) {
			...
			$signo = (in_array($rowEstado['tipoDocumentoN'],array(1,2))) ? 1 : (-1);
			...
			switch($rowEstado['estadoFactura']) {
			...
			switch ($rowEstado['tipoDocumentoN']) {
				case 1 : // 1 = Factura
					$aVerDcto = sprintf("<a id=\"aVerDcto\" href=\"cc_factura.php?id=%s&acc=0\" target=\"_blank\"><img src=\"../img/iconos/ico_view.png\" title=\"".utf8_encode("Ver Factura Venta")."\"/><a>",
						$rowEstado['idFactura']);
					switch ($rowEstado['id_modulo']) {
						case 0 : // REPUESTOS
				...
				case 2 : // 2 = Nota de Débito
					$aVerDcto = sprintf("<a id=\"aVerDcto\" href=\"cc_nota_de_cargo.php?id=%s&acc=0\" target=\"_blank\"><img src=\"../img/iconos/ico_view.png\" title=\"".utf8_encode("Ver Nota de Débito")."\"/><a>",
						$rowEstado['idFactura']);
					...
					break;
				case 3 : // 3 = Anticipo
					$aVerDcto = sprintf("<a id=\"aVerDcto\" href=\"cc_anticipo.php?id=%s&acc=0\" target=\"_blank\"><img src=\"../img/iconos/ico_view.png\" title=\"".utf8_encode("Ver Anticipo")."\"/><a>",
						$rowEstado['idFactura']);
					if (in_array($rowEstado['id_modulo'],array(0,1,3))) {
						$aVerDctoAux = sprintf("javascript:verVentana('../caja_rs/reportes/cjrs_comprobante_pago_anticipo_pdf.php?valBusq=%s', 960, 550);",
							$rowEstado['idFactura']);
					} else if (in_array($rowEstado['id_modulo'],array(2))) {
						$aVerDctoAux = sprintf("javascript:verVentana('../cajax3/reportes/cj_comprobante_pago_anticipo_pdf.php?valBusq=%s', 960, 550);",
							$rowEstado['idFactura']);
					}
				...
				case 4 : // 4 = Nota Credito
					$aVerDcto = sprintf("<a id=\"aVerDcto\" href=\"cc_nota_de_credito_xajax.php?id=%s&acc=0\" target=\"_blank\"><img src=\"../img/iconos/ico_view.png\" title=\"".utf8_encode("Ver Nota de Crédito")."\"/><a>",
						$rowEstado['idFactura']);
					switch ($rowEstado['id_modulo']) {
						case 0 : // REPUESTOS
				...
				case 5 : // 5 = Cheque
					switch ($rowEstado['id_modulo']) {
						case 2 : // VEHICULOS
							$aVerDcto = sprintf("<a href=\"javascript:verVentana('../caja_vh/reportes/cj_comprobante_pago_cheque_pdf.php?valBusq=%s', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"".utf8_encode("Recibo(s) de Pago(s)")."\"/><a>",
								$rowEstado['idFactura']);
							break;
						default : $aVerDcto = "";
					}
				...
				case 6 : // 6 = Transferencia
					switch ($rowEstado['id_modulo']) {
						case 2 : // VEHICULOS
							$aVerDcto = sprintf("<a href=\"javascript:verVentana('../caja_vh/reportes/cj_comprobante_pago_transferencia_pdf.php?valBusq=%s', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"".utf8_encode("Recibo(s) de Pago(s)")."\"/><a>",
								$rowEstado['idFactura']);
							break;
						default : $aVerDcto = "";
					}
			...
			if (in_array($valCadBusq[4],array(3,4))) { // 3 = General por Cliente, 4 = General por Dcto.
				...
				$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
					...
					$htmlTb .= "<td align=\"center\" nowrap=\"nowrap\">".date("d-m-Y",strtotime($rowEstado['fechaRegistroFactura']))."</td>";
					$htmlTb .= "<td align=\"center\" nowrap=\"nowrap\">".(($rowEstado['fechaVencimientoFactura'] != "") ? date("d-m-Y",strtotime($rowEstado['fechaVencimientoFactura'])) : "-")."</td>";
					$htmlTb .= "<td align=\"center\" nowrap=\"nowrap\">".(($rowEstado['fecha_ultimo_pago'] != "") ? date("d-m-Y",strtotime($rowEstado['fecha_ultimo_pago'])) : "-")."</td>";
					...
					$htmlTb .= "<td>";
						$htmlTb .= "<table width=\"100%\">";
						$htmlTb .= "<tr>";
							$htmlTb .= "<td width=\"100%\">".utf8_encode($rowEstado['nombre_cliente'])."</td>";
						$htmlTb .= "</tr>";
						$htmlTb .= (strlen($rowEstado['descripcion_concepto_forma_pago']) > 0) ? "<tr><td><span class=\"textoNegrita_9px\">".utf8_encode($rowEstado['descripcion_concepto_forma_pago'])."</span></td></tr>" : "";
						$htmlTb .= (strlen($rowEstado['observacionFactura']) > 0) ? "<tr><td><span class=\"textoNegritaCursiva_9px\">".utf8_encode($rowEstado['observacionFactura'])."<span></td></tr>" : "";
						$htmlTb .= "</table>";
					$htmlTb .= "</td>";
					$htmlTb .= "<td align=\"center\" ".$class.">".$rowEstado['estado_documento']."</td>";
					$htmlTb .= "<td align=\"right\">".$rowEstado['abreviacion_moneda_local'].number_format($signo * $rowEstado['saldoFactura'], 2, ".", ",")."</td>";
					$htmlTb .= "<td align=\"right\">".$rowEstado['abreviacion_moneda_local'].number_format($signo * $rowEstado['montoTotalFactura'], 2, ".", ",")."</td>";
				$htmlTb .= "</tr>";
			}
			
			$arrayTotalSaldos[$rowEstado['tipoDocumentoN']]['cant_dctos'] = $arrayTotalSaldos[$rowEstado['tipoDocumentoN']]['cant_dctos'] + 1;
			$arrayTotalSaldos[$rowEstado['tipoDocumentoN']]['saldo_dctos'] = $arrayTotalSaldos[$rowEstado['tipoDocumentoN']]['saldo_dctos'] + ($signo * $rowEstado['saldoFactura']);
			$arrayTotalSaldos[$rowEstado['tipoDocumentoN']]['total_dctos'] = $arrayTotalSaldos[$rowEstado['tipoDocumentoN']]['total_dctos'] + ($signo * $rowEstado['montoTotalFactura']);
			
			$totalSaldoCliente += $signo * $rowEstado['saldoFactura'];
			$totalMontoCliente += $signo * $rowEstado['montoTotalFactura'];
		}
		
		if (in_array($valCadBusq[4],array(3))) { // 3 = General por Cliente, 4 = General por Dcto.
			$htmlTb .= "<tr align=\"left\" class=\"trResaltarTotal\" height=\"24\">";
				$htmlTb .= "<td align=\"right\" class=\"tituloCampo\" colspan=\"10\">".utf8_encode($row['nombre_cliente']).":</td>";
			...
			$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
				...
				$htmlTb .= ($valCadBusq[5] == 1) ? "<td>".utf8_encode($row['nombre_empresa'])."</td>" : "";
	...
	if ($contFila > 0) {
		...
		if ($pageNum == $totalPages) {
			$rsEstado = mysql_query(str_replace($ultIdCliente,"",$queryEstado));
			while ($rowEstado = mysql_fetch_assoc($rsEstado)) {
				// 1 = Factura, 2 = Nota de Débito, 3 = Anticipo, 4 = Nota Credito, 5 = Cheque, 6 = Transferencia
				$signo = (in_array($rowEstado['tipoDocumentoN'],array(1,2))) ? 1 : (-1);
				
				$arrayTotalFinal[11] += $signo * $rowEstado['saldoFactura'];
				$arrayTotalFinal[12] += $signo * $rowEstado['montoTotalFactura'];
			}
	...
	while ($row = mysql_fetch_array($rsLimit)) {
		...
		switch ($row['tipoDocumentoN']) {
			...
			case 3 : // 3 = Anticipo
				$aVerDcto = sprintf("<a id=\"aVerDcto\" href=\"cc_anticipo.php?id=%s&acc=0\" target=\"_blank\"><img src=\"../img/iconos/ico_view.png\" title=\"".utf8_encode("Ver Anticipo")."\"/><a>",
					$row['idFactura']);
				if (in_array($row['id_modulo'],array(0,1,3))) {
					$aVerDctoAux = sprintf("javascript:verVentana('../caja_rs/reportes/cjrs_comprobante_pago_anticipo_pdf.php?valBusq=%s', 960, 550);",
						$row['idFactura']);
				} else if (in_array($row['id_modulo'],array(2))) {
					$aVerDctoAux = sprintf("javascript:verVentana('../cajax3/reportes/cj_comprobante_pago_anticipo_pdf.php?valBusq=%s', 960, 550);",
						$row['idFactura']);
				}
				$aVerDcto .= (strlen($aVerDctoAux) > 0) ? "<a href=\"".$aVerDctoAux."\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"".utf8_encode("Recibo(s) de Pago(s)")."\"/></a>" : "";
