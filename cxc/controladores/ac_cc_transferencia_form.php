...
function cargarDcto($idTransferencia){
	...
	if ($idTransferencia > 0) {
		...
		if ($totalRowsTransferencia > 0) {
			if (in_array($rowTransferencia['id_departamento'],array(2,4))) {
				$aVerPago = sprintf("verVentana('../caja_vh/reportes/cjvh_recibo_impresion_pdf.php?idTpDcto=6&id=%s', 960, 550);", $idTransferencia);
			} else if (in_array($rowTransferencia['id_departamento'],array(0,1,3))) {
				$aVerPago = sprintf("verVentana('../caja_rs/reportes/cjrs_recibo_impresion_pdf.php?idTpDcto=6&id=%s', 960, 550);", $idTransferencia);
			}
