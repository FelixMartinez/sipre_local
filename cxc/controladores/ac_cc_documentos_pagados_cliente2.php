	...
	if (mysql_num_rows($rsEstado) > 0){
		while ($rowEstado = mysql_fetch_array($rsEstado)){
			...
			$html .= "<tr class=\"".$clase."\" onmouseover=\"this.className = 'trSobre';\" onmouseout=\"this.className = '".$clase."';\" height=\"24\">";
				...
				$html .= "<td>";
				if ($rowEstado['tipoDocumentoN'] == 1){ // FACTURA
					$html .= sprintf("<a href=\"cc_factura.php?id=%s&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/ title=\"Factura\"></a>", $rowEstado['idDocumento']);
				} else if ($rowEstado['tipoDocumentoN'] == 2){ // NOTA DE DÉBITO
					$html .= sprintf("<a href=\"cc_nota_de_cargo.php?id=%s&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/ title=\"".utf8_encode("Nota de Débito")."\"></a>", $rowEstado['idDocumento']);
				} else if ($rowEstado['tipoDocumentoN'] == 3){ // ANTICIPO
					$html .= sprintf("<a href=\"cc_anticipo.php?id=%s&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/ title=\"Anticipo\"></a>", $rowEstado['idDocumento']);
				} else if ($rowEstado['tipoDocumentoN'] == 4){ // NOTA DE CREDITO
					$html .= sprintf("<a href=\"cc_nota_de_credito_xajax.php?id=%s&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/ title=\"".utf8_encode("Nota de Crédito")."\"></a>", $rowEstado['idDocumento']);
				}
				$html .= "</td>";
			$html .= "</tr>";
