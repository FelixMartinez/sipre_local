...
function listadoLibroVenta($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15000, $totalRows = NULL) {
	...
	while ($rowIva = mysql_fetch_array($rsIva)) {
		...
		$arrayIvas[$rowIva['iva']."|".$rowIva['tipo']] = "";//ivas existentes para el libro
		
		$arrayIvasContribuyente["iva|tipo"] = 0;//cantidad ivas segun contribuyentes por dia
		$arrayIvasNoContribuyente["iva|tipo"] = 0;//cantidad ivas segun no contribuyentes por dia
		
		$arrayTotalIvasContribuyente["iva|tipo"] = 0;//total ivas segun contribuyentes por dia
		$arrayTotalIvasNoContribuyente["iva|tipo"] = 0;//total ivas segun no contribuyentes por dia
	}
	...
	$queryRetencionesOtrosPeriodos = sprintf("SELECT
		...
		WHERE (pa.fechaPago BETWEEN '%s' AND '%s')
			...
			%s
		GROUP BY de.idRetencionDetalle
	...
	while ($rowRetencionesOtrosPeriodos = mysql_fetch_array($rsRetencionesOtrosPeriodos)){
			...
			$htmlTb .= repetir("<td align=\"right\"></td>",($contColum)*2);
	...
	$htmlTb.= "<tr class=\"trResaltar6\">";
			$htmlTb .= "<td align=\"center\" colspan='".(5+($contColum)*2)."'>Total Retenciones de otros Periodos </td>";
			$htmlTb .= "<td align=\"right\"></td>";
			$htmlTb .= "<td align=\"right\"></td>";
			$htmlTb .= "<td align=\"right\"></td>";
			$htmlTb .= "<td align=\"right\"></td>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		foreach($arrayIvas as $indice => $valor){//deben reiniciarse a cero por dia para el total por dia
				$arrayIvasContribuyente[$indice] = 0;
				$arrayIvasNoContribuyente[$indice] = 0;                
		}
		...
		while($rowEstadoCuenta = mysql_fetch_array($rsEstadoCuenta)) {
			...
			if($rowEstadoCuenta['tipoDocumento'] == "FA") { /*-------FACTURA--------*/ //AHORA DEBE CAMBIARSE A MULTI IMPUESTO
				$sqlDocumento = sprintf("SELECT *, porcentajeIvaFactura as iva_normal, porcentajeIvaDeLujoFactura as iva_lujo 				
				...
				$sqlDocumento = sprintf("SELECT *, 
                                IF(porcentajeIvaNotaCredito != 0, porcentajeIvaNotaCredito, (SELECT porcentajeIvaFactura FROM cj_cc_encabezadofactura WHERE idFactura = cj_cc_notacredito.idDocumento)) as iva_normal, 
                                IF(porcentajeIvaDeLujoNotaCredito != 0, porcentajeIvaDeLujoNotaCredito, (SELECT porcentajeIvaDeLujoFactura FROM cj_cc_encabezadofactura WHERE idFactura = cj_cc_notacredito.idDocumento)) as iva_lujo
				...
				$sqlDocumento = sprintf("SELECT *, porcentajeIvaNotaCargo as iva_normal, 0.00 as iva_lujo FROM cj_cc_notadecargo
				WHERE idNotaCargo = '%s'
			...
			if($rowEstadoCuenta["tipoDocumento"] == "FA" && mysql_num_rows($rsDocumento) == 1) {
				...
				$queryFacturaGastos = sprintf("SELECT * FROM cj_cc_factura_gasto WHERE id_factura = %s;",
			...
			/*--------NOTA DE CREDITO---------*/
				...
				// CONSULTA PARA SUMAR LOS GASTOS DEL DOCUMENTO
				$queryNotaCreditoGastos = sprintf("SELECT * FROM cj_cc_nota_credito_gasto WHERE id_nota_credito = %s;",
				...
				while($rowNotaCreditoGasto = mysql_fetch_array($rsNotaCreditoGasto)) {
					$montoGastos += $rowNotaCreditoGasto['monto'];
				}
				
				/* CONSULTA PARA SUMAR LOS IVAS DEL DOCUMENTO
				$queryNotaCreditoIva = sprintf("SELECT * FROM cp_notacredito_iva WHERE id_notacredito = '%s'", $rowDocumento["idNotaCredito"]);
				...
				$totalCompraConIva = -($rowNotaCredito['subtotalNotaCredito'] - $rowNotaCredito['subtotal_descuento']) + $montoGastos + $montoIva;
				$nroComprobanteRetencion75 = "-";
				$nroComprobanteRetencion3 = "-";
			...
			if($rowDocumento['idFactura'] != "" || $rowDocumento['idNotaCredito'] != "" || $rowDocumento['idNotaCargo'] != "") {
				...
				$htmlTb .= "<tr title=\"reglon documento\" align=\"left\" class=\"".$clase."\" onmouseover=\"this.className='trSobre';\" onmouseout=\"this.className='".$clase."';\" height=\"24\">";
					$htmlTb .= "<td align=\"right\">".$fechaDocumento."</td>";
					$htmlTb .= "<td align=\"center\" title=\"id: ".$idDocumento." Origen: ".$origenDocumento."\">".$tipoDocumento."</td>";
				...
				if ($rowCliente['contribuyente'] == 'No') {
                                        ...
                                        $arrayIvasNoContribuyente["".$rowDocumento['iva_normal']."|6"] += $ivaDocumento;//6 = iva venta
                                        $arrayIvasNoContribuyente["".$rowDocumento['iva_lujo']."|2"] += $ivaLujoDocumento;//2 = iva lujo
                                        
                                        $arrayTotalIvasNoContribuyente["".$rowDocumento['iva_normal']."|6"] += $ivaDocumento;//6 = iva venta
                                        $arrayTotalIvasNoContribuyente["".$rowDocumento['iva_lujo']."|2"] += $ivaLujoDocumento;//2 = iva lujo
                                    
					$htmlTb .= "<td align=\"right\" title=\"base imponible no contribuyente\">".valTpDato(number_format(doubleval($montoBaseImponible),2,",",""),"cero_por_vacio")."</td>";
						foreach($arrayIvas as $indice => $valor){
							if($indice == $rowDocumento['iva_normal']."|6"){//6 = iva venta
								$htmlTb .= "<td align=\"right\" title=\"iva no contribuyente\">".valTpDato(number_format(doubleval($ivaDocumento),2,",",""),"cero_por_vacio")."</td>";
							}elseif($indice == $rowDocumento['iva_lujo']."|2"){//2 = iva lujo
								$htmlTb .= "<td align=\"right\" title=\"iva lujo no contribuyente\">".valTpDato(number_format(doubleval($ivaLujoDocumento),2,",",""),"cero_por_vacio")."</td>";
							}else{
								$htmlTb .= "<td align=\"right\" title=\"sin iva no contribuyente\">0.00</td>";
							}
						}
						$htmlTb .= repetir("<td align=\"center\">-</td>",$contColum);
				...
				if ($rowCliente['contribuyente'] == 'Si') {
					...
					$arrayIvasContribuyente["".$rowDocumento['iva_normal']."|6"] += $ivaDocumento;//6 = iva venta
					$arrayIvasContribuyente["".$rowDocumento['iva_lujo']."|2"] += $ivaLujoDocumento;//2 = iva lujo
					
					$arrayTotalIvasContribuyente["".$rowDocumento['iva_normal']."|6"] += $ivaDocumento;//6 = iva venta
					$arrayTotalIvasContribuyente["".$rowDocumento['iva_lujo']."|2"] += $ivaLujoDocumento;//2 = iva lujo
					
					$htmlTb .= repetir("<td align=\"center\">-</td>",$contColum);
					$htmlTb .= "<td align=\"right\" title=\"base imponible contribuyente\">".valTpDato(number_format(doubleval($montoBaseImponible),2,",",""),"cero_por_vacio")."</td>";
						foreach($arrayIvas as $indice => $valor){                                            
							if($indice == $rowDocumento['iva_normal']."|6"){//6 = iva venta
								$htmlTb .= "<td align=\"right\" title=\"iva contribuyente\">".valTpDato(number_format(doubleval($ivaDocumento),2,",",""),"cero_por_vacio")."</td>";
							}elseif($indice == $rowDocumento['iva_lujo']."|2"){//2 = iva lujo
								$htmlTb .= "<td align=\"right\" title=\"iva lujo contribuyente\">".valTpDato(number_format(doubleval($ivaLujoDocumento),2,",",""),"cero_por_vacio")."</td>";
							}else{
								$htmlTb .= "<td align=\"right\" title=\"sin iva contribuyente\">0.00</td>";
							}
						}
		...
		$htmlTb.= "<tr class=\"trResaltar6\">";
			...
			$htmlTb .= "<td align=\"right\">".number_format($totalBaseImponibleDiaNoContribuyente,2,",","")."</td>";
				foreach($arrayIvas as $indice => $valor){
					$htmlTb .= "<td align=\"right\">".number_format($arrayIvasNoContribuyente[$indice],2,",","")."</td>";
				}
			$htmlTb .= "<td align=\"right\">".number_format($totalBaseImponibleDiaContribuyente,2,",","")."</td>";
				foreach($arrayIvas as $indice => $valor){
					$htmlTb .= "<td align=\"right\">".number_format($arrayIvasContribuyente[$indice],2,",","")."</td>";
				}
	...
	$htmlTb.= "<tr class=\"trResaltarTotal\" height=\"24\">";
		...
		$htmlTb .= "<td align=\"right\">".number_format($TotalesBaseImponibleDocNoContribuyente,2,",","")."</td>";
                foreach($arrayIvas as $indice => $valor){
                    $htmlTb .= "<td align=\"right\">".number_format($arrayTotalIvasNoContribuyente[$indice],2,",","")."</td>";
                }

		$htmlTb .= "<td align=\"right\">".number_format($TotalesBaseImponibleDocContribuyente,2,",","")."</td>";
                foreach($arrayIvas as $indice => $valor){
                    $htmlTb .= "<td align=\"right\">".number_format($arrayTotalIvasContribuyente[$indice],2,",","")."</td>";
                }
	...
	$objResponse->script("xajax_encabezadoEmpresa2(".$idEmpresa.")");
...
function encabezadoEmpresa2($idEmpresa) {
	$objResponse = new xajaxResponse();
	...
	if ($row['id_empresa'] != "") {
		...
		$html .= "<tr align=\"center\">";
			...
			if (strlen($row['direccion']) > 1) {
				$html .= "<tr align=\"center\">";
					$html .= "<td>";
						$html .= htmlentities(utf8_encode($row['direccion']));
					$html .= "</td>";
				$html .= "</tr>";
			}
			if (strlen($row['web']) > 1) {
...
$xajax->register(XAJAX_FUNCTION,"encabezadoEmpresa2");
