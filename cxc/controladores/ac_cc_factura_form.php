...
function asignarDepartamento($frmDcto, $frmListaArticulo, $frmTotalDcto) {
	...
	if ($frmDcto['lstModulo'] == 3 && $frmDcto['lstAplicaLibro'] == 1) {
		...
		$fechaVencimiento = suma_fechas("d-m-Y",date("d-m-Y"),$frmDcto['txtDiasCreditoCliente']);
		
		$objResponse->assign("tdtxtFechaFactura","innerHTML","<input type=\"text\" id=\"txtFechaFactura\" name=\"txtFechaFactura\" size=\"10\" style=\"text-align:center\" value=\"".date("d-m-Y")."\"/>");
		...
		jQuery(function($){
			$('#txtFechaFactura').maskInput('99-99-9999',{placeholder:' '});
			$('#txtFechaVencimiento').maskInput('99-99-9999',{placeholder:' '});
		});
		new JsDatePick({
			useMode:2,
		...
		$queryIva = sprintf("SELECT iva.*, IF (iva.tipo IN (3,2), 1, NULL) AS lujo FROM pg_iva iva WHERE estado = 1 AND tipo IN (6,2) ORDER BY iva");
		$rsIva = mysql_query($queryIva);
		if (!$rsIva) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
		$indiceIva = 0;
		while ($rowIva = mysql_fetch_assoc($rsIva)) {
	...
	if (count($arrayObj) > 0 && $frmDcto['txtIdCliente'] > 0) {
		...
		if (isset($frmTotalDcto['cbxIva'])) {
			foreach ($frmTotalDcto['cbxIva'] as $indice => $valor) {
				...
				$query = sprintf("SELECT iva.*, IF (iva.tipo IN (3,2), 1, NULL) AS lujo FROM pg_iva iva WHERE estado = 1 AND tipo IN (6) AND activo = 1 AND idIva = %s ORDER BY iva",
...
function cargaLstIva($nombreObjeto, $selId = "", $selVal = "") {
	...
	$queryIva = sprintf("SELECT iva.*, IF (iva.tipo IN (3,2), 1, NULL) AS lujo FROM pg_iva iva WHERE iva.estado = 1 AND iva.tipo IN (6,2)
	ORDER BY iva;");
	$rsIva = mysql_query($queryIva);
	if (!$rsIva) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
...
function formDcto($idFactura, $acc){
	...
	if ($idFactura > 0) {
		...
		$objResponse->assign("txtFechaFactura","value",date("d-m-Y",strtotime($rowFactura['fechaRegistroFactura'])));
		...
		switch ($rowFactura['idDepartamentoOrigenFactura']) {
			case 0 : // REPUESTOS
				$aVerDcto = sprintf("verVentana('../repuestos/reportes/iv_factura_venta_pdf.php?valBusq=%s', 960, 550);",
					$idFactura);
				break;
			case 1 : // SERVICIOS
				$aVerDcto = sprintf("verVentana('../servicios/reportes/sa_factura_venta_pdf.php?valBusq=%s', 960, 550);",
					$idFactura);
				break;
			case 2 : // VEHICULOS
				$aVerDcto = sprintf("verVentana('../vehiculos/reportes/an_factura_venta_pdf.php?valBusq=%s', 960, 550);",
					$idFactura);
				break;
			case 3 : // ADMINISTRACION
				$aVerDcto = sprintf("verVentana('../repuestos/reportes/ga_factura_venta_pdf.php?valBusq=%s', 960, 550);",
					$idFactura);
				break;
		}
		...
		if ($totalRowsNotaCredito > 0) {
			...
			while ($rowNotaCredito = mysql_fetch_array($rsNotaCredito)){
				...
				$aVerDcto = "<a id=\"aVerDcto\" href=\"cc_nota_credito_form.php?id=".$rowNotaCredito['idNotaCredito']."&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\" title=\"Ver Nota de Crédito\"/><a>";
				
				$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
					$htmlTb .= "<td align=\"center\">".implode("-",array_reverse(explode("-",$rowNotaCredito['fechaNotaCredito'])))."</td>";
...
function guardarDcto($frmDcto, $frmListaArticulo, $frmTotalDcto){
	...
	if ($idFactura > 0) {
		...
		VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
			...
			valTpDato(implode("-",array_reverse(explode("-",$frmDcto['txtFechaVencimiento']))), "date"),
...
function cargaLstIvaItm($nombreObjeto, $selId = "", $selVal = "") {
	// 1 = IVA COMPRA, 3 = IMPUESTO LUJO COMPRA, 6 = IVA VENTA, 2 = IMPUESTO LUJO VENTA
	$queryIva = sprintf("SELECT iva.*, IF (iva.tipo IN (3,2), 1, NULL) AS lujo FROM pg_iva iva WHERE iva.estado = 1 AND iva.tipo IN (6,2) ORDER BY iva");
	$rsIva = mysql_query($queryIva);
	if (!$rsIva) return $objResponse->alert(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
...
function insertarItemArticulo($contFila, $hddIdFacturaVentaDetalle = "", $idCliente = "", $idConcepto = "", $cantPedida = "", $precioUnitario = "", $costoUnitario = "", $abrevMonedaCostoUnitario = "", $idIva = "") {
	...
	WHERE ((iva.tipo IN (6) AND iva.activo = 1)
			OR (iva.tipo IN (6,2) AND idIva = %s AND %s IS NOT NULL))
	...
	$htmlItmPie = sprintf("$('#trItmPie').before('".
		"<tr align=\"left\" id=\"trItmArticulo:%s\" class=\"textoGris_11px %s\">".
			...
			"<td>%s</td>".
			"<td><input type=\"text\" id=\"txtDescItm%s\" name=\"txtDescItm%s\" class=\"inputCompleto\" maxlength=\"44\" value=\"%s\"/></td>".
		...
		$contFila, $clase,
			...
			$contFila, $contFila, $rowConcepto['descripcion'],
