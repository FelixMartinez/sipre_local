...
function listaNotaDebito($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 20, $totalRows = NULL){
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" height=\"24\">";
			...
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"cc_nota_debito_form.php?id=%s&acc=0\" target=\"_self\"><img src=\"../img/iconos/ico_view.png\"/ title=\"Ver\"></a>",
					$row['idNotaCargo']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
				$htmlTb .= sprintf("<a href=\"javascript:verVentana('reportes/cc_nota_cargo_pdf.php?valBusq=%s', 960, 550);\"><img src=\"../img/iconos/page_white_acrobat.png\" title=\"Nota de Débito PDF\"/></a>",
					$row['idNotaCargo']);
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			if ($row['saldoNotaCargo'] < $row['total']) {
				if (in_array($row['id_modulo'],array(2,4))) {
					$aVerDcto = sprintf("../caja_vh/reportes/cjvh_recibo_pago_pdf.php?idTpDcto=2&id=%s", $row['idNotaCargo']);
				} else if (in_array($row['id_modulo'],array(0,1,3))) {
					$aVerDcto = sprintf("../caja_rs/reportes/cjrs_recibo_pago_pdf.php?idTpDcto=2&id=%s", $row['idNotaCargo']);
				}
			} else {
				$aVerDcto = "";
			}
				$htmlTb .= (strlen($aVerDcto) > 0) ? "<a href=\"javascript:verVentana('".$aVerDcto."', 960, 550);\"><img src=\"../img/iconos/print.png\" title=\"Recibo(s) de Pago(s)\"/></a>" : "";
			$htmlTb .= "</td>";
			$htmlTb .= "<td>";
			// MODIFICADO ERNESTO
			...
			if (in_array($row['id_modulo'],array(0,1,2))){ //RESPUESTOS O SERVICIOS O VEHICULOS
