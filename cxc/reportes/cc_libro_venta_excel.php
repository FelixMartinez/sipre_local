...
header("Content-Disposition: attachment; filename=libro_venta_excel.xls");  //File name extension was wrong
header("Expires: 0");
...
function listadoLibroVentaExcel($pageNum = 0, $campOrd = "", $tpOrd = "", $valBusq = "", $maxRows = 15000, $totalRows = NULL) {
	...
	if (!$rsIva) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	...
	if (!$rsConfig403) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	...
	$htmlTh .= "<tr class=\"tituloColumna\">
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Fecha del Documento"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Tipo De Documento"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Nro. de Factura"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Nro. de Control"."</th>";
	
	$colspan = '11';
	
	if ($valor == 2 || $valor == 3) {//1 = VENEZUELA ; 2 = PANAMA ; 3 = PUERTO RICO
		$colspan = '12';
		$htmlTh .= "<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Nro. Fiscal"."</th>";
	}
				
	$htmlTh .= "<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Codigo"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">".$spanClienteCxC."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Cliente"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Nro. Nota de Credito / Debito"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Numero de Factura Afectada"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Numero de Comprobante de Retencion"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Fecha de Comprobante de Retencion. "."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Total de Ventas Incluyendo el IVA"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Ventas Exentas"."</th>
				<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Ventas Exoneradas"."</th>";
	
	$htmlTh .= "<th colspan=\"".$contColum."\" style=\"background-color: #bfbfbf;\" width=\"\">"."No Contribuyente"."</th>
				<th colspan=\"".$contColum."\" style=\"background-color: #bfbfbf;\" width=\"\">"."Contribuyente"."</th>";
	
	$htmlTh .= "<th rowspan=\"3\" style=\"background-color: #bfbfbf;\" width=\"\">"."Impuesto Retenido"."</th>
			</tr>";
	
	$htmlTh .= "<tr class=\"tituloColumna\">";
	$htmlTh .= "<th rowspan=\"2\" style=\"background-color: #bfbfbf;\" width=\"\">"."Base Imponible"."</th>";
	
	while ($rowIva = mysql_fetch_array($rsIva)) {
		$htmlTh .= "<th style=\"background-color: #bfbfbf;\" width=\"\">"."Alicuota IVA"."</th>";
	}
	...
	while ($rowIva = mysql_fetch_array($rsIva)) {
		$htmlTh .= "<td style=\"background-color: #bfbfbf;\" width=\"\">".$rowIva['iva']."%</td>";
	}
	...
	if (!$rsRetencionesOtrosPeriodos) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	...
	$htmlTb.= "<tr style=\"background-color: #ffeed5;\" class=\"trResaltar6\">";
			$htmlTb .= "<td align=\"center\" colspan='".$colspan."'>Total Retenciones de otros Periodos </td>";/*alo*/
			$htmlTb .= "<td align=\"right\"></td>";
			$htmlTb .= "<td align=\"right\"></td>";
			$htmlTb .= "<td align=\"right\"></td>";
			$htmlTb .= "<td align=\"right\"></td>";
	...
	while ($row = mysql_fetch_assoc($rsLimit)) {
		...
		if (!$rsEstadoCuenta) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
		...
		while($rowEstadoCuenta = mysql_fetch_array($rsEstadoCuenta)) {
			...
			if($rowEstadoCuenta['tipoDocumento'] == "FA") { /*-------FACTURA--------*/
				...
				if (!$rsFacturaGasto) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				...
				if (!$rsFacturaIva) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			...
			if (!$rsDocumento) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			...
			if($rowEstadoCuenta["tipoDocumento"] == "FA" && mysql_num_rows($rsDocumento) == 1) {
				...
				if (!$rsRetenciones) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			...
			/*--------NOTA DE CREDITO---------*/
				...
				if($rowDocumento["tipoDocumento"] == "ND") {
					...
					if (!$consulnota) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
					...
					if (!$rsFact) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				...
				if (!$rsNotaCredito) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				...
				if (!$rsNotaCreditoGasto) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				$montoGastos = 0;
				...
				if (!$rsNotaCreditoIva) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				$montoIva = 0;
				$montoBaseImpIvas = 0;
			...
			/*-------NOTA DE DÉBITO---------*/
				...
				if (!$rsNotaCargoGasto) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				$montoGastos = 0;
				$montoBaseImpIvas = 0;
				...
				if (!$rsNotaCargoIva) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				$montoIva = 0;
			...
			if (!$retencionesconsulta) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			...
			if (!$detallesretencionesconsulta) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
			...
			if($rowDocumento['idFactura'] != "" || $rowDocumento['idNotaCredito'] != "" || $rowDocumento['idNotaCargo'] != "") {
				...
				FROM cj_cc_cliente cliente WHERE id = '".$rowDocumento['idCliente']."'";
				$rsCliente = mysql_query($sqlCliente);
				if (!$rsCliente) return die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
				...
				$htmlTb .= "<tr align=\"left\" class=\"".$clase."\" onmouseover=\"this.className='trSobre';\" onmouseout=\"this.className='".$clase."';\" height=\"24\">";
					...
					$htmlTb .= "<td align=\"right\">".$rowCliente['ci_cliente']."</td>";
	...
	$htmlTb.= "<tr style=\"background-color: #e6ffe6;\" class=\"trResaltarTotal\" height=\"24\">";
			...
			$htmlTf .= "<tr class=\"tituloCampo\">";
				...
				$htmlTf .= "<td align=\"center\" class=\"tituloColumna\" width=\"210\">";
					...
					$htmlTf .= "<tr align=\"center\">";
						...
						$htmlTf .= "<td width=\"25\">";
						if ($pageNum > 0) {
							$htmlTf .= sprintf("<a class=\"puntero\" onclick=\"xajax_listadoLibroVentaExcel(%s,'%s','%s','%s',%s);\">%s</a>",
						...
						$htmlTf .= "<td width=\"100\">";
							...
							$htmlTf .= sprintf("<select id=\"pageNum\" name=\"pageNum\" onchange=\"xajax_listadoLibroVentaExcel(%s,'%s','%s','%s',%s)\">",
	...
	$htmlCuadro .="<tr style=\"background-color: #bfbfbf;\" class=\"tituloColumna\" height=\"24\">";
		$htmlCuadro .="<th width=\"30%\"></th>";
		$htmlCuadro .="<th align=\"center\" width=\"5%\">Desde</th>";
		$htmlCuadro .="<th align=\"center\" width=\"5%\">Hasta</th>";
		$htmlCuadro .="<th align=\"center\" width=\"10%\">Base</th>";
		$htmlCuadro .="<th align=\"center\" width=\"10%\">Iva</th>";
		$htmlCuadro .="<th align=\"center\" width=\"10%\">Exentas</th>";
		$htmlCuadro .="<th align=\"center\" width=\"10%\">Exoneradas</th>";
		$htmlCuadro .="<th align=\"center\" width=\"10%\">Total</th>";
		$htmlCuadro .="<th align=\"center\" width=\"10%\">Iva Retenido</th>";
	$htmlCuadro .="</tr>";
	...
	//return $objResponse;
	return $htmlTableIni.$htmlTh.$htmlTb.$htmlTableFin."<br>".$htmlCuadro;
}

echo listadoLibroVentaExcel(0,'','', $_GET['f1']."|".$_GET['f2']."|".$_GET['modulos']."|".$_GET['idEmpresa']);



//$xajax->register(XAJAX_FUNCTION,"listadoLibroVentaExcel");
?>