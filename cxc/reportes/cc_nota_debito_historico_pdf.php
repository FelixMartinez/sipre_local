...
if ($totalRows > 0) {
	...
	while ($row = mysql_fetch_assoc($rs)) {
		...
		$pdf->Cell($arrayCol[3]['tamano'],14,date("d-m-Y", strtotime($row['fechaRegistroNotaCargo'])),'LR',0,'C',true);
		$pdf->Cell($arrayCol[4]['tamano'],14,($row['numeroNotaCargo']),'LR',0,'R',true);
		$pdf->Cell($arrayCol[5]['tamano'],14,($row['nombre_cliente']),'LR',0,'L',true);
		$pdf->Cell($arrayCol[6]['tamano'],14,$row['id_motivo'].".- ".($row['descripcion_motivo']),'LR',0,'C',true);
