...
if ($valCadBusq[5] != "-1" && $valCadBusq[5] != "") {
	$rs = mysql_query(sprintf("SELECT * FROM pg_modulos WHERE id_enlace_concepto IN (%s);", valTpDato($valCadBusq[5], "campo")));
	if (!$rs) die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	while ($row = mysql_fetch_assoc($rs)) {
...
if ($valCadBusq[6] != "-1" && $valCadBusq[6] != "") {
	$lstTipoDcto = array(1 => "Factura", 2 => "Nota de Débito", 3 => "Anticipo", 4 => "Nota de Crédito", 5 => "Cheque", 6 => "Transferencia");
	foreach ($lstTipoDcto as $indice => $valor) {
...
if (in_array($valCadBusq[3],array(1))) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("H".$contFilaY, "Id");
	$objPHPExcel->getActiveSheet()->setCellValue("I".$contFilaY, "Cliente");
	$objPHPExcel->getActiveSheet()->setCellValue("J".$contFilaY, "Saldo");
	$objPHPExcel->getActiveSheet()->setCellValue("K".$contFilaY, "Cta. Corriente");
	$objPHPExcel->getActiveSheet()->setCellValue("L".$contFilaY, "De ".$rowGrupoEstado['desde1']." A ".$rowGrupoEstado['hasta1']);
	$objPHPExcel->getActiveSheet()->setCellValue("M".$contFilaY, "De ".$rowGrupoEstado['desde2']." A ".$rowGrupoEstado['hasta2']);
	$objPHPExcel->getActiveSheet()->setCellValue("N".$contFilaY, "De ".$rowGrupoEstado['desde3']." A ".$rowGrupoEstado['hasta3']);
	$objPHPExcel->getActiveSheet()->setCellValue("O".$contFilaY, "Mas de ".$rowGrupoEstado['masDe']);
	
	$objPHPExcel->getActiveSheet()->getStyle("A".$contFilaY.":O".$contFilaY)->applyFromArray($styleArrayColumna);
} else {
	$contColum = "A";
...
while ($row = mysql_fetch_assoc($rs)) {
	...
	if (in_array($valCadBusq[3],array(1))) {
		...
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("D".$contFilaY, date("d-m-Y",strtotime($row['fechaRegistroFactura'])), PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("E".$contFilaY, date("d-m-Y",strtotime($row['fechaVencimientoFactura'])), PHPExcel_Cell_DataType::TYPE_STRING);
		...
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("H".$contFilaY, $row['idCliente'], PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValueExplicit("I".$contFilaY, utf8_encode($row['nombre_cliente']), PHPExcel_Cell_DataType::TYPE_STRING);
		$objPHPExcel->getActiveSheet()->setCellValue("J".$contFilaY, $totalSaldo);
		$objPHPExcel->getActiveSheet()->setCellValue("K".$contFilaY, $totalCorriente);
		$objPHPExcel->getActiveSheet()->setCellValue("L".$contFilaY, $totalEntre1);
		$objPHPExcel->getActiveSheet()->setCellValue("M".$contFilaY, $totalEntre2);
		$objPHPExcel->getActiveSheet()->setCellValue("N".$contFilaY, $totalEntre3);
		$objPHPExcel->getActiveSheet()->setCellValue("O".$contFilaY, $totalMasDe);
			
		$objPHPExcel->getActiveSheet()->getStyle("A".$contFilaY.":O".$contFilaY)->applyFromArray($clase);
		...
		$objPHPExcel->getActiveSheet()->getStyle("I".$contFilaY)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		
		$objPHPExcel->getActiveSheet()->getStyle("J".$contFilaY)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_COMMA_SEPARATED1);
		...
		while ($rowEstado = mysql_fetch_array($rsEstado)) {
			...
			if (in_array($valCadBusq[3],array(3,4))) { // 3 = General por Cliente, 4 = General por Dcto.
				...
				$objPHPExcel->getActiveSheet()->setCellValueExplicit(($contColum++).$contFilaY, date("d-m-Y",strtotime($rowEstado['fechaRegistroFactura'])), PHPExcel_Cell_DataType::TYPE_STRING);
				$objPHPExcel->getActiveSheet()->setCellValueExplicit(($contColum++).$contFilaY, date("d-m-Y",strtotime($rowEstado['fechaVencimientoFactura'])), PHPExcel_Cell_DataType::TYPE_STRING);
		...
		if (in_array($valCadBusq[3],array(3))) { // 3 = General por Cliente, 4 = General por Dcto.
			...
			$contColum = "A";
			$objPHPExcel->getActiveSheet()->getStyle(($contColum).$contFilaY.":G".$contFilaY)->applyFromArray($styleArrayCampo);
			$objPHPExcel->getActiveSheet()->mergeCells(($contColum).$contFilaY.":G".$contFilaY);
			$contColum = "H";
		...
		$arrayTotalPagina[4] += $totalSaldoCliente;
		$arrayTotalPagina[5] += $totalCorrienteCliente;
		$arrayTotalPagina[6] += $totalEntre1Cliente;
		$arrayTotalPagina[7] += $totalEntre2Cliente;
		$arrayTotalPagina[8] += $totalEntre3Cliente;
		$arrayTotalPagina[9] += $totalMasDeCliente;
	}
}
$ultimo = $contFilaY;
if (in_array($valCadBusq[3],array(1))) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("J".$contFilaY, $arrayTotalPagina[8]);
	$objPHPExcel->getActiveSheet()->setCellValue("K".$contFilaY, $arrayTotalPagina[9]);
	$objPHPExcel->getActiveSheet()->setCellValue("L".$contFilaY, $arrayTotalPagina[10]);
	$objPHPExcel->getActiveSheet()->setCellValue("M".$contFilaY, $arrayTotalPagina[11]);
	$objPHPExcel->getActiveSheet()->setCellValue("N".$contFilaY, $arrayTotalPagina[12]);
	$objPHPExcel->getActiveSheet()->setCellValue("O".$contFilaY, $arrayTotalPagina[13]);
		
	
	$objPHPExcel->getActiveSheet()->getStyle("A".$contFilaY.":I".$contFilaY)->applyFromArray($styleArrayCampo);
	$objPHPExcel->getActiveSheet()->mergeCells("A".$contFilaY.":I".$contFilaY);
	$objPHPExcel->getActiveSheet()->getStyle("J".$contFilaY.":O".$contFilaY)->applyFromArray($styleArrayResaltarTotal);
	...
	if (in_array($valCadBusq[3],array(3))) { // 3 = General por Cliente, 4 = General por Dcto.
		...
		$contColum = "A";
		$objPHPExcel->getActiveSheet()->getStyle(($contColum).$contFilaY.":I".$contFilaY)->applyFromArray($styleArrayCampo);
		$objPHPExcel->getActiveSheet()->mergeCells(($contColum).$contFilaY.":I".$contFilaY);
		$contColum = "J";
	} else {
...
$objPHPExcel->getActiveSheet()->setAutoFilter((in_array($valCadBusq[3],array(1))) ? "A".$primero.":O".$ultimo : "A".$primero.":".($contColumUlt).$ultimo);

cabeceraExcel($objPHPExcel, $idEmpresa, (in_array($valCadBusq[3],array(1))) ? "O" : ($contColumUlt), true, ((isset($arrayCriterioBusqueda)) ? 9 : 8));

$tituloDcto = "Estado de Cuenta CxC (Antigüedad de Saldos) al ".$valCadBusq[2];
$objPHPExcel->getActiveSheet()->setCellValue("A7", $tituloDcto);
$objPHPExcel->getActiveSheet()->getStyle("A7")->applyFromArray($styleArrayTitulo);
$objPHPExcel->getActiveSheet()->mergeCells((in_array($valCadBusq[3],array(1))) ? "A7:O7" : "A7:".($contColumUlt)."7");
...
if (isset($arrayCriterioBusqueda)) {
	...
	$objPHPExcel->getActiveSheet()->mergeCells((in_array($valCadBusq[3],array(1))) ? "A9:O9" : "A9:".($contColumUlt)."9");
}
