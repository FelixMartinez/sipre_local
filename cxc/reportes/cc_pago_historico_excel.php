...
if ($valCadBusq[4] != "-1" && $valCadBusq[4] != "") {
	$rs = mysql_query(sprintf("SELECT * FROM pg_modulos WHERE id_enlace_concepto IN (%s);", valTpDato($valCadBusq[4], "campo")));
	if (!$rs) die(mysql_error()."\nError Nro: ".mysql_errno()."\nLine: ".__LINE__);
	while ($row = mysql_fetch_assoc($rs)) {
...
$sqlBusq2 .= $cond.sprintf("cxc_fact.idDepartamentoOrigenFactura IN (2,4)
AND cxc_pago.estatus IN (1,2)");
...
while ($row = mysql_fetch_assoc($rs)) {
	...
	$objPHPExcel->getActiveSheet()->setCellValue("K".$contFila, date("d-m-Y",strtotime($row['fechapago'])));
